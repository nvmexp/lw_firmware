 ...
 */
FUB_STATUS
fubCheckIfAutoQs2AsbIsApplicable_TU104
(
...
)
{
    ...
    *pbIsFuseBurningApplicable = LW_FALSE;
    //
    // Check if use cases passed by RegKey includes burning fuse for ASB QS1 to QS2.
    ...
    //
    if (!(g_desc.useCaseMask & LW_REG_STR_RM_FUB_USE_CASE_MASK_AUTO_QS2_ASB_FUSE))
    {
        *pbIsFuseBurningApplicable = LW_FALSE;
        fubStatus                  = FUB_OK;
        goto ErrorExit;
    ...
    {
        fubReportApplicabilityFailure_HAL(pFub, LW_FUB_USE_CASE_MASK_AUTO_QS2_ASB_FUSE);
    }
 ...
 */
FUB_STATUS
fubCheckAndBurnAutoQs2AsbFuse_TU104
(
...
)
{
    ...
    if (*pfubUseCasMask & LW_FUB_USE_CASE_MASK_AUTO_QS2_ASB_FUSE)
    {
        CHECK_FUB_STATUS(FUB_ERR_AUTO_QS2_ASB_ALREADY_SET_IN_MASK);
    }
    ...
    if(bIsFuseBurningApplicable)
    {
        ...
        if (fuseInfo.fuseAdjustedBurlwalue)
        {
            ...
            //
            *pfubUseCasMask = (*pfubUseCasMask | LW_FUB_USE_CASE_MASK_AUTO_QS2_ASB_FUSE);
        }
        else
        {
