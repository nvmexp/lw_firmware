...
RC TraceChannel::SetupAmpereA(LWGpuSubChannel* pSubch)
{
    RC rc;

    DebugPrintf("Setting up initial AmpereA Graphics on gpu %d:0\n",
    ...
    if (params->ParamPresent("-pagepool_size"))
    {
        ...
        CHECK_RC(pSubch->MethodWriteRC(LWC697_WAIT_FOR_IDLE, 0));
        CHECK_RC(pSubch->MethodWriteRC(LWC697_SET_MME_SHADOW_SCRATCH(0), 0));
        CHECK_RC(pSubch->MethodWriteRC(LWC697_SET_MME_SHADOW_SCRATCH(1), data));
        CHECK_RC(pSubch->MethodWriteRC(LWC697_SET_MME_SHADOW_SCRATCH(2), mask));
        CHECK_RC(FalconMethodWrite(pSubch, LWC697_SET_FALCON04, regHal.LookupAddress(MODS_PGRAPH_PRI_SCC_RM_PAGEPOOL), LWC697_NO_OPERATION));

        CHECK_RC(pSubch->MethodWriteRC(LWC697_WAIT_FOR_IDLE, 0));
        CHECK_RC(pSubch->MethodWriteRC(LWC697_SET_MME_SHADOW_SCRATCH(0), 0));
        CHECK_RC(pSubch->MethodWriteRC(LWC697_SET_MME_SHADOW_SCRATCH(1), data));
        CHECK_RC(pSubch->MethodWriteRC(LWC697_SET_MME_SHADOW_SCRATCH(2), mask));
        CHECK_RC(FalconMethodWrite(pSubch, LWC697_SET_FALCON04, regHal.LookupAddress(MODS_PGRAPH_PRI_GPCS_GCC_RM_PAGEPOOL), LWC697_NO_OPERATION));
    }

    if (params->ParamPresent("-coarse_shading"))
    {
        const UINT32 setShadingRateIndexSurfaceSizeA =
            DRF_NUM(C697, _SET_SHADING_RATE_INDEX_SURFACE_SIZE_A, _WIDTH, 0) |
            DRF_NUM(C697, _SET_SHADING_RATE_INDEX_SURFACE_SIZE_A, _HEIGHT, 0);

        const UINT32 setVariablePixelShadingControl =
            DRF_DEF(C697, _SET_VARIABLE_PIXEL_RATE_SHADING_CONTROL,
                _ENABLE, _TRUE);
        ...
        CHECK_RC(pSubch->MethodWriteRC(
                LWC697_SET_SHADING_RATE_INDEX_SURFACE_SIZE_A(0),
                setShadingRateIndexSurfaceSizeA));

        const auto numberOfViewports =
            sizeof (ampere_a_struct().SetVariablePixelRateShading) /
            sizeof (ampere_a_struct().SetVariablePixelRateShading[0]);

        for (auto i = decltype(numberOfViewports){0};
             i < numberOfViewports; ++i)
        {
            CHECK_RC(pSubch->MethodWriteRC(
                    LWC697_SET_VARIABLE_PIXEL_RATE_SHADING_CONTROL(i),
                    setVariablePixelShadingControl));
            CHECK_RC(pSubch->MethodWriteRC(
                    LWC697_SET_VARIABLE_PIXEL_RATE_SHADING_INDEX_TO_RATE_A(i),
                    setVariablePixelRateShadingIndexToRateA));
        }
    }

    DebugPrintf("Done setting up initial AmpereA Graphics state\n");
    return rc;
}
