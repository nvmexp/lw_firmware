...
////////////////////////////////////////////////////////////////////////////////
// TuringBlockLinearRemapperTestBase Implementation
////////////////////////////////////////////////////////////////////////////////

//! Constructor
TuringBlockLinearRemapperTest::TuringBlockLinearRemapperTest(bl_remapperTest& parent,
  ArgReader *params):
...
void TuringBlockLinearRemapperTest::SetupDev(void)
{
    ...
    if(!pGpuSubdev->GetPteKindFromName("GENERIC_MEMORY", &pitchSurfacePteKind))
        //if(!pGpuSubdev->GetPteKindFromName("S8", &pitchSurfacePteKind))            
    {
    ...
    // support for BX16 is now removed, now picking S8 instead
    // the following are the available kinds under Turing:
    // GENERIC_MEMORY, GENERIC_MEMORY_COMPRESSIBLE, GENERIC_MEMORY_COMPRESSIBLE_DISABLE_PLC, S8_COMPRESSIBLE_DISABLE_PLC
    // Z16_COMPRESSIBLE_DISABLE_PLC, S8Z24_COMPRESSIBLE_DISABLE_PLC, Z24S8_COMPRESSIBLE_DISABLE_PLC, ZF32_X24S8_COMPRESSIBLE_DISABLE_PLC
    if(!pGpuSubdev->GetPteKindFromName("S8", &blockSurfacePteKind))
        //if(!pGpuSubdev->GetPteKindFromName("GENERIC_MEMORY", &blockSurfacePteKind))        
    {
...
int TuringBlockLinearRemapperTest::SetupHostBLRemapper(int remapper, MdiagSurf &baseBuf,
...
void TuringBlockLinearRemapperTest::DisableRemapper(int remapper)
{
    // Enable the Remapper, and set the base address
...
//! Does a Block2Pitch TURING_DMA_COPY_A dma transfer.
//!
...
int TuringBlockLinearRemapperTest::DoBlock2PitchDma
(
    MdiagSurf &srcBuffer,
    MdiagSurf &dstBuffer,
    MdiagSurf &semaphore,
...
)
{
    ...
    for(UINT32 i = startZ ; i <= maxZ ; ++i)
    {
        ...
        status = status && m_testObject.getCh()->MethodWriteN(subChannel, LWC06F_SEMAPHOREA, 3, methodData, INCREMENTING);

        status = status && m_testObject.getCh()->MethodWrite(subChannel, LWC06F_SEMAPHORED,
                        DRF_NUM(C06F, _SEMAPHORED, _OPERATION, LWC06F_SEMAPHORED_OPERATION_ACQUIRE) |
                        DRF_NUM(C06F, _SEMAPHORED, _ACQUIRE_SWITCH, LWC06F_SEMAPHORED_ACQUIRE_SWITCH_DISABLED));

        // Set up the src offset
        offset = srcBuffer.GetCtxDmaOffsetGpu();
        methodData[0] = static_cast<UINT32>(offset >> 32);
        methodData[1] = static_cast<UINT32>(offset);
        status = status && m_testObject.getCh()->MethodWriteN(subChannel, LWC5B5_OFFSET_IN_UPPER,2,methodData, INCREMENTING);

        // Set up block linear source parameters
        // BlockSize, Width, Height, Depth, Layer
        //DebugPrintf(T_NAME"::DoBlock2PitchDma() - \n");

        methodData[0] = DRF_NUM(C5B5, _SET_SRC_BLOCK_SIZE, _WIDTH, m2mBlockSizeWidth) |
                        DRF_NUM(C5B5, _SET_SRC_BLOCK_SIZE, _HEIGHT, m2mBlockSizeHeight) |
                        DRF_NUM(C5B5, _SET_SRC_BLOCK_SIZE, _DEPTH, m2mBlockSizeDepth) |
                        DRF_NUM(C5B5, _SET_SRC_BLOCK_SIZE, _GOB_HEIGHT, LWC5B5_SET_DST_BLOCK_SIZE_GOB_HEIGHT_GOB_HEIGHT_FERMI_8);
        ...
        status = status && m_testObject.getCh()->MethodWriteN(subChannel, LWC5B5_SET_SRC_BLOCK_SIZE, 4, methodData, INCREMENTING);

        // Sets the layer (z coord) of the source surface if 3d surface
        status = status && m_testObject.getCh()->MethodWrite(subChannel, LWC5B5_SET_SRC_LAYER, z);

        // Set up Origin X bytes and Y samples
        // X is in pixels, y is in pixels
        methodData[0] = DRF_NUM(C5B5, _SET_SRC_ORIGIN, _X, x) |     // First pixel in x
                        DRF_NUM(C5B5, _SET_SRC_ORIGIN, _Y, y);      // First pixel in y
        status = status && m_testObject.getCh()->MethodWrite(subChannel, LWC5B5_SET_SRC_ORIGIN, methodData[0]);

        // Set line length in and number of lines
        UINT32 lineLength = imgPitch[remapper] - x*imgBpp[remapper];
        status = status && m_testObject.getCh()->MethodWrite(subChannel, LWC5B5_LINE_LENGTH_IN, lineLength);
        status = status && m_testObject.getCh()->MethodWrite(subChannel, LWC5B5_LINE_COUNT,  numLines);
        ...
        methodData[0] = static_cast<UINT32>(offset >> 32);
        methodData[1] = static_cast<UINT32>(offset);
        status = status && m_testObject.getCh()->MethodWriteN(subChannel, LWC5B5_OFFSET_OUT_UPPER,2,methodData, INCREMENTING);

        // Set up the Pitch Out
        status = status && m_testObject.getCh()->MethodWrite(subChannel, LWC5B5_PITCH_OUT, imgPitch[remapper]);
        ...
        status = status && m_testObject.getCh()->MethodWriteN(subChannel, LWC5B5_SET_SEMAPHORE_A,3,methodData, INCREMENTING);

        // start the DMA of src surface to dst surface
        status = status &&
        m_testObject.getCh()->MethodWrite(subChannel, LWC5B5_LAUNCH_DMA,
            DRF_NUM(C5B5, _LAUNCH_DMA, _DATA_TRANSFER_TYPE,     LWC5B5_LAUNCH_DMA_DATA_TRANSFER_TYPE_NON_PIPELINED) |
            DRF_NUM(C5B5, _LAUNCH_DMA, _FLUSH_ENABLE,           LWC5B5_LAUNCH_DMA_FLUSH_ENABLE_TRUE) |
            DRF_NUM(C5B5, _LAUNCH_DMA, _SEMAPHORE_TYPE,         (z == maxZ) ? LWC5B5_LAUNCH_DMA_SEMAPHORE_TYPE_RELEASE_ONE_WORD_SEMAPHORE : LWC5B5_LAUNCH_DMA_SEMAPHORE_TYPE_NONE) |
            DRF_NUM(C5B5, _LAUNCH_DMA, _INTERRUPT_TYPE,         LWC5B5_LAUNCH_DMA_INTERRUPT_TYPE_NONE) |
            DRF_NUM(C5B5, _LAUNCH_DMA, _SRC_MEMORY_LAYOUT,      LWC5B5_LAUNCH_DMA_SRC_MEMORY_LAYOUT_BLOCKLINEAR) |
...
int TuringBlockLinearRemapperTest::DoPitch2BlockDma
(
    MdiagSurf &srcBuffer,
    MdiagSurf &dstBuffer,
    MdiagSurf &semaphore,
...
)
{
    ...
    // Set up the Pitch In
    status = status && m_testObject.getCh()->MethodWrite(subChannel, LWC5B5_PITCH_IN, imgPitch[remapper]);
    ...
    // Set up block linear source parameters
    // BlockSize, Width, Height, Depth, Layer
    methodData[0] = DRF_NUM(C5B5, _SET_DST_BLOCK_SIZE, _WIDTH, m2mBlockSizeWidth) |
                    DRF_NUM(C5B5, _SET_DST_BLOCK_SIZE, _HEIGHT, m2mBlockSizeHeight) |
                    DRF_NUM(C5B5, _SET_DST_BLOCK_SIZE, _DEPTH, m2mBlockSizeDepth) |
                    DRF_NUM(C5B5, _SET_DST_BLOCK_SIZE, _GOB_HEIGHT, LWC5B5_SET_DST_BLOCK_SIZE_GOB_HEIGHT_GOB_HEIGHT_FERMI_8);
    methodData[1] = imgPitch[remapper]; // Width in bytes of source
    methodData[2] = imgHeight[remapper]; // Height in Pixels of source
    methodData[3] = imgDepth[remapper]; // Depth i Pixels of source
    status = status && m_testObject.getCh()->MethodWriteN(subChannel, LWC5B5_SET_DST_BLOCK_SIZE, 4, methodData, INCREMENTING);

    // Sets the layer (z coord) of the source surface if 3d surface
    status = status && m_testObject.getCh()->MethodWrite(subChannel, LWC5B5_SET_DST_LAYER, z);

    // Set up Origin X bytes and Y samples
    // X is in pixels, y is in pixels
    methodData[0] = DRF_NUM(C5B5, _SET_DST_ORIGIN, _X, x) |     // First pixel in x
                    DRF_NUM(C5B5, _SET_DST_ORIGIN, _Y, y);      // First pixel in y
    status = status && m_testObject.getCh()->MethodWrite(subChannel, LWC5B5_SET_DST_ORIGIN, methodData[0]);

    // Set line length in and number of lines
    status = status && m_testObject.getCh()->MethodWrite(subChannel, LWC5B5_LINE_LENGTH_IN, imgPitch[remapper]-x*imgBpp[remapper]);
    status = status && m_testObject.getCh()->MethodWrite(subChannel, LWC5B5_LINE_COUNT, numLines);
    ...
    m_testObject.getCh()->MethodWrite(subChannel, LWC5B5_LAUNCH_DMA,
        ...
        DRF_NUM(C5B5, _LAUNCH_DMA, _DST_MEMORY_LAYOUT,      LWC5B5_LAUNCH_DMA_DST_MEMORY_LAYOUT_BLOCKLINEAR) |
        DRF_NUM(C5B5, _LAUNCH_DMA, _MULTI_LINE_ENABLE,      LWC5B5_LAUNCH_DMA_MULTI_LINE_ENABLE_TRUE) |
        DRF_NUM(C5B5, _LAUNCH_DMA, _REMAP_ENABLE,           LWC5B5_LAUNCH_DMA_REMAP_ENABLE_FALSE) | 
        DRF_NUM(C5B5, _LAUNCH_DMA, _SRC_BYPASS_L2,          LWC5B5_LAUNCH_DMA_SRC_BYPASS_L2_USE_PTE_SETTING) |
        DRF_NUM(C5B5, _LAUNCH_DMA, _DST_BYPASS_L2,          LWC5B5_LAUNCH_DMA_DST_BYPASS_L2_USE_PTE_SETTING) |
...
void TuringBlockLinearRemapperTest::BackupHostBlRegisters(void)
{
...
void TuringBlockLinearRemapperTest::RestoreHostBlRegisters(void)
{
...
int TuringBlockLinearRemapperTest::ParseParameters(void)
{
