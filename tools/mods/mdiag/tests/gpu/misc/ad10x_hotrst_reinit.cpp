...
extern const ParamDecl ad10x_hotrst_reinit_params[] = {
...
Ad10xHotRstReInit::Ad10xHotRstReInit(ArgReader *params) : LWGpuSingleChannelTest(params)
{
...
Ad10xHotRstReInit::~Ad10xHotRstReInit(void)
{
}

STD_TEST_FACTORY(ad10x_hotrst_reinit, Ad10xHotRstReInit)

int Ad10xHotRstReInit::Setup(void) 
{

    int returlwal = SetupGpuPointers();

    getStateReport()->init("ad10x_hotrst_reinit");
    getStateReport()->enable();

    return returlwal;
}

int Ad10xHotRstReInit::SetupGpuPointers(void)
{
    ...
    if(!ch)
    {
        ErrPrintf("Ad10xHotRstReInit::Setup failed to create channel\n");
        return 0;
    }
    ...
    if(!m_regMap)
    {
        ErrPrintf("Ad10xHotRstReInit::Setup: can't get a register map from the GPU!\n");
        return 0;
    }
...
void Ad10xHotRstReInit::CleanUp(void)
{
    if(ch)
    {
        delete ch;
    ...
    if (lwgpu) 
    {
        DebugPrintf("Ad10xHotRstReInit::CleanUp(): Releasing GPU.\n");
        lwgpu = 0;
    }
    LWGpuSingleChannelTest::CleanUp();
}

bool Ad10xHotRstReInit::GetInitValue(IRegisterClass* reg, UINT32 testAddr, UINT32 *initVal,UINT32 *initMask)
{
...
bool  Ad10xHotRstReInit::check_results( UINT32 address, UINT32 expected_data)
{
    UINT32 read_data = 0x0;
...
void Ad10xHotRstReInit::Run(void) 
{
...
void Ad10xHotRstReInit::disable_sbr_on_hotrst(void)
{
    UINT32 address = 0x0;
    UINT32 wdata = 0x0;
    UINT32 init_val = 0x0;
    ...
    InfoPrintf("Ad10xHotRstReInit::disable_sbr_on_hotrst done!\n");

    lwgpu->GetGpuSubdevice()->RegWr32(address,wdata);
}

void Ad10xHotRstReInit::enable_gpu_hot_rst(void)
{
    UINT32 address = 0x0;
    UINT32 wdata = 0x0;
...
void Ad10xHotRstReInit::enable_cfg_hot_rst(void)
{
    UINT32 address = 0x0;
    UINT32 wdata = 0x0;
...
void Ad10xHotRstReInit::disable_gpu_hot_rst(void)
{
    UINT32 address = 0x0;
    UINT32 wdata = 0x0;
    UINT32 init_val = 0x00000100;
...
void Ad10xHotRstReInit::disable_cfg_hot_rst(void)
{
    UINT32 address = 0x0;
    UINT32 wdata = 0x0;
    UINT32 init_val = 0x00000101;
...
void Ad10xHotRstReInit::disable_fundamental_reset_on_hot_rst (void)
{
...
void Ad10xHotRstReInit::corrupt_registers(void)
{
    ...
    address = LW_PBUS_SW_INTR_1;
    lwgpu->GetGpuSubdevice()->RegWr32(address,wdata);

    address = LW_PBUS_SW_INTR_2;
    lwgpu->GetGpuSubdevice()->RegWr32(address,wdata);

    address = LW_PBUS_SW_INTR_3;
    lwgpu->GetGpuSubdevice()->RegWr32(address,0x00020000);

    wdata = 0x00060705;
    address = LW_FUSE_FUSETIME_RD1;
    lwgpu->GetGpuSubdevice()->RegWr32(address,wdata);

    Platform::Delay(100);
}

bool Ad10xHotRstReInit::check_register_reset(void)
{
    InfoPrintf("Checking corrupted registers got reset...\n");

    if (!check_results(LW_PBUS_SW_INTR_1, 0x0)) return false;
    if (!check_results(LW_PBUS_SW_INTR_2, 0x0)) return false;
    if (!check_results(LW_PBUS_SW_INTR_3, 0x0)) return false;

    return true;
}

bool Ad10xHotRstReInit::check_register_not_reset(void)
{
    InfoPrintf("Checking corrupted registers didn't get reset...\n");

    if (!check_results(LW_PBUS_SW_INTR_1, 0xdeadbeef)) return false;
    if (!check_results(LW_PBUS_SW_INTR_2, 0xdeadbeef)) return false;
    if (!check_results(LW_PBUS_SW_INTR_3, 0x00020000)) return false;
    if (!check_results(LW_FUSE_FUSETIME_RD1,0x00060705)) return false;

    return true;

}

void Ad10xHotRstReInit::hot_reset(void)
{
...
UINT32 Ad10xHotRstReInit::gpu_initialize(void)
{
    GpuPtr pGpu;
    JavaScriptPtr pJs;
    RC     rc;
...
UINT32 Ad10xHotRstReInit::testhotrst1()
{
    ...
    if(!check_register_reset()) errCnt++;

    m_reg = m_regMap->FindRegister("LW_XVE_PRIV_MISC");
    MASSERT(m_reg);
    GetInitValue(m_reg.get(), UINT32(m_reg->GetAddress()), &m_initVal,&m_initMask);
    //XVE_PRIV does not get reset:
    wdata = 0x00000100| (0x00000001 << (0 ? LW_XVE_PRIV_MISC_CYA_RESET_GPU_HOT_RESET));
    address = LW_XVE_PRIV_MISC_CYA_RESET_GPU + DEVICE_BASE(LW_PCFG);
    if (!check_results(address,wdata)) {errCnt++;}

    address = LW_XVE_PRIV_XV_0_CYA_RESET_CFG + DEVICE_BASE(LW_PCFG);
    wdata = 0x00000101  | (0x00000001 << (0 ? LW_XVE_PRIV_XV_0_CYA_RESET_CFG_HOT_RESET));
...
UINT32 Ad10xHotRstReInit::testhotrst2()
{
    GpuSubdevice * pSubdev;
    RC             rc;
    ...
    // Revert SBR to gk10x behavior in GK110 & GK20a
    //UINT32 arch = lwgpu->GetArchitecture();
    //if((arch == Gpu::GK110) || (arch == Gpu::GK208) ||(arch == Gpu::GK106))
    //{
    //  disable_sbr_on_hotrst();
    //}
    ...
    if(!check_register_not_reset()) errCnt++;
...
UINT32 Ad10xHotRstReInit::testhotrst3()
{
...
UINT32 Ad10xHotRstReInit::testhotrst4()
{
    GpuSubdevice * pSubdev;
    RC             rc;
