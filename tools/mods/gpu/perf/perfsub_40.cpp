...
static const UINT32 PerfLimitsIDs[] =
{
    ...
    LW2080_CTRL_PERF_LIMIT_CLIENT_LOW_STRICT_XBAR_MIN,
    LW2080_CTRL_PERF_LIMIT_CLIENT_LOW_STRICT_XBAR_MAX,
    ...
    LW2080_CTRL_PERF_LIMIT_CLIENT_STRICT_XBAR_MIN,
    LW2080_CTRL_PERF_LIMIT_CLIENT_STRICT_XBAR_MAX,
    ...
    LW2080_CTRL_PERF_LIMIT_MODS_RULES_STRICT_XBAR_MIN,
    LW2080_CTRL_PERF_LIMIT_MODS_RULES_STRICT_XBAR_MAX,
...
Perf40::Perf40(GpuSubdevice *pSubdevice, bool owned) :
...
Perf40::~Perf40()
{
    ...
    for (UINT32 vfSwitchType = VfSwitchVal::Mods; vfSwitchType < VfSwitchVal::NumTypes; ++vfSwitchType)
    {
        MASSERT(vfSwitchType < VfSwitchVal::NumTypes);
...
RC Perf40::Cleanup()
{
    ...
    // ClearForcedPState() which is obsolete/unsupported in Perf40.
    return rc;
}

RC Perf40::GetVfSwitchTimePriority(UINT32* priority)
{
    *priority = m_VfSwitchTimePriority;
    return OK;
}

RC Perf40::SetVfSwitchTimePriority(UINT32 priority)
{
    m_VfSwitchTimePriority = static_cast<Tee::Priority>(priority);
    return OK;
}

RC Perf40::GetCoreVoltageMv(UINT32 *pVoltage)
{
    // TODO - Initialize Volt3x
    if (m_pSubdevice->HasBug(2864561))
        return RC::OK;
...
RC Perf40::SetCoreVoltageMv(UINT32 Voltage, UINT32 PStateNum/*= ILWALID_PSTATE*/)
{
    return RC::UNSUPPORTED_FUNCTION;
}

RC Perf40::SetVoltage(Gpu::VoltageDomain Domain, UINT32 VoltMv, UINT32 PState)
{
    return RC::UNSUPPORTED_FUNCTION;
}

RC Perf40::PrintAllPStateInfo()
{
    return RC::UNSUPPORTED_FUNCTION;
}

//TODO: Replace the function with Status MODSGA10X-1467
RC Perf40::GetClkPropRegimesInfo(LW2080_CTRL_CLK_CLK_PROP_REGIMES_INFO* pClkPropRegimeInfo) const
{
    MASSERT(pClkPropRegimeInfo);
    pClkPropRegimeInfo->super = m_CachedRmInfo.clkPropRegimes.super;
    return LwRmPtr()->ControlBySubdevice(
        m_pSubdevice
       ,LW2080_CTRL_CMD_CLK_CLK_PROP_REGIMES_GET_INFO
       ,pClkPropRegimeInfo
       ,sizeof(*pClkPropRegimeInfo));
}

RC Perf40::GetClockAtPStateNoCache(UINT32 PStateNum,
                                   ...
                                   UINT64 *pFreqHz)
{
    ...
    if (clkIndex == (std::numeric_limits<UINT32>::max)())
    {
        Printf(Tee::PriError, "Invalid clock domain %s\n",
    ...
    UINT64 factor = (Domain == Gpu::ClkPexGen) ? 1 : 1000;
...
RC Perf40::GetPexSpeedAtPState(UINT32 PStateNum, UINT32 *pLinkSpeed)
{
    RC rc;
    ...
    {
        Printf(Tee::PriError, "Invalid clock domain %s\n",
            PerfUtil::ClkDomainToStr(Gpu::ClkPexGen));
        return RC::ILWALID_CLOCK_DOMAIN;
    }
    ...
    UINT32 genNumber = clkEntryInfo.nom.origFreqkHz;
    switch (genNumber)
    {
        case 5:
...
RC Perf40::GetLinkWidthAtPState(UINT32 PStateNum, UINT32 *pLinkWidth)
{
    RC rc;
...
RC Perf40::GetVdtToJs(JSContext *pCtx, JSObject *pJsObj)
{
    return RC::UNSUPPORTED_FUNCTION;
}

RC Perf40::CheckSplitRailDomainExists(Gpu::SplitVoltageDomain voltDom) const
{
    RC rc;
...
RC Perf40::SetPStateOffsetMv(UINT32 PStateNum, INT32 OffsetMv)
{
    return RC::UNSUPPORTED_FUNCTION;
}

RC Perf40::SetRailVoltTuningOffsetuV(Gpu::SplitVoltageDomain Rail, INT32 OffsetuV)
{
    RC rc;

    CHECK_RC(ValidateVoltRail(Rail));

    RC tempRC;
    if ((tempRC = Perf40::CheckSplitRailDomainExists(Rail)) != OK)
    {
    ...
    Volt3x* pVolt3x = m_pSubdevice->GetVolt3x();
    UINT32 voltRailIdx = pVolt3x->GpuSplitVoltageDomainToRailIdx(Rail);
    m_RailVoltTuningOffsetsuV[voltRailIdx] = OffsetuV;
    return SetVoltTuningParams(writeIfPStateLocked);
}

RC Perf40::SetRailClkDomailwoltTuningOffsetuV
(
...
RC Perf40::ValidateClkDomain(Gpu::ClkDomain clkDom)
{
    ...
    {
        Printf(Tee::PriError, "ClockDomain: %s is not supported\n",
...
RC Perf40::SetFreqClkDomainOffsetkHz(Gpu::ClkDomain ClkDomain, INT32 FrequencyInkHz)
{
    RC rc;
...
RC Perf40::SetFreqClkDomainOffsetPercent(Gpu::ClkDomain ClkDomain, FLOAT32 percent)
{
    RC rc;
...
RC Perf40::GetClkProgEntriesMHz
(
    UINT32 pstateNum,
...
namespace
{
    // Returns a clock VF Rel entry index and the offset that must be
    // applied to achieve targetFreqkHz
    // Reuse this Clock Programming Entry map as the data being fetched
    // is same even from Clock VF relationship
    void GetClosestVFrelEntryOffsetkHz
    (
...
RC Perf40::ConfigureDramclkSettings(UINT32 pstateNum, const ClkSetting& dramclkSetting)
{
    RC rc;
    ...
    // Cannot adjust clock VF Rel entries if dramclk is fixed
    ...
    // Get all the VF Rel entries for this pstate. These are
    // the entries that are eligible to be offset.
    // Reuse this Clock Programming Entry map as the data being fetched
    // is same even from Clock VF relationship
    map<LwU8, Perf::clkProgEntry> pstateProgEntries;
    CHECK_RC(GetVfRelEntriesMHz(pstateNum, dramclkSetting.Domain, ProgEntryOffset::DISABLED,
                                  &pstateProgEntries));

    std::pair<LwU8, INT32> closestPStateProgEntryOffsetkHz;
    GetClosestVFrelEntryOffsetkHz(pstateProgEntries, dramclkSetting.FreqHz / 1000,
    ...
    if (!m_OverrideOVOC)
    {
        // Remove all entries derived from VF rel that will be unable to
    ...
    // Get all the clock VF Relationship entries for this domain. These will
    // be used to sanity check user input.
    // Reuse this Clock Programming Entry map as the data being fetched
    // is same even from Clock VF relationship
    map<LwU8, Perf::clkProgEntry> allProgEntries;
    CHECK_RC(GetVfRelEntriesMHz(ILWALID_PSTATE, dramclkSetting.Domain, ProgEntryOffset::DISABLED,
                                  &allProgEntries));
    std::pair<LwU8, INT32> closestEntryOffsetkHz;
    GetClosestVFrelEntryOffsetkHz(allProgEntries, dramclkSetting.FreqHz / 1000,
...
RC Perf40::ClearDramclkProgEntryOffsets()
{
    RC rc;
    ...
    for (const auto& progItr : m_AdjustedDramclkProgEntries)
    {
        if (progItr.offsetkHz)
        {
            ...
            CHECK_RC(SetClkVfRelationshipOffsetkHz(progItr.index, 0));
        }
    }
...
RC Perf40::SetClkVfRelationshipOffsetkHz(UINT32 RelIdx, INT32 FrequencyInkHz)
{
    RC rc;

    LW2080_CTRL_CLK_CLK_VF_RELS_CONTROL clkVfRelsControl;
    memset(&clkVfRelsControl, 0, sizeof(clkVfRelsControl));
    BoardObjGrpMaskBitSet(&clkVfRelsControl.super.objMask.super, RelIdx);

    CHECK_RC(LwRmPtr()->ControlBySubdevice(
        m_pSubdevice
        ,LW2080_CTRL_CMD_CLK_CLK_VF_RELS_GET_CONTROL
        ,&clkVfRelsControl
        ,sizeof(clkVfRelsControl)));

    PLW2080_CTRL_CLK_FREQ_DELTA pClkVfRelsFreqDelta =
        &clkVfRelsControl.vfRels[RelIdx].delta.freqDelta;

    Printf(Tee::PriLow, "Offsetting clock vf relationship entry %u by %dkHz\n",
        RelIdx, FrequencyInkHz);

    pClkVfRelsFreqDelta->type = LW2080_CTRL_CLK_CLK_FREQ_DELTA_TYPE_STATIC;
    pClkVfRelsFreqDelta->data.staticOffset.deltakHz = FrequencyInkHz;
...
)
{
    ...
    LW2080_CTRL_BOARDOBJGRP_MASK_E32_FOR_EACH_INDEX(ii, &m_CachedRmInfo.clkDomains.super.objMask.super)
        ...
        {
            const LW2080_CTRL_CLK_CLK_DOMAIN_INFO_40_PROG& infoEntry =
                m_CachedRmInfo.clkDomains.domains[ii].data.v40Prog;
            ...
            // Retrieve the clock VF rel status
            LW2080_CTRL_CLK_CLK_VF_RELS_STATUS vfPointsStatus = {};
            // RailIdx
            INT32 railIdx = Utility::BitScanForward(infoEntry.railMask);
            MASSERT(railIdx >= 0);
            for (UINT32 jj = infoEntry.railVfItem[railIdx].data.master.clkVfRelIdxFirst;
                 jj <= infoEntry.railVfItem[railIdx].data.master.clkVfRelIdxLast; jj++)
            {
                BoardObjGrpMaskBitSet(&vfPointsStatus.super.objMask.super, jj);
            }
            CHECK_RC(LwRmPtr()->ControlBySubdevice(
                m_pSubdevice
            ...
            // This approach is similar to getting entries through
            // Clock Programming Table in PS3.5
            // Clock VF Relationship entries come in two flavors:
            ...
            // clock VF rel entry set by VBIOS. RM always adds the factory
            ...
            for (UINT32 jj = infoEntry.railVfItem[railIdx].data.master.clkVfRelIdxFirst;
                 jj <= infoEntry.railVfItem[railIdx].data.master.clkVfRelIdxLast; jj++)
            {
                const auto& clkVfRelInfo = m_CachedRmInfo.clkVfRels.vfRels[jj];
                const auto& clkVfRelStatus = vfPointsStatus.vfRels[jj];
                ...
                switch (clkVfRelInfo.type)
                {
                    ...
                    case LW2080_CTRL_CLK_CLK_VF_REL_TYPE_TABLE_FREQ:
                        baseEntryMHz = clkVfRelInfo.freqMaxMHz;
                        dynamicEntryMHz = clkVfRelStatus.offsettedFreqMaxMHz;
                        overclockingEnabled = (clkVfRelInfo.bOCOVEnabled == LW_TRUE);
                        break;
                    default:
                        MASSERT(!"Unknown or invalid clock VF rel entry type");
                        return RC::SOFTWARE_ERROR;
                }
                ...
                // is always enabled for every entry.
            ...
            {
                Printf(Tee::PriError,
                    "Cannot find pstate %u clock VF Rel entries for %s\n",
                    pstateNum,
                    PerfUtil::ClkDomainToStr(clkDom));
                return RC::SOFTWARE_ERROR;
            }
    ...
    Printf(Tee::PriError, "Could not find any VF Rel entries for %s\n",
        PerfUtil::ClkDomainToStr(clkDom));
    return RC::ILWALID_CLOCK_DOMAIN;
}

RC Perf40::GetClkProgrammingTableSlaveRatio
(
    UINT32  tableIdx,
...
RC Perf40::SetClkProgrammingTableSlaveRatio
(
    UINT32 tableIdx,
    UINT32 slaveEntryIdx,
    UINT08 ratio
...
RC Perf40::SetMaskForModifyVFPoints
(
    VFPointType Type,
    LW2080_CTRL_CLK_CLK_VF_POINTS_CONTROL* pClkVFPointsControl,
...
{
    RC rc;
    memset(pClkVFPointsControl, 0, sizeof(*pClkVFPointsControl));
    if (TableIdxStart >= LW2080_CTRL_BOARDOBJGRP_E512_MAX_OBJECTS || TableIdxEnd >=
        LW2080_CTRL_BOARDOBJGRP_E512_MAX_OBJECTS || TableIdxStart > TableIdxEnd)
    {
        Printf(Tee::PriError, "VF Point index Range {Start, End} = {%d, %d} is invalid \n",
        TableIdxStart, TableIdxEnd);
        return RC::BAD_COMMAND_LINE_ARGUMENT;
    }

    if (lwrveIdx > LW2080_CTRL_CLK_CLK_VF_REL_VF_LWRVE_IDX_MAX)
    {
        Printf(Tee::PriError, "VF lwrve Idx = {%d} is greater than the max supported VF lwrve Idx = %d \n",
        lwrveIdx, LW2080_CTRL_CLK_CLK_VF_REL_VF_LWRVE_IDX_MAX);
        return RC::BAD_COMMAND_LINE_ARGUMENT;
    }
    /*
        The LW2080_CTRL_CLK_CLK_VF_POINTS_CONTROL stores the secondary VF lwrves in an array where
        0 - 127 denotes first secondary VF lwrve, 128 - 255 next secondary VF lwrve and so on
    */
    UINT32 secVfLwrveIdx = 0;
    if (lwrveIdx == LW2080_CTRL_CLK_CLK_VF_REL_VF_LWRVE_IDX_PRI)
    {
        for (UINT32 idx = TableIdxStart; idx <= TableIdxEnd; idx++)
        {
            BoardObjGrpMaskBitSet(&pClkVFPointsControl->super.objMask.super, idx);
        }
    }
    else
    {
        /*lwrveIdx has valid values from 1 to LW2080_CTRL_CLK_CLK_VF_REL_VF_LWRVE_IDX_SEC_0*/
        if (m_SecondaryVFPointsSizes.size() <= lwrveIdx)
        {
            Printf(Tee::PriError, "VF Lwrve with index = %d is unavailable \n", lwrveIdx);
            return RC::BAD_COMMAND_LINE_ARGUMENT;
        }
        secVfLwrveIdx = m_SecondaryVFPointsSizes[lwrveIdx - 1];
        TableIdxStart = secVfLwrveIdx + TableIdxStart;
        TableIdxEnd = secVfLwrveIdx + TableIdxEnd;
        for (UINT32 idx = TableIdxStart; idx <= TableIdxEnd; idx++)
        {
            BoardObjGrpMaskBitSet(&pClkVFPointsControl->sec.super.objMask.super, idx);
        }
    }
    ...
    CHECK_RC(LwRmPtr()->ControlBySubdevice(
        ...
        ,pClkVFPointsControl
        ,sizeof(*pClkVFPointsControl)));

    for (UINT32 idx = TableIdxStart; idx <= TableIdxEnd; idx++)
    {
        if (lwrveIdx == LW2080_CTRL_CLK_CLK_VF_REL_VF_LWRVE_IDX_PRI)
        {
            if (GetVFPointType(pClkVFPointsControl->vfPoints[idx].type) != Type)
            {
                Printf(Tee::PriError,
        ...
        {
            if (GetVFPointType(pClkVFPointsControl->sec.vfPoints[idx].type) != Type)
            {
                Printf(Tee::PriError,
                    "SetVFPointsOffset error: VF point index %d is not of expected type\n",
                    idx);
...
)
{
    ...
    CHECK_RC(SetMaskForModifyVFPoints(Type, &clkVFPointsControl, TableIdxStart, TableIdxEnd, lwrveIdx));
    ...
    if (lwrveIdx == LW2080_CTRL_CLK_CLK_VF_REL_VF_LWRVE_IDX_PRI)
    {
        switch (clkVFPointsControl.vfPoints[TableIdxStart].type)
        {
            case LW2080_CTRL_CLK_CLK_VF_POINT_TYPE_40_VOLT_PRI:
            {
                ...
                for (UINT32 idx = TableIdxStart; idx <= TableIdxEnd; idx++)
                {
                    PLW2080_CTRL_CLK_FREQ_DELTA pClkVFPointsCtrlFreqDelta =
                        &clkVFPointsControl.vfPoints[idx].data.v40Volt.freqDelta;
            ...
            case LW2080_CTRL_CLK_CLK_VF_POINT_TYPE_40_FREQ:
            {
                ...
                for (UINT32 idx = TableIdxStart; idx <= TableIdxEnd; idx++)
                {
                    if (clkVFPointsControl.vfPoints[idx].data.v40Freq.voltDeltauV != Value)
                    {
                        changeDetected = true;
                        clkVFPointsControl.vfPoints[idx].data.v40Freq.voltDeltauV = Value;
                    }
                }
                if (!changeDetected)
                {
            ...
            default:
                Printf(Tee::PriError, "SetVFPointsOffset error: unrecognized type = %d\n",
                    clkVFPointsControl.vfPoints[TableIdxStart].type);
                return RC::SOFTWARE_ERROR;
        }
    }
    else
    {
        switch (clkVFPointsControl.sec.vfPoints[TableIdxStart].type)
        {
            case LW2080_CTRL_CLK_CLK_VF_POINT_TYPE_40_VOLT_SEC:
            {
                ...
                {
                    PLW2080_CTRL_CLK_FREQ_DELTA pClkVFPointsCtrlFreqDelta =
                        &clkVFPointsControl.sec.vfPoints[idx].data.v40VoltSec.super.freqDelta;
            ...
            default:
                Printf(Tee::PriError, "SetVFPointsOffset error: unrecognized type = %d\n",
                    clkVFPointsControl.sec.vfPoints[TableIdxStart].type);
                return RC::SOFTWARE_ERROR;
        }
    }
...
RC Perf40::SetVFPointsOffsetkHz
(
    UINT32 TableIdxStart,
    UINT32 TableIdxEnd,
...
)
{
    return SetVFPointsOffset(VFPointType::Volt,
        TableIdxStart, TableIdxEnd, FrequencyInkHz, lwrveIdx);
}

RC Perf40::SetVFPointsOffsetuV
(
    UINT32 TableIdxStart,
    UINT32 TableIdxEnd,
    INT32 OffsetInuV
...
RC Perf40::SetVFPointsOffsetDvcoCode
(
    UINT32 TableIdxStart,
    UINT32 TableIdxEnd,
    UINT32 dvcoOffsetCode,
    UINT32 lwrveIdx
)
{
    RC rc;
    LW2080_CTRL_CLK_CLK_VF_POINTS_CONTROL clkVFPointsControl;
    CHECK_RC(SetMaskForModifyVFPoints(VFPointType::Volt, &clkVFPointsControl, TableIdxStart, TableIdxEnd, lwrveIdx));
    if (lwrveIdx < LW2080_CTRL_CLK_CLK_VF_REL_VF_LWRVE_IDX_SEC_0)
    {
        Printf(Tee::PriError, "Overriding DVCO offset code is supported only for secondary VF lwrves \n");
        return RC::BAD_COMMAND_LINE_ARGUMENT;
    }
    switch (clkVFPointsControl.sec.vfPoints[TableIdxStart].type)
    {
        case LW2080_CTRL_CLK_CLK_VF_POINT_TYPE_40_VOLT_SEC:
        {
            ...
            for (UINT32 idx = TableIdxStart; idx <= TableIdxEnd; idx++)
            {
                PLW2080_CTRL_CLK_CLK_VF_POINT_CONTROL_40_VOLT_SEC pClkVFPointSecVfCtrl =
                    &clkVFPointsControl.sec.vfPoints[idx].data.v40VoltSec;

                if (pClkVFPointSecVfCtrl->dvcoOffsetCodeOverride != dvcoOffsetCode)
                {
                    changeDetected = true;
                    pClkVFPointSecVfCtrl->dvcoOffsetCodeOverride = dvcoOffsetCode;
                }
            }
            if (!changeDetected)
                return RC::OK;
        ...
        default:
            Printf(Tee::PriNormal, "SetVFPointsOffsetDvcoCode error: unrecognized type\n");
            return RC::SOFTWARE_ERROR;
    }
...
// Unsupported in this subclass
RC Perf40::SetVdtTuningOffsetMv(INT32 OffsetMv)
{
    return RC::UNSUPPORTED_FUNCTION;
}

RC Perf40::GetVoltageAtPStateNoCache
(
    UINT32 PStateNum,
...
RC Perf40::GetLwrrentPerfValuesNoCache(PStateInfos *pPstatesInfo)
{
    Printf(Tee::PriLow, "Perf40::GetLwrrentPerfValeusNoCache\n");
    RC rc;
    LW2080_CTRL_PERF_PSTATES_STATUS pstatesStatus = {};
    pstatesStatus.super = m_CachedRmInfo.pstates.super;
    CHECK_RC(LwRmPtr()->ControlBySubdevice(
...
RC Perf40::SetPStateTableInternal
(
    UINT32 pstateNum,
...
)
{
    ...
    for (auto& clkItr : clks)
    {
        ...
        if (rmClockIndex == (std::numeric_limits<UINT32>::max)())
        {
            Printf(Tee::PriError, "Invalid clock domain %s\n",
                PerfUtil::ClkDomainToStr(dom));
            return RC::ILWALID_CLOCK_DOMAIN;
...
RC Perf40::InjectPerfPoint(const InjectedPerfPoint &point)
{
    // TODO - Initialize Volt3x
    if (m_pSubdevice->HasBug(2864561))
        return RC::OK;
...
RC Perf40::GetRmPStateIndex(UINT32 pstateNum, UINT32 *rmPStateIndex) const
{
...
RC Perf40::GetRmPStateLevel(UINT32 pstateNum, UINT32 *rmPStateLevel) const
{
...
RC Perf40::GetClkDomainsControl()
{
    if (!m_InitDone)
        return RC::SOFTWARE_ERROR;
...
RC Perf40::SetClkDomainsControl()
{
    if (!m_InitDone)
        return RC::SOFTWARE_ERROR;
...
RC Perf40::SetVoltTuningParams(VoltTuningWriteMode m)
{
    // TODO - Initialize Volt3x
    if (m_pSubdevice->HasBug(2864561))
        return RC::OK;
    ...
    while (voltRailMask)
    {
        ...
        CHECK_RC(ValidateVoltRail(pVolt3x->RailIdxToGpuSplitVoltageDomain(railIdx)));
    ...
    for (const auto& clkDom : m_ClockDomains)
    {
        ...
        {
            Printf(Tee::PriError, "Invalid clock domain %s\n",
                PerfUtil::ClkDomainToStr(clkDom.first));
            return RC::ILWALID_CLOCK_DOMAIN;
        }
        ...
        auto& deltaCtrl = m_ClkDomainsControl.domains[clkIdx].data.v40Prog.deltas.freqDelta;
    ...
    for (const auto& percentOffset : m_FreqClkPercentOffsets)
    {
        ...
        {
            Printf(Tee::PriError, "Invalid clock domain %s\n",
                PerfUtil::ClkDomainToStr(percentOffset.first));
            return RC::ILWALID_CLOCK_DOMAIN;
        }
    ...
    for (const auto& clkDom : m_ClockDomains)
    {
        clkObjIdx = GetClkDomainClkObjIndex(clkDom.first);
        if (clkObjIdx == (std::numeric_limits<UINT32>::max)())
        {
            Printf(Tee::PriError, "Invalid clock domain %s\n",
                PerfUtil::ClkDomainToStr(clkDom.first));
            return RC::ILWALID_CLOCK_DOMAIN;
        ...
        while (voltRailMask)
        {
            ...
            INT32 offsetuV = 0;
            if (m_RailClkDomailwoltTuningOffsetsuV[voltDom].find(clkObjIdx) !=
                m_RailClkDomailwoltTuningOffsetsuV[voltDom].end())
            {
                offsetuV = m_RailClkDomailwoltTuningOffsetsuV[voltDom][clkObjIdx];
            }
            else
            {
                continue;
            }
            if (m_ClkDomainsControl.domains[clkObjIdx].data.v3xProg.deltas.voltDeltauV[railIdx] != offsetuV)
            {
                m_ClkDomainsControl.domains[clkObjIdx].data.v3xProg.deltas.voltDeltauV[railIdx] = offsetuV;
                changeDetected = true;
            }
        }
    }
        ...
        ((writeIfPStateLocked == m) && (GetPStateLockType() == NotLocked)))
    {
        if (!changeDetected)
        {
            Printf(Tee::PriLow, "No Change Detected when trying to apply any Offsets to the Clock Domains \n");
        }
        if (GetPStateLockType() == NotLocked)
        {
            Printf(Tee::PriLow, " PState is not locked, so no offsets can be applied to Clock Domains \n");
        }
        return OK;
    }
...
RC Perf40::SetOverrideOVOC(bool override)
{
    m_OverrideOVOC = override;
    return OK;
}

RC Perf40::ApplyFreqTuneOffset(INT32 freqOffsetKhz, UINT32 pstateNum)
{
    return RC::UNSUPPORTED_FUNCTION;
}

RC Perf40::ParseVfTable()
{
    RC rc;

    Printf(Tee::PriLow,
           "Clock VF relationship mask = 0x%x\n"
           "slave entry count = %d\n"
           "secondary VF entry counts = %d\n",
           m_CachedRmInfo.clkVfRels.super.objMask.super.pData[0],
           m_CachedRmInfo.clkVfRels.slaveEntryCount,
           m_CachedRmInfo.clkVfRels.vfEntryCountSec);
    ...
    LW2080_CTRL_BOARDOBJGRP_MASK_E32_FOR_EACH_INDEX(i, &m_CachedRmInfo.clkDomains.super.objMask.super)
        ...
        if (domInfo.super.type != LW2080_CTRL_CLK_CLK_DOMAIN_TYPE_40_PROG)
        {
            continue;
        }
        ...
        m_ClockDomains[domain].freqDeltaRangeMHz.minFreqMHz =
            domInfo.data.v40Prog.freqDeltaMinMHz;
        m_ClockDomains[domain].freqDeltaRangeMHz.maxFreqMHz =
            domInfo.data.v40Prog.freqDeltaMaxMHz;
...
RC Perf40::GetVirtualPerfPoints(PerfPoints *points)
{
    RC rc;
...
LwU32 Perf40::GetVPStateId(GpcPerfMode m) const
{
    switch (m)
    {
        case GpcPerf_TDP:
...
bool Perf40::HasVirtualPState(GpcPerfMode m) const
{
...
RC Perf40::GetVPStatesInfo()
{
    ...
    for (UINT32 ii = 0; ii < NUM_GpcPerfMode; ii++)
    {
        ...
        for (UINT32 jj = 0; jj < NUMELEMS(m_CachedRmInfo.vpstates.vpstates[vpstateIdx].data.v3x.clocks); jj++)
        {
            ...
            if (clk.Domain == Gpu::ClkUnkUnsp ||
                (ClkDomainType(clk.Domain) == FIXED &&
                 !(clk.Domain == Gpu::ClkGpc || clk.Domain == Gpu::ClkGpc2)))
            {
                continue;
            }
...
RC Perf40::GetTDPGpcHz(UINT64* tdpGpcHz)
{
    // For each VPState
    for (const auto& vpstateItr : m_VPStates)
    {
...
Gpu::ClkDomain Perf40::GetDomainFromClockProgrammingOrder(UINT32 idx) const
{
    for (const auto& clkDomItr : m_ClockDomains)
    {
...
RC Perf40::GetCachedRmInfo()
{
    ...
    GET_RM_INFO(LW2080_CTRL_CMD_CLK_CLK_ENUMS_GET_INFO, clkEnums);
    GET_RM_INFO(LW2080_CTRL_CMD_CLK_CLK_VF_RELS_GET_INFO, clkVfRels);
    GET_RM_INFO(LW2080_CTRL_CMD_CLK_CLK_PROP_REGIMES_GET_INFO, clkPropRegimes);
    GET_RM_INFO(LW2080_CTRL_CMD_CLK_CLK_PROP_TOPS_GET_INFO, clkPropTops);
    GET_RM_INFO(LW2080_CTRL_CMD_CLK_CLK_PROP_TOP_RELS_GET_INFO, clkPropTopRels);
    return rc;
}
...
RC Perf40::Init()
{
    ...
    Printf(Tee::PriLow, "Beginning mods Perf40 Init\n");
    JavaScriptPtr pJs;
    RC rc;
    ...
    LW2080_CTRL_BOARDOBJGRP_MASK_E32_FOR_EACH_INDEX(i, &m_CachedRmInfo.clkDomains.super.objMask.super)
        ...
        switch (domInfo.super.type)
        {
            ...
            {
                Printf(Tee::PriLow, "Programmable\n");
                newClockDomain.Type = PROGRAMMABLE;

                //Read the DOMAIN_40 information
                LW2080_CTRL_CLK_CLK_DOMAIN_INFO_40_PROG &domInfo40 =
                    m_CachedRmInfo.clkDomains.domains[i].data.v40Prog;

                 // Copy CLK_DOMAIN_40_PROG parameters.
                newClockDomain.infoType40.clkEnumIdxFirst       = domInfo40.clkEnumIdxFirst;
                newClockDomain.infoType40.clkEnumIdxLast        = domInfo40.clkEnumIdxLast;
                newClockDomain.infoType40.preVoltOrderingIndex  = domInfo40.preVoltOrderingIndex;
                newClockDomain.infoType40.postVoltOrderingIndex = domInfo40.postVoltOrderingIndex;
                newClockDomain.infoType40.clkVFLwrveCount       = domInfo40.clkVFLwrveCount;
                newClockDomain.infoType40.railMask              = domInfo40.railMask;

                for (UINT08 index = 0; index < LW2080_CTRL_CLK_CLK_DOMAIN_PROG_RAIL_VF_ITEM_MAX; index++)
                {
                    if (domInfo40.railVfItem[index].type ==
                        LW2080_CTRL_CLK_CLK_DOMAIN_40_PROG_RAIL_VF_TYPE_PRIMARY)
                    {
                        // Copy VF Relationship information
                        newClockDomain.infoType40.RailVfItems[index].master.clkVfRelIdxFirst =
                            domInfo40.railVfItem[index].data.master.clkVfRelIdxFirst;
                        newClockDomain.infoType40.RailVfItems[index].master.clkVfRelIdxLast =
                            domInfo40.railVfItem[index].data.master.clkVfRelIdxLast;

                        // Copy slave clock domain index mask
                        newClockDomain.infoType40.RailVfItems[index].master.slaveDomainsMask =
                            domInfo40.railVfItem[index].data.master.slaveDomainsMask;

                        // Copy Type
                        newClockDomain.infoType40.RailVfItems[index].Type =
                            LW2080_CTRL_CLK_CLK_DOMAIN_40_PROG_RAIL_VF_TYPE_PRIMARY;
                    }
                    else if (domInfo40.railVfItem[index].type ==
                             LW2080_CTRL_CLK_CLK_DOMAIN_40_PROG_RAIL_VF_TYPE_SECONDARY)
                    {
                        // Copy slace information on this voltage rail
                        newClockDomain.infoType40.RailVfItems[index].slave.masterIdx =
                            domInfo40.railVfItem[index].data.slave.masterIdx;
                    ...
                    {
                        // This clock domain is independent of this voltage rail
                        continue;
                    }
                }
                break;
        ...
        // TODO: Implement parsing for Clock Propagation Topology Table

        // Store the noise aware capable domains
        switch (domInfo.super.type)
        {
            case LW2080_CTRL_CLK_CLK_DOMAIN_TYPE_40_PROG:
                if (m_CachedRmInfo.clkDomains.domains[i].data.v40Prog.super.bNoiseAwareCapable)
                {
    ...
    {
        case LW2080_CTRL_PERF_PSTATE_VERSION_40:
            m_PStateVersion = PStateVersion::PSTATE_40;
            break;
        default:
            Printf(Tee::PriError,
                       "Invalid Pstate Version: %d\n", m_CachedRmInfo.pstates.version);
                return RC::SOFTWARE_ERROR;
            break;
    }
    ...
    // EMU/SIM platforms don't support power policies
    ...
    Printf(Tee::PriLow, "Perf40 Init done\n");
    m_InitDone = true;
    return rc;
}

RC Perf40::GenerateStandardPerfPoints
(
...
)
{
    ...
    {
        if (m_pSubdevice->HasDomain(dom.first) && IsDomainTypeMaster(dom.first))
        {
    ...
    for (UINT32 i = 0; i < pstateInfo.size(); ++i)
    {
        ...
        vector<GpcPerfMode> additionalPerfModes =
        ...
        for (auto mode : perfPointsToGenerate)
        {
            ...
            if (decoupledDomains.find(Gpu::ClkM) != decoupledDomains.end())
            {
                ...
                if (orgClkInfo.find(rmDomain) != orgClkInfo.end())
                {
                    ...
                    p.Clks[Gpu::ClkM] = ClkSetting(Gpu::ClkM, freqHz, FORCE_PERF_LIMIT_MIN);
                }
            }
...
RC Perf40::SetRegime(Gpu::ClkDomain dom, RegimeSetting regime)
{
    RC rc;
...
RC Perf40::GetRegime(Gpu::ClkDomain dom, RegimeSetting* regime)
{
    ...
    switch (dom)
    {
        ...
        case Gpu::ClkGpc:
        {
            ...
            const INT32 gpcIndex = Utility::BitScanForward(gpcMask);
            MASSERT(gpcIndex >= 0);
            const Avfs::NafllId gpcNafll = static_cast<Avfs::NafllId>(
                Avfs::NAFLL_GPC0 + gpcIndex);
            return pAvfs->GetNafllDeviceRegimeViaId(gpcNafll, regime);
        }
        case Gpu::ClkXbar2:
        case Gpu::ClkXbar:
...
RC Perf40::ForceNafllRegimes(RegimeSetting regime)
{
...
RC Perf40::SetRegimesForClocks(const ClkMap& clks)
{
    RC rc;
...
RC Perf40::CheckVfSwitchTimeForGivenType(VfSwitchVal type)
{
    RC rc;
    const bool vfSwitchCheck =
        (GetAllowedPerfSwitchTimeNs(type) != (std::numeric_limits<UINT32>::max)())
         || m_VfSwitchTimePriority != Tee::PriNone;

    string vfvaltype = "MODS";

    if (type == VfSwitchVal::Pmu)
    {
        vfvaltype = "PMU";
    }
    else if (type == VfSwitchVal::RmLimits)
    {
        vfvaltype = "RMLIMITS";
    }
    if (vfSwitchCheck && (m_LwrrentVfSwitchTimeUs[type] > (GetAllowedPerfSwitchTimeNs(type) *1e3)))
    {
        Printf(Tee::PriError,
            "%s VF Switch took %lldUs (limit %uUs)\n",
            vfvaltype.c_str(), m_LwrrentVfSwitchTimeUs[type],
            static_cast<UINT32>(GetAllowedPerfSwitchTimeNs(type) * 1e3));
        rc = RC::VF_SWITCH_TOO_SLOW;
    }
    return rc;
}

RC Perf40::InnerSetPerfPoint(const PerfPoint& Setting)
{
    RC rc;
    ...
    // Need VF Switch information to be updated below even if this call
    // fails due to VF_SWITCH_TOO_SLOW
    ...
    m_NumVfSwitches++;
    UpdateVfSwitchStats(VfSwitchVal::Mods, elapsedTimeUs);

    for (UINT08 i = VfSwitchVal::Mods; i < VfSwitchVal::NumTypes; i++)
    {
        rc = CheckVfSwitchTimeForGivenType(static_cast<Perf::VfSwitchVal>(i));
    }

    return rc;
}

RC Perf40::InnerSetPStateLock
(
...
RC Perf40::GetLwrrentPerfPoint(PerfPoint *pPoint)
{
    MASSERT(pPoint);
    RC rc;
    UINT32 numPState = 0;
...
LimitInfo clientLowStrictFreqLockLimits[] =
{
    ...
    { Gpu::ClkPexGen, { LW2080_CTRL_PERF_LIMIT_CLIENT_LOW_STRICT_PCIE_MIN,
                        LW2080_CTRL_PERF_LIMIT_CLIENT_LOW_STRICT_PCIE_MAX }},
    { Gpu::ClkXbar,   { LW2080_CTRL_PERF_LIMIT_CLIENT_LOW_STRICT_XBAR_MIN,
                        LW2080_CTRL_PERF_LIMIT_CLIENT_LOW_STRICT_XBAR_MAX }}
};
...
LimitInfo strictFreqLockLimits[] =
{
    ...
    { Gpu::ClkPexGen, { LW2080_CTRL_PERF_LIMIT_CLIENT_STRICT_PCIE_MIN,
                        LW2080_CTRL_PERF_LIMIT_CLIENT_STRICT_PCIE_MAX }},
    { Gpu::ClkXbar,   { LW2080_CTRL_PERF_LIMIT_CLIENT_STRICT_XBAR_MIN,
                        LW2080_CTRL_PERF_LIMIT_CLIENT_STRICT_XBAR_MAX }}
};
...
LimitInfo modsRulesStrictFreqLockLimits[] =
{
    ...
    { Gpu::ClkPexGen, { LW2080_CTRL_PERF_LIMIT_MODS_RULES_STRICT_PCIE_MIN,
                        LW2080_CTRL_PERF_LIMIT_MODS_RULES_STRICT_PCIE_MAX }},
    { Gpu::ClkXbar,   { LW2080_CTRL_PERF_LIMIT_MODS_RULES_STRICT_XBAR_MIN,
                        LW2080_CTRL_PERF_LIMIT_MODS_RULES_STRICT_XBAR_MAX }}
};
...
RC Perf40::ConstructPStateLimit
(
    const PerfPoint &Setting,
    LW2080_CTRL_PERF_LIMIT_INPUT *pInputLimit
) const
...
RC Perf40::ConstructPStatePerfLimits(const PerfPoint& Setting)
{
...
RC Perf40::ValidatePerfPoint(const PerfPoint& Setting)
{
    RC rc;
    ...
    // if limits are requested for a specific clock domain, sanity check them
    for (const auto & clkItr: Setting.Clks)
    {
        Gpu::ClkDomain dom = clkItr.first;
        auto it = std::find_if(std::begin(strictFreqLockLimits), std::end(strictFreqLockLimits),
            [=] (LimitInfo const& limitInfo)
            {
                return (limitInfo.clkDomain == dom);
            });
        bool isStrictLimitOnClkDomain = (it != std::end(strictFreqLockLimits));
        if ((clkItr.second.Flags & FORCE_STRICT_LIMIT) && !isStrictLimitOnClkDomain)
        {
            Printf(Tee::PriError, "Strict limit cannot be applied on domain = %s \n",
                PerfUtil::ClkDomainToStr(dom));
            return RC::BAD_PARAMETER;
        }
    }
        ...
        (m_NoiseAwareDomains.find(GpcDomain) != m_NoiseAwareDomains.end()))
    {
        ...
        {
            UINT32 idx;
            CHECK_RC(GetPStateIdx(Setting.PStateNum, &idx));
            const INT32 freqTuneOffsetKHz =  GetFreqTuningOffsetKHz(Setting.PStateNum);
            const INT32 minKHz = m_CachedPStates[idx].ClkVals[PerfUtil::GpuClkDomainToCtrl2080Bit(GpcDomain)].MinKHz + freqTuneOffsetKHz;
            const INT32 maxKHz = m_CachedPStates[idx].ClkVals[PerfUtil::GpuClkDomainToCtrl2080Bit(GpcDomain)].MaxKHz + freqTuneOffsetKHz;
            Printf(Tee::PriLow,
                  "clock freq not in a range for specified pstate. "
                  "PState=%d RmDomain=%x ClkDomain = %d FreqKHz=%d. MaxKHz=%d MinKHz=%d\n",
                   Setting.PStateNum, PerfUtil::GpuClkDomainToCtrl2080Bit(GpcDomain), GpcDomain,
                   GpcPerfkHz, maxKHz, minKHz);
            return RC::BAD_PARAMETER;
        }
    }
...
RC Perf40::ConstructClksPerfLimits(const PerfPoint& Setting, const ClkMap& Clks)
{
    RC rc;
    ...
    for (const auto& clkItr : Clks)
    {
        ...
        Gpu::ClkDomain dom = clkItr.first;
        UINT32 freqkHz = static_cast<UINT32>(clkItr.second.FreqHz * 1.0f / 1000);

        // Sanitize Clk Domain Flags
        // Fail if multiple flags are being requested
        if (Utility::CountBits(clkItr.second.Flags) > 1)
        {
            Printf(Tee::PriError, "Invalid Flags requested for %s domain\n",
                        PerfUtil::ClkDomainToStr(dom));
            return RC::SOFTWARE_ERROR;
        }
        ...
        if (dom == Gpu::ClkM)
        {
            if (ClkDomainType(dom) == FIXED)
            {
                // Cannot set fixed clocks via RM Perf ARBITER, only programmable clocks can be set
                continue;
            }
            else if (ClkDomainType(dom) == PROGRAMMABLE || !CanSetClockDomailwiaPState(dom))
            {
            ...
            // Send a limit only if set explicitly
        ...
        auto it = std::find_if(std::begin(strictFreqLockLimits), std::end(strictFreqLockLimits), [=] (LimitInfo const& limitInfo) {
            return (limitInfo.clkDomain == dom);
            });
        bool isStrictLimitOnClkDomain = (it != std::end(strictFreqLockLimits));

        if (isStrictLimitOnClkDomain && !(clkItr.second.Flags & FORCE_LOOSE_LIMIT))
        {
            CHECK_RC(LookupPerfStrictLimits(dom, &pMinLim, &pMaxLim));
        }
        else if (IsDomainTypeProgrammable(dom))
        {
        ...
        ConstructFreqLimit(dom, freqkHz, &inputLimit);
        // Bug - 200648409
        // Send a MIN or MAX limit
        if (clkItr.second.Flags & FORCE_PERF_LIMIT_MIN)
        {
            *pMinLim = inputLimit;
        }
        else if (clkItr.second.Flags & FORCE_PERF_LIMIT_MAX)
        {
            *pMaxLim = inputLimit;
        }
        else
...
RC Perf40::SetPStateLockViaLimits(const PerfPoint &Setting)
{
    RC rc;
...
RC Perf40::ResetLimitsIfPStateLockChanged(PStateLockType newLockType)
{
    RC rc;
...
RC Perf40::SetForcedPStateLockType(PStateLockType type)
{
    RC rc;
...
RC Perf40::SetUseHardPStateLocks(bool useHard)
{
    RC rc;
...
RC Perf40::UseIMPCompatibleLimits()
{
    RC rc;
...
RC Perf40::RestoreDefaultLimits()
{
    RC rc;
...
RC Perf40::ClearForcedPState()
{
    return ClearPerfLimits();
}

RC Perf40::ClearForcePStateEx(UINT32 flags)
{
    return RC::UNSUPPORTED_FUNCTION;
}

RC Perf40::ClearPerfLimits()
{
    RC rc;
...
RC Perf40::SendPerfLimitsToRM()
{
    RC rc;
    LW2080_CTRL_PERF_LIMITS_SET_STATUS_V2_PARAMS limitParams = {0};
    limitParams.numLimits = static_cast<LwU32>(m_PerfLimits.size());

    for (UINT32 limitIdx = 0; limitIdx < m_PerfLimits.size(); limitIdx++)
    {
        limitParams.limitsList[limitIdx].limitId = m_PerfLimits[limitIdx].limitId;
        limitParams.limitsList[limitIdx].input = m_PerfLimits[limitIdx].input;
    }
    ...
    CHECK_RC(LwRmPtr()->ControlBySubdevice(
        m_pSubdevice, LW2080_CTRL_CMD_PERF_LIMITS_SET_STATUS_V2,
        &limitParams, sizeof(limitParams)));

    UINT64 endTime = Xp::QueryPerformanceCounter();
    UINT64 elapsedTimeUs =
        (endTime - startTime) * 1000000 / Xp::QueryPerformanceFrequency();
    UpdateVfSwitchStats(VfSwitchVal::RmLimits, elapsedTimeUs);
...
RC Perf40::SendPerfPointToRM()
{
    ...
    {
        CHECK_RC(SendPerfLimitsToRM());
        CHECK_RC(SetClkVfRelationshipOffsetkHz(
...
RC Perf40::PrintPerfLimits() const
{
    RC rc;
...
void Perf40::PrintPerfLimitStatus_v35
(
    const LW2080_CTRL_PERF_PERF_LIMIT_INFO& limitInfo,
    const LW2080_CTRL_PERF_PERF_LIMIT_STATUS& limitStatus
) const
...
void Perf40::PrintArbiterOutput
(
...
void Perf40::PrintArbiterOutput_PMU
(
    const LW2080_CTRL_PERF_PERF_LIMITS_STATUS& limitsStatus
) const
{
...
void Perf40::PrintArbiterOutput_v35
(
    const LW2080_CTRL_PERF_PERF_LIMITS_STATUS& limitsStatus
) const
{
...
RC Perf40::SetIntersectPt(const PerfPoint &setting)
{
    RC rc;
...
RC Perf40::ConstructIntersectLimits
(
...
RC Perf40::InnerConstructIntersectLimit
(
...
//      input VPState
RC Perf40::ConstructBoundLimits
(
    const PerfPoint& perfPoint,
...
RC Perf40::AssignIntersectAndBoundLimits
(
    ...
    *pPStateIntLimit = intersectLimit;
    ...
    for (const auto& upperBoundLimit : boundLimits)
    {
        ...
        else if (upperBoundLimit.type == LW2080_CTRL_PERF_LIMIT_INPUT_TYPE_FREQ)
        {
            ...
            else if (IsDomainTypeSlave(clkDom))
            {
...
RC Perf40::CheckIntersectSettings(const set<IntersectSetting>& intersectSettings)
{
    RC rc;
...
RC Perf40::AppendVminLimit(LW2080_CTRL_PERF_LIMIT_INPUT* pIntersectLimit)
{
...
RC Perf40::ClearVoltTuningOffsets()
{
    // TODO - Initialize Volt3x
    if (m_pSubdevice->HasBug(2864561))
        return RC::OK;
...
RC Perf40::GetClkDomailwfEntryTables(Gpu::ClkDomain dom)
{
    RC rc;
    ...
    CHECK_RC(GetClkDomailwfPoints(pri, dom, &domainPoints));
...
RC Perf40::PrintVFTable()
{
    ...
    LW2080_CTRL_BOARDOBJGRP_MASK_E32_FOR_EACH_INDEX(i, &m_CachedRmInfo.clkDomains.super.objMask.super)
        LW2080_CTRL_CLK_CLK_DOMAIN_INFO &domInfo = m_CachedRmInfo.clkDomains.domains[i];
        Gpu::ClkDomain dom = PerfUtil::ClkCtrl2080BitToGpuClkDomain(domInfo.domain);
        if (domInfo.super.type != LW2080_CTRL_CLK_CLK_DOMAIN_TYPE_40_PROG)
        {
            continue;
        }
...
RC Perf40::FindVfPointsInPState
(
    Gpu::ClkDomain dom,
    UINT32 pstateNum,
...
RC Perf40::GetClkDomailwfPoints
(
    Tee::Priority pri,
    Gpu::ClkDomain dom,
...
)
{
    ...
    const auto& clkInfo = m_ClockDomains.find(dom);
    if (clkInfo == m_ClockDomains.end() ||
        !clkInfo->second.IsRmClkDomain ||
        clkInfo->second.Type != PROGRAMMABLE)
    {
        Printf(Tee::PriError, "Cannot retrieve VF points for clock domain: %s\n",
            PerfUtil::ClkDomainToStr(dom));
        return RC::ILWALID_CLOCK_DOMAIN;
    }

    const LW2080_CTRL_CLK_CLK_VF_RELS_INFO* clkVfRelsInfo = GetClkVfRelsInfo();
    LW2080_CTRL_CLK_CLK_VF_POINTS_STATUS vfPointsStatus = {};
    vfPointsStatus.super = m_CachedRmInfo.vfPoints.super;
    vfPointsStatus.sec.super = m_CachedRmInfo.vfPoints.sec.super;
    CHECK_RC(LwRmPtr()->ControlBySubdevice(
        m_pSubdevice
    ...
    ClkDomainInfo clkDomInfo = clkInfo->second;
    UINT32 railMask = clkDomInfo.infoType40.railMask;
    // Go through all voltage rails
    while (railMask)
    {
        UINT32 railIdx = Utility::BitScanForward(railMask);
        railMask ^= 1 << railIdx;

        // Only master has VF lwrve
        if (clkDomInfo.infoType40.RailVfItems[railIdx].Type !=
            LW2080_CTRL_CLK_CLK_DOMAIN_40_PROG_RAIL_VF_TYPE_PRIMARY)
        {
            Printf(pri, "railIdx = %u, type = SLAVE\n", railIdx);
            continue;
        }
        Printf(pri, "railIdx = %u, type = MASTER\n", railIdx);
        for (UINT32 relIdx = clkDomInfo.infoType40.RailVfItems[railIdx].master.clkVfRelIdxFirst;
             relIdx <= clkDomInfo.infoType40.RailVfItems[railIdx].master.clkVfRelIdxLast;
             relIdx++)
        {
            Printf(pri, "Primary VF Lwrve \n");
            for (UINT32 vfPointIdx = clkVfRelsInfo->vfRels[relIdx].vfEntryPri.vfPointIdxFirst;
                 vfPointIdx <= clkVfRelsInfo->vfRels[relIdx].vfEntryPri.vfPointIdxLast;
                 vfPointIdx++)
            {
                VfPoint point = {};
                point.VfeIdx = clkVfRelsInfo->vfRels[relIdx].vfEntryPri.vfeIdx;
                point.VfIdx = vfPointIdx;

                LW2080_CTRL_CLK_CLK_VF_POINT_VF_TUPLE offsetedVFTuple;
                switch (vfPointsStatus.vfPoints[vfPointIdx].super.type)
                {
                    case LW2080_CTRL_CLK_CLK_VF_POINT_TYPE_40_FREQ:
                        offsetedVFTuple = vfPointsStatus.vfPoints[vfPointIdx].data.v40Freq.offsetedVFTuple[0];
                        break;
                    case LW2080_CTRL_CLK_CLK_VF_POINT_TYPE_40_VOLT_PRI:
                        offsetedVFTuple = vfPointsStatus.vfPoints[vfPointIdx].data.v40VoltPri.offsetedVFTuple[0];
                        break;
                    default:
                        Printf(Tee::PriError, "Unknown VF point type (0x%x)\n",
                            vfPointsStatus.vfPoints[vfPointIdx].super.type);
                        return RC::SOFTWARE_ERROR;
                }
                point.ClkFreqKHz = offsetedVFTuple.freqMHz * ((dom == Gpu::ClkPexGen) ? 1 : 1000);
                Printf(pri, "    vfPointIdx = %u, type = %u, vfeIdx = %u, voltageuV = %u, freqMHz = %u\n",
                    point.VfIdx, vfPointsStatus.vfPoints[vfPointIdx].super.type,
                    point.VfeIdx, offsetedVFTuple.voltageuV, point.ClkFreqKHz);
                pClkVfPoints->push_back(point);
            }

            if (m_SecondaryVFSizesNotFilled)
                m_SecondaryVFPointsSizes.push_back(0);
            for (UINT32 i = 0; i < clkVfRelsInfo->vfEntryCountSec; i++)
            {
                if (clkVfRelsInfo->vfRels[relIdx].vfEntriesSec[i].vfeIdx ==
                    LW2080_CTRL_PERF_VFE_EQU_INDEX_ILWALID)
                {
                    Printf(pri, "Secondary VF lwrve unavailable for relIdx = %d \n", relIdx);
                    continue;
                }
                Printf(pri, "Secondary VF Lwrve, index = %d \n", i);
                UINT32 vfPtdxLast = clkVfRelsInfo->vfRels[relIdx].vfEntriesSec[i].vfPointIdxLast;
                UINT32 vfPtIdxFirst = clkVfRelsInfo->vfRels[relIdx].vfEntriesSec[i].vfPointIdxFirst;
                if (m_SecondaryVFSizesNotFilled)
                    m_SecondaryVFPointsSizes.push_back(vfPtdxLast + 1);
                for (UINT32 vfPointIdx = vfPtIdxFirst;
                    vfPointIdx <= vfPtdxLast;
                    vfPointIdx++)
                {
                    VfPoint point = {};
                    point.VfeIdx = clkVfRelsInfo->vfRels[relIdx].vfEntriesSec[i].vfeIdx;
                    point.VfIdx = vfPointIdx;

                    LW2080_CTRL_CLK_CLK_VF_POINT_VF_TUPLE offsetedVFTuple;
                    switch (vfPointsStatus.sec.vfPoints[vfPointIdx].super.type)
                    {
                        case LW2080_CTRL_CLK_CLK_VF_POINT_TYPE_40_FREQ:
                            // PLL clock
                            offsetedVFTuple =
                            vfPointsStatus.sec.vfPoints[vfPointIdx].data.v40Freq.offsetedVFTuple[0];
                            break;
                        case LW2080_CTRL_CLK_CLK_VF_POINT_TYPE_40_VOLT_SEC:
                            // NAFLL clock
                            offsetedVFTuple =
                            vfPointsStatus.sec.vfPoints[vfPointIdx].data.v40VoltSec.offsetedVFTuple[0];
                            break;
                        default:
                            Printf(Tee::PriError, "Unknown VF point type (0x%x)\n",
                                vfPointsStatus.sec.vfPoints[vfPointIdx].super.type);
                            return RC::SOFTWARE_ERROR;
                    }
                    point.ClkFreqKHz =
                        offsetedVFTuple.freqMHz * ((dom == Gpu::ClkPexGen) ? 1 : 1000);
                    point.DvcoOffsetCode = vfPointsStatus.sec.vfPoints[vfPointIdx].data.v40VoltSec.baseVFTuple.dvcoOffsetCode;
                    Printf(pri, "    vfPointIdx = %u, type = %u, vfeIdx = %u, voltageuV = %u, freqKHz = %u, dvcoOffsetCode = %u\n",
                        point.VfIdx, vfPointsStatus.sec.vfPoints[vfPointIdx].super.type,
                        point.VfeIdx, offsetedVFTuple.voltageuV, point.ClkFreqKHz, point.DvcoOffsetCode);
                }
            }
            m_SecondaryVFSizesNotFilled = false;
        }
    }
    return rc;
}

RC Perf40::GetFrequencieskHz
(
    Gpu::ClkDomain dom,
...
RC Perf40::GetVfPointPerfPoints
(
...
RC Perf40::GetVfPointsStatusForDomain
(
    Gpu::ClkDomain dom,
...
// bug 1712759.
RC Perf40::GetPStateErrorCodeDigits(UINT32 *pThreeDigits)
{
    ...
    if (!lwrrPP.IntersectSettings.empty())
    {
        ...
        switch (is.VoltageDomain)
        {
            ...
            case Gpu::VOLTAGE_MSVDD:
                intersectRailDigit = IntersectRail::Msvdd;
                break;
            default:
...
RC Perf40::PrintVfeIndepVars()
{
    ...
    LW2080_CTRL_BOARDOBJGRP_MASK_E32_FOR_EACH_INDEX(i, &m_CachedRmInfo.vfeVars.super.objMask.super)
        ...
        switch (m_CachedRmInfo.vfeVars.vars[i].super.type)
        {
            ...
            {
                Printf(Tee::PriNormal, " Type: Sensed Fuse 2.0\n");
                if (Utility::GetSelwrityUnlockLevel() < Utility::SUL_LWIDIA_NETWORK)
                {
                    break;
                }
                if (m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse20.super.fuseValue.bSigned)
                {
                    Printf(Tee::PriNormal, " Value: %d\n",
                           m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse20.super.fuseValue.data.signedValue);
                }
                else
                {
                    Printf(Tee::PriNormal, " Value: 0x%08x (%u)\n",
                           m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse20.super.fuseValue.data.unsignedValue,
                           m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse20.super.fuseValue.data.unsignedValue);
                }
                Printf(Tee::PriNormal, " Default Value: 0x%08x (%d)\n",
                       m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse20.super.fuseValDefault.data.unsignedValue,
                       m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse20.super.fuseValDefault.data.unsignedValue);
                const auto fuseId =
                    m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse20.fuseId;
                Printf(Tee::PriNormal, " FUSE ID: %s (0x%02x)\n",
                       PerfUtil::FuseIdToStr(fuseId), fuseId);
                Printf(Tee::PriNormal, " FUSE ID Version: %u\n",
                       m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse20.fuseIdVer);
                Printf(Tee::PriNormal, " Expected FUSE ID Version: %u\n",
                       m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse20.super.verInfo.verExpected);
                single = true;
                hasMinMax = true;
                break;
            }
...
)
{
    ...
    LW2080_CTRL_BOARDOBJGRP_MASK_E32_FOR_EACH_INDEX(i, &m_CachedRmInfo.vfeVars.super.objMask.super)
    {
        ...
        case LW2080_CTRL_PERF_VFE_VAR_TYPE_SINGLE_SENSED_FUSE_20:
        {
            CHECK_RC(pJs->SetProperty(pVfeInfo, "Type", CLASS_FUSE_20));
            if (m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse20.super.fuseValue.bSigned)
            {
                CHECK_RC(pJs->SetProperty(pVfeInfo, "Value",
                    m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse20.super.fuseValue.data.signedValue));
            ...
            CHECK_RC(pJs->SetProperty(pVfeInfo, "DefaultValue",
                m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse20.super.fuseValDefault.data.unsignedValue));
            CHECK_RC(pJs->SetProperty(pVfeInfo, "FuseId",
                m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse20.fuseId));
            CHECK_RC(pJs->SetProperty(pVfeInfo, "FuseIdVer",
                m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse20.fuseIdVer));
            CHECK_RC(pJs->SetProperty(pVfeInfo, "VFieldIdVerExp",
                m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse20.super.verInfo.verExpected));
            single = true;
        ...
        case LW2080_CTRL_PERF_VFE_VAR_TYPE_SINGLE_GLOBALLY_SPECIFIED:
            ...
            CHECK_RC(pJs->SetProperty(pVfeInfo, "ValOverride",
                *reinterpret_cast<FLOAT32*>(&vfeVarsControl.vars[i].data.globallySpecified.valOverride)));
            single = true;
            hasMinMax = true;
            break;
        default:
...
RC Perf40::PrintVfeEqus()
{
    ...
    UINT32 i;
    LW2080_CTRL_BOARDOBJGRP_MASK_E1024_FOR_EACH_INDEX(i, &m_CachedRmInfo.vfeEqus.super.objMask.super)
...
RC Perf40::OverrideVfeEquOutputRange
(
    UINT32 idx
   ,bool bMax
   ,FLOAT32 val
...
void Perf40::DisableAllPerfLimits()
{
...
RC Perf40::GetPerfLimitByID(UINT32 limitID, LW2080_CTRL_PERF_LIMIT_INPUT **limit)
{
    for (UINT32 limitIdx = 0; limitIdx < NUMELEMS(PerfLimitsIDs); limitIdx++)
    {
    ...
    Printf(Tee::PriError, "Perf limit not found for id = %u\n", limitID);
    return RC::SOFTWARE_ERROR;
}

RC Perf40::LookupPerfStrictLimits
(
    Gpu::ClkDomain dom,
    LW2080_CTRL_PERF_LIMIT_INPUT **pMinLimit,
    LW2080_CTRL_PERF_LIMIT_INPUT **pMaxLimit
...
RC Perf40::LookupPerfLooseLimits
(
    LW2080_CTRL_PERF_LIMIT_INPUT **pMinLimit,
    LW2080_CTRL_PERF_LIMIT_INPUT **pMaxLimit
)
...
RC Perf40::LookupPerfIntersectLimit(LW2080_CTRL_PERF_LIMIT_INPUT **pIntersectLimit)
{
    RC rc;
...
)
{
    ...
    CHECK_RC(ValidateVoltRail(voltDom));
    ...
    {
        Printf(Tee::PriError, "Invalid clock domain %s\n",
            PerfUtil::ClkDomainToStr(rmClkDom));
        return RC::ILWALID_CLOCK_DOMAIN;
    }

    LW2080_CTRL_CLK_CLK_DOMAIN_RPC clkDomainRpc = { 0 };
    clkDomainRpc.classType = LW2080_CTRL_CLK_CLK_DOMAIN_TYPE_40_PROG;
    clkDomainRpc.clkDomainIdx = clkDomIdx;
    switch (lookupType)
    {
    case VoltToFreq:
        clkDomainRpc.infoData.v3xRpcInfo.type =
            LW2080_CTRL_CLK_DOMAIN_40_PROG_RPC_ID_VOLT_TO_FREQ;
    ...
    case FreqToVolt:
        clkDomainRpc.infoData.v3xRpcInfo.type =
            LW2080_CTRL_CLK_DOMAIN_40_PROG_RPC_ID_FREQ_TO_VOLT;
...
RC Perf40::EnableArbitration(Gpu::ClkDomain dom)
{
    return SetArbitrationStatus(dom, true);
}

RC Perf40::DisableArbitration(Gpu::ClkDomain dom)
{
    return SetArbitrationStatus(dom, false);
}

RC Perf40::SetArbitrationStatus(Gpu::ClkDomain dom, bool enable)
{
    RC rc;
...
bool Perf40::IsArbitrated(Gpu::ClkDomain dom) const
{
    if (!m_ArbiterEnabled)
    {
        return false;
...
RC Perf40::DisablePerfLimit(UINT32 limitId) const
{
    if (!m_ArbiterEnabled)
    {
        return OK;
    ...
    LW2080_CTRL_PERF_LIMITS_SET_STATUS_V2_PARAMS limitParams = {};
    limitParams.numLimits = 1;
    limitParams.limitsList[0].limitId = limitId;
    limitParams.limitsList[0].input.type = LW2080_CTRL_PERF_LIMIT_INPUT_TYPE_DISABLED;
...
RC Perf40::GetVPStateIdx(UINT32 vpstateID, UINT32 *pIdx) const
{
    MASSERT(pIdx);
...
void Perf40::UpdateVfSwitchStats(UINT32 vfSwitchType, UINT64 timeUs)
{
    MASSERT(vfSwitchType < VfSwitchVal::NumTypes);
    ...
    m_LwrrentVfSwitchTimeUs[vfSwitchType] = timeUs;
...
void Perf40::UpdatePmuVfSwitchStats(UINT64 totalPmuTimeNs,
                        UINT64 totalPmuBuildTimeNs,
                        UINT64 totalPmuExecTimeNs)
{
    m_PmuTotalBuildTimeUs = totalPmuBuildTimeNs/1000;
    m_PmuTotalExelwtionTimeUs = totalPmuExecTimeNs/1000;
    UpdateVfSwitchStats(VfSwitchVal::Pmu, totalPmuTimeNs/1000);
}

RC Perf40::SetClocksViaInjection(const PerfPoint& Setting, ClockInjectionType injType)
{
...
RC Perf40::HandleRmChgSeq()
{
    ...
    UINT64 totaltimeTakenNs;
    UINT64 totalBuildTimeNs;
    UINT64 totalExecTimeNs;
    switch (seqStatus.version)
    {
        case LW2080_CTRL_PERF_CHANGE_SEQ_VERSION_2X:
            LwU64_ALIGN32_UNPACK(&totaltimeTakenNs,
                                 &seqStatus.data.v2x.scriptLast.profiling.totalTimens);
            LwU64_ALIGN32_UNPACK(&totalBuildTimeNs,
                                 &seqStatus.data.v2x.scriptLast.profiling.totalBuildTimens);
            LwU64_ALIGN32_UNPACK(&totalExecTimeNs,
                                 &seqStatus.data.v2x.scriptLast.profiling.totalExelwtionTimens);
            break;
        ...
        case LW2080_CTRL_PERF_CHANGE_SEQ_VERSION_35:
            ...
            LwU64_ALIGN32_UNPACK(&totalBuildTimeNs,
                                 &seqStatus.data.pmu.scriptLast.hdr.profiling.totalBuildTimens);
            LwU64_ALIGN32_UNPACK(&totalExecTimeNs,
                                 &seqStatus.data.pmu.scriptLast.hdr.profiling.totalExelwtionTimens);
            break;
        default:
    ...
    UpdatePmuVfSwitchStats(totaltimeTakenNs, totalBuildTimeNs, totalExecTimeNs);

    return rc;
}

RC Perf40::SetArbiterState(LockState state)
{
    RC rc;
...
RC Perf40::LockArbiter()
{
    return SetArbiterState(LockState::LOCKED);
}

RC Perf40::UnlockArbiter()
{
    return SetArbiterState(LockState::UNLOCKED);
}

RC Perf40::SetChangeSequencerState(LockState state)
{
    RC rc;
...
RC Perf40::LockChangeSequencer()
{
    return SetChangeSequencerState(LockState::LOCKED);
}

RC Perf40::UnlockChangeSequencer()
{
    return SetChangeSequencerState(LockState::UNLOCKED);
}

bool Perf40::IsChangeSequencerLocked() const
{
    LW2080_CTRL_PERF_CHANGE_SEQ_CONTROL seqCtrl = {};
...
RC Perf40::GetPerfLimitsStatus(LW2080_CTRL_PERF_PERF_LIMITS_STATUS* pStatus) const
{
    MASSERT(pStatus);
...
RC Perf40::GetActivePerfLimitsToJs(JSContext *pCtx, JSObject* pJsObj)
{
    MASSERT(pCtx);
    MASSERT(pJsObj);
    RC rc;
...
RC Perf40::SetFmonTestPoint(UINT32 gpcclk_MHz, UINT32 margin_MHz, UINT32 margin_uV)
{
    RC rc;
    ...
    // Check for validity of Clock Domain
    // Coverity may crib if not checked
    if (clkDomIdx == (std::numeric_limits<UINT32>::max)())
    {
        Printf(Tee::PriError, "Invalid clock domain %s\n",
            PerfUtil::ClkDomainToStr(Gpu::ClkGpc));
        return RC::ILWALID_CLOCK_DOMAIN;
    }
...
)
{
    ...
    *quantizedOutputFreq_MHz = 0;
    ...
    clkDomainRpc.classType = LW2080_CTRL_CLK_CLK_DOMAIN_TYPE_40_PROG;
    clkDomainRpc.clkDomainIdx = clkDomIdx;
    clkDomainRpc.infoData.v3xRpcInfo.type = LW2080_CTRL_CLK_DOMAIN_40_PROG_RPC_ID_FREQ_QUANTIZE;
...
RC Perf40::UnlockClockDomains(const vector<Gpu::ClkDomain>& clkDoms)
{
    RC rc;
...
RC Perf40::GetClkMonFaults(const vector<Gpu::ClkDomain> &clkDomainsToMonitor,
...
RC Perf40::ClearClkMonFaults(const vector<Gpu::ClkDomain>& clkDoms)
{
    if (clkDoms.empty())
    {
        return OK;
    ...
    for (const auto& clkDom : clkDoms)
    {
        ...
        m_ClkDomainsControl.domains[clkIdx].data.v40Prog.clkMon.flags =
        // Same field even for Perf 40
...
RC Perf40::SetClkMonThresholds(const vector<ClkMonOverride>& overrides)
{
    RC rc;
    ...
    for (const auto& ov : overrides)
    {
        ...
        auto& clkMon = m_ClkDomainsControl.domains[clkIdx].data.v40Prog.clkMon;
...
RC Perf40::ClearClkMonThresholds(const vector<Gpu::ClkDomain>& clkDoms)
{
    RC rc;
    ...
    for (const auto clkDom : clkDoms)
    {
        ...
        m_ClkDomainsControl.domains[clkIdx].data.v40Prog.clkMon.lowThresholdOverride =
            LW2080_CTRL_CLK_CLK_DOMAIN_CONTROL_35_PROG_CLK_MON_THRESHOLD_OVERRIDE_IGNORE;
        m_ClkDomainsControl.domains[clkIdx].data.v40Prog.clkMon.highThresholdOverride =
            LW2080_CTRL_CLK_CLK_DOMAIN_CONTROL_35_PROG_CLK_MON_THRESHOLD_OVERRIDE_IGNORE;
...
RC Perf40::SetUseMinSramBin(const string &skuName)
{
    RC rc;
    ...
    LW2080_CTRL_BOARDOBJGRP_MASK_E32_FOR_EACH_INDEX(i, &m_CachedRmInfo.vfeVars.super.objMask.super)
        ...
        if ((m_CachedRmInfo.vfeVars.vars[i].super.type == LW2080_CTRL_PERF_VFE_VAR_TYPE_SINGLE_SENSED_FUSE) &&
            (m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse.vFieldId == LW2080_CTRL_BIOS_VFIELD_ID_STRAP_SRAM_VMIN))
            {
        ...
        else if ((m_CachedRmInfo.vfeVars.vars[i].super.type == LW2080_CTRL_PERF_VFE_VAR_TYPE_SINGLE_SENSED_FUSE_20) &&
                 (m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse20.fuseId == LW2080_CTRL_FUSE_ID_STRAP_SRAM_VMIN))
            {
                Printf(Tee::PriDebug, "Overriding Vmin at index %d\n", i);
                CHECK_RC(OverrideVfeVar(Perf::OVERRIDE_VALUE, Perf::CLASS_FUSE_20, i,
                    static_cast<FLOAT32>(minSram)));
            }
...
RC Perf40::ScaleSpeedo(const FLOAT32 factor)
{
    UINT32 ii;
    LW2080_CTRL_BOARDOBJGRP_MASK_E32_FOR_EACH_INDEX(ii, &m_CachedRmInfo.vfeVars.super.objMask.super)
        if ((m_CachedRmInfo.vfeVars.vars[ii].super.type == LW2080_CTRL_PERF_VFE_VAR_TYPE_SINGLE_SENSED_FUSE) &&
            (m_CachedRmInfo.vfeVars.vars[ii].data.sensedFuse.vFieldId == LW2080_CTRL_BIOS_VFIELD_ID_STRAP_SPEEDO))
        {
            Printf(Tee::PriLow,
        ...
        else if ((m_CachedRmInfo.vfeVars.vars[ii].super.type == LW2080_CTRL_PERF_VFE_VAR_TYPE_SINGLE_SENSED_FUSE_20) &&
                 (m_CachedRmInfo.vfeVars.vars[ii].data.sensedFuse20.fuseId == LW2080_CTRL_FUSE_ID_STRAP_SPEEDO))
        {
            Printf(Tee::PriLow,
                "Overriding speedo at VFE variable index %u with scale factor %f\n",
                ii, factor);
            return OverrideVfeVar(Perf::OVERRIDE_SCALE, Perf::CLASS_FUSE_20, ii, factor);
        }
    LW2080_CTRL_BOARDOBJGRP_MASK_FOR_EACH_INDEX_END
...
RC Perf40::OverrideKappa(const FLOAT32 factor)
{
    RC rc;
    UINT32 indexValid;
    ...
    LW2080_CTRL_BOARDOBJGRP_MASK_E32_FOR_EACH_INDEX(indexValid, &m_CachedRmInfo.vfeVars.super.objMask.super)
        if ((m_CachedRmInfo.vfeVars.vars[indexValid].super.type == LW2080_CTRL_PERF_VFE_VAR_TYPE_SINGLE_SENSED_FUSE) &&
            (m_CachedRmInfo.vfeVars.vars[indexValid].data.sensedFuse.vFieldId == LW2080_CTRL_BIOS_VFIELD_ID_KAPPA_VALID))
        {
        ...
        else if ((m_CachedRmInfo.vfeVars.vars[indexValid].super.type == LW2080_CTRL_PERF_VFE_VAR_TYPE_SINGLE_SENSED_FUSE_20) &&
                 (m_CachedRmInfo.vfeVars.vars[indexValid].data.sensedFuse20.fuseId == LW2080_CTRL_FUSE_ID_KAPPA_VALID))
        {
            matchKappaValid = true;
            Printf(Tee::PriLow,
                "Setting kappa valid bit to 1 using VFE variable %u\n", indexValid);
            CHECK_RC(OverrideVfeVar(Perf::OVERRIDE_VALUE, Perf::CLASS_FUSE_20, indexValid, 1));
            break;
        }
    LW2080_CTRL_BOARDOBJGRP_MASK_FOR_EACH_INDEX_END
    ...
    LW2080_CTRL_BOARDOBJGRP_MASK_E32_FOR_EACH_INDEX(indexKappa, &m_CachedRmInfo.vfeVars.super.objMask.super)
        if ((m_CachedRmInfo.vfeVars.vars[indexKappa].super.type == LW2080_CTRL_PERF_VFE_VAR_TYPE_SINGLE_SENSED_FUSE) &&
            (m_CachedRmInfo.vfeVars.vars[indexKappa].data.sensedFuse.vFieldId == LW2080_CTRL_BIOS_VFIELD_ID_KAPPA))
        {
            matchKappa = true;
            Printf(Tee::PriLow,
        ...
        else if ((m_CachedRmInfo.vfeVars.vars[indexKappa].super.type == LW2080_CTRL_PERF_VFE_VAR_TYPE_SINGLE_SENSED_FUSE_20) &&
                 (m_CachedRmInfo.vfeVars.vars[indexKappa].data.sensedFuse20.fuseId == LW2080_CTRL_FUSE_ID_KAPPA))
        {
            matchKappa = true;
            Printf(Tee::PriLow,
                "Overriding kappa by %.1f using VFE variable %u\n",
                factor * 2, indexKappa);
            CHECK_RC(OverrideVfeVar(Perf::OVERRIDE_VALUE, Perf::CLASS_FUSE_20, indexKappa, factor * 2));
            break;
        }
    LW2080_CTRL_BOARDOBJGRP_MASK_FOR_EACH_INDEX_END
    ...
    LW2080_CTRL_BOARDOBJGRP_MASK_E32_FOR_EACH_INDEX(indexBoardBin, &m_CachedRmInfo.vfeVars.super.objMask.super)
        if ((m_CachedRmInfo.vfeVars.vars[indexBoardBin].super.type == LW2080_CTRL_PERF_VFE_VAR_TYPE_SINGLE_SENSED_FUSE) &&
            (m_CachedRmInfo.vfeVars.vars[indexBoardBin].data.sensedFuse.vFieldId == LW2080_CTRL_BIOS_VFIELD_ID_STRAP_BOARD_BINNING))
        {
            matchBoardBin = true;
            Printf(Tee::PriLow,
        ...
        else if ((m_CachedRmInfo.vfeVars.vars[indexBoardBin].super.type == LW2080_CTRL_PERF_VFE_VAR_TYPE_SINGLE_SENSED_FUSE_20) &&
                 (m_CachedRmInfo.vfeVars.vars[indexBoardBin].data.sensedFuse20.fuseId == LW2080_CTRL_FUSE_ID_STRAP_BOARD_BINNING))
        {
            matchBoardBin = true;
            Printf(Tee::PriLow,
                "Overriding Board Bin by %.1f using VFE variable %u\n",
                factor * 2, indexBoardBin);
            CHECK_RC(OverrideVfeVar(Perf::OVERRIDE_VALUE, Perf::CLASS_FUSE_20, indexBoardBin, factor * 2));
            break;
        }
    LW2080_CTRL_BOARDOBJGRP_MASK_FOR_EACH_INDEX_END
...
RC Perf40::GetOCBilwalue(UINT32 *pOCBilwalue)
{
    MASSERT(pOCBilwalue);
    *pOCBilwalue = m_CachedRmInfo.clkDomains.overClockingBin;
    return RC::OK;
}

RC Perf40::GetOCBinFuseValue(UINT32 *pOCBinFuseValue)
{
    UINT32 i = 0;
    LW2080_CTRL_BOARDOBJGRP_MASK_E32_FOR_EACH_INDEX(i, &m_CachedRmInfo.vfeVars.super.objMask.super)
        if (m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse.vFieldId ==
            LW2080_CTRL_BIOS_VFIELD_ID_OC_BIN)
        {
            *pOCBinFuseValue = m_CachedRmInfo.vfeVars.vars[i].data.sensedFuse.super.fuseValue.data.unsignedValue;
            return RC::OK;
        }
    LW2080_CTRL_BOARDOBJGRP_MASK_FOR_EACH_INDEX_END

    Printf(Tee::PriLow, "VFE variable OC Bin (VFIELD_ID_OC_BIN) not present in BIOS\n");
    return RC::WRONG_BIOS;
}

RC Perf40::GetOverclockRange(const Gpu::ClkDomain clkDom, OverclockRange* pOcRange)
{
    MASSERT(pOcRange);
    RC rc;
    ...
    const auto& prog = m_CachedRmInfo.clkDomains.domains[rmClkIdx].data.v40Prog;
    pOcRange->MinOffsetkHz = prog.freqDeltaMinMHz*1000;
    pOcRange->MaxOffsetkHz = prog.freqDeltaMaxMHz*1000;

    return rc;
}

RC Perf40::ForceClock
(
    Gpu::ClkDomain clkDomain,
    UINT64 targetkHz,
    UINT32 flags /* unused */
...
RC Perf40::GetClkPropRegimesControl(LW2080_CTRL_CLK_CLK_PROP_REGIMES_CONTROL* pClkPropRegimesControl)
{
    MASSERT(pClkPropRegimesControl);
    RC rc;
    pClkPropRegimesControl->super = m_CachedRmInfo.clkPropRegimes.super;
    CHECK_RC(LwRmPtr()->ControlBySubdevice(
        m_pSubdevice
       ,LW2080_CTRL_CMD_CLK_CLK_PROP_REGIMES_GET_CONTROL
       ,pClkPropRegimesControl
       ,sizeof(LW2080_CTRL_CLK_CLK_PROP_REGIMES_CONTROL)));
    return rc;
}

RC Perf40::SetClkPropRegimesControl(LW2080_CTRL_CLK_CLK_PROP_REGIMES_CONTROL* pClkPropRegimesControl)
{
    MASSERT(pClkPropRegimesControl);
    RC rc;
    CHECK_RC(LwRmPtr()->ControlBySubdevice(
...
RC Perf40::GetClkPropRegimeIndex(UINT32 propRegimeID, UINT32 *index) const
{
    UINT32 i = 0;
    RC rc;
    LW2080_CTRL_BOARDOBJGRP_MASK_E32_FOR_EACH_INDEX(i,
        &m_CachedRmInfo.clkPropRegimes.super.objMask.super)
        if (m_CachedRmInfo.clkPropRegimes.propRegimes[i].regimeId ==
            propRegimeID)
        {
            *index = i;
            return RC::OK;
    ...
    LW2080_CTRL_BOARDOBJGRP_MASK_FOR_EACH_INDEX_END
    Printf(Tee::PriError, "Propagation Regime ID = %d does not match regimeID"
                          " in LW2080_CTRL_CLK_CLK_PROP_REGIME_INFO \n", propRegimeID);
    return RC::BAD_PARAMETER;
}

RC Perf40::GetClkVfRelsControl(LW2080_CTRL_CLK_CLK_VF_RELS_CONTROL* pClkVfRelsControl)
{
    MASSERT(pClkVfRelsControl);
    RC rc;
    pClkVfRelsControl->super = m_CachedRmInfo.clkVfRels.super;
    CHECK_RC(LwRmPtr()->ControlBySubdevice(
        m_pSubdevice
       ,LW2080_CTRL_CMD_CLK_CLK_VF_RELS_GET_CONTROL
       ,pClkVfRelsControl
       ,sizeof(LW2080_CTRL_CLK_CLK_VF_RELS_CONTROL)));
    return rc;
}

RC Perf40::SetClkVfRelsControl(LW2080_CTRL_CLK_CLK_VF_RELS_CONTROL* pClkVfRelsControl)
{
    MASSERT(pClkVfRelsControl);
    RC rc;
    CHECK_RC(LwRmPtr()->ControlBySubdevice(
...
RC Perf40::GetClkPropTopRelsControl
(
    LW2080_CTRL_CLK_CLK_PROP_TOP_RELS_CONTROL* pClkPropTopRelsControl
)
{
    MASSERT(pClkPropTopRelsControl);
    RC rc;
    pClkPropTopRelsControl->super = m_CachedRmInfo.clkPropTopRels.super;
    CHECK_RC(LwRmPtr()->ControlBySubdevice(
        m_pSubdevice
       ,LW2080_CTRL_CMD_CLK_CLK_PROP_TOP_RELS_GET_CONTROL
       ,pClkPropTopRelsControl
       ,sizeof(LW2080_CTRL_CLK_CLK_PROP_TOP_RELS_CONTROL)));
    return rc;
}

RC Perf40::SetClkPropTopRelsControl
(
    LW2080_CTRL_CLK_CLK_PROP_TOP_RELS_CONTROL* pClkPropTopRelsControl
)
{
...
RC Perf40::ValidateVoltRail(Gpu::SplitVoltageDomain voltDom)
{
    if (!m_pSubdevice->GetVolt3x()->HasDomain(voltDom))
    {
...
bool Perf40::IsDomainTypeMaster(Gpu::ClkDomain dom)
{
    map<Gpu::ClkDomain, ClkDomainInfo>::const_iterator domIt = m_ClockDomains.find(dom);
    if (domIt == m_ClockDomains.end())
    {
        // CLK Domain not supported
        return false;
    }
    UINT08 railIdx =
        static_cast<UINT08>(Utility::BitScanForward(domIt->second.infoType40.railMask));
    if (railIdx >= LW2080_CTRL_CLK_CLK_DOMAIN_PROG_RAIL_VF_ITEM_MAX)
    {
        // No valid rail index
        return false;
    }
    return domIt->second.infoType40.RailVfItems[railIdx].Type ==
        LW2080_CTRL_CLK_CLK_DOMAIN_40_PROG_RAIL_VF_TYPE_PRIMARY;
}

bool Perf40::IsDomainTypeSlave(Gpu::ClkDomain dom)
{
    map<Gpu::ClkDomain, ClkDomainInfo>::const_iterator domIt = m_ClockDomains.find(dom);
    if (domIt == m_ClockDomains.end())
    {
...
bool Perf40::IsDomainTypeProgrammable(Gpu::ClkDomain dom)
{
    map<Gpu::ClkDomain, ClkDomainInfo>::const_iterator domIt = m_ClockDomains.find(dom);
    if (domIt == m_ClockDomains.end())
    {
    ...
    return domIt->second.Type == PROGRAMMABLE;
}

RC Perf40::GetClkPropTopControl
(
    LW2080_CTRL_CLK_CLK_PROP_TOPS_CONTROL* pClkPropTopsControl
)
{
    MASSERT(pClkPropTopsControl);
    RC rc;
    pClkPropTopsControl->super = m_CachedRmInfo.clkPropTops.super;
    CHECK_RC(LwRmPtr()->ControlBySubdevice(
        m_pSubdevice
       ,LW2080_CTRL_CMD_CLK_CLK_PROP_TOPS_GET_CONTROL
       ,pClkPropTopsControl
       ,sizeof(LW2080_CTRL_CLK_CLK_PROP_TOPS_CONTROL)));
    return rc;
}

RC Perf40::SetClkPropTopControl
(
    LW2080_CTRL_CLK_CLK_PROP_TOPS_CONTROL* pClkPropTopsControl
)
{
...
RC Perf40::GetClkPropTopStatus(LW2080_CTRL_CLK_CLK_PROP_TOPS_STATUS *pStatus)
{
    MASSERT(pStatus);
    pStatus->super = m_CachedRmInfo.clkPropTops.super;
    return LwRmPtr()->ControlBySubdevice(
        m_pSubdevice
       ,LW2080_CTRL_CMD_CLK_CLK_PROP_TOPS_GET_STATUS
       ,pStatus
       ,sizeof(LW2080_CTRL_CLK_CLK_PROP_TOPS_STATUS));
}

Perf::VFPointType Perf40::GetVFPointType(LwU32 rmVFPointType)
{
    VFPointType ret;
...
JS_CLASS(RailVFType);
//-----------------------------------------------------------------------------
RC Perf40::GetClkDomainInfoToJs(JSContext *pCtx, JSObject *pJsOutArray)
{
    RC rc;
    CHECK_RC(GetPStatesInfo());
    ...
    for (UINT32 i = 0; clkDomIter != m_ClockDomains.end(); ++clkDomIter, i++)
    {
        ...
        JSObject* pJsRailVFObj = JS_NewObject(pCtx, &RailVFTypeClass, NULL, NULL);
        for (UINT32 j = 0; j<LW2080_CTRL_CLK_CLK_DOMAIN_PROG_RAIL_VF_ITEM_MAX; j++)
        {
            CHECK_RC(pJs->SetElement(pJsRailVFObj,
                                    j, PerfUtil::RmClkDomTypeToModsClkDomType(static_cast<UINT08>(clkDomIter->second.infoType40.RailVfItems[j].Type))));
        }
        CHECK_RC(pJs->SetProperty(pClkInfo, "RailVFType", pJsRailVFObj));
        jsval ClkInfoJsval;
...
Gpu::ClkDomain Perf40::GetClkObjIndexClkDomain(UINT32 clkIdx) const
{
    if (clkIdx >= LW2080_CTRL_BOARDOBJ_MAX_BOARD_OBJECTS)
        return Gpu::ClkDomain_NUM;
    return PerfUtil::ClkCtrl2080BitToGpuClkDomain(
        m_CachedRmInfo.clkDomains.domains[clkIdx].domain);
}

RC Perf40::SetVfePollingPeriodMs(UINT32 periodMs)
{
    RC rc;
...
RC Perf40::GetVfePollingPeriodMs(UINT32* periodMs)
{
    RC rc;
...
UINT64 Perf40::GetLwrrentVfSwitchTime(VfSwitchVal type)
{
    return m_LwrrentVfSwitchTimeUs[type];
}
