...
/**
 * @file  perfsub_40.h
 * @brief APIs that maintain various performance settings for pstates 4.0
 *
 */

#pragma once
#ifndef INCLUDED_PERFSUB_40_H
#define INCLUDED_PERFSUB_40_H
...
class Perf40 : public Perf
{
public:
    Perf40(GpuSubdevice *pSubdevice, bool owned);
    ~Perf40() override;
    ...
    Gpu::ClkDomain GetClkObjIndexClkDomain(UINT32 clkIdx) const override;
    ...
    VFPointType GetVFPointType(LwU32 rmVFPointType) override;
    ...
    RC GetOCBilwalue(UINT32 *pOCBilwalue) override;
    RC GetOCBinFuseValue(UINT32 *pOCBinFuseValue) override;
    ...
    const LW2080_CTRL_CLK_CLK_ENUMS_INFO * GetClkEnumsInfo() const
    {
        return &m_CachedRmInfo.clkEnums;
    }

    const LW2080_CTRL_CLK_CLK_VF_RELS_INFO* GetClkVfRelsInfo() const
    {
        return &m_CachedRmInfo.clkVfRels;
    }

    const LW2080_CTRL_CLK_CLK_PROP_TOPS_INFO* GetClkPropTopsInfo() const
    {
        return &m_CachedRmInfo.clkPropTops;
    }
    ...
    RC GetClkPropRegimeIndex(UINT32 propRegimeID, UINT32 *index) const override;
    RC GetClkPropRegimesInfo(LW2080_CTRL_CLK_CLK_PROP_REGIMES_INFO* pClkPropRegimeInfo) const;

    const LW2080_CTRL_CLK_CLK_PROP_TOP_RELS_INFO * GetClkTopPropRelsInfo() const
    {
        return &m_CachedRmInfo.clkPropTopRels;
    }

    RC GetClkPropRegimesControl(LW2080_CTRL_CLK_CLK_PROP_REGIMES_CONTROL* clkPropRegimesControl);
    RC SetClkPropRegimesControl(LW2080_CTRL_CLK_CLK_PROP_REGIMES_CONTROL* clkPropRegimesControl);

    RC GetClkVfRelsControl(LW2080_CTRL_CLK_CLK_VF_RELS_CONTROL* clkVfRelsControl);
    RC SetClkVfRelsControl(LW2080_CTRL_CLK_CLK_VF_RELS_CONTROL* clkVfRelsControl);

    RC GetClkPropTopRelsControl(LW2080_CTRL_CLK_CLK_PROP_TOP_RELS_CONTROL* clkPropTopRelsControl);
    RC SetClkPropTopRelsControl(LW2080_CTRL_CLK_CLK_PROP_TOP_RELS_CONTROL* clkPropTopRelsControl);


    RC GetClkPropTopControl
    (
        LW2080_CTRL_CLK_CLK_PROP_TOPS_CONTROL* pClkPropTopsControl
    );

    RC SetClkPropTopControl
    (
        LW2080_CTRL_CLK_CLK_PROP_TOPS_CONTROL* pClkPropTopsControl
    );

    RC GetClkPropTopStatus(LW2080_CTRL_CLK_CLK_PROP_TOPS_STATUS *pStatus);

    bool IsDomainTypeMaster(Gpu::ClkDomain dom);
    bool IsDomainTypeSlave(Gpu::ClkDomain dom);
    bool IsDomainTypeProgrammable(Gpu::ClkDomain dom);

    RC PrintVFTable();

    RC GetVfRelEntriesMHz
    (
        UINT32 pstateNum,
        Gpu::ClkDomain clkDom,
    ...
    UINT64 GetLwrrentVfSwitchTime(VfSwitchVal type);

    UINT64 GetPmuTotalBuildTimeUs() { return m_PmuTotalBuildTimeUs;}
    UINT64 GetPmuTotalExecTimeUs() { return m_PmuTotalExelwtionTimeUs;}

    RC SetMaskForModifyVFPoints
    (
        VFPointType Type,
...
protected:
    ...
    RC GetClkDomainInfoToJs(JSContext *pCtx, JSObject *pJsOutArray) override;

    RC SetVfePollingPeriodMs(UINT32 periodMs) override;
    RC GetVfePollingPeriodMs(UINT32* periodMs) override;

    RC CheckVfSwitchTimeForGivenType(VfSwitchVal type) override;

private:
    //! Map of noise aware domains to their regime status
    vector<UINT32> m_SecondaryVFPointsSizes;
    bool m_SecondaryVFSizesNotFilled = true;
    ...
    UINT64 m_AvgVfSwitchTimeUs[VfSwitchVal::NumTypes] = {0};
    UINT64 m_MilwfSwitchTimeUs[VfSwitchVal::NumTypes] = {0};
    UINT64 m_MaxVfSwitchTimeUs[VfSwitchVal::NumTypes] = {0};
    UINT64 m_LwrrentVfSwitchTimeUs[VfSwitchVal::NumTypes] = {0};
    UINT64 m_PmuTotalBuildTimeUs = 0;
    UINT64 m_PmuTotalExelwtionTimeUs = 0;
    UINT32 m_NumVfSwitches = 0;
    void UpdateVfSwitchStats(UINT32 vfSwitchType, UINT64 timeUs);
    void UpdatePmuVfSwitchStats(UINT64 totalPmuTimeNs,
    ...
    struct CachedRmInfo
    {
        ...
        LW2080_CTRL_CLK_CLK_ENUMS_INFO clkEnums = {};
        LW2080_CTRL_CLK_CLK_VF_RELS_INFO clkVfRels = {};
        LW2080_CTRL_CLK_CLK_PROP_REGIMES_INFO clkPropRegimes = {};
        LW2080_CTRL_CLK_CLK_PROP_TOPS_INFO clkPropTops = {};
        LW2080_CTRL_CLK_CLK_PROP_TOP_RELS_INFO clkPropTopRels = {};
    };
    ...
    RC GetClkDomailwfEntryTables(Gpu::ClkDomain dom);
    ...
    RC ValidateVoltRail(Gpu::SplitVoltageDomain voltDom);
