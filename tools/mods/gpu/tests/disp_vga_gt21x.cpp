...
//      All rights reserved.
//
//      Branched from "disp_vga.cpp" - broken out 28 Janurary 2008
//
...
//
//      VGASetClocks_gt21x                    Programs VPLL coefficients based on desired clock frequency for VGA
//      IsoProgramVPLL_gt21x                  Programs VPLL coefficients based on desired clock frequency
//      IsoCalcVPLLCoeff_gt21x                Callwlate the and set VPLL coefficients
...
#include "gt215/dev_ext_devices.h"
 ...
 *              capture tests.
 *   This version of VGASetClocks is for the 40nm VPLLs.
 ***************************************************************************************/
RC DispTest::VGASetClocks_gt21x(UINT32 head,
                          UINT32 set_pclk,
                          UINT32 pclk_freq,
                          UINT32 dac_index,
                          UINT32 sim_turbo,
                          ...
                          FLOAT64 timeout_ms)
{
    ...
    Printf(Tee::PriNormal, "DispTest::VGASetClocks_gt21x: Crystal Frequency Value = %d \n",crystal_freq);

    // perform EscapeWrites for sim_turbo in RTL mode
    if ( (sim_turbo!=0) && (Platform::GetSimulationMode() == Platform::RTL) ) {
        Printf(Tee::PriNormal, "[VGASetClocks_gt21x]: Setting turbo mode for disp clock model : pclk = %d kHz\n", pclk_freq);
    ...
    //Enable VPLL
    UINT32 vpll_config, rd_vpll_config;
    CHECK_RC(DispTest::RmaRegRd32(LW_PDISP_CLK_REM_VPLL_CONFIG(head), &rd_vpll_config));
    Printf(Tee::PriNormal, "[VGASetClocks_gt21x]: LW_PDISP_CLK_REM_VPLL_CONFIG(%d) == 0x%x\n", head, rd_vpll_config);
    vpll_config = FLD_SET_DRF(_PDISP,_CLK_REM_VPLL_CONFIG,_ENABLE,_YES,rd_vpll_config);
    Printf(Tee::PriNormal, "[VGASetClocks_gt21x]: LW_PDISP_CLK_REM_VPLL_CONFIG(%d) --> 0x%x\n", head, vpll_config);
    CHECK_RC(DispTest::RmaRegWr32(LW_PDISP_CLK_REM_VPLL_CONFIG(head), vpll_config));
    ...
    if ( ((set_pclk!=0)  && (Platform::GetSimulationMode() == Platform::Hardware) && (emu_run!=0)) ||
        ...
        // custom clock
        Printf(Tee::PriNormal, "[VGASetClocks_gt21x]: Setting custom clock mode for Head %d\n", head);
        ...
        Printf(Tee::PriNormal, "[VGASetClocks_gt21x]: Setting default 25 MHz clock mode for Head %d\n", head);
        rc = Platform::PioRead08(LW_VIO_MISC__READ, &data_byte); CHECK_RC(rc);
    ...
    Printf(Tee::PriNormal, "[VGASetClocks_gt21x]: set div-by ratios to 1 for Head %d and Dac %d\n", head, dac_index);
    ...
    if ( (emu_run!=0) && (Platform::GetSimulationMode() == Platform::Hardware) ) {
        ...
        // custom pclk.
        Printf(Tee::PriNormal, "[VGASetClocks_gt21x]: Adjusting 25/28 MHz VPLL pdiv values for emulation.\n");
        ...
        if (set_pclk!=0) {
            ...
            // program VPLL pdiv
            Printf(Tee::PriNormal, "[VGASetClocks_gt21x]: Adjusting Head %d custom VPLL pdiv value for emulation", head);
            Printf(Tee::PriNormal, " (desired frequency %d kHz, configured frequency %d kHz)\n", pclk_freq, frequency);
            CHECK_RC(DispTest::RmaRegWr32(LW_PDISP_CLK_REM_VPLL_DIV(head),
                DRF_DEF(_PDISP,_CLK_REM_VPLL_DIV,_NDIV,_INIT) |
                DRF_DEF(_PDISP,_CLK_REM_VPLL_DIV,_MDIV,_INIT) |
                DRF_NUM(_PDISP,_CLK_REM_VPLL_DIV,_PLDIV,pdiv_value)));
        ...
        Printf(Tee::PriNormal, "[VGASetClocks_gt21x]: Calling IsoCalcVPLLCoeff to configure Head %d VPLL\n", head);
        rc = DispTest::IsoCalcVPLLCoeff_gt21x(head, pclk_freq, LW_VIO_MISC_CLK_SEL_LWSTOM);
        CHECK_RC(rc);

        UINT32 vpll_config;
        CHECK_RC(DispTest::RmaRegRd32(LW_PDISP_CLK_REM_VPLL_DIV(head), &vpll_lo));
        CHECK_RC(DispTest::RmaRegRd32(LW_PDISP_CLK_REM_VPLL_SDM(head), &vpll_hi));
        CHECK_RC(DispTest::RmaRegRd32(LW_PDISP_CLK_REM_VPLL_CONFIG(head), &vpll_config));
        if(DRF_VAL(_PDISP,_CLK_REM_VPLL_CONFIG,_SDM_ENABLE,vpll_config) == LW_PDISP_CLK_REM_VPLL_CONFIG_SDM_ENABLE_YES)
        {
            Printf(Tee::PriNormal, "[VGASetClocks_gt21x]: SDM enabled\n");
            FLOAT32 numerator = FLOAT32(DRF_VAL(_PDISP,_CLK_REM_VPLL_DIV,_NDIV, vpll_lo) + 0.5
                                + 4 * DRF_VAL(_PDISP,_CLK_REM_VPLL_SDM,_DIN,vpll_hi) * (2^-15));

            frequency_f = (crystal_freq * numerator
                        / DRF_VAL(_PDISP,_CLK_REM_VPLL_DIV,_MDIV, vpll_lo)
                        / DRF_VAL(_PDISP,_CLK_REM_VPLL_DIV,_PLDIV, vpll_lo));
        }
        else
        {
            Printf(Tee::PriNormal, "[VGASetClocks_gt21x]: SDM disabled\n");
            frequency_f = FLOAT32(crystal_freq * DRF_VAL(_PDISP,_CLK_REM_VPLL_DIV,_NDIV, vpll_lo)
                                      / DRF_VAL(_PDISP,_CLK_REM_VPLL_DIV,_MDIV, vpll_lo)
                                      / DRF_VAL(_PDISP,_CLK_REM_VPLL_DIV,_PLDIV, vpll_lo));
        }
        Printf(Tee::PriNormal, "[VGASetClocks_gt21x]: (desired frquency %d kHz, configured frequency (assuming %d MHz refclk) %f kHz)\n", pclk_freq, (crystal_freq/1000), frequency_f);
        Printf(Tee::PriNormal, "[VGASetClocks_gt21x]: LW_PDISP_CLK_REM_VPLL_DIV(%d) = 0x%08x\n", head, vpll_lo);
        Printf(Tee::PriNormal, "[VGASetClocks_gt21x]: LW_PDISP_CLK_REM_VPLL_SDM(%d) = 0x%08x\n", head, vpll_hi);
        ...
        if (sim_turbo!=0) {
            Printf(Tee::PriNormal, "[VGASetClocks_gt21x]: Detected -sim_turbo in RTL sims.  Copying Head %d VPLL coefficients into 25 MHz and 28 MHz VPLL coefficients\n", head);
            Printf(Tee::PriNormal, "[VGASetClocks_gt21x]: Overwriting VGA clock settings due to sim_turbo.\n");
 ...
 ***************************************************************************************/
RC DispTest::IsoProgramVPLL_gt21x(UINT32 head,
                            UINT32 pclk_freq,
                            UINT32 mode,
                            UINT32 emu_run)
{
    ...
    crystal_freq = pSubdev->RegRd32(LW_PEXTDEV_BOOT_0);
    crystal_freq = (DRF_VAL(_PEXTDEV,_BOOT_0,_STRAP_CRYSTAL, crystal_freq) == LW_PEXTDEV_BOOT_0_STRAP_CRYSTAL_27000K ) ? 27000 : 25000;
    Printf(Tee::PriNormal, "DispTest::IsoProgramVPLL_gt21x: Crystal Frequency Value = %d \n",crystal_freq);
    ...
    if ( (emu_run!=0) && (Platform::GetSimulationMode() == Platform::Hardware) ) {
        ...
        if (mode == LW_VIO_MISC_CLK_SEL_25_180MHZ) {
            Printf(Tee::PriNormal, "[IsoProgramVPLL_gt21x]: Adjusting 25 MHz VPLL pdiv value for emulation");
            Printf(Tee::PriNormal, " (desired frequency %d kHz, configured frequency %d kHz)\n", pclk_freq, frequency);
            CHECK_RC(DispTest::RmaRegWr32(LW_PDISP_CLK_REM_VPLL_25_DIV,
                DRF_DEF(_PDISP,_CLK_REM_VPLL_25_DIV,_NDIV,_INIT) |
                DRF_DEF(_PDISP,_CLK_REM_VPLL_25_DIV,_MDIV,_INIT) |
        ...
        // program 28 MHz VPLL pdiv
            ...
            Printf(Tee::PriNormal, "[IsoProgramVPLL_gt21x]: Adjusting 28 MHz VPLL pdiv value for emulation");
            Printf(Tee::PriNormal, " (desired frequency %d kHz, configured frequency %d kHz)\n", pclk_freq, frequency);
            CHECK_RC(DispTest::RmaRegWr32(LW_PDISP_CLK_REM_VPLL_28_DIV,
                DRF_DEF(_PDISP,_CLK_REM_VPLL_28_DIV,_NDIV,_INIT) |
                DRF_DEF(_PDISP,_CLK_REM_VPLL_28_DIV,_MDIV,_INIT) |
        ...
        // program custom VPLL pdiv
            ...
            Printf(Tee::PriNormal, "[IsoProgramVPLL_gt21x]: Adjusting Head %d custom VPLL pdiv value for emulation", head);
            Printf(Tee::PriNormal, " (desired frequency %d kHz, configured frequency %d kHz)\n", pclk_freq, frequency);
            CHECK_RC(DispTest::RmaRegWr32(LW_PDISP_CLK_REM_VPLL_DIV(head),
                DRF_DEF(_PDISP,_CLK_REM_VPLL_DIV,_NDIV,_INIT) |
                DRF_DEF(_PDISP,_CLK_REM_VPLL_DIV,_MDIV,_INIT) |
        ...
        // unknown mode
            ...
            Printf(Tee::PriHigh, "ERROR [IsoProgramVPLL_gt21x]: Cannot configure VPLL pdiv for emulation due to unknown mode = %d\n", mode);
            return ~OK;
        }
        ...
        if ((mode == LW_VIO_MISC_CLK_SEL_LWSTOM) || (mode == LW_VIO_MISC_CLK_SEL_25_180MHZ) || (mode == LW_VIO_MISC_CLK_SEL_28_325MHZ))  {
            Printf(Tee::PriNormal, "[IsoProgramVPLL_gt21x]: Calling IsoCalcVPLLCoeff_gt21x to configure Head %d VPLL\n", head);
            rc = DispTest::IsoCalcVPLLCoeff_gt21x(head, pclk_freq, mode); // colwert freq from kHz to Hz
            CHECK_RC(rc);
            ...
            if(DRF_VAL(_PDISP,_CLK_REM_VPLL_CONFIG,_SDM_ENABLE,vpll_config) == LW_PDISP_CLK_REM_VPLL_CONFIG_SDM_ENABLE_YES)
            {
                Printf(Tee::PriNormal, "[IsoProgramVPLL_gt21x]: SDM enabled\n");
                FLOAT32 numerator = FLOAT32(DRF_VAL(_PDISP,_CLK_REM_VPLL_DIV,_NDIV, vpll_lo) + 0.5
                    + 4 * DRF_VAL(_PDISP,_CLK_REM_VPLL_SDM,_DIN,vpll_hi) * (2^-15));
            ...
            {
                Printf(Tee::PriNormal, "[IsoProgramVPLL_gt21x]: SDM disabled\n");
                frequency_f = FLOAT32(crystal_freq * DRF_VAL(_PDISP,_CLK_REM_VPLL_DIV,_NDIV, vpll_lo)
                        / DRF_VAL(_PDISP,_CLK_REM_VPLL_DIV,_MDIV, vpll_lo)
                        / DRF_VAL(_PDISP,_CLK_REM_VPLL_DIV,_PLDIV, vpll_lo));
            }

            Printf(Tee::PriNormal, "[IsoProgramVPLL_gt21x]: (desired frquency %d kHz, configured frequency (assuming %d MHz refclk) %f kHz)\n", pclk_freq, (crystal_freq/1000),
                    frequency_f);
            Printf(Tee::PriNormal, "[IsoProgramVPLL_gt21x]: LW_PDISP_CLK_REM_VPLL_DIV(%d) = 0x%08x\n", head, vpll_lo);
            Printf(Tee::PriNormal, "[IsoProgramVPLL_gt21x]: LW_PDISP_CLK_REM_VPLL_SDM(%d) = 0x%08x\n", head, vpll_hi);

        } else {

            Printf(Tee::PriHigh, "ERROR [IsoProgramVPLL_gt21x]: Cannot configure VPLL due to unknown mode = %d\n", mode);
            return ~OK;
        }
...
/****************************************************************************************
 * DispTest::IsoCalcVPLLCoeff_gt21x
 *
 *  Arguments Description:
 *  - head: head number
 *  - pclk_freq: pixel clock frequency in kHz
 ...
 ***************************************************************************************/
RC DispTest::IsoCalcVPLLCoeff_gt21x(UINT32 head, UINT32 pclk_freq, UINT32 mode)
{
    RC rc = OK;
    UINT32 crystal_freq = 0;
    //FLOAT32 frequency_f;
    ...
    crystal_freq = pSubdev->RegRd32(LW_PEXTDEV_BOOT_0);
    crystal_freq = (DRF_VAL(_PEXTDEV,_BOOT_0,_STRAP_CRYSTAL, crystal_freq) == LW_PEXTDEV_BOOT_0_STRAP_CRYSTAL_27000K ) ? 27000 : 25000;
    Printf(Tee::PriNormal, "DispTest::IsoCalcVPLLCoeff_gt21x: Crystal Frequency Value = %d \n",crystal_freq);

//{
    UINT32  ref_clk;//, clk_in;
    //UINT32  vclk,  // same as pclk_freq
    UINT32 tgtVco;
    UINT32  bestDelta, bestM, bestN, bestPL, bestSDM;//, bestSrcDiv;
    UINT32  lwv, delta, m, n, n2, sdm, pl;//, srcDiv;
    UINT32  Fvco;
    UINT32  fUA;
    ...
    Printf(Tee::PriNormal, "DispTest::IsoCalcVPLLCoeff_gt21x: Fmilwco=%08d FmaxVco=%08d FminU=%08d FmaxU=%08d\n", Fmilwco, FmaxVco, FminU, FmaxU);
    Printf(Tee::PriNormal, "DispTest::IsoCalcVPLLCoeff_gt21x: lowM   =%08d highM  =%08d lowN   =%08d highN  =%08d minPL =%08d maxPL = %08d\n",lowM, highM, lowN, highN, minPL, maxPL);
    ...
    // High linear divider is the largest value where we can still
    // hit the target frequency.
    ...
    //Printf(Tee::PriNormal, "DispTest::IsoCalcVPLLCoeff_gt21x: pclk_freq = %08d tgtVco =%08d ref_clk=%08d FmaxVco=%08d highLDIV=%08d lowLDIV =%08d \n",pclk_freq, tgtVco, ref_clk, FmaxVco, highLDIV, lowLDIV);
    ...
    if (bestDelta == 0xFFFFFFFF) {
      Printf(Tee::PriNormal, "DispTest::IsoCalcVPLLCoeff_gt21x: Error : bestDelta is still 0xFFFFFFFF\n");
      return ~OK;
    }
    else {
      Printf(Tee::PriNormal, "DispTest::IsoCalcVPLLCoeff_gt21x : Callwlated coefficients for target frequency %d ref_clk (%d)- M(%d) N(%d) SDM(%d) PL(%d)\n", pclk_freq, ref_clk, bestM, bestN, bestSDM, bestPL);
      //In some directed tests, the test wants to set the 25/28MHz registers to some other frequency.
      //This function uses the mode to determine where to program the desired frequency.
      if(mode == LW_VIO_MISC_CLK_SEL_25_180MHZ)
      {
          CHECK_RC(DispTest::RmaRegWr32(LW_PDISP_CLK_REM_VPLL_25_DIV,
          ...
          CHECK_RC(DispTest::RmaRegWr32(LW_PDISP_CLK_REM_VPLL_CONFIG(head),
                DRF_DEF(_PDISP,_CLK_REM_VPLL_CONFIG,_SDM_ENABLE,_YES) ));
          ...
          UINT32 vpll_lo, vpll_hi, vpll_config;
            ...
            CHECK_RC(DispTest::RmaRegRd32(LW_PDISP_CLK_REM_VPLL_25_SDM, &vpll_hi));
            CHECK_RC(DispTest::RmaRegRd32(LW_PDISP_CLK_REM_VPLL_CONFIG(head), &vpll_config));
          Printf(Tee::PriNormal, "DispTest::IsoCalcVPLLCoeff_gt21x : LW_PDISP_CLK_REM_VPLL_25_DIV = 0x%x LW_PDISP_CLK_REM_VPLL_25_SDM = 0x%x LW_PDISP_CLK_REM_VPLL_CONFIG = 0x%x \n", vpll_lo, vpll_hi, vpll_config);
      ...
      else if(mode == LW_VIO_MISC_CLK_SEL_28_325MHZ)
      {
          ...
          UINT32 vpll_lo, vpll_hi, vpll_config;
            ...
            CHECK_RC(DispTest::RmaRegRd32(LW_PDISP_CLK_REM_VPLL_28_SDM, &vpll_hi));
            CHECK_RC(DispTest::RmaRegRd32(LW_PDISP_CLK_REM_VPLL_CONFIG(head), &vpll_config));
          Printf(Tee::PriNormal, "DispTest::IsoCalcVPLLCoeff_gt21x : LW_PDISP_CLK_REM_VPLL_28_DIV = 0x%x LW_PDISP_CLK_REM_VPLL_28_SDM = 0x%x LW_PDISP_CLK_REM_VPLL_CONFIG = 0x%x \n", vpll_lo, vpll_hi, vpll_config);
      }
      else if(mode == LW_VIO_MISC_CLK_SEL_LWSTOM)
      {
          CHECK_RC(DispTest::RmaRegWr32(LW_PDISP_CLK_REM_VPLL_DIV(head),
                DRF_NUM(_PDISP,_CLK_REM_VPLL_DIV,_MDIV,bestM) |
                DRF_NUM(_PDISP,_CLK_REM_VPLL_DIV,_PLDIV,bestPL) |
                DRF_NUM(_PDISP,_CLK_REM_VPLL_DIV,_NDIV,bestN) ));

          CHECK_RC(DispTest::RmaRegWr32(LW_PDISP_CLK_REM_VPLL_CONFIG(head),
                DRF_DEF(_PDISP,_CLK_REM_VPLL_CONFIG,_SDM_ENABLE,_YES) ));

          CHECK_RC(DispTest::RmaRegWr32(LW_PDISP_CLK_REM_VPLL_SDM(head),
                DRF_NUM(_PDISP,_CLK_REM_VPLL_SDM,_DIN,bestSDM) ));

          UINT32 vpll_lo, vpll_hi, vpll_config;
            CHECK_RC(DispTest::RmaRegRd32(LW_PDISP_CLK_REM_VPLL_DIV(head), &vpll_lo));
            CHECK_RC(DispTest::RmaRegRd32(LW_PDISP_CLK_REM_VPLL_SDM(head), &vpll_hi));
            CHECK_RC(DispTest::RmaRegRd32(LW_PDISP_CLK_REM_VPLL_CONFIG(head), &vpll_config));
          Printf(Tee::PriNormal, "DispTest::IsoCalcVPLLCoeff_gt21x : LW_PDISP_CLK_REM_VPLL_DIV = 0x%x LW_PDISP_CLK_REM_VPLL_SDM = 0x%x LW_PDISP_CLK_REM_VPLL_CONFIG = 0x%x \n", vpll_lo, vpll_hi, vpll_config);
      }
      else
      {
          Printf(Tee::PriNormal, "DispTest::IsoCalcVPLLCoeff_gt21x : Error: invalid mode 0x%x \n", mode);
      }
