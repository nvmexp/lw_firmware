...
#ifndef H264OLDLAYOUTER_H
#define H264OLDLAYOUTER_H
...
public:
    ...
    void SetStartAddress(UINT08 *startPtr)
    {
        ...
        m_offsetsStore.template Clear<vldCtrlOffsets>();
        m_offsetsStore.template Clear<picSetupOffsets>();
        m_offsetsStore.template Clear<dataInfoOffsets>();
    }
    ...
    {
        static const UINT08 endOfStream[] =
        {
            0x00, 0x00, 0x01, 0x0B,
            0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x01, 0x0B,
        ...
        if (0 == GetFrameOffsetsSize())
        {
            // We don't have to encrypt EOS. Instead we will later place it in a
            // buffer that doesn't support encryption.
            SetEOSOffset(m_writer.Write(
                &endOfStream[0],
                static_cast<UINT32>(sizeof(endOfStream)),
                m_sliceAlign));
        }
        ...
        {
            // We place all slices one by one without gaps. Code in
            // IH264ControlStructs::DoUpdateFromPicture depends on this
            // assumption when fills in slice offsets. If you change this mode
            // of placement here, don't forget to change the correspondent code
            // in every implementation of IH264ControlStructs::DoUpdateFromPicture.
    ...
    void AddVLDControl(void const *what, size_t dataSize)
    {
        AddVldCtrlOffsets(m_writer.Write(
    ...
    void AddDataInfo(void const *what, size_t dataSize)
    {
        AddDataInfoOffsets(m_writer.Write(
            what, static_cast<UINT32>(dataSize), m_controlAlign
        ));
    }
    ...
    const_iterator vld_begin() const
    {
        return m_offsetsStore.template Begin<vldCtrlOffsets>();
    }

    const_iterator vld_end() const
    {
        return m_offsetsStore.template End<vldCtrlOffsets>();
    }
    ...
    {
        return m_offsetsStore.template Begin<dataInfoOffsets>();
    }

    const_iterator data_end() const
    {
        return m_offsetsStore.template End<dataInfoOffsets>();
    }

    UINT32 GetEOSOffset() const
    {
        return m_offsetsStore.template Get<endOfStreamOffset>();
    }
...
private:
    ...
    void AddVldCtrlOffsets(UINT32 v)
    {
        m_offsetsStore.template Add<vldCtrlOffsets>(v);
    }
    ...
    void AddDataInfoOffsets(UINT32 v)
    {
        m_offsetsStore.template Add<dataInfoOffsets>(v);
    }
