...
ClA0B0Engine::ClA0B0Engine(Channel *lwCh, LwRm::Handle hLwdec, Tee::Priority printPri)
...
ClA0B0Engine::~ClA0B0Engine()
{}

template <class H264Layouter>
void ClA0B0Engine::BuildH264Layout(H264Layouter *layouter)
{
    MASSERT(layouter);
...
UINT32 ClA0B0Engine::CalcH264LayoutSize()
{
    if (!m_encryption)
    {
...
RC ClA0B0Engine::InitH264(GpuDevice *dev, GpuSubdevice *subDev, LwRm::Handle chHandle)
{
    RC rc;
...
RC ClA0B0Engine::DoInitFromH264File(const char *fileName, const TarFile *tarFile, GpuDevice *dev,
...
RC ClA0B0Engine::DoInitFromH265File(const char *fileName, const TarFile *tarFile, GpuDevice *dev,
                                    GpuSubdevice *subDev, LwRm::Handle chHandle)
{
    return RC::UNSUPPORTED_FUNCTION;
}

RC ClA0B0Engine::DoInitFromAnother
(
    GpuDevice *dev,
    GpuSubdevice *subDev,
...
)
{
    ...
    auto &ae = dynamic_cast<ClA0B0Engine &>(another);
...
RC ClA0B0Engine::DoInitOutput(GpuDevice * dev, GpuSubdevice * subDev, LwRm::Handle chHandle,
                              FLOAT64 skTimeout)
{
    RC rc;
...
RC ClA0B0Engine::DoUpdateInDataGoldens(GpuGoldenSurfaces * goldSurfs, Goldelwalues * goldValues)
{
    MASSERT(goldSurfs);
    MASSERT(goldValues);
...
RC ClA0B0Engine::DoUpdateOutPicsGoldens(GpuGoldenSurfaces *goldSurfs, Goldelwalues *goldValues)
{
    MASSERT(goldSurfs);
    MASSERT(goldValues);
...
RC ClA0B0Engine::DoUpdateHistGoldens(GpuGoldenSurfaces * goldSurfs, Goldelwalues * goldValues)
{
    MASSERT(goldSurfs);
    MASSERT(goldValues);
...
RC ClA0B0Engine::DoSaveFrames(const string &nameTemplate, UINT32 instNum)
{
    RC rc;
...
size_t ClA0B0Engine::DoGetNumPics() const
{
    return m_h264parser->GetNumFrames();
}

RC ClA0B0Engine::DoInitChannel()
{
    RC rc;
...
RC ClA0B0Engine::DoSetupContentKey(GpuSubdevice *subDev, FLOAT64 skTimeout)
{
    return Engine::DoSetupContentKey(subDev, skTimeout);
}

const UINT32* ClA0B0Engine::DoGetWrappedSessionKey() const
{
    return m_wrappedSessionKey;
}

void ClA0B0Engine::DoSetWrappedSessionKey(const UINT32 wsk[AES::DW])
{
    memcpy(m_wrappedSessionKey, wsk, sizeof(m_wrappedSessionKey));
}

const UINT32* ClA0B0Engine::DoGetWrappedContentKey() const
{
    return m_wrappedContentKey;
}

void ClA0B0Engine::DoSetWrappedContentKey(const UINT32 wck[AES::DW])
{
    memcpy(m_wrappedContentKey, wck, sizeof(m_wrappedContentKey));
}

const UINT32* ClA0B0Engine::DoGetContentKey() const
{
    return m_contentKey;
}

void ClA0B0Engine::DoSetContentKey(const UINT32 ck[AES::DW])
{
    memcpy(m_contentKey, ck, sizeof(m_contentKey));
}

const UINT32* ClA0B0Engine::DoGetInitVector() const
{
    return m_initVector;
}

void ClA0B0Engine::DoSetInitVector(const UINT32 iv[AES::DW])
{
    memcpy(m_initVector, iv, sizeof(m_initVector));
}

UINT08 ClA0B0Engine::DoGetKeyIncrement() const
{
    return m_keyIncrement;
}

void ClA0B0Engine::DoSetKeyIncrement(UINT08 ki)
{
    m_keyIncrement = ki;
}

RC ClA0B0Engine::DoDecodeOneFrame(FLOAT64 timeOut)
{
    RC rc;
...
RC ClA0B0Engine::PrintH264InputData(const Layouter &layouter)
{
    RC rc;
...
RC ClA0B0Engine::DoPrintInputData()
{
    RC rc;
...
RC ClA0B0Engine::DoPrintHistogram()
{
    RC rc;
...
LWDEC::Engine* ClA0B0Engine::DoClone() const
{
    return new ClA0B0Engine(*this);
}

RC ClA0B0Engine::SetApplicationID(ApplicationId id)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->Write(
        0, LWA0B0_SET_APPLICATION_ID,
        DRF_NUM(A0B0, _SET_APPLICATION_ID, _ID, static_cast<unsigned int>(id))));

    return rc;
}

RC ClA0B0Engine::SetControlParams(CodecType codecType, bool gpTimerOn, bool retErr,
                                  bool errConcealOn, unsigned int errorFrmIdx)
{
    ...
    controlParams = DRF_NUM(A0B0, _SET_CONTROL_PARAMS, _CODEC_TYPE, codecType);
    controlParams |= DRF_NUM(A0B0, _SET_CONTROL_PARAMS, _GPTIMER_ON, gpTimerOn);
    controlParams |= DRF_NUM(A0B0, _SET_CONTROL_PARAMS, _RET_ERROR, retErr);
    controlParams |= DRF_NUM(A0B0, _SET_CONTROL_PARAMS, _ERR_CONCEAL_ON, errConcealOn);
    controlParams |= DRF_NUM(A0B0, _SET_CONTROL_PARAMS, _ERROR_FRM_IDX, errorFrmIdx);

    CHECK_RC(m_lwCh->Write(0, LWA0B0_SET_CONTROL_PARAMS, controlParams));

    return rc;
}

RC ClA0B0Engine::SetInBufBaseOffset(const Surface2D& surf, UINT32 offs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWA0B0_SET_IN_BUF_BASE_OFFSET, surf, offs, 8));

    return rc;
}

RC ClA0B0Engine::SetPictureIndex(UINT32 idx)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->Write(
        0, LWA0B0_SET_PICTURE_INDEX,
        DRF_NUM(A0B0, _SET_PICTURE_INDEX, _INDEX, idx)));

    return rc;
}

RC ClA0B0Engine::SetDrvPicSetupOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWA0B0_SET_DRV_PIC_SETUP_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClA0B0Engine::SetHistoryOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWA0B0_SET_HISTORY_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClA0B0Engine::SetColocDataOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWA0B0_SET_COLOC_DATA_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClA0B0Engine::SetHistogramOffset(const Surface2D& surf)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWA0B0_SET_HISTOGRAM_OFFSET, surf, 0, 8));

    return rc;
}

RC ClA0B0Engine::H264SetMBHistBufOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWA0B0_H264_SET_MBHIST_BUF_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClA0B0Engine::SetPictureLumaOffset(size_t picIdx, const Surface2D& surf)
{
    static const UINT32 method[] =
    {
        LWA0B0_SET_PICTURE_LUMA_OFFSET0
      , LWA0B0_SET_PICTURE_LUMA_OFFSET1
      , LWA0B0_SET_PICTURE_LUMA_OFFSET2
      , LWA0B0_SET_PICTURE_LUMA_OFFSET3
      , LWA0B0_SET_PICTURE_LUMA_OFFSET4
      , LWA0B0_SET_PICTURE_LUMA_OFFSET5
      , LWA0B0_SET_PICTURE_LUMA_OFFSET6
      , LWA0B0_SET_PICTURE_LUMA_OFFSET7
      , LWA0B0_SET_PICTURE_LUMA_OFFSET8
      , LWA0B0_SET_PICTURE_LUMA_OFFSET9
      , LWA0B0_SET_PICTURE_LUMA_OFFSET10
      , LWA0B0_SET_PICTURE_LUMA_OFFSET11
      , LWA0B0_SET_PICTURE_LUMA_OFFSET12
      , LWA0B0_SET_PICTURE_LUMA_OFFSET13
      , LWA0B0_SET_PICTURE_LUMA_OFFSET14
      , LWA0B0_SET_PICTURE_LUMA_OFFSET15
      , LWA0B0_SET_PICTURE_LUMA_OFFSET16
    };
...
RC ClA0B0Engine::SetPictureChromaOffset(size_t picIdx, const Surface2D& surf)
{
    static const UINT32 method[] =
    {
        LWA0B0_SET_PICTURE_CHROMA_OFFSET0
      , LWA0B0_SET_PICTURE_CHROMA_OFFSET1
      , LWA0B0_SET_PICTURE_CHROMA_OFFSET2
      , LWA0B0_SET_PICTURE_CHROMA_OFFSET3
      , LWA0B0_SET_PICTURE_CHROMA_OFFSET4
      , LWA0B0_SET_PICTURE_CHROMA_OFFSET5
      , LWA0B0_SET_PICTURE_CHROMA_OFFSET6
      , LWA0B0_SET_PICTURE_CHROMA_OFFSET7
      , LWA0B0_SET_PICTURE_CHROMA_OFFSET8
      , LWA0B0_SET_PICTURE_CHROMA_OFFSET9
      , LWA0B0_SET_PICTURE_CHROMA_OFFSET10
      , LWA0B0_SET_PICTURE_CHROMA_OFFSET11
      , LWA0B0_SET_PICTURE_CHROMA_OFFSET12
      , LWA0B0_SET_PICTURE_CHROMA_OFFSET13
      , LWA0B0_SET_PICTURE_CHROMA_OFFSET14
      , LWA0B0_SET_PICTURE_CHROMA_OFFSET15
      , LWA0B0_SET_PICTURE_CHROMA_OFFSET16
    };
...
RC ClA0B0Engine::SetSliceOffsetsBufOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWA0B0_SET_SLICE_OFFSETS_BUF_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClA0B0Engine::Execute(bool notify, NotifyOn notifyOn, bool awaken)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->Write(
        0, LWA0B0_EXELWTE,
        (notify ?
            DRF_DEF(A0B0, _EXELWTE, _NOTIFY, _ENABLE) :
            DRF_DEF(A0B0, _EXELWTE, _NOTIFY, _DISABLE))
        |
        (awaken ?
            DRF_DEF(A0B0, _EXELWTE, _AWAKEN, _ENABLE) :
            DRF_DEF(A0B0, _EXELWTE, _AWAKEN, _DISABLE))
        |
        (notifyOnBeg == notifyOn ?
            DRF_DEF(A0B0, _EXELWTE, _NOTIFY_ON, _BEGIN) :
            DRF_DEF(A0B0, _EXELWTE, _NOTIFY_ON, _END))
    ));

    return rc;
}

RC ClA0B0Engine::Flush()
{
    RC rc;
...
void ClA0B0Engine::UpdateControlStructsFromPics()
{
    if (h264 != m_initedTo)
    {
        MASSERT(false);
...
RC ClA0B0Engine::InitOutSurfaces(UINT32 width, UINT32 height, UINT32 numPics, GpuDevice *dev,
                                 GpuSubdevice *subDev, LwRm::Handle chHandle)
{
    RC rc;
...
void ClA0B0Engine::BuildH264Picsetup()
{
    using Utility::StrPrintf;
...
RC ClA0B0Engine::CreateH264PushBuffer()
{
    RC rc;
...
ClA0B0Engine::H264Ctrl::H264Ctrl()
{
...
void ClA0B0Engine::H264Ctrl::UpdateFromPicture(const H264::Picture &pic)
{
...
void ClA0B0Engine::H264Ctrl::EnableEncryption(unsigned int keyIncrement, const UINT32 sessionKey[4],
                                              const UINT32 contentKey[4], const UINT32 initVector[4])
{
...
void const * ClA0B0Engine::H264Ctrl::GetPicSetup() const
{
    return &m_ps;
}

size_t ClA0B0Engine::H264Ctrl::GetPicSetupSize() const
{
    return sizeof(m_ps);
}

size_t ClA0B0Engine::H264Ctrl::GetMaxDPBSize() const
{
    return NUMELEMS(m_ps.dpb);
}

unsigned int ClA0B0Engine::H264Ctrl::GetLwrrPicIdx() const
{
    return m_ps.LwrrPicIdx;
}

unsigned int ClA0B0Engine::H264Ctrl::GetLwrrColIdx() const
{
    return m_ps.LwrrColIdx;
}

unsigned int ClA0B0Engine::H264Ctrl::GetDPBPicIdx(size_t idx) const
{
    return m_ps.dpb[idx].index;
}

unsigned int ClA0B0Engine::H264Ctrl::GetDPBColIdx(size_t idx) const
{
    return m_ps.dpb[idx].col_idx;
}

bool ClA0B0Engine::H264Ctrl::IsDPBEntryUsed(size_t idx) const
{
    return 0 != m_ps.dpb[idx].state;
}

void ClA0B0Engine::H264Ctrl::SetLwrrPicIdx(unsigned int val)
{
    m_ps.LwrrPicIdx = val;
    m_ps.LwrrColIdx = val;
}

void ClA0B0Engine::H264Ctrl::UpdateDPB(const H264::Picture &pic, size_t whereIdx,
...
void ClA0B0Engine::H264Ctrl::SetOutputMemoryLayout(MemLayout memLayout)
{
    m_ps.output_memory_layout = MemLayout::LW12 == memLayout ? 0 : 1;
}

void ClA0B0Engine::H264Ctrl::SetHistBufferSize(unsigned int size)
{
    m_ps.HistBufferSize = size;
}

void ClA0B0Engine::H264Ctrl::EnableHistogram(short startX, short startY, short endX, short endY)
{
...
void ClA0B0Engine::H264Ctrl::SetPitchOffset(const Surface2D &surfY, const Surface2D &surfUV)
{
...
void ClA0B0Engine::H264Ctrl::SetSlicesSize(unsigned int slicesSize)
{
    m_ps.stream_len = slicesSize;
}

unsigned int ClA0B0Engine::H264Ctrl::GetSlicesSize(void const *buf) const
{
...
void ClA0B0Engine::H264Ctrl::SetSlicesCount(unsigned int slicesCount)
{
    m_ps.slice_count = slicesCount;
}

void ClA0B0Engine::H264Ctrl::SetMBHistBufSize(unsigned int mbHistBufSize)
{
    m_ps.mbhist_buffer_size = mbHistBufSize;
}

void const * ClA0B0Engine::H264Ctrl::GetSliceOffsets() const
{
    return &m_sliceOffsets[0];
}

size_t ClA0B0Engine::H264Ctrl::GetSliceOffsetsSize() const
{
    return sizeof(m_sliceOffsets[0]) * m_sliceOffsets.size();
}

string ClA0B0Engine::H264Ctrl::PicSetupToText(void const *buf) const
{
    const lwdec_h264_pic_s *ps = reinterpret_cast<const lwdec_h264_pic_s *>(buf);
...
std::string ClA0B0Engine::H264Ctrl::SliceOffsetsToText(void const *buf, size_t numSlices) const
{
    using Utility::StrPrintf;
