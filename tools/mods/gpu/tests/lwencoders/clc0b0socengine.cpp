...
class ClC0B0SocEngine::H265Ctrl
{
public:
    H265Ctrl();
...
ClC0B0SocEngine::ClC0B0SocEngine(Channel *lwCh, Tee::Priority printPri)
  : m_printPri(printPri)
  , m_keyIncrement(1)
  , m_lwCh(lwCh)
{
...
ClC0B0SocEngine::~ClC0B0SocEngine()
{}

template <class H264Layouter>
void ClC0B0SocEngine::BuildH264Layout(H264Layouter *layouter)
{
    MASSERT(layouter);
...
void ClC0B0SocEngine::BuildH265Layout(H265Layouter *layouter)
{
    MASSERT(layouter);
...
UINT32 ClC0B0SocEngine::CalcH264LayoutSize()
{
    if (!m_encryption)
    {
...
UINT32 ClC0B0SocEngine::CalcH265LayoutSize()
{
...
RC ClC0B0SocEngine::InitH264(GpuDevice *dev, GpuSubdevice *subDev, LwRm::Handle chHandle)
{
    RC rc;
...
RC ClC0B0SocEngine::InitH265(GpuDevice *dev, GpuSubdevice *subDev, LwRm::Handle chHandle)
{
    RC rc;
...
RC ClC0B0SocEngine::DoInitFromH264File(const char *fileName, const TarFile *tarFile, GpuDevice *dev,
...
RC ClC0B0SocEngine::DoInitFromH265File(const char *fileName, const TarFile *tarFile, GpuDevice *dev,
                                       GpuSubdevice *subDev, LwRm::Handle chHandle)
{
    using Utility::AlignUp;
...
RC ClC0B0SocEngine::DoInitFromAnother
(
    GpuDevice *dev,
    GpuSubdevice *subDev,
...
)
{
    ...
    auto &ae = dynamic_cast<ClC0B0SocEngine &>(another);
...
RC ClC0B0SocEngine::DoInitOutput(GpuDevice * dev, GpuSubdevice * subDev, LwRm::Handle chHandle,
                                 FLOAT64 skTimeout)
{
    RC rc;
...
RC ClC0B0SocEngine::DoUpdateInDataGoldens(GpuGoldenSurfaces * goldSurfs, Goldelwalues * goldValues)
{
    MASSERT(goldSurfs);
    MASSERT(goldValues);
...
RC ClC0B0SocEngine::DoUpdateOutPicsGoldens(GpuGoldenSurfaces *goldSurfs, Goldelwalues *goldValues)
{
    MASSERT(goldSurfs);
    MASSERT(goldValues);
...
RC ClC0B0SocEngine::DoUpdateHistGoldens(GpuGoldenSurfaces *goldSurfs, Goldelwalues *goldValues)
{
    MASSERT(goldSurfs);
    MASSERT(goldValues);
...
RC ClC0B0SocEngine::DoSaveFrames(const string &nameTemplate, UINT32 instNum)
{
    RC rc;
...
size_t ClC0B0SocEngine::DoGetNumPics() const
{
    return h264 == m_initedTo ? m_h264parser->GetNumFrames() : m_h265parser->GetNumFrames();
}

RC ClC0B0SocEngine::DoInitChannel()
{
    RC rc;
    CHECK_RC(m_lwCh->SetClass(0, LWE26E_CMD_SETCL_CLASSID_LWDEC));
    return rc;
}

RC ClC0B0SocEngine::DoSetupContentKey(GpuSubdevice *subDev, FLOAT64 skTimeout)
{
    return CheetAh::SocPtr()->SetupContentKey(GetContentKey());
}

const UINT32* ClC0B0SocEngine::DoGetWrappedSessionKey() const
{
    return m_wrappedSessionKey;
}

void ClC0B0SocEngine::DoSetWrappedSessionKey(const UINT32 wsk[AES::DW])
{
    memcpy(m_wrappedSessionKey, wsk, sizeof(m_wrappedSessionKey));
}

const UINT32* ClC0B0SocEngine::DoGetWrappedContentKey() const
{
    return m_wrappedContentKey;
}

void ClC0B0SocEngine::DoSetWrappedContentKey(const UINT32 wck[AES::DW])
{
    memcpy(m_wrappedContentKey, wck, sizeof(m_wrappedContentKey));
}

const UINT32* ClC0B0SocEngine::DoGetContentKey() const
{
    return m_contentKey;
}

void ClC0B0SocEngine::DoSetContentKey(const UINT32 ck[AES::DW])
{
    memcpy(m_contentKey, ck, sizeof(m_contentKey));
}

const UINT32* ClC0B0SocEngine::DoGetInitVector() const
{
    return m_initVector;
}

void ClC0B0SocEngine::DoSetInitVector(const UINT32 iv[AES::DW])
{
    memcpy(m_initVector, iv, sizeof(m_initVector));
}

UINT08 ClC0B0SocEngine::DoGetKeyIncrement() const
{
    return m_keyIncrement;
}

void ClC0B0SocEngine::DoSetKeyIncrement(UINT08 ki)
{
    m_keyIncrement = ki;
}

RC ClC0B0SocEngine::DoDecodeOneFrame(FLOAT64 timeOut)
{
    RC rc;
...
RC ClC0B0SocEngine::PrintH264InputData(const Layouter &layouter)
{
    RC rc;
...
RC ClC0B0SocEngine::PrintH265InputData(const H265MemLayouter &layouter)
{
    RC rc;
...
RC ClC0B0SocEngine::DoPrintInputData()
{
    RC rc;
...
RC ClC0B0SocEngine::DoPrintHistogram()
{
    RC rc;
...
LWDEC::Engine* ClC0B0SocEngine::DoClone() const
{
    return new ClC0B0SocEngine(*this);
}

RC ClC0B0SocEngine::SetApplicationID(ApplicationId id)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->Write(
        0, LWC0B0_SET_APPLICATION_ID,
        DRF_NUM(C0B0, _SET_APPLICATION_ID, _ID, static_cast<unsigned int>(id))));

    return rc;
}

RC ClC0B0SocEngine::SetControlParams(CodecType codecType, bool gpTimerOn, bool retErr,
                                     bool errConcealOn, unsigned int errorFrmIdx)
{
    ...
    controlParams  = DRF_NUM(C0B0, _SET_CONTROL_PARAMS, _CODEC_TYPE, codecType);
    controlParams |= DRF_NUM(C0B0, _SET_CONTROL_PARAMS, _GPTIMER_ON, gpTimerOn);
    controlParams |= DRF_NUM(C0B0, _SET_CONTROL_PARAMS, _RET_ERROR, retErr);
    controlParams |= DRF_NUM(C0B0, _SET_CONTROL_PARAMS, _ERR_CONCEAL_ON, errConcealOn);
    controlParams |= DRF_NUM(C0B0, _SET_CONTROL_PARAMS, _ERROR_FRM_IDX, errorFrmIdx);

    CHECK_RC(m_lwCh->Write(0, LWC0B0_SET_CONTROL_PARAMS, controlParams));

    return rc;
}

RC ClC0B0SocEngine::SetInBufBaseOffset(const Surface2D& surf, UINT32 offs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC0B0_SET_IN_BUF_BASE_OFFSET, surf, offs, 8));

    return rc;
}

RC ClC0B0SocEngine::SetPictureIndex(UINT32 idx)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->Write(
        0, LWC0B0_SET_PICTURE_INDEX,
        DRF_NUM(C0B0, _SET_PICTURE_INDEX, _INDEX, idx)));

    return rc;
}

RC ClC0B0SocEngine::SetDrvPicSetupOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC0B0_SET_DRV_PIC_SETUP_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClC0B0SocEngine::SetHistoryOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC0B0_SET_HISTORY_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClC0B0SocEngine::SetColocDataOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC0B0_SET_COLOC_DATA_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClC0B0SocEngine::HEVCSetScalingListOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC0B0_HEVC_SET_SCALING_LIST_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClC0B0SocEngine::HEVCSetFilterBufferOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC0B0_HEVC_SET_FILTER_BUFFER_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClC0B0SocEngine::HEVCSetTileSizesOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC0B0_HEVC_SET_TILE_SIZES_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClC0B0SocEngine::SetHistogramOffset(const Surface2D& surf)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC0B0_SET_HISTOGRAM_OFFSET, surf, 0, 8));

    return rc;
}

RC ClC0B0SocEngine::H264SetMBHistBufOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC0B0_H264_SET_MBHIST_BUF_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClC0B0SocEngine::SetPictureLumaOffset(size_t picIdx, const Surface2D& surf)
{
    static const UINT32 method[] =
    {
        LWC0B0_SET_PICTURE_LUMA_OFFSET0
      , LWC0B0_SET_PICTURE_LUMA_OFFSET1
      , LWC0B0_SET_PICTURE_LUMA_OFFSET2
      , LWC0B0_SET_PICTURE_LUMA_OFFSET3
      , LWC0B0_SET_PICTURE_LUMA_OFFSET4
      , LWC0B0_SET_PICTURE_LUMA_OFFSET5
      , LWC0B0_SET_PICTURE_LUMA_OFFSET6
      , LWC0B0_SET_PICTURE_LUMA_OFFSET7
      , LWC0B0_SET_PICTURE_LUMA_OFFSET8
      , LWC0B0_SET_PICTURE_LUMA_OFFSET9
      , LWC0B0_SET_PICTURE_LUMA_OFFSET10
      , LWC0B0_SET_PICTURE_LUMA_OFFSET11
      , LWC0B0_SET_PICTURE_LUMA_OFFSET12
      , LWC0B0_SET_PICTURE_LUMA_OFFSET13
      , LWC0B0_SET_PICTURE_LUMA_OFFSET14
      , LWC0B0_SET_PICTURE_LUMA_OFFSET15
      , LWC0B0_SET_PICTURE_LUMA_OFFSET16
    };
...
RC ClC0B0SocEngine::SetPictureChromaOffset(size_t picIdx, const Surface2D& surf)
{
    static const UINT32 method[] =
    {
        LWC0B0_SET_PICTURE_CHROMA_OFFSET0
      , LWC0B0_SET_PICTURE_CHROMA_OFFSET1
      , LWC0B0_SET_PICTURE_CHROMA_OFFSET2
      , LWC0B0_SET_PICTURE_CHROMA_OFFSET3
      , LWC0B0_SET_PICTURE_CHROMA_OFFSET4
      , LWC0B0_SET_PICTURE_CHROMA_OFFSET5
      , LWC0B0_SET_PICTURE_CHROMA_OFFSET6
      , LWC0B0_SET_PICTURE_CHROMA_OFFSET7
      , LWC0B0_SET_PICTURE_CHROMA_OFFSET8
      , LWC0B0_SET_PICTURE_CHROMA_OFFSET9
      , LWC0B0_SET_PICTURE_CHROMA_OFFSET10
      , LWC0B0_SET_PICTURE_CHROMA_OFFSET11
      , LWC0B0_SET_PICTURE_CHROMA_OFFSET12
      , LWC0B0_SET_PICTURE_CHROMA_OFFSET13
      , LWC0B0_SET_PICTURE_CHROMA_OFFSET14
      , LWC0B0_SET_PICTURE_CHROMA_OFFSET15
      , LWC0B0_SET_PICTURE_CHROMA_OFFSET16
    };
...
RC ClC0B0SocEngine::SetSliceOffsetsBufOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC0B0_SET_SLICE_OFFSETS_BUF_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClC0B0SocEngine::Execute(bool notify, NotifyOn notifyOn, bool awaken)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->Write(
        0, LWC0B0_EXELWTE,
        (notify ?
            DRF_DEF(C0B0, _EXELWTE, _NOTIFY, _ENABLE) :
            DRF_DEF(C0B0, _EXELWTE, _NOTIFY, _DISABLE))
        |
        (awaken ?
            DRF_DEF(C0B0, _EXELWTE, _AWAKEN, _ENABLE) :
            DRF_DEF(C0B0, _EXELWTE, _AWAKEN, _DISABLE))
        |
        (notifyOnBeg == notifyOn ?
            DRF_DEF(C0B0, _EXELWTE, _NOTIFY_ON, _BEGIN) :
            DRF_DEF(C0B0, _EXELWTE, _NOTIFY_ON, _END))
    ));

    return rc;
}

RC ClC0B0SocEngine::Flush()
{
    RC rc;
...
void ClC0B0SocEngine::UpdateControlStructsFromPics()
{
    if (h264 == m_initedTo)
    {
...
RC ClC0B0SocEngine::InitOutSurfaces(UINT32 width, UINT32 height, UINT32 numPics, GpuDevice *dev,
                                    GpuSubdevice *subDev, LwRm::Handle chHandle)
{
    RC rc;
...
void ClC0B0SocEngine::BuildH264Picsetup()
{
    using Utility::StrPrintf;
...
void ClC0B0SocEngine::BuildH265Picsetup()
{
...
RC ClC0B0SocEngine::CreateH264PushBuffer()
{
    RC rc;
...
RC ClC0B0SocEngine::CreateH265PushBuffer()
{
    RC rc;
    ...
    CHECK_RC(SetControlParams(CODEC_TYPE_HEVC, false, false, false, 0));
    CHECK_RC(SetPictureIndex(m_lwrPic));
    CHECK_RC(SetInBufBaseOffset(m_inSurf, frameOffset));
...
ClC0B0SocEngine::H264Ctrl::H264Ctrl()
{
...
void ClC0B0SocEngine::H264Ctrl::UpdateFromPicture(const H264::Picture &pic)
{
...
void ClC0B0SocEngine::H264Ctrl::EnableEncryption(unsigned int keyIncrement, const UINT32 sessionKey[4],
                                                 const UINT32 contentKey[4], const UINT32 initVector[4])
{
...
void const * ClC0B0SocEngine::H264Ctrl::GetPicSetup() const
{
    return &m_ps;
}

size_t ClC0B0SocEngine::H264Ctrl::GetPicSetupSize() const
{
    return sizeof(m_ps);
}

size_t ClC0B0SocEngine::H264Ctrl::GetMaxDPBSize() const
{
    return NUMELEMS(m_ps.dpb);
}

unsigned int ClC0B0SocEngine::H264Ctrl::GetLwrrPicIdx() const
{
    return m_ps.LwrrPicIdx;
}

unsigned int ClC0B0SocEngine::H264Ctrl::GetLwrrColIdx() const
{
    return m_ps.LwrrColIdx;
}

unsigned int ClC0B0SocEngine::H264Ctrl::GetDPBPicIdx(size_t idx) const
{
    return m_ps.dpb[idx].index;
}

unsigned int ClC0B0SocEngine::H264Ctrl::GetDPBColIdx(size_t idx) const
{
    return m_ps.dpb[idx].col_idx;
}

bool ClC0B0SocEngine::H264Ctrl::IsDPBEntryUsed(size_t idx) const
{
    return 0 != m_ps.dpb[idx].state;
}

void ClC0B0SocEngine::H264Ctrl::SetLwrrPicIdx(unsigned int val)
{
    m_ps.LwrrPicIdx = val;
    m_ps.LwrrColIdx = val;
}

void ClC0B0SocEngine::H264Ctrl::UpdateDPB(const H264::Picture &pic, size_t whereIdx,
...
void ClC0B0SocEngine::H264Ctrl::SetOutputMemoryLayout(MemLayout memLayout)
{
    m_ps.output_memory_layout = MemLayout::LW12 == memLayout ? 0 : 1;
}

void ClC0B0SocEngine::H264Ctrl::SetHistBufferSize(unsigned int size)
{
    m_ps.HistBufferSize = size;
}

void ClC0B0SocEngine::H264Ctrl::EnableHistogram(short startX, short startY, short endX, short endY)
{
...
void ClC0B0SocEngine::H264Ctrl::SetPitchOffset(const Surface2D &surfY, const Surface2D &surfUV)
{
...
void ClC0B0SocEngine::H264Ctrl::SetSlicesSize(unsigned int slicesSize)
{
    m_ps.stream_len = slicesSize;
}

unsigned int ClC0B0SocEngine::H264Ctrl::GetSlicesSize(void const *buf) const
{
...
void ClC0B0SocEngine::H264Ctrl::SetSlicesCount(unsigned int slicesCount)
{
    m_ps.slice_count = slicesCount;
}

void ClC0B0SocEngine::H264Ctrl::SetMBHistBufSize(unsigned int mbHistBufSize)
{
    m_ps.mbhist_buffer_size = mbHistBufSize;
}

void const * ClC0B0SocEngine::H264Ctrl::GetSliceOffsets() const
{
    return &m_sliceOffsets[0];
}

size_t ClC0B0SocEngine::H264Ctrl::GetSliceOffsetsSize() const
{
    return sizeof(m_sliceOffsets[0]) * m_sliceOffsets.size();
}

string ClC0B0SocEngine::H264Ctrl::PicSetupToText(void const *buf) const
{
    const lwdec_h264_pic_s *ps = reinterpret_cast<const lwdec_h264_pic_s *>(buf);
...
std::string ClC0B0SocEngine::H264Ctrl::SliceOffsetsToText(void const *buf, size_t numSlices) const
{
    using Utility::StrPrintf;
...
ClC0B0SocEngine::H265Ctrl::H265Ctrl()
{
    memset(&m_ps, 0, sizeof(m_ps));

    // CheetAh block linear
    m_ps.tileformat = 0;
...
void const* ClC0B0SocEngine::H265Ctrl::GetPicSetup() const
{
    return &m_ps;
}

size_t ClC0B0SocEngine::H265Ctrl::GetPicSetupSize() const
{
    return sizeof(m_ps);
}

void const* ClC0B0SocEngine::H265Ctrl::GetScalingMatrices() const
{
    return &m_sm;
}

size_t ClC0B0SocEngine::H265Ctrl::GetScalingMatricesSize() const
{
    return sizeof(m_sm);
}

void const* ClC0B0SocEngine::H265Ctrl::GetTileInfo() const
{
    return &m_ti[0];
}

size_t ClC0B0SocEngine::H265Ctrl::GetTileInfoSize() const
{
    return m_ti.size() * TileInfoValSize;
}

void ClC0B0SocEngine::H265Ctrl::UpdateFromPicture(const H265::Picture &pic)
{
...
void ClC0B0SocEngine::H265Ctrl::UpdateDPB(INT32 lwrrPicPoc, const UINT08 l0[H265::maxNumRefPics],
...
void ClC0B0SocEngine::H265Ctrl::SetLwrrPicIdx(unsigned int lwrPicIdx)
{
    m_ps.lwrr_pic_idx = lwrPicIdx;
}

void ClC0B0SocEngine::H265Ctrl::SetPitch(const Surface2D &surfY, const Surface2D &surfUV,
                                         OutputSize outSize)
{
...
void ClC0B0SocEngine::H265Ctrl::SetColMvBufSize(unsigned int size)
{
    m_ps.colMvBuffersize = size / 256;
}

void ClC0B0SocEngine::H265Ctrl::SetSaoBufferOffset(unsigned int offset)
{
    m_ps.HevcSaoBufferOffset = offset;
}

void ClC0B0SocEngine::H265Ctrl::SetBsdCtrlOffset(unsigned int offset)
{
    m_ps.HevcBsdCtrlOffset = offset;
}

void ClC0B0SocEngine::H265Ctrl::SetSlicesSize(unsigned int slicesSize)
{
    m_ps.stream_len = slicesSize;
}

unsigned int ClC0B0SocEngine::H265Ctrl::GetSlicesSize(void const *buf) const
{
...
string ClC0B0SocEngine::H265Ctrl::PicSetupToText(void const * buf) const
{
    using Utility::StrPrintf;
    ...
    psStr += StrPrintf("    initreflistidxl0 = {");
    ...
    psStr += StrPrintf("    RefDiffPicOrderCnts = {");
...
string ClC0B0SocEngine::H265Ctrl::ScalingMatricesToText(void const *buf) const
{
    using Utility::StrPrintf;
...
string ClC0B0SocEngine::H265Ctrl::TileInfoToText(void const * buf,
...
ClC0B0SocEngine::H265Ctrl::ScalingMatrices::ScalingMatrices()
{
