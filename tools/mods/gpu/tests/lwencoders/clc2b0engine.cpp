...
// because c2b0_drv.h defines its own copy
#ifdef ALIGN_UP
#   undef ALIGN_UP
#endif
#include "class/c2b0_drv.h"
// Undefine ALIGN_UP coming from c2b0_drv.h
#ifdef ALIGN_UP
#   undef ALIGN_UP
#endif
...
ClC2B0Engine::ClC2B0Engine(Channel *lwCh, LwRm::Handle hLwdec, Memory::Location semLoc,
                           bool useFESem, Tee::Priority printPri)
  ...
  , m_lwCh(lwCh)
{
    ...
    if (!m_useFESem && nullptr != m_lwCh)
    {
        m_semaphore = NotifySemaphore();
...
ClC2B0Engine::ClC2B0Engine(const ClC2B0Engine &x)
  ...
  , m_lwCh(x.m_lwCh)
{
    ...
    if (x.m_semaphore && x.m_semaphore->IsAllocated())
    {
        m_semaphore = NotifySemaphore();
        m_semaphore->Setup(NotifySemaphore::ONE_WORD, m_semLoc, 1);
        m_semAllocResult = m_semaphore->Allocate(m_lwCh, 0);
    ...
    // Don't rely on value of (bool)x.m_semaphore to determine whether the
    // copying class was wired to use frontend or backend semaphores. Because we
    // don't create a semaphore in this if x.m_semaphore is not allocated. It
    // means on the second copy this->m_semaphore won't exist, m_useFESem will
    // be false and m_semAllocResult probably will have a valuable error code.
    else if (!m_useFESem)
    {
...
ClC2B0Engine::~ClC2B0Engine()
{}

template <class H264Layouter>
void ClC2B0Engine::BuildH264Layout(H264Layouter *layouter)
{
    MASSERT(layouter);
...
void ClC2B0Engine::BuildH265Layout(H265Layouter *layouter)
{
    MASSERT(layouter);
...
UINT32 ClC2B0Engine::CalcH264LayoutSize()
{
    if (!m_encryption)
    {
...
UINT32 ClC2B0Engine::CalcH265LayoutSize()
{
...
RC ClC2B0Engine::InitH264(GpuDevice *dev, GpuSubdevice *subDev, LwRm::Handle chHandle)
{
    RC rc;
...
RC ClC2B0Engine::InitH265(GpuDevice *dev, GpuSubdevice *subDev, LwRm::Handle chHandle)
{
    RC rc;
...
RC ClC2B0Engine::DoInitFromH264File(const char *fileName, const TarFile *tarFile, GpuDevice *dev,
...
RC ClC2B0Engine::DoInitFromH265File(const char *fileName, const TarFile *tarFile, GpuDevice *dev,
                                    GpuSubdevice *subDev, LwRm::Handle chHandle)
{
    using Utility::AlignUp;
...
RC ClC2B0Engine::DoInitFromAnother
(
    GpuDevice *dev,
    GpuSubdevice *subDev,
...
)
{
    ...
    auto &ae = dynamic_cast<ClC2B0Engine &>(another);
...
RC ClC2B0Engine::DoInitOutput(GpuDevice * dev, GpuSubdevice * subDev, LwRm::Handle chHandle,
                              FLOAT64 skTimeout)
{
    RC rc;
...
RC ClC2B0Engine::DoUpdateInDataGoldens(GpuGoldenSurfaces * goldSurfs, Goldelwalues * goldValues)
{
    MASSERT(goldSurfs);
    MASSERT(goldValues);
...
RC ClC2B0Engine::DoUpdateOutPicsGoldens(GpuGoldenSurfaces *goldSurfs, Goldelwalues *goldValues)
{
    MASSERT(goldSurfs);
    MASSERT(goldValues);
...
RC ClC2B0Engine::DoUpdateHistGoldens(GpuGoldenSurfaces *goldSurfs, Goldelwalues *goldValues)
{
    MASSERT(goldSurfs);
    MASSERT(goldValues);
...
RC ClC2B0Engine::DoSaveFrames(const string &nameTemplate, UINT32 instNum)
{
    RC rc;
...
RC ClC2B0Engine::DoSetupContentKey(GpuSubdevice *subDev, FLOAT64 skTimeout)
{
    return Engine::DoSetupContentKey(subDev, skTimeout);
}

const UINT32* ClC2B0Engine::DoGetWrappedSessionKey() const
{
    return m_wrappedSessionKey;
}

void ClC2B0Engine::DoSetWrappedSessionKey(const UINT32 wsk[AES::DW])
{
    memcpy(m_wrappedSessionKey, wsk, sizeof(m_wrappedSessionKey));
}

const UINT32* ClC2B0Engine::DoGetWrappedContentKey() const
{
    return m_wrappedContentKey;
}

void ClC2B0Engine::DoSetWrappedContentKey(const UINT32 wck[AES::DW])
{
    memcpy(m_wrappedContentKey, wck, sizeof(m_wrappedContentKey));
}

const UINT32* ClC2B0Engine::DoGetContentKey() const
{
    return m_contentKey;
}

void ClC2B0Engine::DoSetContentKey(const UINT32 ck[AES::DW])
{
    memcpy(m_contentKey, ck, sizeof(m_contentKey));
}

const UINT32* ClC2B0Engine::DoGetInitVector() const
{
    return m_initVector;
}

void ClC2B0Engine::DoSetInitVector(const UINT32 iv[AES::DW])
{
    memcpy(m_initVector, iv, sizeof(m_initVector));
}

UINT08 ClC2B0Engine::DoGetKeyIncrement() const
{
    return m_keyIncrement;
}

void ClC2B0Engine::DoSetKeyIncrement(UINT08 ki)
{
    m_keyIncrement = ki;
}

RC ClC2B0Engine::DoDecodeOneFrame(FLOAT64 timeOut)
{
    RC rc;
    ...
    if (!m_useFESem && (!m_semaphore || !m_semaphore->IsAllocated()))
    {
    ...
    {
        // This class uses a backend semaphore in contrast to other engines. At the
        // point of applying this change the reason why Channel::WaitIdle didn't
        // work is unknown. See bug 200183819 for details.
...
RC ClC2B0Engine::PrintH264InputData(const Layouter &layouter)
{
    RC rc;
...
RC ClC2B0Engine::PrintH265InputData(const H265MemLayouter &layouter)
{
    RC rc;
...
RC ClC2B0Engine::DoPrintInputData()
{
    RC rc;
...
RC ClC2B0Engine::DoPrintHistogram()
{
    RC rc;
...
LWDEC::Engine* ClC2B0Engine::DoClone() const
{
    return new ClC2B0Engine(*this);
}

RC ClC2B0Engine::SetApplicationID(ApplicationId id)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->Write(
        0, LWC2B0_SET_APPLICATION_ID,
        DRF_NUM(C2B0, _SET_APPLICATION_ID, _ID, static_cast<unsigned int>(id))));

    return rc;
}

RC ClC2B0Engine::SetControlParams(CodecType codecType, bool gpTimerOn, bool retErr,
                                  bool errConcealOn, unsigned int errorFrmIdx, bool mbTimerOn)
{
    ...
    controlParams  = DRF_NUM(C2B0, _SET_CONTROL_PARAMS, _CODEC_TYPE, codecType);
    controlParams |= DRF_NUM(C2B0, _SET_CONTROL_PARAMS, _GPTIMER_ON, gpTimerOn);
    controlParams |= DRF_NUM(C2B0, _SET_CONTROL_PARAMS, _MBTIMER_ON, mbTimerOn);
    controlParams |= DRF_NUM(C2B0, _SET_CONTROL_PARAMS, _RET_ERROR, retErr);
    controlParams |= DRF_NUM(C2B0, _SET_CONTROL_PARAMS, _ERR_CONCEAL_ON, errConcealOn);
    controlParams |= DRF_NUM(C2B0, _SET_CONTROL_PARAMS, _ERROR_FRM_IDX, errorFrmIdx);

    CHECK_RC(m_lwCh->Write(0, LWC2B0_SET_CONTROL_PARAMS, controlParams));

    return rc;
}

RC ClC2B0Engine::SetInBufBaseOffset(const Surface2D& surf, UINT32 offs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC2B0_SET_IN_BUF_BASE_OFFSET, surf, offs, 8));

    return rc;
}

RC ClC2B0Engine::SetPictureIndex(UINT32 idx)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->Write(
        0, LWC2B0_SET_PICTURE_INDEX,
        DRF_NUM(C2B0, _SET_PICTURE_INDEX, _INDEX, idx)));

    return rc;
}

RC ClC2B0Engine::SetDrvPicSetupOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC2B0_SET_DRV_PIC_SETUP_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClC2B0Engine::SetHistoryOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC2B0_SET_HISTORY_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClC2B0Engine::SetColocDataOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC2B0_SET_COLOC_DATA_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClC2B0Engine::HEVCSetScalingListOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC2B0_HEVC_SET_SCALING_LIST_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClC2B0Engine::HEVCSetFilterBufferOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC2B0_HEVC_SET_FILTER_BUFFER_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClC2B0Engine::HEVCSetTileSizesOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC2B0_HEVC_SET_TILE_SIZES_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClC2B0Engine::SetHistogramOffset(const Surface2D& surf)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC2B0_SET_HISTOGRAM_OFFSET, surf, 0, 8));

    return rc;
}

RC ClC2B0Engine::H264SetMBHistBufOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC2B0_H264_SET_MBHIST_BUF_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClC2B0Engine::SetPictureLumaOffset(size_t picIdx, const Surface2D& surf)
{
    static const UINT32 method[] =
    {
        LWC2B0_SET_PICTURE_LUMA_OFFSET0
      , LWC2B0_SET_PICTURE_LUMA_OFFSET1
      , LWC2B0_SET_PICTURE_LUMA_OFFSET2
      , LWC2B0_SET_PICTURE_LUMA_OFFSET3
      , LWC2B0_SET_PICTURE_LUMA_OFFSET4
      , LWC2B0_SET_PICTURE_LUMA_OFFSET5
      , LWC2B0_SET_PICTURE_LUMA_OFFSET6
      , LWC2B0_SET_PICTURE_LUMA_OFFSET7
      , LWC2B0_SET_PICTURE_LUMA_OFFSET8
      , LWC2B0_SET_PICTURE_LUMA_OFFSET9
      , LWC2B0_SET_PICTURE_LUMA_OFFSET10
      , LWC2B0_SET_PICTURE_LUMA_OFFSET11
      , LWC2B0_SET_PICTURE_LUMA_OFFSET12
      , LWC2B0_SET_PICTURE_LUMA_OFFSET13
      , LWC2B0_SET_PICTURE_LUMA_OFFSET14
      , LWC2B0_SET_PICTURE_LUMA_OFFSET15
      , LWC2B0_SET_PICTURE_LUMA_OFFSET16
    };
...
RC ClC2B0Engine::SetPictureChromaOffset(size_t picIdx, const Surface2D& surf)
{
    static const UINT32 method[] =
    {
        LWC2B0_SET_PICTURE_CHROMA_OFFSET0
      , LWC2B0_SET_PICTURE_CHROMA_OFFSET1
      , LWC2B0_SET_PICTURE_CHROMA_OFFSET2
      , LWC2B0_SET_PICTURE_CHROMA_OFFSET3
      , LWC2B0_SET_PICTURE_CHROMA_OFFSET4
      , LWC2B0_SET_PICTURE_CHROMA_OFFSET5
      , LWC2B0_SET_PICTURE_CHROMA_OFFSET6
      , LWC2B0_SET_PICTURE_CHROMA_OFFSET7
      , LWC2B0_SET_PICTURE_CHROMA_OFFSET8
      , LWC2B0_SET_PICTURE_CHROMA_OFFSET9
      , LWC2B0_SET_PICTURE_CHROMA_OFFSET10
      , LWC2B0_SET_PICTURE_CHROMA_OFFSET11
      , LWC2B0_SET_PICTURE_CHROMA_OFFSET12
      , LWC2B0_SET_PICTURE_CHROMA_OFFSET13
      , LWC2B0_SET_PICTURE_CHROMA_OFFSET14
      , LWC2B0_SET_PICTURE_CHROMA_OFFSET15
      , LWC2B0_SET_PICTURE_CHROMA_OFFSET16
    };
...
RC ClC2B0Engine::SetSliceOffsetsBufOffset(const Surface2D& surf, UINT32 ofs)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC2B0_SET_SLICE_OFFSETS_BUF_OFFSET, surf, ofs, 8));

    return rc;
}

RC ClC2B0Engine::SemaphoreAB(const Surface2D& surf)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC2B0_SEMAPHORE_A, surf, 0, 32));
    CHECK_RC(m_lwCh->WriteWithSurface(0, LWC2B0_SEMAPHORE_B, surf, 0, 0));

    return rc;
}

RC ClC2B0Engine::SemaphoreC(UINT32 payload)
{
    RC rc;

    Printf(m_printPri, "SemaphoreC, payload = %#.8x\n", payload);

   CHECK_RC(m_lwCh->Write(0, LWC2B0_SEMAPHORE_C, DRF_NUM(C2B0, _SEMAPHORE_C, _PAYLOAD, payload)));

   return rc;
}

RC ClC2B0Engine::Execute(bool notify, NotifyOn notifyOn, bool awaken)
{
    RC rc;
    ...
    CHECK_RC(m_lwCh->Write(
        0, LWC2B0_EXELWTE,
        (notify ?
            DRF_DEF(C2B0, _EXELWTE, _NOTIFY, _ENABLE) :
            DRF_DEF(C2B0, _EXELWTE, _NOTIFY, _DISABLE))
        |
        (awaken ?
            DRF_DEF(C2B0, _EXELWTE, _AWAKEN, _ENABLE) :
            DRF_DEF(C2B0, _EXELWTE, _AWAKEN, _DISABLE))
        |
        (notifyOnBeg == notifyOn ?
            DRF_DEF(C2B0, _EXELWTE, _NOTIFY_ON, _BEGIN) :
            DRF_DEF(C2B0, _EXELWTE, _NOTIFY_ON, _END))
    ));

    return rc;
}

RC ClC2B0Engine::Flush()
{
    RC rc;
...
void ClC2B0Engine::UpdateControlStructsFromPics()
{
    if (h264 == m_initedTo)
    {
...
RC ClC2B0Engine::InitOutSurfaces(UINT32 width, UINT32 height, UINT32 numPics, GpuDevice *dev,
                                 GpuSubdevice *subDev, LwRm::Handle chHandle)
{
    RC rc;
...
void ClC2B0Engine::BuildH264Picsetup()
{
    using Utility::StrPrintf;
...
void ClC2B0Engine::BuildH265Picsetup()
{
...
RC ClC2B0Engine::CreateH264PushBuffer()
{
    RC rc;
...
RC ClC2B0Engine::CreateH265PushBuffer()
{
    RC rc;
...
ClC2B0Engine::H264Ctrl::H264Ctrl()
{
...
void ClC2B0Engine::H264Ctrl::UpdateFromPicture(const H264::Picture &pic)
{
...
void ClC2B0Engine::H264Ctrl::EnableEncryption(unsigned int keyIncrement, const UINT32 sessionKey[4],
                                              const UINT32 contentKey[4], const UINT32 initVector[4])
{
...
void const * ClC2B0Engine::H264Ctrl::GetPicSetup() const
{
    return &m_ps;
}

size_t ClC2B0Engine::H264Ctrl::GetPicSetupSize() const
{
    return sizeof(m_ps);
}

size_t ClC2B0Engine::H264Ctrl::GetMaxDPBSize() const
{
    return NUMELEMS(m_ps.dpb);
}

unsigned int ClC2B0Engine::H264Ctrl::GetLwrrPicIdx() const
{
    return m_ps.LwrrPicIdx;
}

unsigned int ClC2B0Engine::H264Ctrl::GetLwrrColIdx() const
{
    return m_ps.LwrrColIdx;
}

unsigned int ClC2B0Engine::H264Ctrl::GetDPBPicIdx(size_t idx) const
{
    return m_ps.dpb[idx].index;
}

unsigned int ClC2B0Engine::H264Ctrl::GetDPBColIdx(size_t idx) const
{
    return m_ps.dpb[idx].col_idx;
}

bool ClC2B0Engine::H264Ctrl::IsDPBEntryUsed(size_t idx) const
{
    return 0 != m_ps.dpb[idx].state;
}

void ClC2B0Engine::H264Ctrl::SetLwrrPicIdx(unsigned int val)
{
    m_ps.LwrrPicIdx = val;
    m_ps.LwrrColIdx = val;
}

void ClC2B0Engine::H264Ctrl::UpdateDPB(const H264::Picture &pic, size_t whereIdx,
...
void ClC2B0Engine::H264Ctrl::SetOutputMemoryLayout(MemLayout memLayout)
{
    m_ps.output_memory_layout = MemLayout::LW12 == memLayout ? 0 : 1;
}

void ClC2B0Engine::H264Ctrl::SetHistBufferSize(unsigned int size)
{
    m_ps.HistBufferSize = size;
}

void ClC2B0Engine::H264Ctrl::EnableHistogram(short startX, short startY, short endX, short endY)
{
...
void ClC2B0Engine::H264Ctrl::SetPitchOffset(const Surface2D &surfY, const Surface2D &surfUV)
{
...
void ClC2B0Engine::H264Ctrl::SetSlicesSize(unsigned int slicesSize)
{
    m_ps.stream_len = slicesSize;
}

unsigned int ClC2B0Engine::H264Ctrl::GetSlicesSize(void const *buf) const
{
...
void ClC2B0Engine::H264Ctrl::SetSlicesCount(unsigned int slicesCount)
{
    m_ps.slice_count = slicesCount;
}

void ClC2B0Engine::H264Ctrl::SetMBHistBufSize(unsigned int mbHistBufSize)
{
    m_ps.mbhist_buffer_size = mbHistBufSize;
}

void const * ClC2B0Engine::H264Ctrl::GetSliceOffsets() const
{
    return &m_sliceOffsets[0];
}

size_t ClC2B0Engine::H264Ctrl::GetSliceOffsetsSize() const
{
    return sizeof(m_sliceOffsets[0]) * m_sliceOffsets.size();
}

string ClC2B0Engine::H264Ctrl::PicSetupToText(void const *buf) const
{
    const lwdec_h264_pic_s *ps = reinterpret_cast<const lwdec_h264_pic_s *>(buf);
...
std::string ClC2B0Engine::H264Ctrl::SliceOffsetsToText(void const *buf, size_t numSlices) const
{
    using Utility::StrPrintf;
...
ClC2B0Engine::H265Ctrl::H265Ctrl()
{
    memset(&m_ps, 0, sizeof(m_ps));
...
void const* ClC2B0Engine::H265Ctrl::GetPicSetup() const
{
    return &m_ps;
}

size_t ClC2B0Engine::H265Ctrl::GetPicSetupSize() const
{
    return sizeof(m_ps);
}

void const* ClC2B0Engine::H265Ctrl::GetScalingMatrices() const
{
    return &m_sm;
}

size_t ClC2B0Engine::H265Ctrl::GetScalingMatricesSize() const
{
    return sizeof(m_sm);
}

void const* ClC2B0Engine::H265Ctrl::GetTileInfo() const
{
    return &m_ti[0];
}

size_t ClC2B0Engine::H265Ctrl::GetTileInfoSize() const
{
    return m_ti.size() * TileInfoValSize;
}

void ClC2B0Engine::H265Ctrl::UpdateFromPicture(const H265::Picture &pic)
{
...
void ClC2B0Engine::H265Ctrl::UpdateDPB(INT32 lwrrPicPoc, const UINT08 l0[H265::maxNumRefPics],
...
void ClC2B0Engine::H265Ctrl::SetLwrrPicIdx(unsigned int lwrPicIdx)
{
    m_ps.lwrr_pic_idx = lwrPicIdx;
}

void ClC2B0Engine::H265Ctrl::SetPitch(const Surface2D &surfY, const Surface2D &surfUV,
                                         OutputSize outSize)
{
...
void ClC2B0Engine::H265Ctrl::SetColMvBufSize(unsigned int size)
{
    m_ps.colMvBuffersize = size / 256;
}

void ClC2B0Engine::H265Ctrl::SetSaoBufferOffset(unsigned int offset)
{
    m_ps.HevcSaoBufferOffset = offset;
}

void ClC2B0Engine::H265Ctrl::SetBsdCtrlOffset(unsigned int offset)
{
    m_ps.HevcBsdCtrlOffset = offset;
}

void ClC2B0Engine::H265Ctrl::SetFilterAboveBufferOffset(unsigned int offset)
{
    m_ps.v1.hevc_main10_444_ext.HevcFltAboveOffset = offset;
}

void ClC2B0Engine::H265Ctrl::SetSaoAboveBufferOffset(unsigned int offset)
{
    m_ps.v1.hevc_main10_444_ext.HevcSaoAboveOffset = offset;
}

void ClC2B0Engine::H265Ctrl::SetSlicesSize(unsigned int slicesSize)
{
    m_ps.stream_len = slicesSize;
}

unsigned int ClC2B0Engine::H265Ctrl::GetSlicesSize(void const *buf) const
{
...
string ClC2B0Engine::H265Ctrl::PicSetupToText(void const * buf) const
{
    using Utility::StrPrintf;
...
string ClC2B0Engine::H265Ctrl::ScalingMatricesToText(void const *buf) const
{
    using Utility::StrPrintf;
...
string ClC2B0Engine::H265Ctrl::TileInfoToText(void const * buf,
...
ClC2B0Engine::H265Ctrl::ScalingMatrices::ScalingMatrices()
{
