...
//
// pseudo-code:
//  assume we are already in hi-res mode with HDCP display
//  use RM's existing LW0073_CTRL_CMD_SPECIFIC_HDCP_CTRL interface
//    data to RM:
//     command = _READ_LINK_STATUS
...
//     bKsv (B's key-selection-vector)
//     aN (A's 64-bit random key)
//     cS (connection state)
//     stStatus
...
//         cN, bKsv,
//         aN, cS, stStatus)
//  Check all Ksv values for 20 0's and 20 1's, else FAIL
...
JS_CLASS_INHERIT(HdcpKey, GpuTest,
                 "HDCP key test.");

CLASS_PROP_READWRITE(HdcpKey, MaxKpToTest, UINT32,
                     "Max tmds links to check Kp for");

CLASS_PROP_READWRITE(HdcpKey, DisplayToTest, UINT32,
                     "Select a display to test from multiple active displays");

CLASS_PROP_READWRITE(HdcpKey, SkipHDCPUpstreamErrors, bool,
            "Skip the kp mismatch error in the upstream and treat it as warning (Bug 3494696)");
//----------------------------------------------------------------------------
HdcpKey::HdcpKey()
{
    SetName("HdcpKey");
    m_MaxKpToTest = 2;
    m_DisplayToTest = 0;
    m_SkipHDCPUpstreamErrors = false;
}
...
RC HdcpKey::Run()
{
    ...
    if(m_DisplayToTest != 0)
        selected = m_DisplayToTest;

    Printf(Tee::PriLow, "HdcpKey::Run on display 0x%06x\n", selected);

    if(GetBoundGpuDevice()->HasBug(529851))
    {
        UINT32 lwrrentPState = 0;
        CHECK_RC(GetBoundGpuSubdevice()->GetPerf()->
                 GetLwrrentPState(&lwrrentPState));
        if(lwrrentPState == 12)
        {
            // See bug 549500
            Printf(Tee::PriNormal, " Current PState = 12, HDCP-key test skipped.\n");
            return OK;
        }
    }
    ...
    bool isHdcpEnabled;
    bool isDisplayCapable;
    bool isGpuCapable;
    bool isHdcp22Capable = false;
    bool isHdcp22Enabled = false;

    CHECK_RC(pDisplay->GetHDCPInfo(selected, &isHdcpEnabled,
                &isDisplayCapable, &isGpuCapable, &isHdcp22Capable,
                &isHdcp22Enabled, true)); //hdcp cached status
    Printf(Tee::PriLow, " HDCP for display 0x%06x is %s.\n",
           selected, isHdcpEnabled ? "ON" : "OFF");
    Printf(Tee::PriLow, "  Board is %scapable of HDCP.\n",
           isGpuCapable ? "" : "not ");
    Printf(Tee::PriLow, " Display is %scapable of",
            (isDisplayCapable || isHdcp22Capable)? "": "not ");
    Printf(Tee::PriLow, " HDCP %s", isHdcp22Capable ? "and HDCP2.2\n": "\n");

    if (!(isHdcpEnabled || isHdcp22Enabled))
    {
        Printf(Tee::PriNormal, " HDCP is not on, HDCP-key test skipped.\n");
        return OK;
    }

    // Get hdcp data from the RM on current display.
    ...
    pparams->subDeviceInstance = GetBoundGpuDevice()->GetDisplaySubdeviceInst();
    pparams->displayId = selected;
    pparams->cmd = LW0073_CTRL_SPECIFIC_HDCP_CTRL_CMD_READ_LINK_STATUS;
...

    {
        ...
        UINT32 ApsToTest = Utility::CountBits((UINT32)
            DRF_VAL(0073, _CTRL_SPECIFIC_HDCP_CTRL, _CS_ATTACH_POINTS, pparams->cS));

        // Check all the Key-Selection-Vectors to be sure they all have
        // exactly 20 1's in them.
        // Report all the errors before exiting the test.
        UINT32 i;
        RC tempRc;

        for (i = 0; i < ApsToTest; i++)
        {
            tempRc = CheckKsv(pparams->aKsv[i], "A", i);
            if (OK == rc)
                rc = tempRc;
        }
        if (!isHdcp22Enabled)
        {
            //Bksv should not be checked for hdcp2.2 encryption
            for (i = 0; i < ApsToTest; i++)
            {
                tempRc = CheckKsv(pparams->bKsv[i], "B", i);
                if (OK == rc)
                    rc = tempRc;
            }
        }

        tempRc = CheckKsv(pparams->cKsv, "C", 0);
        if (OK == rc)
            rc = tempRc;

        for (i = 0; i < ApsToTest && i < m_MaxKpToTest; i++)
        {
            tempRc = CheckKsv(pparams->dKsv[i], "D", i);
            if (OK == rc)
                rc = tempRc;
        }

        // @@@ TO DO:
        // As David Wyatt warns me, this logic is not quite right.
        // It will fail in one particular scenario: two HDCP displays in clone
        // mode.  We can't hit this scenario right now (winter 2007).
        //
        // The right thing to do is to enable all detected HDCP displays at once
        // in clone mode, query Cs, and iterate over each attach-point in Cs.
        // Need to revisit this in g9x timeframe.

        std::vector<UINT64> NumPortsOnApIndex;
        ...
        for (i = 0; i < ApsToTest && i < m_MaxKpToTest; i++)
        {
            ...
            Printf(Tee::PriDebug,
                   "  link %d (of 0 to %d) reports Cs is 0x%llx,  S is 0x%llx\n",
                   i, pparams->linkCount-1, pparams->cS, S);

            if (pparams->apIndex[i] != 0)
            {
                NumPortsOnApIndex.push_back( (((UINT64)pparams->apIndex[i])<<32) |
                    DRF_VAL(0073, _CTRL_SPECIFIC_HDCP_CTRL, _STATUS_S_NUM_PORTS,
                            pparams->stStatus[pparams->apIndex[i]].S) );
            }
            ...
            {
                // Bug 3494696
                // SkipHDCPUpstreamErrors = true => Skip the kp mismatch error in the
                // upstream and treat it as warning
                Tee::Priority kpMismatchPri = m_SkipHDCPUpstreamErrors ? Tee::PriWarn :
                    Tee::PriError;
                Printf(kpMismatchPri, " kP[%d] doesn't match: %llx %llx\n",
                       i, kP, pparams->kP[i]);
                if (!m_SkipHDCPUpstreamErrors)
                { 
        ...
        // Status[11:8] check requested in bug 492222:
        bool AllNumPortsMatch = true;
        for (UINT32 Idx = 1; Idx < NumPortsOnApIndex.size(); Idx++)
        {
            if ( (NumPortsOnApIndex[0] & 0xFFFFFFFF) !=
                 (NumPortsOnApIndex[Idx] & 0xFFFFFFFF) )
            {
                AllNumPortsMatch = false;
                break;
            }
        }
        ...
        {
            Printf(Tee::PriError,
                   " NUM_PORTS mismatch on display 0x%08x:\n", selected);

            for (UINT32 Idx = 0; Idx < NumPortsOnApIndex.size(); Idx++)
            {
                Printf(Tee::PriHigh,
                   "  ApIdx%d: NUM_PORTS = %d\n",
                   UINT32(NumPortsOnApIndex[Idx] >> 32),
                   UINT32(NumPortsOnApIndex[Idx] & 0xFFFFFFFF));
            }
...
)
{
    ...
    if (init != HdcpKey::LfsrInitMode::NoInit)
    {
        s_A[i] = 0;
        s_B[i] = 1;
    }
...
)
{
    ...
    if (init==HdcpKey::LfsrInitMode::InitA)
    {
        for (i=22;i>19;i--)
    ...
    else if (init==HdcpKey::LfsrInitMode::InitB)
    {
        for (i=22;i>19;i--)
...
//         mode == HdcpKey::LfsrInitMode::InitA ? "initA" : mode == HdcpKey::LfsrInitMode::InitB ? "initB" : "",
//         data,
//         key,
//         Result);
//     PrintGlobalState();
...
)
{
    ...
    Cn56 = Cn;
    PartCn = Cn56 & MASK40;
    OneWay(PartCn, Ku, HdcpKey::LfsrInitMode::InitA);

    for (i=0; i<56+32; i++)
    {
        K1 = K1 >> 1;
        K1 |= OneWay(PartCn, Ku, HdcpKey::LfsrInitMode::NoInit) ? MSB56 : 0;
    }
    ...
    OneWay(Bksv, K1, HdcpKey::LfsrInitMode::InitA);
    for (i=0; i<56+32; i++)
    {
        K2 = K2 >> 1;
        K2 |= OneWay(Bksv, K1, HdcpKey::LfsrInitMode::NoInit) ? MSB56 : 0;
    }
    ...
    An >>= 24;
    OneWay(An, K2, HdcpKey::LfsrInitMode::InitA);
    for (i=0; i<56+32; i++)
    {
        K3 = K3 >> 1;
        K3 |= OneWay(An, K2, HdcpKey::LfsrInitMode::NoInit) ? MSB56 : 0;
    }
    ...
    OneWay(StatCn, K3, HdcpKey::LfsrInitMode::InitA);
    for (i=0; i<56+32; i++)
    {
        K4 = K4 >> 1;
        K4 |= OneWay(StatCn, K3, HdcpKey::LfsrInitMode::NoInit) ? MSB56 : 0;
    }
    ...
    {
        // Kp' = oneWay-A56(K4', Cs)
        OneWay(Cs, K4, HdcpKey::LfsrInitMode::InitA);

        for (i=0; i<56+32; i++)
        {
            Kp = Kp >> 1;
            Kp |= OneWay(Cs, K4, HdcpKey::LfsrInitMode::NoInit) ? MSB56 : 0;
        }
    }
    else
    {
