...
#include "gpu/tests/lwca/mats/tiledmats.h"

//------------------------------------------------------------------------------
//! \brief LWCA memory test utilizing the Tensor Memory Access (TMA) unit
//!
class LwdaTiledMats final : public LwdaMemTest
{
public:
    LwdaTiledMats();
    virtual ~LwdaTiledMats() = default;
...
LwdaTiledMats::LwdaTiledMats()
{
    SetName("LwdaTiledMats");
}

RC LwdaTiledMats::SetPatterns(JsArray *val)
{
    JavaScriptPtr pJs;
    return pJs->FromJsArray(*val, &m_Patterns);
}

RC LwdaTiledMats::GetPatterns(JsArray *val) const
{
    JavaScriptPtr pJs;
    return pJs->ToJsArray(m_Patterns, val);
}

RC LwdaTiledMats::SetBoxDims(JsArray *val)
{
    JavaScriptPtr pJs;
    return pJs->FromJsArray(*val, &m_BoxDims);
}

RC LwdaTiledMats::GetBoxDims(JsArray *val) const
{
    JavaScriptPtr pJs;
...
//!
RC LwdaTiledMats::GetPatternByName()
{
    RC rc;
...
//!
void LwdaTiledMats::PrintJsProperties(Tee::Priority pri)
{
    LwdaMemTest::PrintJsProperties(pri);
    Printf(pri, "LwdaTiledMats Js Properties:\n");
...
//------------------------------------------------------------------------------
//! \brief Check whether LwdaTiledMats is supported
//!
bool LwdaTiledMats::IsSupported()
{
...
RC LwdaTiledMats::Setup()
{
    ...
    CHECK_RC(GetLwdaInstance().LoadNewestSupportedModule("tiledmats", &m_Module));
    ...
    m_TiledMatsFunc = m_Module.GetFunction("TiledMats", m_NumBlocks, m_NumStressThreads);
    m_TiledMatsFunc.InitCheck();
    ...
    CHECK_RC(m_TiledMatsFunc.SetSharedSizeMax());
    ...
    if (m_UseTMA)
    {
        ...
        m_TiledMatsFunc.GetSharedSize(&dynamicSharedMemSize);
...
//!
void LwdaTiledMats::CallwlateKernelGeometry()
{
    ...
    // TODO: revisit this Tiled MATS kernel - just 1 thread per block enough?
...
//!
const char * LwdaTiledMats::GetFillFuncName()
{
...
//!
const char * LwdaTiledMats::GetCheckFuncName()
{
...
//!
RC LwdaTiledMats::SetupErrorReporting()
{
    RC rc;
...
//!
RC LwdaTiledMats::DetermineTestableChunks(vector<ChunkData> *pTestableChunks)
{
    RC rc;
...
//!
bool LwdaTiledMats::GetTestRange
(
    UINT32 ichunk,
    UINT64* pStartOffset,
    UINT64* pEndOffset
...
//!       [12, 15, 18]
//!
RC LwdaTiledMats::SetupTensorDims(ChunkData *pChunk)
{
    RC rc;
...
//!
void LwdaTiledMats::InitTMATensorDescriptor
(
    ChunkData *pChunk
)
{
    ...
    /*GlobalMemAddr*/          reinterpret_cast<void*>(pChunk->tiledMatsInput.mem),
...
//!
void LwdaTiledMats::InitBaseInput(ChunkData *pChunk)
{
    ...
    (
        GetVerbosePrintPri(),
        "LwdaTiledMats: Running %s on %.1fMB of memory,"
...
//!
RC LwdaTiledMats::SetTensorDescriptor
(
    lwdaTmaDescv2 *pDesc
)
{
...
)
{
    ...
    pChunk->tiledMatsInput.startingBoxIdx = startingBoxIdx;
    pChunk->tiledMatsInput.numBoxesInBatch = numBoxesInBatch;

    CHECK_RC(m_TiledMatsFunc.Launch(pChunk->tiledMatsInput));
...
//!
RC LwdaTiledMats::LaunchBatchesWithPulse
(
    ChunkData *pChunk
)
{
    ...
    const UINT64 totalNumBoxes =
        pChunk->tiledMatsInput.numBoxesInTensor * m_NumInnerIterations;
    ...
    while (numBoxesIteratedSoFar < totalNumBoxes)
    {
        ...
        const UINT64 startingBoxIdx =
            numBoxesIteratedSoFar % pChunk->tiledMatsInput.numBoxesInTensor;
...
//!
RC LwdaTiledMats::RunChunkOuterIteration
(
    ...
    for (auto& chunk : *pTestableChunks)
    {
        ...
        else
        {
            const UINT64 numBoxesInBatch =
                chunk.tiledMatsInput.numBoxesInTensor * m_NumInnerIterations;
...
//!
bool LwdaTiledMats::ErrorsFound()
{
...
//! Valid error results
//!
RC LwdaTiledMats::CheckResultsCorrupted()
{
    const RangeErrors* const results =
        reinterpret_cast<const RangeErrors*>(m_HostResults.GetPtr());
...
//!
RC LwdaTiledMats::ChunkData::Initialize
(
    UINT32 seed,
...
JS_CLASS_INHERIT(LwdaTiledMats, LwdaMemTest,
                 "Lwca Tiled MATS test");
CLASS_PROP_READWRITE(LwdaTiledMats, RuntimeMs, UINT32,
                     "How long to run the active portion of the test, in milliseconds. Overrides \"Iterations\"");
CLASS_PROP_READWRITE(LwdaTiledMats, Iterations, UINT32,
                     "Number of times to repeat test");
CLASS_PROP_READWRITE(LwdaTiledMats, MaxInnerIterations, UINT32,
                     "Maximum number of iterations with which to ilwoke the kernel");
CLASS_PROP_READWRITE(LwdaTiledMats, StartPhys, UINT64,
                     "Starting physical address");
CLASS_PROP_READWRITE(LwdaTiledMats, EndPhys, UINT64,
                     "Ending physical address");
CLASS_PROP_READWRITE(LwdaTiledMats, UseRandomData, bool,
                     "Use random data in place of Patterns. Each LWCA thread uses different random values");
CLASS_PROP_READWRITE(LwdaTiledMats, SkipErrorReporting, UINT64,
                     "Skip reporting of memory errors");
CLASS_PROP_READWRITE_JSARRAY(LwdaTiledMats, Patterns, JsArray,
                     "User-defined array of patterns");
CLASS_PROP_READWRITE(LwdaTiledMats, PatternSubsetSize, UINT32,
                     "Size of pattern subsets. Each box is filled sequentially with patterns from the corresponding subset");
CLASS_PROP_READWRITE(LwdaTiledMats, PatternName, string,
                     "Name of pattern to be used in LwdaTiledMats when explicit Patterns are not provided.");
CLASS_PROP_READWRITE_JSARRAY(LwdaTiledMats, BoxDims, JsArray,
                     "List of box dimension lengths.");
CLASS_PROP_READWRITE(LwdaTiledMats, UseTMA, bool,
                     "Use TMA hardware and instructions to perform memory test");
CLASS_PROP_READWRITE(LwdaTiledMats, PrefetchMode, bool,
                     "Prefetch patterns from FB to cache. Will not check for errors in this mode");
CLASS_PROP_READWRITE(LwdaTiledMats, PulseUs, double,
                     "Time spent by the kernel doing memory accesses in microseconds.");
CLASS_PROP_READWRITE(LwdaTiledMats, DelayUs, double,
