...
typedef enum gemmType_t {   // aclwmulator type
    ...
    I16832LWDNN,   // Int8-precision using IMMA instruction
    I8816LWDNN,  // Int8-precision using IMMA instruction
...
// Enum values match SW's lwdaDataType (lwblasDataType_t) taken from //sw/gpgpu/lwca/tools/lwdart/library_types.h
...
inline size_t elementDataSize(const elementDataType_t e) {
    switch (e) {
    case R_16F : return  2;
    case C_16F : return  4;
    case R_32F : return  4;
    ...
    default    :
#ifdef LW_MODS
        MASSERT(false && "Unknown elementDataType_t value"); return 0;
#else
        assert(false && "Unknown elementDataType_t value"); return 0;
#endif
    } // switch
} // elementDataSize
...
// Enum values match SW's lwblasFillMode_t, with RECTANGLE added
...
typedef enum lwDnnLayout_t {
    NCHW,
    NHWC
} lwDnnLayout_t;

/*
// Defined in //sw/gpgpu/lwblas/src/lwblas_api.h
typedef enum {
    LWBLAS_OP_N=0,  
    LWBLAS_OP_T=1,  
    LWBLAS_OP_C=2  
...
/*  GEMM modes of operation.  Mode is a combination of mode flags.
    ...
    and atomic vs spin-lock aclwmulation.  TODO: can't you do both, to get
    ...
    mode&0x0004: Split-k-across-CTAs GEMM, chunks indexed by CTAid.Z.  This
                 ...
                 aclwmulation" flag set.

    mode&0x0008: Split-k-across-CTAS deterministic aclwmulation of C via
                 spin-waits on counters in client-zeroed "Sync" memory.

    mode&0x0010: Split-k-across-CTAS non-deterministic aclwmulation via
    ...
    mode&0x0020: Split-k-across-CTAS, deterministic aclwmulation in another 
                 kernel
...
struct ShaderParams {
    ...
    // LWDNN properties
    edgeType_t    lwDnnEdges;
    lwDnnLayout_t lwDnnLayout;
    bool          lwDnnStridedB;
    bool          lwDnnSplitK;
    bool          lwDnnDgrad;
    bool          lwDnnWgrad;
