...
public:
    ...
    public: 
        ConstDiv(unsigned divisor = 1U) : divisor(divisor) {
            MASSERT(divisor != 0 && "ConstDiv cannot divide by 0");            
    ...
    Pos2 colwStride;    // (row, col) size of implicit GEMM filter's stride.
    ...
    CtaSwizzle(
        ...
        const Pos2 &ctaTile,
        const Pos2 &filter,
        const Pos2 &colwStride,
        const Pos2 &outputSize) : gridDim(gridDim), ctaTile(ctaTile), filter(filter),
            colwStride(colwStride), outputSize(outputSize)
    {
#if 0
        Printf(Tee::PriDebug, "CtaSwizzle created with gridDim(%d, %d, %d), ctaTile(%d, %d)\n", 
        ...
        // data nearly filter.col / colwStride.col times within a CTA as we 
        ...
        // (filter.row / colwStride.row) * (filter.col / colwStride.col)
        ...
        // So we'll just play it safe and use filter.col / ColwStride_W for
        ...
        defaultAReplication = std::max(1U, filter.col / colwStride.col);
              ...
              bool     serpentine) const
    {
        ...
        if (groupCols > gridDim.col) {
            Printf(Tee::PriDebug, "groupCols=%d > gridDim.col=%d, which not only causes SASS to waste cycles reducing to a reasonable value, but also results in incorrect computation of up/down direction.  The client should ensure the stated condition.", groupCols, gridDim.col);
            MASSERT(false);
        }
        if ((pos.row >= gridDim.row) || (pos.col >= gridDim.col)
                || (pos.batch >= gridDim.batch)) {
            Printf(Tee::PriDebug, "pos(%d, %d, %d) is outside of gridDim(%d, %d, %d)\n",
...
#if 0
        Printf(Tee::PriDebug, "linearLocal=%d, colInGroup3=%d\n", linearLocal, colInGroup3);
#endif
        ...
        const bool     serpentine) const
    {
        ...
        if (index >= ctasPerBatch) {
            ...
            if ((batch != batch2) || (batchMod != batchMod2)) {
                Printf(Tee::PriDebug, "Const div failed: batch = %d, batch2 = %d, batchMod = %d, batchMod2 = %d\n",
        ...
        if ((col != col2) || (row != row2)) {
            Printf(Tee::PriDebug, "Const div failed: col = %d, col2 = %d, row = %d, row2 = %d\n",
                col, col2, row, row2);
            MASSERT(false);
        }
#endif
        ...
        const bool serpentine) const
    {
        if (first.batch != last.batch) {
            Printf(Tee::PriDebug, "first.batch=%d != last.batch=%d\n",
        ...
        if (firstBColBase == lastBColBase) {
            ...
            if (serpentine) {
                ...
                // the last CTA.  
#ifdef DEBUG
                MASSERT((first.bottomToTop != last.bottomToTop)
        ...
        const bool     serpentine)
    {
        ...
        if ((ctasPerWave <= groupCols) && (groupCols > 1)) {
            Printf(Tee::PriDebug, "It makes no sense to have ctasPerWave=%d <= groupCols=%d, as the whole point is to share *both* A and B data.  Further, this code depends upon this condition.", ctasPerWave, groupCols);
        }
#endif
        Pos2 total = Pos2(0, 0);
...
#if defined(DEBUG)
           Printf(Tee::PriDebug, "log2 = %d, ctas %3d..%3d, batch = %d..%d: A height = %d, B width = %d\n",
    ...
    unsigned chooseBestLog2GroupCols(unsigned ctasPerWave) {
        ...
        for (unsigned log2GroupCols = 0;
             ...
             log2GroupCols++) {
            unsigned groupCols = 1 << log2GroupCols;
            if (groupCols > gridDim.col) break;
            ...
            if (filter.col != 1 || filter.row != 1) { 
                ...
                if (outputSize.col < rowFetchedPerWave) {
                    aReplication *= std::max(1U, std::min(rowFetchedPerWave / outputSize.col, filter.row) / colwStride.row);
                }
            }
...
#if defined(DEBUG)
            Printf(Tee::PriDebug, "A total = %d, B total = %d, scaled fetches = %d\n",
