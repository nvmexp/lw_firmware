...
namespace details {
    ...
    struct scan_dispatch<details::coalesced_group_id, TyScan> {
        ...
        _CG_STATIC_QUALIFIER auto scan(const TyGroup& group, TyVal&& val, TyFn&& op) -> decltype(op(val, val)) {
            ...
            if (TyScan == ScanType::exclusive) {
                scan_result = colwert_inclusive_to_exclusive(group,
                                                             scan_result,
    ...
    struct scan_dispatch<details::multi_tile_group_id, TyScan> {
        ...
        _CG_STATIC_QUALIFIER auto scan(const thread_block_tile<Size, ParentT>& group, TyVal&& val, TyFn&& op) -> decltype(op(val, val)) {
            ...
            // In warp scan result, callwlated in warp_lambda
            TyRet warp_scan;
            ...
            auto inter_warp_lambda =
                [&] (const details::internal_thread_block_tile<num_warps, warpType>& subwarp, TyRet* thread_scratch_location) {
                    ...
                    *thread_scratch_location = colwert_inclusive_to_exclusive(subwarp, result, thread_val, op);
            };
    ...
    struct scan_update_dispatch<details::coalesced_group_id, TyScan> {
        ...
        _CG_STATIC_QUALIFIER auto scan(const TyGroup& group, TyAtomic& dst, TyVal&& val, TyFn&& op) -> decltype(op(val, val)) {
            ...
            if (TyScan == ScanType::exclusive) {
                scan_result = colwert_inclusive_to_exclusive(group, scan_result, _CG_STL_NAMESPACE::forward<TyVal>(val), op);
            }
        ...
        _CG_STATIC_QUALIFIER auto scan(const thread_block_tile<Size, ParentT>& group, TyAtomic& dst, TyVal&& val, TyFn&& op) -> decltype(op(val, val)) {
            ...
            auto inter_warp_lambda =
                [&] (const details::internal_thread_block_tile<num_warps, warpType>& subwarp, TyRet* thread_scratch_location) {
                    ...
                    scan_result = colwert_inclusive_to_exclusive(subwarp, scan_result, thread_val, op);
                    // Add offset read from the atomic to the scanned warp sum.
                    // Skipping first thread, since it got defautly constructed value from the colwersion,
...
namespace experimental {
    template<typename TyGroup, typename TyVal, typename TyInputVal, lwca::thread_scope Sco, typename TyFn>
    _CG_QUALIFIER auto inclusive_scan_update(const TyGroup& group, lwca::atomic<TyVal, Sco>& dst, TyInputVal&& val, TyFn&& op) -> decltype(op(val, val)) {
    ...
    template<typename TyGroup, typename TyVal, typename TyInputVal, lwca::thread_scope Sco>
    _CG_QUALIFIER TyVal inclusive_scan_update(const TyGroup& group, lwca::atomic<TyVal, Sco> & dst, TyInputVal&& val) {
        return inclusive_scan_update(group, dst, _CG_STL_NAMESPACE::forward<TyInputVal>(val), cooperative_groups::plus<TyVal>());
    }

    template<typename TyGroup, typename TyVal, typename TyInputVal, lwca::thread_scope Sco, typename TyFn>
    _CG_QUALIFIER auto exclusive_scan_update(const TyGroup& group, lwca::atomic<TyVal, Sco>& dst, TyInputVal&& val, TyFn&& op) -> decltype(op(val, val)) {
        details::check_scan_update_params<TyGroup, TyVal, details::remove_qual<TyInputVal>, decltype(op(val, val))>();
    ...
    template<typename TyGroup, typename TyVal, typename TyInputVal, lwca::thread_scope Sco>
    _CG_QUALIFIER TyVal exclusive_scan_update(const TyGroup& group, lwca::atomic<TyVal, Sco>& dst, TyInputVal&& val) {
        return exclusive_scan_update(group, dst, _CG_STL_NAMESPACE::forward<TyInputVal>(val), cooperative_groups::plus<TyVal>());
    }

    template<typename TyGroup, typename TyVal, typename TyInputVal, lwca::thread_scope Sco, typename TyFn>
    _CG_QUALIFIER auto inclusive_scan_update(const TyGroup& group, const lwca::atomic_ref<TyVal, Sco>& dst, TyInputVal&& val, TyFn&& op) -> decltype(op(val, val)) {
        details::check_scan_update_params<TyGroup, TyVal, details::remove_qual<TyInputVal>, decltype(op(val, val))>();
    ...
    template<typename TyGroup, typename TyVal, typename TyInputVal, lwca::thread_scope Sco>
    _CG_QUALIFIER TyVal inclusive_scan_update(const TyGroup& group, const lwca::atomic_ref<TyVal, Sco> & dst, TyInputVal&& val) {
        return inclusive_scan_update(group, dst, _CG_STL_NAMESPACE::forward<TyInputVal>(val), cooperative_groups::plus<TyVal>());
    }

    template<typename TyGroup, typename TyVal, typename TyInputVal, lwca::thread_scope Sco, typename TyFn>
    _CG_QUALIFIER auto exclusive_scan_update(const TyGroup& group, const lwca::atomic_ref<TyVal, Sco>& dst, TyInputVal&& val, TyFn&& op) -> decltype(op(val, val)) {
        details::check_scan_update_params<TyGroup, TyVal, details::remove_qual<TyInputVal>, decltype(op(val, val))>();
    ...
    template<typename TyGroup, typename TyVal, typename TyInputVal, lwca::thread_scope Sco>
    _CG_QUALIFIER TyVal exclusive_scan_update(const TyGroup& group, const lwca::atomic_ref<TyVal, Sco>& dst, TyInputVal&& val) {
        return exclusive_scan_update(group, dst, _CG_STL_NAMESPACE::forward<TyInputVal>(val), cooperative_groups::plus<TyVal>());
    }
}
