...
struct scan_choose_colwertion;

template<>
struct scan_choose_colwertion<true> {
    template <typename TyGroup, typename TyRes, typename TyVal>
    _CG_STATIC_QUALIFIER details::remove_qual<TyVal> colwert_inclusive_to_exclusive(const TyGroup& group, TyRes& result, TyVal&& val) {
        return result - val;
    }
};

template<>
struct scan_choose_colwertion<false> {
    template <typename TyGroup, typename TyRes, typename TyVal>
    _CG_STATIC_QUALIFIER details::remove_qual<TyVal> colwert_inclusive_to_exclusive(const TyGroup& group, TyRes& result, TyVal&& val) {
...
_CG_QUALIFIER auto colwert_inclusive_to_exclusive(const TyGroup& group, TyRes& result, TyVal&& val, TyFn&& op) -> decltype(op(val, val)) {
    using colwersion = scan_choose_colwertion<_CG_STL_NAMESPACE::is_same<remove_qual<TyFn>, cooperative_groups::plus<remove_qual<TyVal>>>::value
                                 && _CG_STL_NAMESPACE::is_integral<remove_qual<TyVal>>::value>;
    return colwersion::colwert_inclusive_to_exclusive(group, result, _CG_STL_NAMESPACE::forward<TyVal>(val));
}
