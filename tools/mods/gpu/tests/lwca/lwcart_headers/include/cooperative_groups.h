...
#include <lwca/atomic>
#define _CG_THREAD_SCOPE(scope) _CG_STATIC_CONST_DECL lwca::thread_scope thread_scope = scope;
#else
 ...
 * class thread_group;
 *
 * Generic thread group type, into which all groups are colwertible.
...
public:
    _CG_THREAD_SCOPE(lwca::thread_scope::thread_scope_device)
 ...
 * lwLaunchCooperativeKernelMultiDevice (or the LWCA Runtime equivalent),
...
public:
    _CG_THREAD_SCOPE(lwca::thread_scope::thread_scope_system)
 ...
 * must have been launched with lwLaunchCooperativeKernel (or the LWCA Runtime equivalent),
 * and the device must support it (queryable device attribute).
 *
 ...
 * class cluster_group
 *
 * Every GPU kernel is exelwted by a grid of thread blocks. A grid can be evenly
 ...
 * grids are subject to additional oclwpancy limitations due to per-cluster
...
class cluster_group : public thread_group_base<details::cluster_group_id>
{
    ...
    {
        // FIXME: Hack until PTX handles calling sync from emulated clusters. As
        // of now, if this ctor is called from something that is not a real
        // cluster, the sync() call throws an exception. To avoid this, before
        // calling sync(), we check to see that we are in a real cluster.
        if (details::cluster::isReal()) {
            // FIXME: Hack until PTX exposes cluster.arrive.relaxed. This should
            // be a barrier.cluster.arrive.relaxed, followed by
 ...
 public:
    //_CG_THREAD_SCOPE(lwca::thread_scope::thread_scope_cluster)
    ...
    _CG_QUALIFIER void barrier_arrive() const
    {
        return details::cluster::barrier_arrive();
    }

    _CG_QUALIFIER void barrier_wait() const
    {
    ...
    template <typename T>
    _CG_QUALIFIER T* map_shared_rank(T *addr, int rank) const
    {
 ...
 * class thread_block
 *
 * Every GPU kernel is exelwted by a grid of thread blocks, and threads within
...
class thread_block : public thread_group_base<details::thread_block_id>
{
 ...
 public:
    ...
    _CG_THREAD_SCOPE(lwca::thread_scope::thread_scope_block)
 ...
 *
 * A group representing the current set of colwerged threads in a warp.
 ...
 * The only specializations lwrrently supported are those with native
 ...
 * of the parent group have ilwoked the operation before resuming exelwtion.
 *
 ...
 * <group_type>.sync()
 *
 * Exelwtes a barrier across the group
 *
