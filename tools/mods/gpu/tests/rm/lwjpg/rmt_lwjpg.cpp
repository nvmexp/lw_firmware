...
//
// RM test to test LWJPG class
//
//
//!
//! \file rmt_lwjpg.cpp
//! \brief RM test to test LWJPG class

//!
//! This file tests LWJPG. The test allocates an object of the class, and tries to
//! write a value to a memory and read it back to make sure it went through. It
//! also checks that lwjpg was able to notify the CPU about the semaphore release
...
class LwjpgRmTest: public RmTest
{
public:
    LwjpgRmTest();
    RC B8D1Test(UINT32 &);
...
//! \brief LwjpgRmTest constructor.
//!
//! \sa Setup
//------------------------------------------------------------------------------
LwjpgRmTest::LwjpgRmTest()
{
    m_gpuAddr = 0LL;
    m_cpuAddr = NULL;
    m_hObj    = 0;
    ...
    m_classAllocator = new LwjpgAlloc();

    SetName("LwjpgRmTest");
}

//! \brief LwjpgRmTest destructor.
//!
//! \sa Cleanup
//------------------------------------------------------------------------------
LwjpgRmTest::~LwjpgRmTest()
...
//! \return true/false
//------------------------------------------------------------------------------
bool LwjpgRmTest::IsInstanceSupported(UINT32 instNum)
{
    RC     rc;
    ...
    return (engineId == LW2080_ENGINE_TYPE_LWJPEG(instNum));
}
...
//! \return True
//------------------------------------------------------------------------------
string LwjpgRmTest::IsTestSupported()
{
    ...
    if (m_classAllocator->IsSupported(pGpuDev))
    {
        ...
        {
            Printf(Tee::PriHigh,
                   "Test is supported on the current GPU/platform, preparing to run on LWJPG%d...\n",
                   GetInstanceId());
    ...
    else
    {
        ...
        {
            case LWB8D1_VIDEO_LWJPG:
                return "Class LWB8D1_VIDEO_LWJPG Not supported on GPU";
            case LWC4D1_VIDEO_LWJPG:
                return "Class LWC4D1_VIDEO_LWJPG Not supported on GPU";
            case LWC9D1_VIDEO_LWJPG:
                return "Class LWC9D1_VIDEO_LWJPG Not supported on GPU";
...
RC LwjpgRmTest::Setup()
{
    ...
    // Allocate channel
    CHECK_RC(m_TestConfig.AllocateChannel(&m_pCh, &m_hCh, LW2080_ENGINE_TYPE_LWJPEG(GetInstanceId())));
    ...
    CHECK_RC(m_classAllocator->AllocOnEngine(m_hCh,
                                             LW2080_ENGINE_TYPE_LWJPEG(GetInstanceId()),
    ...
    CHECK_RC(pLwRm->AllocEvent(pLwRm->GetSubdeviceHandle(GetBoundGpuSubdevice()),
                               &hNotifyEvent,
                               LW01_EVENT_OS_EVENT,
                               LW2080_NOTIFIERS_LWJPEG(GetInstanceId()) | LW01_EVENT_NONSTALL_INTR,
                               pEventAddr));

    eventParams.event  = LW2080_NOTIFIERS_LWJPEG(GetInstanceId());
...
//-----------------------------------------------------------------------------
//! \brief B8D1Test test by writing different value to the same memory through
//!        GPU mapping for B8D1 class.
//!
//! \return void
//-----------------------------------------------------------------------------
RC LwjpgRmTest::B8D1Test(UINT32 &subch)
{
    if (m_TestRc)
    {
        RC errorRc;
        ...
        m_pCh->Write(subch, LWB8D1_SEMAPHORE_A,
                        DRF_NUM(B8D1, _SEMAPHORE_A, _UPPER, (UINT32)(0)));
        m_pCh->Write(subch, LWB8D1_SEMAPHORE_B, (UINT32)(0));
        m_pCh->Write(subch, LWB8D1_SEMAPHORE_C,
                        DRF_NUM(B8D1, _SEMAPHORE_C, _PAYLOAD, 0xdeaddead));
        m_pCh->Write(subch, LWB8D1_SEMAPHORE_D,
                        DRF_DEF(B8D1, _SEMAPHORE_D, _STRUCTURE_SIZE, _ONE) |
                        DRF_DEF(B8D1, _SEMAPHORE_D, _AWAKEN_ENABLE, _FALSE));
        m_pCh->Write(subch, LWB8D1_SEMAPHORE_D,
                        DRF_DEF(B8D1, _SEMAPHORE_D, _OPERATION, _TRAP));
        ...
        Printf(Tee::PriHigh,
            "%s: %s RC error oclwrred on LWJPG%d in RC test: %s\n",
    ...
    m_pCh->Write(subch, LWB8D1_SEMAPHORE_A,
                    DRF_NUM(B8D1, _SEMAPHORE_A, _UPPER, (UINT32)(m_gpuAddr >> 32LL)));
    m_pCh->Write(subch, LWB8D1_SEMAPHORE_B, (UINT32)(m_gpuAddr & 0xffffffffLL));
    m_pCh->Write(subch, LWB8D1_SEMAPHORE_C,
                    DRF_NUM(B8D1, _SEMAPHORE_C, _PAYLOAD, 0x12345678));
    m_pCh->Write(subch, LWB8D1_SEMAPHORE_D,
                    DRF_DEF(B8D1, _SEMAPHORE_D, _STRUCTURE_SIZE, _ONE) |
                    DRF_DEF(B8D1, _SEMAPHORE_D, _AWAKEN_ENABLE, _FALSE));
    m_pCh->Write(subch, LWB8D1_SEMAPHORE_D,
...
RC LwjpgRmTest::Run()
{
    ...
    {
        Printf(Tee::PriHigh,
              "LwjpgRmTest: Read/Write matched as expected.\n");
    }
...
//!
//! Set up a JavaScript class that creates and owns a C++ LwjpgRmTest object.
//------------------------------------------------------------------------------
JS_CLASS_INHERIT(LwjpgRmTest, RmTest, "LWJPG RM test.");
CLASS_PROP_READWRITE(LwjpgRmTest, InstanceId, UINT32,
                     "Instance number to test");
CLASS_PROP_READWRITE(LwjpgRmTest, TestRc, bool,
                     "RC recovery test");
