...
//!
//! \file rmt_dp12sst.cpp
//! \brief Tests DP 1.2 sst functionality
//!
...
class DP12SstTest : public RmTest
{
public:
    DP12SstTest();
    virtual ~DP12SstTest();
...
//!         false otherwise
//------------------------------------------------------------------------------
string DP12SstTest::IsTestSupported()
{
...
RC DP12SstTest::sstAllocation()
{
    ...
    CHECK_RC(m_pDisplay->EnumAllocAndGetDPDisplays(dpConnectors[0], &m_pDpConnector, &displayIDs, m_branchDevice, 1));
    ...
    vector<unsigned> width(displayIDs.size(), 1024);
    vector<unsigned> height(displayIDs.size(), 768);
    ...
    {
        CHECK_RC(VerifyImage(*dispItem, width[it], height[it], 32, refreshRate[it], &CoreImage[it]));
        Printf(Tee::PriHigh, "- DisplayID: 0x%x Verified\n", (UINT32) *dispItem);
        if (CoreImage[it].GetMemHandle() != 0)
            CoreImage[it].Free();
        CHECK_RC(m_pDpConnector->DetachDisplay(DisplayIDs(1, *dispItem), *groupItem));
    }
...
RC DP12SstTest::VerifyImage(DisplayID displayID, LwU32 width, LwU32 height, LwU32 depth, LwU32 refreshRate, Surface2D* CoreImage)
{
    RC rc;
...
//------------------------------------------------------------------------------
JS_CLASS_INHERIT(DP12SstTest, RmTest, "DP1.2 Lane count & Bandwidth change test \n");
CLASS_PROP_READWRITE(DP12SstTest, branchDevice, bool,
                     "Actual Branch Device present default = 0");
CLASS_PROP_READWRITE(DP12SstTest, manualVerif, bool,
