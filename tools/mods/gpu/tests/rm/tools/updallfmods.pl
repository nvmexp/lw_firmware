...
# Config Var's
my $date = `date`;
my $logdir = "/home/maheshak/log/";
my $validatefmodellog = $ELW{HOME}."/tmp/chipsa.log";
my $fmodelList = $ELW{HOME}."/.fmodelAttempted";
...
my %regsys =  (
   ...
   "gt215" => "/home/scratch.regress04/linux50/tesla2_tree",
   "gt216" => "/home/scratch.regress04/linux50/tesla2_tree",
   "gt218" => "/home/scratch.regress04/linux50/tesla2_tree",
   "igt21a" => "/home/scratch.regress04/linux50/tesla2_tree",
   "gf100" => "/home/scratch.regress04/linux50/fermi1_gf100"
);

my %fmodlocation =  (
   "gt212" => "/home/scratch.maheshak_fermi-arch/src/",
   "gt215" => "/home/scratch.maheshak_fermi-arch/src/",
   "gt216" => "/home/scratch.maheshak_fermi-arch/src/",
   "gt218" => "/home/scratch.maheshak_fermi-arch/src/",
   "igt21a" => "/home/scratch.maheshak_fermi-arch/src/",
   "gf100" => "/home/scratch.maheshak_fermi-arch/src/"
);
...
sub getlatesthwclregsys {
    ...
    my @revlist = reverse sort(@list);    
    
    print "regsys : $regsysPath  chip : $chip  hwcl : $revlist[0]\n" if $debug;
...
sub fillSwclForChipHwcl {
    ...
    open(SWCL, "$modsChangelist") or die "Can't open file $modsChangelist";
    $swcl = <SWCL>;
    chomp($swcl);
    close(SWCL); 
...
#    system("rm -rf $fmodlocation{$chip}/hw/tesla2") == 0 or die "Unable to remove dir $! : $?";
...
sub syncChipsa {
    ...
    my @p4synclog = `$p4spec sync $ELW{P4ROOT}/sw/dev/gpu_drv/chips_a/... 2>&1`;
...
sub runRmTest {
    ...
    my @modsrunlog = `$ELW{MODS_RUNSPACE}/sim.pl -chip $chip -fmod rmtest.js -dvs 2>&1`; 
        ...
        foreach my $line (@fmodAttempt) {
            ...
            if ($line =~ /$chip/i) {
                if (defined $hwcl) {
                    chomp($line);
                    $line = "$line"." "."$hwcl";
                    print "Updated .fmodelAttempted file with hwcl $hwcl\n";
                    print FMODN "$line\n";
...
#
sub mailStatus {
    my $email_status = "mail -s \"Status of RMTESTS on TOT with latest regsys fmodels \" maheshak\@lwpu.com < $emailLog" ;
