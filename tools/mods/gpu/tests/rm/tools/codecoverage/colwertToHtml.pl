...
use Getopt::Long;
use Cwd;
my ($file,$outputDir,$outputFileName);
...
sub main()
{
    my ($funcName,$functionNameTemp,$coverageData,@tokens);
    ...
    my $outputFile = "$outputDir/$outputFileName";
    ...
    foreach my $line(@lines)
    {
        if($line =~ /\(/)
        {
            ($funcName,$coverageData) = split(/\)\"\,/,$line);
            ($funcName,$functionNameTemp) = split(/\(/,$funcName);
            @tokens = split(',',$coverageData);
            unshift(@tokens,$funcName);		
        }
        else
        {
            @tokens = split(/,/,$line);
