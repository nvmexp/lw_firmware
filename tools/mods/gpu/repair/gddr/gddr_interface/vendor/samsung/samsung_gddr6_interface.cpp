...
#include "samsung_gddr6_interface.h"
...
RC Gddr::SamsungGddr6::Setup()
{
    RC rc;

    CHECK_RC(GddrInterface::Setup());

    // Support checks
    if (m_GddrModel.vendorId != RamVendorId::RamSAMSUNG)
    {
        Printf(Tee::PriError, "Attempt to setup Samsung mem interface with a "
                              "non-Samsung model:\n  %s\n",
                               m_GddrModel.vendorName.c_str());
        return RC::SOFTWARE_ERROR;
    }

    if (m_GddrModel.ramProtocol != RamProtocol::RamGDDR6)
    {
        Printf(Tee::PriError, "Unsupported Samsung RAM protocol:\n  %s\n",
                               m_GddrModel.protocolName.c_str());
...
RC Gddr::SamsungGddr6::RowRepair
(
...
RC Gddr::SamsungGddr6::SoftRowRepair
(
    const Settings& settings,
    const vector<RowError>& rowErrors
)
...
RC Gddr::SamsungGddr6::HardRowRepair
(
    const Settings& settings,
    const vector<RowError>& rowErrors
)
{
    ...
    // For GDDR_ROW_REPAIR_FIRST_RC_SKIP
    constexpr RepairType repairType = Memory::RepairType::HARD;

    StickyRC firstRepairRc;
    for (const auto& rowErr : rowErrors)
    {
        const Row& row = rowErr.row;

        GDDR_ROW_REPAIR_FIRST_RC_SKIP(PreSingleRowRepair(rowErr));

        Printf(Tee::PriNormal,
                   "[repair]   Repair type: hard row repair\n"
                   "[repair]   MEM location: FBPA %u, subp %u, pseudochannel %u, bank %u, row %u\n"
                   "[repair]   pseudo_hard_repair: %s\n",
                   row.hwFbio.Number(), row.subpartition.Number(), row.pseudoChannel.Number(),
                   row.bank.Number(), row.row,
                   Utility::ToString(settings.modifiers.pseudoHardRepair).c_str());

        if (!settings.modifiers.pseudoHardRepair)
        {
            GDDR_ROW_REPAIR_FIRST_RC_SKIP(DoHardRowRepair(row));
        }

        AddRowToRepairList(row);
        firstRepairRc = PostSingleRowRepair(settings, repairType, rowErr, RC::OK);
    }

    return firstRepairRc;
}

bool Gddr::SamsungGddr6::IsRowRepairable
(
    Row row
) const
{
    UINT32 numRepairsInBank = 0;
    for (auto &repair : m_RepairedRows)
    {
        if (row.hwFbio.Number()        == repair.hwFbio.Number() &&
            row.subpartition.Number()  == repair.subpartition.Number() &&
            row.pseudoChannel.Number() == repair.pseudoChannel.Number() &&
            row.bank.Number()          == repair.bank.Number())
        {
            numRepairsInBank++;
        }
    }
    return numRepairsInBank < s_NumRepairsPerBank;
}

void Gddr::SamsungGddr6::AddRowToRepairList
(
    Row row
)
{
    m_RepairedRows.push_back(row);
}

RC Gddr::SamsungGddr6::DoHardRowRepair
(
    Row row
) const
{
    ...
    if (pseudoChannel == 1)
    {
        ...
        // On Tuning PC1_CA[3] falling connected to PC0 CA[3] falling by mistake
        regs.SetField(&testCmdExt, MODS_PFB_FBPA_0_TESTCMD_EXT_ADR_DRAM1, hwFbpa, (rowNum & 0x7F));
        regs.SetField(&testCmdExt, MODS_PFB_FBPA_0_TESTCMD_EXT_LAM_CMD,  hwFbpa, 1);

        // FBPA_TESTCMD:
        testCmd |= (1 << 30);
        testCmd |= (((rowNum >> 7) & 0x1) << 19);
        testCmd |= (bank << 8);
        testCmd |= (1 << 6);
