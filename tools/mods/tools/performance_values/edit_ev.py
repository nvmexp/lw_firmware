...
# PURPOSE:
# For mass changes to a PvsStatExp.js file!
# Unfortunately requires python 2.6+ for the json library

import optparse, json, sys, re
...
python edit_ev.py [-h] [-t INCLUDED_TEST] [-T EXCLUDED_TEST]
                         ...
                         [-o OUTPUT_FILEPATH] [--gpugen] [--gputest]
                         ...
                         [--delete] TARGET"""
description="""  This script is for making mass changes to a PvsStatExp.js file!
...
Also:
  ...
  just as it appears in pvsstatexp.js or stat.jso.

  By default, outputs to ev_out.js. Change to whatever you like. Also,
...
p.add_option('-o', action='store', dest='output_filepath', help='output results to this path', default="ev_out.js")
...
if len(args)==0:
    print("TARGET not found. As in, no input filename was supplied.")
    sys.exit()
if len(args)!=1:
    print(dedent("""
...
#Will output a file? Otherwise, be in debug mode.
...
#jsondata[machine][infpt][gentest][test][metric]
for machine in list(jsondata.keys()):
   if ((not options.included_machine or
        ...
        for infpt in list(jsondata[machine].keys()):
            if ((not options.included_pstate or
                ...
                #it matches gentest.
                for gentest in list(jsondata[machine][infpt].keys()):
                    if ((options.gputest == False or gentest == "gputest") and
                        ...
                        for test in list(jsondata[machine][infpt][gentest].keys()):
                            if ((not options.included_test or
                                ...
                                for metricname in list(jsondata[machine][infpt][gentest][test].keys()):
                                    if ((not options.included_metric or
                                        metricname in options.included_metric) and
                                        metricname not in options.excluded_metric):

                                        #Shorter name.
                                        metric = jsondata[machine][infpt][gentest][test][metricname]
                                        ...
                                        if options.delete == True:
                                            del jsondata[machine][infpt][gentest][test][metricname]
                                    
                                    #Remove empty nodes
                                if not jsondata[machine][infpt][gentest][test]:
                                    del jsondata[machine][infpt][gentest][test]
                        if not jsondata[machine][infpt][gentest]:
                            del jsondata[machine][infpt][gentest]
                if not jsondata[machine][infpt]:
                    del jsondata[machine][infpt]
        if not jsondata[machine]:
            del jsondata[machine]

#Output the new and improved Expected Value file
if fileaction == True:
    with open(options.output_filepath, 'wt') as f:
        #Still no lwrlies-get-own-line support. :(
        jsonstring = "var TestEVs = " + json.dumps(jsondata, sort_keys = True, indent=4)
        f.write(jsonstring)
