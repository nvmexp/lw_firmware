...
#include "cheetah/include/t194preinit.h"
#include "cheetah/include/t234preinit.h"
...
#include "cheetah/include/uphymgr.h"
#include "cheetah/include/t194uphy.h"
#include "cheetah/include/t234uphy.h"
#include "t19x/t194/arxusb_padctl.h"
#include "t19x/t194/arclk_rst.h"
...
#include "xusbpadctrl.h"
    ...
    #undef CLASS_NAME
#endif
#define CLASS_NAME "T194XhciPreInit"
...
UINT32 IsAdbSafe= 0;
UINT32 FalconClkDiv = 2;

RC T194PadctlClkReset()
{
    UINT32 regVal;
...
RC T194EnPlle()
{
    ...
    //ClkMgr::ClkDev()->Enable("plle", true);

    CheetAh::RegRd(LW_DEVID_CAR, 0, CLK_RST_CONTROLLER_PLLE_MISC_0, &regVal, 4);
    if (FLD_TEST_RF_NUM(CLK_RST_CONTROLLER_PLLE_MISC_0_PLLE_LOCK_RANGE, 1, regVal))
    {
        Printf(Tee::PriNormal,"PLLE already enabled, skip EnPlle \n");
        return OK;
    }
    CheetAh::RegRd(LW_DEVID_CAR, 0, CLK_RST_CONTROLLER_PLLE_AUX_0, &regVal, 4);
    if (FLD_TEST_RF_NUM(CLK_RST_CONTROLLER_PLLE_AUX_0_PLLE_SEQ_ENABLE_RANGE, 1, regVal))
    {
        Printf(Tee::PriNormal,"PLLE HW SEQ already enabled, skip EnPlle \n");
        return OK;
    }

    Printf(Tee::PriLow,"Disabling PLLE \n");
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_PLLE_MISC_0,PLLE_PTS,1);
    ...
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_PLLE_AUX_0,PLLE_SEQ_ENABLE,0);
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_PLLE_MISC_0,PLLE_IDDQ_SWCTL,1);
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_PLLE_AUX_0,PLLE_ENABLE_SWCTL,1);
    ...
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_PLLE_MISC1_0,PLLE_EN_DITHER,1);
    ...
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_PLLE_SS_CNTL1_0,PLLE_SDM_DIN,0);

    CLK_REG_UPDATE(CLK_RST_CONTROLLER_PLLE_BASE_0,PLLE_MDIV,2);
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_PLLE_BASE_0,PLLE_NDIV,125);
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_PLLE_BASE1_0,PLLE_PLDIV_CML,24);
    ...
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_PLLE_SS_CNTL_0,PLLE_SSCMAX,0x25);
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_PLLE_SS_CNTL_0,PLLE_SSCINCINTRV,0x20);
    ...
    // Enabling HW sequencer for PLLE

    CLK_REG_UPDATE(CLK_RST_CONTROLLER_PLLE_MISC_0,PLLE_IDDQ_SWCTL,0);
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_PLLE_AUX_0,PLLE_ENABLE_SWCTL,0);
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_PLLE_AUX_0,PLLE_SS_SWCTL,0);
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_PLLE_AUX_0,PLLE_SS_SEQ_INCLUDE,1);
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_PLLE_AUX_0,PLLE_USE_LOCKDET,1);
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_PLLE_AUX_0,PLLE_SEQ_ENABLE,1);
...
RC T194InitClks()
{
    ...
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_CLK_SOURCE_XUSB_CORE_HOST_0,XUSB_CORE_HOST_CLK_DIVISOR,0x6);
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_CLK_SOURCE_XUSB_CORE_HOST_0,XUSB_CORE_HOST_CLK_SRC,1);
    // XUSB_CORE_DEV
    // SELECT SOURCE = PLLP 408MHz / 4
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_CLK_SOURCE_XUSB_CORE_DEV_0,XUSB_CORE_DEV_CLK_DIVISOR,0x6);
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_CLK_SOURCE_XUSB_CORE_DEV_0,XUSB_CORE_DEV_CLK_SRC,1);
    // FALCON
    // SELECT SOURCE = PLLP 408MHz/2
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_CLK_SOURCE_XUSB_FALCON_0,XUSB_FALCON_CLK_DIVISOR,FalconClkDiv);
...
RC T194PadTrack()
{
    ...
    // Enable Clocks
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_CLK_OUT_ENB_USB2_HSIC_TRK_0,CLK_ENB_USB2_TRK, 1);

    CHECK_RC(XusbPadCtrlMgr::GetPadCtrl()->EnablePadTrack());

    // disable the track clocks
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_CLK_OUT_ENB_USB2_HSIC_TRK_0,CLK_ENB_USB2_TRK, 0);

    return rc;
}
RC T194EnUtmipSamplers()
{
    UINT32 regVal;
...
RC T234ReleaseRsts()
{
    ...
    T194ReleaseRsts();
    Printf(Tee::PriLow, "Releasing XUSB_AON resets\n");
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_RST_DEV_XUSB_AON_0, SWR_XUSB_VAUX_AON_RST,1);
    CLK_REG_UPDATE(CLK_RST_CONTROLLER_RST_DEV_XUSB_AON_0, SWR_XUSB_VAUX_AON_RST,0);
...
RC T194PreInit::XhciCfgPmu(const vector<string> &ConfigParams)
{
    RC rc = OK;
...
RC T194PreInit::XhciRstrPmu(const vector<string> &ConfigParams)
{
    RC rc = OK;
    CHECK_RC(SysUtil::Board::ApplySettings("XUSB_PWR_RESTORE"));
    return OK;
}

RC T194PreInit::XhciCfgCtrl(const vector<string> &ConfigParams)
{
    LOG_ENT();
    RC rc;
    UINT32 regVal;
    ...
    UINT32 isForceFs = 0;
    UINT32 devModeUsb2PortIndex = 0xff;
    UINT32 isForceHS = 0;
    UINT32 isForceSS = 0;
    ...
    */
    ...
    CHECK_RC(GetOptionalParam(ConfigParams, "ForceSS", isForceSS, 0));
    CHECK_RC(GetOptionalParam(ConfigParams, "ForceFS", isForceFs, 0));
    CHECK_RC(GetOptionalParam(ConfigParams, "AdbSafe", IsAdbSafe, 0));
    CHECK_RC(GetOptionalParam(ConfigParams, "FalconClkDiv", FalconClkDiv, 2));
    CHECK_RC(GetOptionalParam(ConfigParams, "Debug", debugExit, ""));

    if(isDeviceMode)
    {
        CHECK_RC(GetOptionalParam(ConfigParams, "DevModePort", devModeUsb2PortIndex, 0));
        Printf(Tee::PriNormal, "Device Mode on USB2 port %d\n", devModeUsb2PortIndex);
    }
    Printf(Tee::PriNormal,
           "XHCI PreInit(): %s %s %s %s %s\n",
           IsPrivatePlle ? "Private PLLe" : "",
           isForceHS ? "ForceHS" : "",
           isForceSS ? "ForceSS" : "",
           IsAdbSafe ? "Adb Safe Mode" : "",
           (FalconClkDiv == 0) ? "Falcon Clk 408MHz" : "");
    if(isForceFs)
    {
    ...
    Printf(Tee::PriLow, "XHCI PreInit(): disable SMMU\n");
    if (0xff == devModeUsb2PortIndex)
    {
        CheetAh::PhysWr(0x02c00250, 0x7f);
        CheetAh::PhysWr(0x02c00258, 0x7f);
    }
    else
    {   // for device mode
        CheetAh::PhysWr(0x02c00260, 0x7f);
        CheetAh::PhysWr(0x02c00268, 0x7f);
    }
    ...
    CHECK_RC(UphyMgr::Setup("HSIO", T194Uphy::CFG_XUSB_SATA));
    if(isForceHS)
    {
        CHECK_RC(UphyMgr::LaneForceIddq("HSIO", 0x852, 0));
    }

    CHECK_RC(XusbPadCtrlMgr::GetPadCtrl()->PadInit(devModeUsb2PortIndex, IsAdbSafe));
    if (isForceSS)
    {
        CHECK_RC(XusbPadCtrlMgr::GetPadCtrl()->ForceSsSpd(0));
    }
    else
    {
        CHECK_RC(XusbPadCtrlMgr::GetPadCtrl()->ForceSsSpd(1));
    }
/*
...
RC T194PreInit::XhciRstrCtrl(const vector<string> &ConfigParams)
{
    LOG_ENT();
    RC rc = OK;
    ...
    CHECK_RC(SysUtil::Board::ApplySettings("XUSB_BRDMUX_RESTORE"));
    LOG_EXT();
    return OK;
}
    ...
    #undef CLASS_NAME
#endif
#define CLASS_NAME "T234XhciPreInit"

RC T234PreInit::XhciCfgPmu(const vector<string> &ConfigParams)
{
    RC rc = OK;
    CHECK_RC(SysUtil::Board::ApplySettings("XUSB_PWR"));
    return rc;
...
RC T234PreInit::XhciRstrPmu(const vector<string> &ConfigParams)
{
    RC rc = OK;
    CHECK_RC(SysUtil::Board::ApplySettings("XUSB_PWR_RESTORE"));
    return OK;
...
RC T234PreInit::XhciCfgCtrl(const vector<string> &ConfigParams)
{
    LOG_ENT();
    RC rc;
    UINT32 regVal;
    ...
    {   // for device mode
        CHECK_RC(IOMMUtil::DisableSmmuForDev("XUSB_DEVW"));
        CHECK_RC(IOMMUtil::DisableSmmuForDev("XUSB_DEVR"));
    }
    ...
    if (!IsAdbSafe)
    {
        // On Orin-SLT(E2425) J2009(TypeA) and J2001(microAB) share same SS/HS port
        // For testing J2009, need disable microAB's brdmux
        CHECK_RC(SysUtil::Board::ApplySettings("XUSB_MICROAB_BRDMUX_DISABLE"));
    }

    CHECK_RC(XusbPadCtrlMgr::GetPadCtrl()->PadInit(devModeUsb2PortIndex, IsAdbSafe, isForceHS));
    if (isForceSS)
    {
        CHECK_RC(XusbPadCtrlMgr::GetPadCtrl()->ForceSsSpd(0));
    }
    ...
    {
        T194InitClks();
        ClkMgr::ClkDev()->Enable("UTMIP_PLL", 1); // for Orin-SLT bringup
    }
    T234ReleaseRsts();
    T194PadTrack();

    CHECK_RC(UphyMgr::Setup("HSIO", T234Uphy::HSIO_UPHY_LANE_OPT_2));
    ...
    CHECK_RC(XusbPadCtrlMgr::GetPadCtrl()->SetProdSetting());

    LOG_EXT();
    return OK;
}

RC T234PreInit::XhciRstrCtrl(const vector<string> &ConfigParams)
{
    LOG_ENT();
    RC rc = OK;
