...
#ifndef INCLUDED_INTEL_SMBPORT_H
#define INCLUDED_INTEL_SMBPORT_H
...
class IntelSmbPort : public SmbPort
{
public:
    IntelSmbPort(SmbDevice *pSmbDev, UINT32 PortIndex, UINT32 CtrlBase);//contructor
    ~IntelSmbPort();//destructor
    RC Initialize();
    RC Uninitialize();
...
private:
    ...
    RC Clear();
    RC SetData(vector<UINT08> *pData, UINT08 Protocol);
    RC GetData(vector<UINT08> *pData, UINT08 Protocol);
    RC Protocol(UINT16 SlvAddr,UINT08 Protocol,vector<UINT08> *pDataWr,vector<UINT08> *pDataRd,UINT08 Cmd);
    RC SetAddr(UINT08 Addr, UINT08 ReadBit);
...
#endif //INCLUDED_INTEL_SMBPORT_H
