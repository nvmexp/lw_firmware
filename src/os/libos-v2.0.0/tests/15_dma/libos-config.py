...
task(name='TASK_TEST', pasid='PASID_TEST', entry='task_test_entry', stack=[8192, memory_region.CACHE], waiting_on='TASK_TEST_PORT', randomize_initial_registers=True)
port(name='TASK_TEST_PORT', receiver='TASK_TEST', senders=['TASK_INIT'])
log(name='LOG_TEST', task='TASK_TEST', id = 'LOGTEST')

# test memory allocations
memory_region('TASK_RM_TEST_DMA', ['TASK_TEST'], ['TASK_TEST'], 65536, memory_region.DMA_ONLY)
memory_region('TASK_RM_TEST_DMA_SYS', ['TASK_TEST'], ['TASK_TEST'], 65536, memory_region.DMA_ONLY | memory_region.INIT_ARG_REGION, init_arg_id8='SYS')
memory_region('TASK_RM_TEST_TCM_SRC', ['TASK_TEST'], ['TASK_TEST'], 65536, memory_region.PAGED_TCM)
memory_region('TASK_RM_TEST_TCM_DEST', ['TASK_TEST'], ['TASK_TEST'], 65536, memory_region.PAGED_TCM)
