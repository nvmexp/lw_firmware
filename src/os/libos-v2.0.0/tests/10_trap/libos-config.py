...
address_space(
     name='PASID_TEST',
     images=["test.elf"],
     grant_debug=['PASID_INIT']
)

# Interrupt port
port(name='PORT_EXTINTR', receiver='TASK_INIT', senders=[])

task(name='TASK_INIT', pasid='PASID_INIT', entry='task_init_entry', stack=[8192, memory_region.CACHE | memory_region.PREBOOT_INIT_MAPPED], waiting_on = None, randomize_initial_registers=False)
port(name='TASK_INIT_PANIC_PORT', receiver='TASK_INIT', senders=[])
port(name='TASK_INIT_UTIL_PORT', receiver='TASK_INIT', senders=['TASK_TEST'])
shuttle(name='TASK_INIT_SHUTTLE_PANIC', owner_name='TASK_INIT')
shuttle(name='TASK_INIT_SHUTTLE_UTIL', owner_name='TASK_INIT')
...
task(name='TASK_TEST',
     pasid='PASID_TEST',
     entry='task_test_entry',
     stack=[8192, memory_region.CACHE],
     waiting_on='TASK_TEST_PORT',
     randomize_initial_registers=False)
port(name='TASK_TEST_PORT', receiver='TASK_TEST', senders=['TASK_INIT'])

# Generic port for use with replies.  No one has access by default (libosPortSendRecv will grant one-time access)
port(name='TASK_TEST_REPLY', receiver='TASK_TEST', senders=[])
