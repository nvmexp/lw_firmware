...
void task_init_entry(LwU64 elfPhysicalBase)
{
    ...
    LOG_INIT(LOG_LEVEL_INFO, "Starting IMEM task.\n");
    // Start the subsidiary tasks
    libosPortSyncSend(LIBOS_SHUTTLE_DEFAULT, TASK_IMEM_PORT, 0, 0, 0, 0);

    LOG_INIT(LOG_LEVEL_INFO, "Starting DMEM task.\n");
    libosPortSyncSend(LIBOS_SHUTTLE_DEFAULT, TASK_DMEM_PORT, 0, 0, 0, 0);

    LOG_INIT(LOG_LEVEL_INFO, "Init task ending.\n");
}
