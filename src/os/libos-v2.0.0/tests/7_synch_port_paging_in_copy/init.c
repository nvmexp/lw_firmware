...
void task_init_entry(LwU64 elfPhysicalBase)
{
    ...
    *(volatile LwU64 *)TASK_INIT_INITIAL_RECV_BUFFER = 0;

    // Wake up the test task
    LOG_INIT(LOG_LEVEL_INFO, "Wake up test task.\n");
    libosPortSyncSend(LIBOS_SHUTTLE_DEFAULT, TASK_TEST_PORT, 0, 0, 0, 0);

    // Send the task a test message
    LwU64 payload = 0xDEADBEEFF00D0000ULL;
    LOG_INIT(LOG_LEVEL_INFO, "Send %llx to test port.\n", payload);
    libosPortSyncSend(
        LIBOS_SHUTTLE_DEFAULT, TASK_TEST_PORT, &payload, sizeof(payload), 0, LIBOS_TIMEOUT_INFINITE);

    // Wait for the task to send back a payload
    LwU64 msgid, incomingMessageSize = sizeof(LwU64);
    libosPortSyncRecv(
        LIBOS_SHUTTLE_DEFAULT, TASK_INIT_PORT, TASK_INIT_INITIAL_RECV_BUFFER, sizeof(LwU64),
        &incomingMessageSize, LIBOS_TIMEOUT_INFINITE);
    LOG_INIT(
        LOG_LEVEL_INFO, "Received %llx (size=%d) from init port.\n",
        *(volatile LwU64 *)TASK_INIT_INITIAL_RECV_BUFFER, incomingMessageSize);
}
