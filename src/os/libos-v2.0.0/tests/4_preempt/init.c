...
void task_init_entry(LwU64 elfPhysicalBase)
{
    ...
    LOG_INIT(LOG_LEVEL_INFO, "Waking up test task\n");
    LOG_AND_VALIDATE(
        LOG_INIT, libosPortSyncSend(LIBOS_SHUTTLE_DEFAULT, TASK_TEST_PORT, 0, 0, 0, LIBOS_TIMEOUT_INFINITE) ==
                      LIBOS_OK);

    LOG_INIT(LOG_LEVEL_INFO, "Init 1\n");
    __asm__ __volatile__("wfi" ::: "memory");
    LOG_INIT(LOG_LEVEL_INFO, "Init 2\n");
    __asm__ __volatile__("wfi" ::: "memory");
    LOG_INIT(LOG_LEVEL_INFO, "Init 3\n");
}
