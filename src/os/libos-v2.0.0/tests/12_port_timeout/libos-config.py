...
address_space(name='DOMAIN_INIT', images=["init.elf"])
address_space(name='DOMAIN_TEST', images=["test.elf"])

# Interrupt port
port(name='PORT_EXTINTR', receiver='TASK_INIT', senders=[])

task(name='TASK_INIT', pasid='DOMAIN_INIT', entry='task_init_entry', stack=[8192, memory_region.CACHE  | memory_region.PREBOOT_INIT_MAPPED], waiting_on = None, randomize_initial_registers=True)
port(name='TASK_INIT_PANIC_PORT', receiver='TASK_INIT', senders=[])
port(name='TASK_INIT_PORT', receiver='TASK_INIT', senders=['TASK_TEST', 'TASK_INIT'])
...
task(name='TASK_TEST', pasid='DOMAIN_TEST', entry='task_test_entry', stack=[8192, memory_region.CACHE], waiting_on='TASK_TEST_PORT', randomize_initial_registers=True)
port(name='TASK_TEST_PORT', receiver='TASK_TEST', senders=['TASK_INIT'])
port(name='TASK_TEST_PORT_START', receiver='TASK_TEST', senders=['TASK_INIT'])
port(name='TASK_TEST_PORT2', receiver='TASK_TEST', senders=['TASK_INIT'])
