...
struct gsp : riscv_core, lwstom_memory, gsp_mmpu_state, gsp_falcon_state
{
    chip_t                          chip;
    ...
    LwU64 fb_for_chip(chip_t chip) {
        switch (chip)
        {
            case tu11x:
            case ga100:
                return 0x6180000000000000ULL;

            case ga102:
                return 0x6060000000000000ULL;
    ...
    LwU64 dmem_for_chip(chip_t chip) {
        switch (chip)
        {
            case tu11x:
            case ga100:
   ...
   LwU64 imem_for_chip(chip_t chip) {
        switch (chip)
        {
            case tu11x:
            case ga100:
    ...
    LwU64 dmem_size_for_chip(chip_t chip) {
        switch (chip)
        {
            case tu11x:
            case ga100:
            case ga102:
                return 0x0000000000010000ull;
   ...
   LwU64 imem_size_for_chip(chip_t chip) {
        switch (chip)
        {
            case tu11x:
            case ga100:
    ...
    LwU64 priv_for_chip(chip_t chip)    {
        switch (chip)
        {
            case tu11x:
            case ga100:
    ...
    gsp(chip_t chip, image *firmware)
        : chip(chip), riscv_core(firmware),  // @todo: , fb_for_chip(chip)
          mmpuIMEM('ITCM', imem_for_chip(chip), std::vector<LwU8>(imem_size_for_chip(chip), 0)),
          mmpuDMEM('DTCM', dmem_for_chip(chip), std::vector<LwU8>(dmem_size_for_chip(chip), 0))
    {
        if (chip == tu11x || chip == ga100)
