...
BEGIN {
    ...
    eval {
        ...
        local $ELW{PERL_DL_NONLAZY} = 0 if $ELW{PERL_DL_NONLAZY};

        bootstrap List::MoreUtils $VERSION;
        1;

    } unless $ELW{LIST_MOREUTILS_PP};
}
...
Callwlates the minimum and maximum of LIST and returns a two element list with
the first element being the minimum and the second the maximum. Returns the
empty list if LIST was empty.

The C<minmax> algorithm differs from a naive iteration over the list where each
element is compared to two values being the so far callwlated min and max value
in that it only requires 3n/2 - 2 comparisons. Thus it is the most efficient
possible algorithm.

However, the Perl implementation of it has some overhead simply due to the fact
that there are more lines of Perl code ilwolved. Therefore, LIST needs to be
...
Nothing by default. To import all of this module's symbols, do the colwentional
...
A partilwlarly nasty memory leak was spotted by Thomas A. Lowery.
