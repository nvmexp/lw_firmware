 ...
 */
sysTASK_DATA static LwU8 pmgrMutexPhysicalToLogical_TU10X[] =
{
 ...
 */
sysTASK_DATA static LwU8 gspMutexPhysicalToLogical_TU10X[] =
{
 ...
 */
sysSYSLIB_CODE void
mutexEstablishMapping_TU10X(void)
{
    LwU8 i;
    LwU8 j;
    LwBool bFind;
    ...
    {
        bFind = LW_FALSE;
        for (j = 0; ((j < sizeof(gspMutexPhysicalToLogical_TU10X)) && !bFind); j++)
        {
            if (gspMutexPhysicalToLogical_TU10X[j] == i)
            {
        ...
        for (j = 0; ((j < sizeof(pmgrMutexPhysicalToLogical_TU10X)) && !bFind); j++)
        {
            if (pmgrMutexPhysicalToLogical_TU10X[j] == i)
            {
