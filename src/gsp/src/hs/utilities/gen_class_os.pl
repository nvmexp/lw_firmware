...
sub create_offsets_array
{
  ...
  {
    my $app_size       = dec2hex(getsize_roundup("ovl$cnt"));
    if ($app_size > 0)
    {
  ...
  for($cnt = 1; $cnt < 11; $cnt++)
  {
    my $app_size       = dec2hex(getsize_roundup("data_ovl$cnt"));
    if($app_size > 0)
    {
      @offsets_array = (@offsets_array, $app_offset, $app_size);
      $num_elems+=2;
...
sub write_hdr
{
  ...
  print SRC_FILE " *"."\n";
  print SRC_FILE " * Copyright 2013 by LWPU Corporation.  All rights reserved.  All"."\n";
  ...
  print SRC_FILE ""."\n";
  print SRC_FILE ""."\n";
  print SRC_FILE "LwU32 gsphs_ucode_data_".$chip."[] = {\n";
  ...
  print SRC_FILE "// \n";
  print SRC_FILE "// \n";
  print SRC_FILE " LwU32 gsphs_ucode_header_".$chip."[] = {\n";
  my $count;
...
# print("array size: $arrsize\n");
  ...
  print SRC_FILE "\nLwU32 gsphs_ucode_data"."_size_".$chip." = $ucode_size_dwords;\n"; #ucode_size in DWORDS
