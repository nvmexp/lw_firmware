...
#ifndef GSP_BAR0_HS_H
#define GSP_BAR0_HS_H
 ...
 */
    ...
    //
    #define EXT_REG_RD32_HS_ERRCHK(addr, pReadVal)    dpuBar0ErrChkRegRd32Hs_HAL(&Dpu.hal, addr, pReadVal)
    #define EXT_REG_WR32_HS_ERRCHK(addr, data)        dpuBar0ErrChkRegWr32NonPostedHs_HAL(&Dpu.hal, addr, data)
...
// not be required once we have EMEM apertures in GV10X. We should be able to
...
//
static inline FLCN_STATUS _dpuBar0WaitIdle_GV10X(LwBool bEnablePrivErrChk)
 ...
 * EMEM aperture on GV10X, this code should disappear, so the code size bloat 
 * because of inlining is only temporary for testing.
 */
static inline FLCN_STATUS
_dpuBar0WaitIdle_GV10X(LwBool bEnablePrivErrChk)
{
    ...
    while (!bDone)
    {
        switch (DRF_VAL(_CGSP, _BAR0_CSR, _STATUS,
                    REG_RD32(CSB, LW_CGSP_BAR0_CSR)))
        {
            case LW_CGSP_BAR0_CSR_STATUS_IDLE:
                bDone = LW_TRUE;
                break;

            case LW_CGSP_BAR0_CSR_STATUS_BUSY:
                break;

            // unexpected error conditions
            case LW_CGSP_BAR0_CSR_STATUS_TMOUT:
            case LW_CGSP_BAR0_CSR_STATUS_DIS:
            case LW_CGSP_BAR0_CSR_STATUS_ERR:
            default:
            {
                //
...
static inline void _dpuBar0RegWr32NonPosted_GV10X(LwU32 addr, LwU32 data)
    GCC_ATTRIB_ALWAYSINLINE();
 ...
 */
static inline void
_dpuBar0RegWr32NonPosted_GV10X
(
    LwU32  addr,
    LwU32  data
)
{
    ...
    REG_WR32(CSB, LW_CGSP_BAR0_ADDR, addr);
    REG_WR32(CSB, LW_CGSP_BAR0_DATA, data);

    REG_WR32(CSB, LW_CGSP_BAR0_CSR,
...
static inline LwU32 _dpuBar0RegRd32_GV10X(LwU32 addr)
    GCC_ATTRIB_ALWAYSINLINE();
...
)
{
    ...
    // If we are not doing PEH,  we don't need to check the return value.
    (void)_dpuBar0WaitIdle_GV10X(LW_FALSE);
    val = REG_RD32(CSB, LW_CGSP_BAR0_DATA);
    return val;
}
