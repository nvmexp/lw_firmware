 ...
 */
SEC2_SEMAPHORE_MTHD_ARRAY SemaphoreMthdArray
...
)
{
    ...
    //
    engStateAddr = sec2GetEngStateAddr_HAL(pSec2, ctx);
...
)
{
    ...
    if ((SemaphoreMthdArray.inputBitMask & SEC2_SEMAPHORE_INPUT_VALID) == SEC2_SEMAPHORE_INPUT_VALID)
    {
        if (!bLongReport)
        {
...
lwrtosTASK_FUNCTION(chnlMgmtMain, pvParameters)
{
    ...
    SemaphoreMthdArray.semDesc.params = FLD_SET_DRF_NUM(_RM_FLCN_MEM_DESC, _PARAMS, _DMA_IDX, RM_SEC2_DMAIDX_VIRT,
                                        SemaphoreMthdArray.semDesc.params);
