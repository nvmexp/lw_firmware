...
#define SEC2_BREAKPOINT()            lwrtosHALT()
 ...
 */
SEC2_CTX_DUMMY_PATTERN sec2CtxDummyPattern
    GCC_ATTRIB_SECTION("dmem_", "sec2CtxDummyPattern")
 ...
 */
SEC2_CTX_DUMMY_PATTERN sec2CtxDummyCheck
    GCC_ATTRIB_SECTION("dmem_", "sec2CtxDummyCheck")
    GCC_ATTRIB_ALIGN(DMA_MIN_READ_ALIGNMENT);

SEC2_CTX_WORKLAUNCH worklaunch
...
{    
    ...
    sec2GetInstBlkOffsetAndDmaIdx_HAL(pSec2, ctx, LW_TRUE, &address, &dmaIdx);
...
)
{
    ...
    sec2GetInstBlkOffsetAndDmaIdx_HAL(pSec2, ctx, LW_FALSE, &address, &dmaIdx);
...
)
{
    ...
    //
    engStateAddr = _sec2GetEngStateAddr(ctx);
    ...
    pMemDesc->params =
        FLD_SET_DRF_NUM(_RM_FLCN_MEM_DESC, _PARAMS, _DMA_IDX,
                        (bPhysical ?
                         sec2GetEngCtxPhysDmaIdx_HAL(pSec2, target) :
                         RM_SEC2_DMAIDX_VIRT),
                         pMemDesc->params);
...
)
{
    ...
    lwrtosQueueSendBlocking(Sec2ChnMgmtCmdDispQueue, &completion, sizeof(completion));
}
...
static void _RCRecoverySendUnblockEvent(void)
{
    ...
    msg.msg.chnmgmt.unblockRC.msgType = RM_SEC2_MSG_TYPE(CHNMGMT, UNBLOCK_RC_RECOVERY);
...
_RCRecoverySendTriggerEvent(void)
{
    ...
    msg.msg.chnmgmt.unblockRC.msgType = RM_SEC2_MSG_TYPE(CHNMGMT, TRIGGER_RC_RECOVERY);
...
_saveRestoreCtx_Iv:
    ...
    // in one structure.
    //
    if (_sec2SaveRestoreIv(ctx, worklaunch.iv, GCM_IV_SIZE_BYTES, bSave) != FLCN_OK)
    {
...
_saveRestoreCtx_ack:
    if (status == FLCN_OK)
    {
        ...
        else
        {
            ...
            //
            icSetNonstallIntrCtrlGfid_HAL(&Ic, sec2GetNextCtxGfid_HAL(pSec2));
            sec2AckCtxRestore_HAL(pSec2);
        }
    }
...
_processMethods(void)
{
    ...
    while (sec2PopMthd_HAL(pSec2, &mthdId, &mthdData))
    {
        // Allow handling of host internal methods first (not sent to worklaunch partition)
        if (sec2ProcessHostInternalMethods_HAL(pSec2, mthdId, mthdData) == FLCN_OK)
        {
            continue;
        }
        ...
        worklaunch.methods[worklaunch.methodCount][WORKLAUNCH_METHOD_ARRAY_IDX_ADDR] = SEC2_METHOD_ID_TO_METHOD(mthdId);
        ...
        if (mthdId == SEC2_METHOD_TO_METHOD_ID(LWCBA2_ENABLE_DBG_PRINT))
            worklaunch.bDbgPrint = LW_TRUE;
#endif

        // Check if we just got a "trigger" method
        if (mthdId == SEC2_METHOD_TO_METHOD_ID(LWCBA2_EXELWTE) || mthdId == SEC2_METHOD_TO_METHOD_ID(LWCBA2_SEMAPHORE_D))
        {
            ...
            memcpy(rpcGetRequest(), &worklaunch, sizeof(SEC2_CTX_WORKLAUNCH));
            ...
            if (errorCode != LWCBA2_ERROR_NONE)
            {
                ...
                csbWrite(LW_CSEC_FALCON_MAILBOX1, errorCode);
                SEC2_BREAKPOINT(); // TODO-ssapre: trigger RC
            }
            if (worklaunch.bDbgPrint)
 ...
 */
LwBool _sec2TriggerRCRecoveryMthdRcvdForIlwalidCtx(void)
{
    LwU16 mthdId;
    LwU32 mthdData;
...
_processMethodIntr(void)
{
    ...
    if (_sec2TriggerRCRecoveryMthdRcvdForIlwalidCtx())
    {
        return;
    }
    
    if (sec2IsMthdFifoEmpty_HAL(pSec2))
    {
        //
        ...
        //
        dbgPrintf(LEVEL_ALWAYS, "processMethodIntr sec2IsMthdFifoEmpty_HAL returned false\n");
        icHostIfIntrUnmask_HAL(pSec2);
        return;
    }
    ...
    sec2HostIdleProgramBusy_HAL(pSec2);
    if (_processMethods())
    {
        //
        ...
        // it.
        //
        sec2HostIdleProgramIdle_HAL(pSec2);
...
/*
    if (g_isRcRecoveryInProgress == LW_TRUE)
    {
        ...
        //
        icHostIfIntrCtxswUnmask_HAL(pSec2);
    }
    else
*/
    {
        bLwrCtxValid  = sec2IsLwrrentChannelCtxValid_HAL(pSec2, &lwrCtx);

        bNextCtxValid = sec2IsNextChannelCtxValid_HAL(pSec2, &nxtCtx);
...
)
{
    ...
    switch (pChnmgmtEvt->cmd.pCmd->cmd.chnmgmt.cmdType)
    {
        case RM_SEC2_CMD_TYPE(CHNMGMT, ENGINE_RC_RECOVERY):
        {
            ...
            msg.hdr       = hdr;
            msg.msg.chnmgmt.engineRC.msgType = RM_SEC2_MSG_TYPE(CHNMGMT, ENGINE_RC_RECOVERY);
    ...
    // the command.
    //
    cmdQueueSweep(&(pChnmgmtEvt->cmd.pCmd->hdr), pChnmgmtEvt->cmd.cmdQueueId);    
}
...
lwrtosTASK_FUNCTION(chnlMgmtMain, pvParameters)
{
    ...
    for (;;)
    {
        if (FLCN_OK == lwrtosQueueReceive(Sec2ChnMgmtCmdDispQueue, &dispatch, sizeof(dispatch), lwrtosMAX_DELAY))
        {
