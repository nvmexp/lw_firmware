...
// Also have FLCNLIBCFG_CHIP_ENABLED(), FLCNLIBCFG_FEATURE_ENABLED(), etc
// from {{$XXCONFIG_H_FILE}}.
//
// NOTE: these definitions are "flat" (ie they don't use some more general
//       FLCNLIBCFG_ENABLED(CHIP,X) form because the pre-processor would re-evaluate
...
#define {{$XXCFG}}_CHIP_ENABLED_OR_BAIL(W)                \
     do {                                           \
         if ( ! {{$XXCFG}}_CHIP_##W)                      \
         {                                          \
             DBG_PRINTF((DBG_MODULE_GLOBAL, DBG_LEVEL_ERRORS, "FLCNLIB: %s: CHIP " #W " not enabled, bailing\n", __func__)); \
             return {{$STATUS_ERROR}};           \
         }                                          \
     } while (LW_FALSE)
#define {{$XXCFG}}_CHIP_ENABLED_OR_ASSERT_AND_BAIL(W)     \
     do {                                           \
         if ( ! {{$XXCFG}}_CHIP_##W)                      \
         {                                          \
             DBG_PRINTF((DBG_MODULE_GLOBAL, DBG_LEVEL_ERRORS, "FLCNLIB: %s: CHIP " #W " not enabled, assert and bail\n", __func__)); \
...
#define {{$XXCFG}}_FEATURE_ENABLED_OR_BAIL(W)            \
     do {                                           \
         ...
         {                                          \
             DBG_PRINTF((DBG_MODULE_GLOBAL, DBG_LEVEL_ERRORS, "FLCNLIB: %s: FEATURE " #W " not enabled, bailing\n", __func__)); \
             return {{$STATUS_ERROR}};           \
         }                                          \
     } while (LW_FALSE)
#define {{$XXCFG}}_FEATURE_ENABLED_OR_ASSERT_AND_BAIL(W) \
     do {                                           \
         if ( ! {{$XXCFG}}_FEATURE_##W)                  \
         {                                          \
             DBG_PRINTF((DBG_MODULE_GLOBAL, DBG_LEVEL_ERRORS, "FLCNLIB: %s: FEATURE " #W " not enabled, assert and bail\n", __func__)); \
...
// MODULE's
#define {{$XXCFG}}_MODULE_ENABLED_STATUS(W)   ({{$XXCFG}}_MODULE_##W ? {{$STATUS_OK}} : {{$STATUS_ERROR}})
#define {{$XXCFG}}_MODULE_ENABLED_OR_BAIL(W)             \
     do {                                           \
         if ( ! {{$XXCFG}}_MODULE_##W)                   \
         {                                          \
             DBG_PRINTF((DBG_MODULE_GLOBAL, DBG_LEVEL_ERRORS, "FLCNLIB: %s: MODULE " #W " not enabled, bailing\n", __func__)); \
             return {{$STATUS_ERROR}};           \
         }                                          \
     } while (LW_FALSE)
#define {{$XXCFG}}_MODULE_ENABLED_OR_ASSERT_AND_BAIL(W)  \
     do {                                           \
         if ( ! {{$XXCFG}}_MODULE_##W)                   \
         {                                          \
             DBG_PRINTF((DBG_MODULE_GLOBAL, DBG_LEVEL_ERRORS, "FLCNLIB: %s: MODULE " #W " not enabled, assert and bail\n", __func__)); \
             {{$ASSERT}}({{$XXCFG}}_MODULE_##W);           \
             return {{$STATUS_ERROR}};           \
         }                                          \
     } while (LW_FALSE)
#define {{$XXCFG}}_MODULE_ENABLED_OR_GOTO(W,L)  \
    do {                                    \
        if (! {{$XXCFG}}_MODULE_##W)            \
        {                                   \
            goto L;                         \
...
#define {{$XXCFG}}_MODULE_ENABLED_OR_BREAK(W)   \
    {                                       \
        if (! {{$XXCFG}}_MODULE_##W)            \
        {                                   \
            break;                          \
