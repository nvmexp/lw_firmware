...
#
# dpaux-config.pl and other FLCN Libraries share one chip-config client.
...
if (!grep {$_ =~ m/--output-dir/} @ARGV) {
    ...
    my $outDir = '_out';
    if (not -e $outDir) {
        print "[dpaux-config] creating output directory '$outDir'\n";
...
unshift @ARGV, "--config", "dpaux-config.cfg";
