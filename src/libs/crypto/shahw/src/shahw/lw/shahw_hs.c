...
/*!
 * @file   shahw_hs.c
 * @brief  HS implementations of SHA & HMAC utilizing SHA HW.
 *
...
/* ------------------------ Function Prototypes ---------------------------- */
static void _shahwEntry(void)
    GCC_ATTRIB_USED()
    GCC_ATTRIB_SECTION("imem_libShahwHs", "start");

/* ------------------------ Private Functions ------------------------------ */
/*!
 * _shahwEntry
 *
 * @brief Entry function of SHA HW library. This function merely returns. It
 ...
 */
FLCN_STATUS
shahwSoftReset_hs(void)
{
    return shahwEngineSoftResetHs_HAL(&shahw, SHAHW_ENGINE_SW_RESET_TIMEOUT_NS);
}
 ...
 */
FLCN_STATUS
shahwAcquireMutex_hs
(
    LwU8 mutexId
)
{
    return shahwAcquireEngineMutexHs_HAL(&shahw, mutexId);
}
 ...
 */
FLCN_STATUS
shahwReleaseMutex_hs
(
    LwU8 mutexId
)
{
    return shahwReleaseEngineMutexHs_HAL(&shahw, mutexId);
}
...
)
{
    ...
    CHECK_FLCN_STATUS(shahwInitializeEngineHs_HAL(&shahw, pShaContext->algoId,
                                                  pShaContext->msgSize));
 ...
 *        completion can be polled via shahwWaitTaskComplete_hs. Ensure input
 *        data will be valid until task has completed.
 * 
...
)
{
    ...
    return shahwInsertTaskHs_HAL(&shahw, pShaContext->algoId, size,
...
)
{
    ...
    CHECK_FLCN_STATUS(shahwReadHashResultHs_HAL(&shahw, pShaContext->algoId,
...
)
{
    ...
    CHECK_FLCN_STATUS(shahwInitializeEngineHs_HAL(&shahw, pHmacContext->algoId,
                                                  msgSize));

    // Generate inner key.
    memset_hs(ipad, 0, SHA_256_BLOCK_SIZE_BYTE);
    memcpy_hs(ipad, &(pHmacContext->keyBuffer), pHmacContext->keySize);
    ...
    CHECK_FLCN_STATUS(shahwInsertTaskHs_HAL(&shahw, pHmacContext->algoId,
...
/*!
 * @brief Wrapper around shahwInsertTaskHs_HAL() to update inner digest in
...
)
{
    ...
    return shahwInsertTaskHs_HAL(&shahw, pHmacContext->algoId, size,
...
)
{
    ...
    CHECK_FLCN_STATUS(shahwReadHashResultHs_HAL(&shahw, pHmacContext->algoId,
                                                innerDigest, bScrubReg,
                                                pIntStatus));

    // Generate outer key.
    memset_hs(opad, 0, SHA_256_BLOCK_SIZE_BYTE);
    memcpy_hs(opad, &(pHmacContext->keyBuffer), pHmacContext->keySize);
    for (i = 0; i < SHA_256_BLOCK_SIZE_BYTE; i++)
    {
...
/*!
 * @brief To be used when waiting is not performed for shahw*Update_hs operations.
 ...
 *                       relevant error otherwise.
 */
FLCN_STATUS shahwWaitTaskComplete_hs
(
    LwU32 *pIntStatus
)
{
    return shahwWaitTaskCompleteHs_HAL(&shahw, pIntStatus);
}
