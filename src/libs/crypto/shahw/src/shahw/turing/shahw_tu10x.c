...
/*!
 * @file   shahw_tux10x.c
 * @brief  HAL implementations for SHA HW functionality on TU10X.
 * 
...
/* ------------------------ Function Prototypes ---------------------------- */
static FLCN_STATUS _shahwCheckTaskSize(SHA_ID algoId, LwU32 taskSize,
     LwU32 msgSize)
    GCC_ATTRIB_SECTION("imem_libShahw", "_shahwCheckTaskSize");

static FLCN_STATUS _shahwSetShaInitVector(SHA_ID algoId)
    GCC_ATTRIB_SECTION("imem_libShahw", "_shahwSetShaInitVector");

static FLCN_STATUS _shahwGetConfigEncodeMode(SHA_ID algoId, LwU8 *pMode)
    GCC_ATTRIB_SECTION("imem_libShahw", "_shahwGetConfigEncodeMode");
...
)
{
    ...
    if ((flcnStatus = shahwAcquireEngineMutex_TU10X(SHA_MUTEX_ID_SHAHW_SOFTRESET))
                      != FLCN_OK)
    {
    ...
    do
    {
        ...
        if (osPTimerTimeNsElapsedGet(&startTimestamp) > timeOut)
        {
...
ErrorExit:
    ...
    if (flcnStatus != FLCN_OK)
    {
        ...
        (void)shahwReleaseEngineMutex_TU10X(SHA_MUTEX_ID_SHAHW_SOFTRESET);
    }
...
)
{
    ...
    CHECK_FLCN_STATUS(_shahwGetConfigEncodeMode(algoId, &encMode));
 ...
 *        not, use shahwWaitTaskComplete_TU10X to wait for completion and check
...
)
{
    ...
    CHECK_FLCN_STATUS(_shahwCheckTaskSize(algoId, taskSize, *pMsgSize));
    CHECK_FLCN_STATUS(shahwWaitTaskComplete_TU10X(pIntStatus));
    ...
    *pMsgSize = *pMsgSize - taskSize;
    ...
    if (bDefaultIv)
    {
        ...
        {
            CHECK_FLCN_STATUS(_shahwSetShaInitVector(algoId));
 ...
 */
FLCN_STATUS
shahwCheckAndClearIntStatus_TU10X
(
...
)
{
    ...
    CHECK_FLCN_STATUS(shahwWaitEngineIdle_TU10X(SHAHW_ENGINE_IDLE_TIMEOUT_NS));

    //
    // Check status, returning success if task completed without error,
    // or if no previous task has been exelwted.
    //
    CHECK_FLCN_STATUS(shahwCheckAndClearIntStatus_TU10X(pIntStatus));
