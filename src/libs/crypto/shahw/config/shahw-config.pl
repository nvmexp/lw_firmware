...
#
# shahw-config.pl and other Falcon Libraries share one chip-config client.
# This file is a wrapper script to trigger chip-config.pl
#
...
if (!grep {$_ =~ m/--output-dir/} @ARGV) {
    ...
    my $outDir = '_out';
    if (not -e $outDir) {
        print "[shahw-config] creating output directory '$outDir'\n";
...
unshift @ARGV, "--config", "shahw-config.cfg";
