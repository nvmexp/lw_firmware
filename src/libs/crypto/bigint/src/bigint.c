...
/* ------------------------- Function Prototypes ---------------------------- */
static LwU32 _bigIntGetBit(const LwU32 *pX, LwU32 i)
    GCC_ATTRIB_SECTION("imem_libBigInt", "_bigIntGetBit");
static void _bigIntSwapEndianness(LwU8 *pOutput, const LwU8 *pInput, LwU32 size)
    GCC_ATTRIB_SECTION("imem_libBigInt", "_bigIntSwapEndianness");
static LwU32 _bigIntIlwerseDigit(LwU32 x)
    GCC_ATTRIB_SECTION("imem_libBigInt", "_bigIntIlwerseDigit");
...
)
{
    ...
    memset(pModulus, 0, sizeof(BigIntModulus));

    // Initialize digits
    pModulus->digits = (size + sizeof(LwU32) - 1) / sizeof(LwU32);

    // Initialize pModulus->n
    _bigIntSwapEndianness((LwU8*)pModulus->n, pData, size);

    // Callwlate pModulus->n
    pModulus->ilwdigit = _bigIntIlwerseDigit(pModulus->n[0]);

    // Callwlate pModulus->r2
    pModulus->r2[0] = 1;
    for (i = 0; i < pModulus->digits * 64; i++)
    {
        bigIntDoubleMod(pModulus->r2, pModulus->r2, pModulus);
    }
}
...
)
{
    ...
    for (i = e_digits * 32 - 1; (i >= 0) && !_bigIntGetBit(pE, i); i--)
    {
