...
static inline void s_riscvScpDoCbcCrypt(LwU8 *pSrc, LwU32 size, LwU8 *pDest,
    ...
    LwBool bShortlwt)
    GCC_ATTRIB_SECTION("imem_libScpCryptHs", "s_riscvScpDoCbcCrypt");
...
)
{
    ...
    if (FLCN_OK != sysVirtToEngineOffset((LwU64*)pSrc, &srcPa, NULL, LW_TRUE))
    {
        return;
    }

    if (FLCN_OK != sysVirtToEngineOffset((LwU64*)pDest, &destPa, NULL, LW_TRUE))
    {
        return;
    }

    LwU64 ivPa = 0;
    sysVirtToEngineOffset((void*)pIvBuf, &ivPa, NULL, LW_TRUE);
...
)
{
    ...
    sysVirtToEngineOffset((void*)pKey, &keyPa, NULL, LW_TRUE);
