...
/* ------------------------ Macros & Defines ------------------------------- */
#define hdcpReadRegister_hs(addr, pDataOut)  dioReadReg(addr, pDataOut)
#define hdcpWriteRegister_hs(addr, dataIn)   dioWriteReg(addr, dataIn)

/* ------------------------ Static Variables ------------------------------- */

/* ------------------------ Public Functions ------------------------------- */
//TODO: 200586859 Need to check CHEETAH's implementation and modify accordingly
/*!
...
)
{
    ...
    if (FLD_TEST_DRF(_FLCN, _HDCP_CTL_OPTIONS, _KEY_TYPE, _PROD, CtlOptions))
    {
        ...
        CHECK_STATUS(hdcpReadRegister_hs(LW_PDISP_UPSTREAM_HDCP_M0_MSB, &reg));

        m0 = (LwU64)reg << 32;
        CHECK_STATUS(hdcpReadRegister_hs(LW_PDISP_UPSTREAM_HDCP_M0_LSB, &reg));
        m0 |= reg;
    ...
    *pM0 = m0;
    ...
    CHECK_STATUS(hdcpReadRegister_hs(LW_PDISP_UPSTREAM_HDCP_STATUS_SOR(0), &data1));
    CHECK_STATUS(hdcpWriteRegister_hs(LW_PDISP_UPSTREAM_HDCP_STATUS_SOR(0), 0xffffffff));
    CHECK_STATUS(hdcpReadRegister_hs(LW_PDISP_UPSTREAM_HDCP_STATUS_SOR(0), &data2));
    CHECK_STATUS(hdcpWriteRegister_hs(LW_PDISP_UPSTREAM_HDCP_STATUS_SOR(0), data1));
...
)
{
    ...
    CHECK_STATUS(hdcpReadRegister_hs(LW_PDISP_UPSTREAM_HDCP_STATUS_SOR(idx), pData));
...
)
{
    ...
    CHECK_STATUS(hdcpWriteRegister_hs(LW_PDISP_UPSTREAM_HDCP_STATUS_SOR(idx), data));
