...
#
# hdcp22wired-config.pl and other FLCN Libraries share one chip-config client.
...
BEGIN {
    ...
    # Adding the major directory of flcnlib-config to @INC so that
...
if (!grep {$_ =~ m/--output-dir/} @ARGV) {
    ...
    my $outDir = '_out';
    if (not -e $outDir) {
        print "[hdcp22wired-config] creating output directory '$outDir'\n";
...
unshift @ARGV, "--config", "hdcp22wired-config.cfg";
