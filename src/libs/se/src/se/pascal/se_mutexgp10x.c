 ...
 */
SE_STATUS seMutexAcquire_GP10X(void)
{
...
/*!
 * @brief Tries to acquire the SE mutex in HS Mode. If successful, SE_OK is returned.
 *
 * @return SE_OK if successful. SE_ERR_MUTEX_ACQUIRE if unsuccessful.
 */
SE_STATUS seMutexAcquireHs_GP10X(void)
{
    SE_STATUS status = SE_OK;
    LwU32     val    = 0;
    FLCN_TIMESTAMP     timeStart;
    ...
    // before starting mutex acquire sequence
    //
    CHECK_SE_STATUS(seSelwreBusReadRegisterErrChkHs(LW_SSE_SE_CTRL_SE_MUTEX_TMOUT_DFTVAL, &val));
    if (val != LW_SSE_SE_CTRL_SE_MUTEX_TMOUT_DFTVAL_TMOUT_MAX)
    {
        // If this register has been tampered with,  HALT
        lwuc_halt();
    ...
    // Initialize default timeout register
    CHECK_SE_STATUS(seSelwreBusWriteRegisterErrChkHs(LW_SSE_SE_CTRL_SE_MUTEX_TMOUT_DFTVAL,
                                                   LW_SSE_SE_CTRL_SE_MUTEX_TMOUT_DFTVAL_TMOUT_MAX));
    ...
    do
    {
        CHECK_SE_STATUS(seSelwreBusReadRegisterErrChkHs(LW_SSE_SE_CTRL_PKA_MUTEX, &val));
        val = DRF_VAL(_SSE, _SE_CTRL_PKA_MUTEX, _RC_VAL, val);
    } while ((val != LW_SSE_SE_CTRL_PKA_MUTEX_RC_SUCCESS) &&
             (SE_MUTEX_TIMEOUT_VAL_USEC > osPTimerTimeNsElapsedGet_hs(&timeStart)));
    ...
    // IAS section 6.1.2.3  - https://p4viewer/get/p4hw:2001//hw/ar/doc/t18x/se/SE_PKA_IAS.doc
    //
    CHECK_SE_STATUS(seMutexSetTimeoutValueHs_HAL(&Se, LW_SSE_SE_CTRL_PKA_MUTEX_WDTMR_VAL_DISABLE));

    // set PKA mutex timeout action to be release mutex & reset PKA core
    CHECK_SE_STATUS(seSelwreBusWriteRegisterErrChkHs(LW_SSE_SE_CTRL_PKA_MUTEX_TMOUT_ACTION,
                                                   DRF_DEF(_SSE, _SE_CTRL_PKA_MUTEX_TMOUT_ACTION, _INTERRUPT,     _ENABLE) |
                                                   DRF_DEF(_SSE, _SE_CTRL_PKA_MUTEX_TMOUT_ACTION, _RELEASE_MUTEX, _ENABLE) |
                                                   DRF_DEF(_SSE, _SE_CTRL_PKA_MUTEX_TMOUT_ACTION, _RESET_PKA,     _ENABLE)));

    // Make sure we own the mutex
    CHECK_SE_STATUS(seSelwreBusReadRegisterErrChkHs(LW_SSE_SE_CTRL_PKA_MUTEX_STATUS, &val));
    val = DRF_VAL(_SSE, _SE_CTRL_PKA_MUTEX_STATUS, _LOCKED, val);
    ...
    // Make sure the watchdog timer is still disabled
    CHECK_SE_STATUS(seSelwreBusReadRegisterErrChkHs(LW_SSE_SE_CTRL_PKA_MUTEX_WDTMR, &val));
    if (val != LW_SSE_SE_CTRL_PKA_MUTEX_WDTMR_VAL_DISABLE)
    {
        return SE_ERR_MUTEX_ACQUIRE;
    }
 ...
 */
SE_STATUS seMutexRelease_GP10X(void)
{
    SE_STATUS status = SE_OK;

    //
    // Write the mutex register to release the mutex.
    // PKA operand memory will be scrubbed automatically to clear any confidential
    // information. While scrubbing is in progress, mutex will not be granted by
    // SE HW to any client.
    //
    CHECK_SE_STATUS(seSelwreBusWriteRegisterErrChk(LW_SSE_SE_CTRL_PKA_MUTEX,
                                                   DRF_DEF(_SSE, _SE_CTRL_PKA_MUTEX, _SE_LOCK, _RELEASE)));
 ...
 * @return SE_OK if successful. Error code if unsuccessful.
 */
SE_STATUS seMutexReleaseHs_GP10X(void)
{
    SE_STATUS status = SE_OK;
    ...
    // SE HW to any client.
    //
    CHECK_SE_STATUS(seSelwreBusWriteRegisterErrChkHs(LW_SSE_SE_CTRL_PKA_MUTEX,
                                                   DRF_DEF(_SSE, _SE_CTRL_PKA_MUTEX, _SE_LOCK, _RELEASE)));
...
/*!
 * @brief Sets the timeout value for SE mutex
 *
 * @param[in] timeoutVal  Value to be programmed in the watchdog
 *                        counter
 *
 * @return SE_OK if successful. Error code if unsuccessful.
 */
SE_STATUS seMutexSetTimeoutValue_GP10X(LwU32 timeoutVal)
{
    SE_STATUS status = SE_OK;

    CHECK_SE_STATUS(seSelwreBusWriteRegisterErrChk(LW_SSE_SE_CTRL_PKA_MUTEX_WDTMR, timeoutVal));
...
/*!
 * @brief Sets the timeout value for SE mutex in HS Mode
 *
 * @param[in] timeoutVal  Value to be programmed in the watchdog
 *                        counter
 *
 * @return SE_OK if successful. Error code if unsuccessful.
 */
SE_STATUS seMutexSetTimeoutValueHs_GP10X(LwU32 timeoutVal)
{
    SE_STATUS status = SE_OK;

    CHECK_SE_STATUS(seSelwreBusWriteRegisterErrChkHs(LW_SSE_SE_CTRL_PKA_MUTEX_WDTMR, timeoutVal));
