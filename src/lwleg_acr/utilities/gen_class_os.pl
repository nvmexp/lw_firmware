...
use constant BSI_BOOT_ADDR_MARKER => 0x1100;
use constant BSI_LOCKDOWN_ADDR_MARKER => 0xB00;
...
{
  my $arg = "";
  my $hdr_file = "", my $dmp_file = "", my $path_vpm = "$ELW{'LW_TOOLS'}/falcon-gcc/falcon6/6.5.4/Linux/falcon-elf-gcc/bin";
...
sub verify_size
{
    ...
    if ($bin_name =~ "bsi_boot")
    {
        ...
        if($bin_size > BSI_BOOT_ADDR_MARKER)
        {
    ...
    if ($bin_name =~ "bsi_lockdown")
    {
        ...
        if($bin_size > BSI_LOCKDOWN_ADDR_MARKER)
        {
            exit_error("size exceeded");
        }        
    }
...
sub write_hdr
{
  ...
  print SRC_FILE " *"."\n";
  print SRC_FILE " * Copyright 2013-2015 by LWPU Corporation.  All rights reserved.  All"."\n";
