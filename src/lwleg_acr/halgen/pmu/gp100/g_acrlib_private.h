...
#if defined(ACRCFG_HAL_SETUP_GP100)

static void     acrlibHalIfacesSetup_GP100(ACRLIB_HAL_IFACES *) 
                    GCC_ATTRIB_SECTION("imem_acr", "acrlibHalIfacesSetup_GP100");

static void acrlibHalIfacesSetup_GP100(ACRLIB_HAL_IFACES *pAcrlibHal)
{

    // GP100's ACRLIB hal interface function pointer block
    static const ACRLIB_HAL_IFACES acrlibHalIfacesInitStruct_GP100 =
    {

        0 // avoid empty typedef
    };  // acrlibHalIfacesInitStruct_GP100

    // init GP100's ACRLIB function ptrs using the init struct above
    *pAcrlibHal = acrlibHalIfacesInitStruct_GP100;
}

#endif  // GP10X or GP100
