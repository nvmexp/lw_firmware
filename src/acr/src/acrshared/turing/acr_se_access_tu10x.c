 ...
 */
ACR_STATUS
acrAcquireSelwreSECommonMutex_TU10X
(
    LwU8  mutexId,
    LwU8 *pToken
)
{
//
// This is to WAR S3/Resume issue on Fmodel.
// SE engine doen't behave properly.
//
#if defined(ACR_FMODEL_BUILD) && defined(ASB)
    return ACR_OK;
#endif
    ...
    CHECK_STATUS_AND_RETURN_IF_NOT_OK(
        acrSelwreBusReadRegister_HAL(pAcr, busTarget, LW_SSE_SE_COMMON_MUTEX_ID(groupIndex),
            &genId));
    ...
    do
    {
        ...
        if (ACR_OK != (status = acrSelwreBusReadRegister_HAL(pAcr, busTarget,
...
ACR_STATUS
acrReleaseSelwreSECommonMutex_TU10X
(
    LwU8  mutexId,
...
)
{
    ...
    CHECK_STATUS_AND_RETURN_IF_NOT_OK (acrSelwreBusReadRegister_HAL(pAcr, busTarget,
    ...
    status2 = acrSelwreBusWriteRegister_HAL(pAcr, busTarget,
                 LW_SSE_SE_COMMON_MUTEX_ID_RELEASE(groupIndex), hwTokenFromCaller);


    return ((status == ACR_OK) ? status2 : status);
}
 ...
 */
ACR_STATUS
acrSelwreBusSendRequest_TU10X
(
    ACR_SELWREBUS_TARGET    busTarget,
    LwBool                  bRead,
    LwU32                   addr,
    LwU32                   valueToWrite
)
{
...
#if defined(AHESASC) || defined(ASB) || defined(BSI_LOCK) || defined(ACR_UNLOAD_ON_SEC2)
    if (busTarget == SEC2_SELWREBUS_TARGET_HUB)
    {
    ...
    else if (busTarget == SEC2_SELWREBUS_TARGET_SE)
    {
...
#elif defined(ACR_UNLOAD)
    if (busTarget == PMU_SELWREBUS_TARGET_SE)
    {
        dioReadyVal =   DRF_DEF(_CPWR, _FALCON_DOC_CTRL, _EMPTY,       _INIT) |
                        DRF_DEF(_CPWR, _FALCON_DOC_CTRL, _WR_FINISHED, _INIT) |
                        DRF_DEF(_CPWR, _FALCON_DOC_CTRL, _RD_FINISHED, _INIT);

        ctrlAddr = LW_CPWR_FALCON_DOC_CTRL;
        d0Addr   = LW_CPWR_FALCON_DOC_D0;
        d1Addr   = LW_CPWR_FALCON_DOC_D1;
    }
    else
    {
        return ACR_ERROR_ILWALID_ARGUMENT;
    ...
    do
    {
        status = ACR_REG_RD32(CSB, ctrlAddr) & dioReadyVal;
    }
    ...
    {
        ACR_REG_WR32(CSB, d1Addr, valueToWrite);
    }

    // 3. Issue request
    ACR_REG_WR32(CSB, d0Addr, addr);

    return ACR_OK;
}

/*!
 * @brief Once a read request happens through acrSelwreBusSendRequest,
...
)
{
    ...
    if (busTarget == SEC2_SELWREBUS_TARGET_HUB)
    {
        dicCtrlAddr = LW_CSEC_FALCON_DIO_DIC_CTRL(0);
        docCtrlAddr = LW_CSEC_FALCON_DIO_DOC_CTRL(0);
        d0Addr      = LW_CSEC_FALCON_DIO_DIC_D0(0);
     }
    else if (busTarget ==  SEC2_SELWREBUS_TARGET_SE)
    {
    ...
    if (busTarget == PMU_SELWREBUS_TARGET_SE)
    {
        dicCtrlAddr = LW_CPWR_FALCON_DIC_CTRL;
        docCtrlAddr = LW_CPWR_FALCON_DOC_CTRL;
        d0Addr      = LW_CPWR_FALCON_DIC_D0;
        pop1Dword   = FLD_SET_DRF_NUM(_CPWR, _FALCON_DIC_CTRL, _POP, 0x1, pop1Dword);
    }
    else
    {
        return ACR_ERROR_ILWALID_ARGUMENT;
    ...
    do
    {
        data = ACR_REG_RD32(CSB, dicCtrlAddr);
    }
    // TODO for jamesx for Bug 1732094: Timeout support
    ...
    // TODO for jamesx for Bug 1732094: Timeout support
    while (FLD_TEST_DRF_NUM(_CPWR, _FALCON_DIC_CTRL, _COUNT, 0x0, data));
#endif


    // 2. Pop read data
    ACR_REG_WR32(CSB, dicCtrlAddr, pop1Dword);

    // 2.5 Check read error from the DOC control
    error = ACR_REG_RD32(CSB, docCtrlAddr);
...
#elif defined(ACR_UNLOAD)
    if (FLD_TEST_DRF_NUM(_CPWR, _FALCON_DOC_CTRL, _RD_ERROR,       0x1, error) ||
        FLD_TEST_DRF_NUM(_CPWR, _FALCON_DOC_CTRL, _PROTOCOL_ERROR, 0x1, error))
    {
        return ACR_ERROR_SELWRE_BUS_REQUEST_FAILED;
    }
#else
    ...
    *pVal = ACR_REG_RD32(CSB, d0Addr);
    ...
    //
    if (acrIgnoreSePkaBankRegForBadValueCheck_HAL(pAcr, addr))
    {
        return ACR_OK;
    }
    ...
    if ((*pVal & CSB_INTERFACE_MASK_VALUE) == CSB_INTERFACE_BAD_VALUE)
    {
        ...
        FAIL_ACR_WITH_ERROR(ACR_ERROR_PRI_FAILURE_FOR_SE_REG_ACCESS_OVER_SE_BUS);
    }
#endif
 ...
 */
ACR_STATUS
acrSelwreBusWriteRegister_TU10X
(
    ACR_SELWREBUS_TARGET busTarget,
    LwU32 addr,
    LwU32 val
...
{
    return acrSelwreBusSendRequest_HAL(pAcr, busTarget, LW_FALSE, addr, val);
}
 ...
 *
 * @return ACR_STATUS whether the read was successful or not
 */
ACR_STATUS
acrSelwreBusReadRegister_TU10X
(
    ACR_SELWREBUS_TARGET busTarget,
    LwU32 addr,
    LwU32 *pVal
)
{
    ...
    status = acrSelwreBusSendRequest_HAL(pAcr, busTarget, LW_TRUE, addr, 0);

    if (status == ACR_OK)
    {
        acrSelwreBusGetData_HAL(pAcr, busTarget, addr, pVal);
        return ACR_OK;
    }
