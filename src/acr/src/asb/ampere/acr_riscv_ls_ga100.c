 ...
 */
/*!
 * @file: acr_riscv_ls_ga100.c
 */

// Sanity check.
#ifndef ACR_RISCV_LS
#error "Including acr_riscv_ls_ga100.c in build but RISC-V LS is not enabled!"
#endif
 ...
 *     an ugly hack to work around a GA100 bug wherein RISC-V cores must boot from
 *     ITCM, rather than FB as we would like. This function is only intended to be used
 *     for prototyping and testing of RISC-V LS on GA100. The code here should not be
 ...
 * @param[in] wprAddress   WPR base address of image
 */
ACR_STATUS
acrlibSetupBootvecRiscv_GA100
(
...
)
{
    ...
    LwU64 bootvec = LW_RISCV_AMAP_FBGPA_START + wprAddress;
    ...
    acrlibFlcnRegWrite_HAL(pAcrlib, pFlcnCfg, BAR0_FLCN, LW_PFALCON_FALCON_IMEMC(0), cmd);

    // Need to write an IMEM tag even though we shouldn't be using it.
    acrlibFlcnRegWrite_HAL(pAcrlib, pFlcnCfg, BAR0_FLCN, LW_PFALCON_FALCON_IMEMT(0), 0);

    {
        ...
        // accessing external recources to zero (NS level) and two (FALCON_SCTL.LSMODE_LEVEL).
        //
        LwU64 mspm = DRF_DEF64(_RISCV_CSR, _MSPM, _MPLM_LEVEL0, _ENABLE) |
                     DRF_DEF64(_RISCV_CSR, _MSPM, _MPLM_LEVEL2, _ENABLE) |
                     DRF_DEF64(_RISCV_CSR, _MSPM, _UPLM_LEVEL0, _ENABLE) |
                     DRF_DEF64(_RISCV_CSR, _MSPM, _UPLM_LEVEL2, _ENABLE);
        ...
        // same level.
        //
        LwU64 mrsp = DRF_DEF64(_RISCV_CSR, _MRSP, _MRPL, _LEVEL2) |
                     DRF_DEF64(_RISCV_CSR, _MRSP, _URPL, _LEVEL2) |
                     DRF_DEF64(_RISCV_CSR, _MRSP, _ICD_PL, _USE_LWR);
        ...
        const LwU32 imemBootstub[] = {
            ...
            0x00000067 | (6 << 15),                                     //      jalr    zero, t1, 0
                                                                        //  data_pool: [@0x28]
            bootvec, bootvec >> 32,                                     //      .long bootvec
            uacc, uacc >> 32,                                           //      .long uaccessattr
            mspm, mspm >> 32,                                           //      .long mspm
            mrsp, mrsp >> 32,                                           //      .long mrsp
        };

        for (i = 0; i < sizeof(imemBootstub) / sizeof(*imemBootstub); i++)
        {
            acrlibFlcnRegWrite_HAL(pAcrlib, pFlcnCfg, BAR0_FLCN, LW_PFALCON_FALCON_IMEMD(0),
                                   imemBootstub[i]);
        }
    }
    ...
    acrlibFlcnRegWrite_HAL(pAcrlib, pFlcnCfg, BAR0_FLCN, LW_PFALCON_FALCON_IMEMC(0), cmd);
    acrlibFlcnRegWrite_HAL(pAcrlib, pFlcnCfg, BAR0_FLCN, LW_PFALCON_FALCON_IMEMD(0), 0);

    // Bootstrap was written to the beginning of IMEM, so set the boot vector to 0.
    acrlibFlcnRegLabelWrite_HAL(pAcrlib, pFlcnCfg, REG_LABEL_RISCV_BOOT_VECTOR_HI, 0);
    acrlibFlcnRegLabelWrite_HAL(pAcrlib, pFlcnCfg, REG_LABEL_RISCV_BOOT_VECTOR_LO, 0);
