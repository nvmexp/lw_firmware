...
/*
**      Modular_exponentiation routines.
**
...
typedef struct {
    /* Temps and interface for mod_exp and helpers */
...
static DRM_BOOL DRM_CALL _basepower_squaring(
    __inout_ecount( temps->modulo->length ) digit_t         *basepower,
    __inout_ecount( 1 )                     temps_t         *temps,
    __inout                                 struct bigctx_t *f_pBigCtx )
{
    DRM_BOOL OK = TRUE;
    const mp_modulus_t *modulo = temps->modulo;

    OK = OK && mod_mul( basepower, basepower, basepower, modulo, temps->modmultemps, f_pBigCtx );
    return OK;
}

static DRM_NO_INLINE DRM_BOOL DRM_CALL _bucket_multiply(
    __in                                 const DRM_DWORD         ibucket,
    __in_ecount( temps->modulo->length ) const digit_t          *multiply_by,
    __inout_ecount( 1 )                        temps_t          *temps,
    __inout                                    struct bigctx_t  *f_pBigCtx )
{
    DRM_BOOL OK = TRUE;
...
static const unsigned short width_lwtoffs[] PR_ATTR_DATA_OVLY(_width_lwtoffs) = {6, 24, 80, 240, 672};  /* (k + 2)*(k + 3) * 2^k */
...

{
    DRM_BOOL OK = TRUE;
    const DRM_DWORD elng = modulo->length;
    const DRM_DWORD exponent_bits_used = mp_significant_bit_count( exponent, lngexpon );

    temps_t *temps = NULL;          /* malloc-ed */
    digit_t *basepower = answer;    /* Same array used for both */
    digit_t *bucket_data = NULL;    /* malloc-ed */

    DRM_DWORD bucket_mask;
    DRM_DWORD bucket_width;
    DRM_DWORD carried;
    DRM_DWORD ibit;
    DRM_DWORD ibucket;
    DRM_DWORD max_bucket;
    DRM_DWORD ndoubling;
    DRM_BOOL base2;
    ...
    {
        bucket_data = digit_allocate( elng*max_bucket + modulo->modmul_algorithm_temps, f_pBigCtx );
    }
    ...
    if( OK )
    {
        ...
        */
        ...
        OK = OK && add_mod( modulo->one, modulo->one, bucket_data, modulo->modulus, modulo->length );     /* bucket_data = 2 */
    ...
    else if( base2 && exponent_bits_used != 0 )
    {
        ...
        */
        for( ibit = exponent_bits_used; OK && ( ibit-- ) != 0; /* no-op */ )
        {
            ...
            if( high_bits_processed )
            {
                ...
                OK = OK && mod_mul( dtemp, dtemp, dtemp, modulo, temps->modmultemps, f_pBigCtx );
                if( iexpbit != 0 )
                {
                    OK = OK && add_mod( dtemp, dtemp, dtemp, modulo->modulus, modulo->length );
                }
            }
            else
            {
                    ...
                    || 2 * high_expon_bits >= shift_max )
                {
                    ...
                    OK = OK && mod_shift( modulo->one, (DRM_LONG)high_expon_bits, dtemp, modulo );

                }
            }
        }
        temps->bucket_location[ 1 ] = dtemp;

        if( !high_bits_processed )
        {
            OK = FALSE;
            SetMpErrno_clue( MP_ERRNO_INTERNAL_ERROR, "mod_exp -- high_bits_processed = FALSE" );
        }
    }
    else
    {
        ...
        */
        ...
        {
            digit_t *bloc = bucket_data + elng * ( ibucket - 1 + ( DRM_IS_EVEN( ibucket ) ? max_bucket : 0 ) ) / 2;

            temps->bucket_location[ ibucket ] = bloc;
            temps->bucket_oclwpied[ ibucket ] = FALSE;

            OEM_SELWRE_DIGITTCPY( bloc, modulo->one, elng );   /* Set bucket contents = 1 */
        }
        ...
        for( ibit = 0; OK && ibit != exponent_bits_used; ibit++ )
        {
            ...
            */
            ...
            {
                OK = FALSE;
                SetMpErrno_clue( MP_ERRNO_INTERNAL_ERROR, "mod_exp -- carried overflow" );
            }
            ...
            if( bit_now != 0 )
            {
                while( OK && ndoubling >= bucket_width + 1 )
                {
                    if( DRM_IS_ODD( carried ) )
                    {
                        ...
                        carried -= ibucket;
                        OK = OK && _bucket_multiply( ibucket, basepower, temps, f_pBigCtx );
                    }
                    OK = OK && _basepower_squaring( basepower, temps, f_pBigCtx );
                    carried /= 2;
                    ndoubling--;
                }
