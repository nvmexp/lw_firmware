...
***@@@---@@@@******************************************************************
*/
#define DRM_BUILDING_ECEX2001_C
#include "bignum.h"
#include "elwrve.h"
...
/*
**          Let E be an elliptic lwrve (over GF(2^m) or GF(p), where p is prime).
**          The costs of some group operations (using affine coordinates) are
**          (I = ilwersion, S = squaring, M = multiplication)
**
...
**      ilwersions by three multiplications.
...
**           2^(w-3) - 1 ilwocations of (both P +- Q) code
**           w - 3       ilwocations of doubling code
**           2^(w-3) - 2 replacements of an ilwersion by 3 multiplications
**
**          After this table is built, the idea is to use windows of width w,
**      with odd signed digits from -oddmax to +oddmax.
**      A simplified, relwrsive, description is
**
...
**      That is log2(e)*w/(w + 1) ilwocations of the doubling code and
**      log2(e)/(w + 1) ilwocations of the 2*P + Q code
**
...
**       oddmax =        (1, 3, 7, 15, 31)
**
**       if (expleft is high or more bits left) then
**           delay
...
**           Use DRM_MIN(expleft - 1, oddmax) and another odd value
...
/*
** Compute Pesult = exponent*P0 in the elliptic lwrve group.
** This code allows P0 and Pesult to overlap.
*/
    ...
    __inout                                       struct bigctx_t   *f_pBigCtx )
{
    ...
    static const DRM_DWORD wlwtoffs[] PR_ATTR_DATA_OVLY(_wlwtoffs) = { 22, 50, 110, 256, 600 };
#define WMAX ((DRM_DWORD)(2 + sizeof(wlwtoffs)/sizeof(wlwtoffs[0])))
    ...
    **
    **  wlwtoffs has approximations to the lwtoffs suggested by the
    **  Maple program at the end of this file.
    **  [Actual lwtoffs are about 20% lower for GF(2^n) than
    **  for GF(p) -- this table has compromise values.]
     */
    w = 2;
    while( w < WMAX && expbits > wlwtoffs[ w - 2 ] ) w++;

    /*
    ** Check for Koblitz lwrves (ec_Koblitz2_exponentiation).
    ...
    */
    if( OK )
    {
        const DRM_DWORD ndigtemp = E->ndigtemps + ( (DRM_DWORD)oddmax + 3 ) * elng + lng_exponent;
        ...
        {
            dtemps = digit_allocate( ndigtemp, f_pBigCtx );
            if( dtemps == NULL )
            {
                OK = FALSE;
            }
            ...
            {
                OEM_SELWRE_ZERO_MEMORY( dtemps, ( ndigtemp ) * sizeof( digit_t ) );
...
#if DRM_DBG
#define p0location(iodd) (DRMASSERT((DRM_BOOL)((iodd) & 1)), \
                          DRMASSERT((digit_t)(iodd) <= oddmax), \
                          &p0muls[elng*(iodd) - elng])
    /* Return address in p0muls array, after validating  subscript in debug mode. */
#else  /* DRM_DBG */
#define p0location(iodd) (&p0muls[elng*(iodd) - elng])
#endif /* DRM_DBG */
    ...
    if( OK )
    {
        ...
        */
        OEM_SELWRE_DIGITTCPY( p0location( 1 ), P0, 2 * E->fdesc->elng );
        if( w == 2 )
        {
            OEM_SELWRE_DIGITTCPY( Presult, P0, 2 * E->fdesc->elng );    /* Only P0 stored in table */
        }
        else
        {
            doublings3pwr = 3;
            OK = OK && ecaffine_PPQ( P0, P0, p0location( 3 ), +1, E, etemps, f_pBigCtx );   /* 3*P0 */

            if( w == 3 )
            {
                OEM_SELWRE_DIGITTCPY( Presult, p0location( 3 ), 2 * E->fdesc->elng );  /* P0 and 3*P0 saved */
            }
            else
            {
                OK = OK && ecaffine_addition( p0location( 3 ), p0location( 3 ), Presult, +1, E, etemps, f_pBigCtx );  /* Presult = 6*P0 */
                ...
                for( iw = 4; iw <= w; iw++ )
                {
                    ...
                    ** combining the ilwersions
                    */
                    for( j = 1; j >> ( iw - 3 ) == 0; j += 2 )
                    {
                        OK = OK && ecaffine_addition_subtraction( Presult, p0location( j ), p0location( doublings3pwr + j ), p0location( doublings3pwr - j ), E, etemps, f_pBigCtx );
                    }
                    if( iw != w )
                    {
                        OK = OK && ecaffine_addition( Presult, Presult, Presult, +1, E, etemps, f_pBigCtx );
                    }
                }
            }
        }
        if( !OK )
        {
            DRM_DBG_TRACE( ( "ecaffine_exponentiation -- OK = FALSE initializing multiples of P0\n" ) );
        }
    }
    ...
    for( iexpbit = 0; iexpbit != expbits && OK; iexpbit++ )
    {
        expon_partial += mp_getbit( exponent, iexpbit ) << ( iexpbit - numdoubling );
        ...
        {
            if( DRM_IS_ODD( expon_partial ) )
            {
                const sdigit_t sindex = (const sdigit_t)( ( ( expon_partial + oddmax ) & ( 2 * oddmax ) ) - oddmax );
                expon_partial -= (digit_t)sindex;
                DRMASSERT( ( expon_partial & ( 2 * oddmax + 1 ) ) == 0 );
                mp_setbit( adjustment_locs, lng_exponent, numdoubling, 1 );
            }
            expon_partial >>= 1;
            numdoubling++;
        }
    ...
    while( OK && delay )
    {
        const DRM_BOOL expon_partial_odd = ( 0 != DRM_IS_ODD( expon_partial ) );
        const digit_t expon_partial3 = (const digit_t)( expon_partial - doublings3pwr );    /* May be negative */
        const DRM_BOOL expon_partial3_odd = ( 0 != DRM_IS_ODD( expon_partial3 ) );
        const digit_t expon_partial_div3 = (const digit_t)( expon_partial / 3 );
        ...
        {
            OEM_SELWRE_DIGITTCPY( Presult, p0location( expon_partial ), 2 * E->fdesc->elng );
        }
        ...
        {
            const digit_t odd1 = DRM_MIN( expon_partial - 1, oddmax );
            const digit_t odd2 = expon_partial - odd1;

            OK = OK && ecaffine_addition( p0location( odd1 ), p0location( odd2 ), Presult, +1, E, etemps, f_pBigCtx );
        }
        else if( expon_partial3_odd && expon_partial <= doublings3pwr + oddmax )
        {
            OK = OK && ecaffine_addition( Presult, p0location( expon_partial3 ), Presult, +1, E, etemps, f_pBigCtx );
        }
        else if( expon_partial_odd && expon_partial <= 3 * oddmax )
        {
            ...
            OK = OK && ecaffine_PPQ( p0location( odd1 ), p0location( odd2 ), Presult, +1, E, etemps, f_pBigCtx );
        }
        else if( ( expon_partial3 & 3 ) == 2 && expon_partial3 <= 2 * oddmax )
        {
            OK = OK && ecaffine_PPQ( p0location( expon_partial3 >> 1 ), Presult, Presult, +1, E, etemps, f_pBigCtx );
        }
        else if( !expon_partial_odd && expon_partial <= 2 * ( oddmax + doublings3pwr ) )
        {
            delay = TRUE;
        }
        ...
        else if( !expon_partial_odd && expon_partial_mod3 == 0 && expon_partial_div3 <= 2 * oddmax )
        {
            const digit_t odd1 = DRM_MIN( expon_partial_div3 - 1, oddmax );
            const digit_t odd2 = expon_partial_div3 - odd1;  /* expon_partial = 3*(odd1 + odd2) */

            OK = OK && ecaffine_addition( p0location( odd1 ), p0location( odd2 ), Presult, +1, E, etemps, f_pBigCtx );
            OK = OK && ecaffine_PPQ( Presult, Presult, Presult, +1, E, etemps, f_pBigCtx );
        }
        else if( expon_partial_mod3 == 0 && expon_partial3_odd && expon_partial_div3 - doublings3pwr <= oddmax )
        {
            DRMASSERT( expon_partial == 3 * ( doublings3pwr + ( expon_partial_div3 - doublings3pwr ) ) );

            OK = OK && ecaffine_addition( p0location( expon_partial_div3 - doublings3pwr ), Presult, Presult, +1, E, etemps, f_pBigCtx );
            OK = OK && ecaffine_PPQ( Presult, Presult, Presult, +1, E, etemps, f_pBigCtx );
        }
        else if( expon_partial_odd && expon_partial <= 5 * oddmax )
        {
            ...
            OK = OK && ecaffine_addition( p0location( oddmax ), p0location( even1 - oddmax ), Presult, +1, E, etemps, f_pBigCtx );
            OK = OK && ecaffine_PPQ( Presult, p0location( expon_partial - 2 * even1 ), Presult, +1, E, etemps, f_pBigCtx );
        }
        else
        {
            delay = TRUE;
        ...
        if( !OK )
        {
            DRM_DBG_TRACE( ( "ecaffine_exponentiation -- OK = FALSE" ) );
            DRM_DBG_TRACE( ( "multiplying by expon_partial = %ld\n", (DRM_LONG)expon_partial ) );
        }
        else if( delay )
        {
            ...
            {
                const sdigit_t sindex = (const sdigit_t)( ( ( expon_partial + oddmax ) & ( 2 * oddmax ) ) - oddmax );
                expon_partial = (digit_t)( expon_partial - (digit_t)sindex );
                DRMASSERT( ( expon_partial & ( 2 * oddmax + 1 ) ) == 0 );
                mp_setbit( adjustment_locs, lng_exponent, numdoubling, 1 );
            }
            numdoubling++;
    ...
    */
    ...
    for( iexpbit = expbits; iexpbit-- && OK; /* no-op */ )
    {
        ...
        */
        ...
        else
        {
            ...
            expon_excess = (sdigit_t)( ( expon_excess << pre_doublings ) - exponent_bit );
            pm1 = ( expon_excess > 0 ? ( (DRM_LONG)-1 ) : ( (DRM_LONG)+1 ) );
            ...
            {
                OK = FALSE;
                SetMpErrno_clue( MP_ERRNO_INTERNAL_ERROR, "ecaffine_exponentiation pre_doublings" );
                DRM_DBG_TRACE( ( "ecaffine_exponentiation -- pre_doublings = %ld out of range\n",
                    (DRM_LONG)pre_doublings ) );
            }
            else if( index > oddmax )
            {
                OK = FALSE;
                SetMpErrno_clue( MP_ERRNO_INTERNAL_ERROR, "ecaffine_exponentiation index > oddmax" );
                DRM_DBG_TRACE( ( "ecaffine_exponentiation -- index = %ld exceeds oddmax = %ld\n",
            ...
            {
                OK = OK && ecaffine_addition( Presult, p0location( index ), Presult, pm1, E, etemps, f_pBigCtx );
            }
            else if( pre_doublings == 1 )
            {
                OK = OK && ecaffine_PPQ( Presult, p0location( index ), Presult, pm1, E, etemps, f_pBigCtx );
            }
            else
            {
                DRMASSERT( FALSE );
                OK = FALSE;
                SetMpErrno_clue( MP_ERRNO_INTERNAL_ERROR, "ecaffine_exponentiation - 3" );
            }
        }
    }
    ...
    if( !OK )
    {
        DRM_DBG_TRACE( ( "ecaffine_exponentiation exiting unsuccessfully\n" ) );
    }
    return OK;
#undef p0location
#undef WMAX
