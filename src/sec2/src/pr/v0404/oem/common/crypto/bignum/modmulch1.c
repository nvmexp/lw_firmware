    ...
    __inout_ecount( pmodulo->length * 2 )       digit_t       *temps )
{
    ...
    OK = OK && divide( temp1, 2 * lng, pmodulo->modulus, lng, &pmodulo->left_reciprocal_1, NULL, c );
    return OK;
}
    ...
    __inout_ecount( pmodulo->length * 2 )       digit_t       *temps )
{
    DRM_BOOL OK = TRUE;
    const DRM_DWORD lng = pmodulo->length;
    digit_t *temp1 = temps;         /* length lng */
    digit_t *temp2 = temps + lng;   /* length lng */
    ...
    __inout_ecount( pmodulo->length * 2 )       digit_t       *temps )
{
    ...
    PRAGMA_WARNING_PUSH;
    PRAGMA_WARNING_DISABLE( 4146 );    /* Unary minus operator applied to unsigned type, result still unsigned. */
    ...
    __analysis_assume( pmodulo->length == LNGQ_MODULO_8 );
    ...
    __inout_ecount( pmodulo->length * 2 )       digit_t       *temps )
{
    ...
    const digit_t milwa0 = milw * a[ 0 ];    /* mod RADIX */
    DRM_DWORD j;
    DRM_UINT64 carry1full;
    DRM_UINT64 carry2full;
    digit_t mul1;
    digit_t mul2;
    DRM_UINT64 prod1;
    DRM_UINT64 prod2;
