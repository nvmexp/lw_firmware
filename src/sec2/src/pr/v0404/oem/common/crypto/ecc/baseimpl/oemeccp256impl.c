...
//
// NVE (kwilson) - We will use these values directly to prevent the need to declare another curve.
// Once all ECC functionality has been ported to HW SE, then there will only be a need for one curve
...
/*************************************************************************************************
**
** Function: _Convert_P256_BigEndianBytesToModular
**
...
**
*************************************************************************************************/
static DRM_NO_INLINE DRM_RESULT DRM_CALL _Convert_P256_BigEndianBytesToModular(
    ...
    __inout                                                    struct bigctx_t *f_pBigCtx )
{
    ...
    */
    ChkDR( Convert_P256_BigEndianBytesToDigitsImpl( f_rgbInBytes, rgdTemp ) );
...
ErrorExit:
    ...
    return dr;
} /* end _Convert_P256_BigEndianBytesToModular */
    ...
    __inout                                       struct bigctx_t *f_pBigCtx )
{
    ...
    OEM_SECURE_DIGITZEROMEMORY( rgdYSqrd, DRM_NO_OF(rgdYSqrd) );

    __analysis_assume( g_Ecurve.fdesc->elng == ECC_P256_INTEGER_SIZE_IN_DIGITS );
    __analysis_assume( g_Ecurve.fdesc->ndigtemps_mul == ECC_P256_INTEGER_SIZE_IN_DIGITS * 2 );
...
**
*************************************************************************************************/
static DRM_NO_INLINE DRM_RESULT DRM_CALL Convert_P256_PlaintextToPoint(
    __in_ecount( 1 )                                 const  PLAINTEXT_P256 *f_pPlaintext,
    __in_ecount( 1 )                                 const  ecurve_t       *f_pEcurve,
    __out_ecount( ECC_P256_ENCRYPT_TEMPS )                  digit_t         f_rgdSuppliedTemps[],
    ...
    __inout                                          struct bigctx_t       *f_pBigCtx )
{
    ...
    */
    ChkDR( _Convert_P256_BigEndianBytesToModular( f_pPlaintext->m_rgbPlaintext,
                                                 f_pEcurve,
                                                 f_rgptOutPoint,
                                                 f_pBigCtx ) );
    ...
    __out_ecount( ECC_P256_POINT_SIZE_IN_BYTES )                    DRM_BYTE  f_rgbOutBytes[] )
{
    ...
    */
    ChkBOOL( from_modular( f_rgptInPoint,
                           ...
                           f_pEcurve->fdesc->modulo ),
             DRM_E_P256_CONVERSION_FAILURE );
    ChkDR( Convert_P256_DigitsToBigEndianBytesImpl( rgdTemp, f_rgbOutBytes ) );
    ...
    */
    ChkBOOL( from_modular( f_rgptInPoint + ECC_P256_POINT_SIZE_IN_ECAFFINES / 2,
                           ...
                           f_pEcurve->fdesc->modulo ),
             DRM_E_P256_CONVERSION_FAILURE );
    ChkDR( Convert_P256_DigitsToBigEndianBytesImpl( rgdTemp,
...
/*************************************************************************************************
**
** Function: Convert_P256_PointToPlaintextImpl
**
...
**
*************************************************************************************************/
DRM_NO_INLINE DRM_API DRM_RESULT DRM_CALL Convert_P256_PointToPlaintextImpl(
    ...
    __out_ecount( 1 )                                     PLAINTEXT_P256 *f_pPlaintext )
{
    ...
    */
    ChkDR( Convert_P256_DigitsToBigEndianBytesImpl( rgdTemp, f_pPlaintext->m_rgbPlaintext ) );

ErrorExit:
    return dr;
} /* end Convert_P256_PointToPlaintextImpl */
    ...
    __in                        DRM_DWORD        f_cbBigCtx )
{
    ...
    */
    ...
    OEM_SECURE_DIGITZEROMEMORY( rgdRandNum, DRM_NO_OF(rgdRandNum) );
    ...
    */
    OEM_SECURE_MEMSET( f_pKeys, 0, ECC_P256_INTEGER_SIZE_IN_BYTES / 2 );

    /*
    ** Convert the first key to digits
    */
    ChkDR( Convert_P256_BigEndianBytesToDigitsImpl( f_pKeys->m_rgbPlaintext, rgdRandNum ) );
    ...
    */
    for ( i = 0; i < P256_KEY_GEN_TRIES; ++i )
    {
        ...
        */
PREFAST_PUSH_DISABLE_EXPLAINED( __WARNING_READ_OVERRUN_6385, "OACR gets confused by the math being used to create rgdRandNum - it thinks it's less than ECC_P256_INTEGER_SIZE_IN_DIGITS" )
        DRMCASSERT( DRM_NO_OF( rgdRandNum ) >= ECC_P256_INTEGER_SIZE_IN_DIGITS );
...
PREFAST_POP     /* __WARNING_READ_OVERRUN_6385 */
        ...
        if ( DRM_SUCCESS == dr )
        {
            ...
            */
            ChkDR( Convert_P256_DigitsToBigEndianBytesImpl( rgdRandNum, f_pKeys->m_rgbPlaintext ) );
            break;
        } /* end if */
    ...
    __inout                                        struct bigctx_t *f_pBigCtx )
{
    ...
    */
    ...
    OEM_SECURE_MEMSET( rgptTemp, 0, sizeof( rgptTemp ) );
...
/************************************************************************************************
**
** Function: OEM_ECC_GenerateTeeSigningPublicKey_P256Impl
**
** Synopsis: Doubles the ECDSA Public Key in order to verify the TEE Signed data in the License
**           challenge
**
** Arguments:
**    [f_pPubkeyOriginal]:  Original, unmodified public key
**    [f_rgbPubkeyDoubled]: Resulting doubled public key
**    [f_pBigCtx]:          Pointer to DRMBIGNUM_CONTEXT_STRUCT for temporary memory allocations.
**    [f_cbBigCtx]:         Size of allocated f_pBigCtx
**
** Returns: DRM_RESULT indicating success or the correct error code. Both parameters will be set
**          if function executes successfully.
**
** Notes: It is put in the [f_rgbPubkeyDoubled].
**
*************************************************************************************************/
DRM_NO_INLINE DRM_API DRM_RESULT DRM_CALL OEM_ECC_GenerateTeeSigningPublicKey_P256Impl(
    ...
    __inout  struct bigctx_t       *f_pBigCtx,
    __in            DRM_DWORD       f_cbBigCt )
{
    CLAW_AUTO_RANDOM_CIPHER
    DRM_RESULT dr                                                   = DRM_SUCCESS;
    digit_t    rgptPubOriginal[ ECC_P256_POINT_SIZE_IN_ECAFFINES ];
    digit_t    rgptPubDoubled[ ECC_P256_POINT_SIZE_IN_ECAFFINES ];

    ChkArg( NULL != f_pPubkeyOriginal );
    ChkArg( NULL != f_rgbPubkeyDoubled );
    ChkArg( NULL != f_pBigCtx );

    ChkDR( OEM_ECC_InitializeBignumStack( f_pBigCtx ) );

    ChkDR( Convert_P256_BigEndianBytesToPointImpl(
        f_pPubkeyOriginal->m_rgbPubkey,
        &g_Ecurve,
        rgptPubOriginal,
        f_pBigCtx ) );

    /* rgptPubDoubled = rgptPubOriginal + rgptPubOriginal */
    ChkBOOL( ecaffine_addition(
        rgptPubOriginal,
        rgptPubOriginal,
        rgptPubDoubled,
        1,
        &g_Ecurve,
        NULL,
        f_pBigCtx ), DRM_E_PKCRYPTO_FAILURE );

    /* Verify that the doubled public key lies on the curve */
    ChkBOOL( ecaffine_on_curve(
        rgptPubDoubled,
        &g_Ecurve,
        NULL,
        NULL,
    ...
    ChkDR( Convert_P256_PointToBigEndianBytes( rgptPubDoubled, &g_Ecurve, f_rgbPubkeyDoubled ) );

ErrorExit:
    return dr;
} /* end OEM_ECC_GenerateTeeSigningPublicKey_P256 */
#endif
    ...
    __in           DRM_DWORD        f_cbBigCtx )
{
    ...
    */
    ...
    const CIPHERTEXT_P256_2POINTS *pCtPoints = DRM_REINTERPRET_CONST_CAST( const CIPHERTEXT_P256_2POINTS, f_pCiphertext );
    ...
    */
    ...
    __analysis_assume( g_Ecurve.fdesc->elng == ECC_P256_INTEGER_SIZE_IN_DIGITS );
    __analysis_assume( g_Ecurve.fdesc->ndigtemps_mul     == ECC_P256_INTEGER_SIZE_IN_DIGITS * 2 );
    __analysis_assume( g_Ecurve.fdesc->ndigtemps_invert1 == 10 * ECC_P256_INTEGER_SIZE_IN_DIGITS + 6 );
    __analysis_assume( g_Ecurve.ndigtemps == ECC_P256_NDIGTEMPS );
    ...
    */
    ChkDR( Convert_P256_BigEndianBytesToDigitsImpl( f_pPrivkey->m_rgbPrivkey, rgdPrivkey ) );

    /*
    ** Convert the ciphertext byte stream to two modular format points
    */
    ChkDR( Convert_P256_BigEndianBytesToPointImpl( pCtPoints->m_rgbC1,
                                              &g_Ecurve,
                                               rgptC1,
                                               f_pBigCtx ) );
    ChkDR( Convert_P256_BigEndianBytesToPointImpl( pCtPoints->m_rgbC2,
                                              &g_Ecurve,
                                               rgptC2,
                                               f_pBigCtx ) );
    ...
    */
    ...
    ChkDR( Convert_P256_PointToPlaintextImpl( rgptResult,
                                        &g_Ecurve,
                                         f_pPlaintext ) );
...
**                     This function does not check that it is on the curve.
    ...
    __inout                                         struct bigctx_t        *f_pBigCtx )
{
    CLAW_AUTO_RANDOM_CIPHER
    DRM_RESULT               dr         = DRM_SUCCESS;
    CIPHERTEXT_P256_2POINTS *pCtPoints  = DRM_REINTERPRET_CAST( CIPHERTEXT_P256_2POINTS, f_pCiphertext );
    digit_t                  rgdR[ ECC_P256_INTEGER_SIZE_IN_DIGITS ];
    digit_t                  rgptPubkey[ ECC_P256_POINT_SIZE_IN_ECAFFINES ];
    /*
        these are initialized during calculation
    */
    digit_t    rgptC1[ ECC_P256_POINT_SIZE_IN_ECAFFINES ];
    digit_t    rgptC2[ ECC_P256_POINT_SIZE_IN_ECAFFINES ];
    ...
    OEM_SECURE_MEMSET( rgdR, 0, sizeof( rgdR ) );

    /*
    ** Convert the public key and generator point ( aka base point ) into point format
    */
    ChkDR( Convert_P256_BigEndianBytesToPointImpl( f_pPubkey->m_rgbPubkey,
                                              &g_Ecurve,
                                               rgptPubkey,
                                               f_pBigCtx ) );
    ...
    */
    ChkBOOL( !ecaffine_is_infinite( rgptPubkey, &g_Ecurve, f_pBigCtx ), DRM_E_PKCRYPTO_FAILURE );
    ...
    __in           DRM_DWORD        f_cbBigCtx )
{
    ...
    */
    ...
    OEM_SECURE_MEMSET( rgptPlaintext, 0, sizeof( rgptPlaintext ) );
    ...
    __in           DRM_DWORD     f_cbBigCtx )
{
    ...
    */
    ChkDR( Convert_P256_DigitsToBigEndianBytesImpl( rgdPriv, f_pPrivKey->m_rgbPrivkey ) );
...
ErrorExit:
  ...
  **
  ** Synopsis:  Generates a public-private key pair for the P256 FIPS Elliptic Curve using Nvidia
    ...
    __in           DRM_DWORD     f_cbBigCtx)
{
    ...
    ** Create the private key such that it is any random number < curve order
    */
    ChkBOOL(random_mod_nonzero(g_Ecurve.gorder, rgdPriv,
                               ECC_P256_INTEGER_SIZE_IN_DIGITS, f_pBigCtx),
            DRM_E_P256_PKCRYPTO_FAILURE);

    /*
    ** Compute the Public key ( privKey * G )
    ** We will use the pre-Montgomery conversion parameters directly, rather than declare a new curve.
    */
    if (seECPointMult((NvU32 *)g_Ecurve.fdesc->modulo->modulus, (NvU32 *)&g_rgdConstA_P256_PreM[0],
                      (NvU32 *)&g_rgecConstG_P256_PreM[0], (NvU32 *)&g_rgecConstG_P256_PreM[8],
                               rgdPriv, rgdR, rgdS, SE_ECC_KEY_SIZE_256) != SE_OK)
    {
    ...
    ** Just to be safe lets make sure the pubkey is on the curve
    */
    if (seECPointVerification((NvU32 *)g_Ecurve.fdesc->modulo->modulus, (NvU32 *)&g_rgdConstA_P256_PreM[0],
                              (NvU32 *)&g_rgdConstB_P256_PreM[0], rgdR, rgdS, SE_ECC_KEY_SIZE_256) != SE_OK)
    {
        dr = DRM_E_SE_CRYPTO_POINT_NOT_ON_CURVE;
        goto ErrorExit;
    }
    ...
    */
    ChkDR(Convert_P256_DigitsToBigEndianBytesImpl(rgdPriv, f_pPrivKey->m_rgbPrivkey));
    ChkDR(Convert_P256_DigitsToBigEndianBytesImpl(rgdR, f_pPubKey->m_rgbPubkey));
    ChkDR(Convert_P256_DigitsToBigEndianBytesImpl(rgdS, &f_pPubKey->m_rgbPubkey[ECC_P256_INTEGER_SIZE_IN_BYTES]));
    ...
    __in           DRM_DWORD     f_cbBigCtx )
{
    ...
    */
    ChkDR( Convert_P256_DigitsToBigEndianBytesImpl( rgdPriv, f_pPrivKey->m_rgbPrivkey ) );
    ChkDR( Convert_P256_PointToBigEndianBytes( rgptPub, &g_Ecurve, f_pPubKey->m_rgbPubkey ) );
    ...
    __in                                 DRM_DWORD       f_cbBigCtx )
{
    ...
    */
    ...
    digit_t     rgdRandNumInverse[ ECC_P256_INTEGER_SIZE_IN_DIGITS ];
    ...
    */
    ...
    ChkArg( NULL !=  f_pBigCtx );
    OEM_SECURE_MEMSET( f_pSignature, 0, sizeof( *f_pSignature ) );
    ...
    */
    ...
    ChkDR( Convert_P256_BigEndianBytesToDigitsModOrderImpl( f_rgbHash,
                                                           &g_Ecurve,
                                                            rgdMessageHash ) );
    /*
    ...
    */
    for( dwNumTry = 0; dwNumTry < 1000; ++dwNumTry )
    {
        ...
        */
        ChkDR( Convert_P256_ModularIntToDigitsModOrderImpl( rgptRandPoint,
                                                       &g_Ecurve,
                                                        rgdSigElement ) );
        ...
        */
        ...
        ChkDR( Convert_P256_DigitsToBigEndianBytesImpl( rgdSigElement, f_pSignature->m_rgbSignature ) );
        ...
        */
        ChkDR( Convert_P256_BigEndianBytesToDigitsImpl( f_pPrivkey->m_rgbPrivkey, rgdRandNum ) );
  ...
  .      ** Calculate k*s and zero the priv key afterwards regardless of success or failure
        ...
        */
        ...
        OEM_SECURE_ZERO_MEMORY( rgdRandNum, sizeof( rgdRandNum ) );
        ...
        */
        ...
        /*
        ** Calculate x + (k*s MOD q) MOD q, where x is the message hash
        */
    ...
    */
    ChkDR( Convert_P256_DigitsToBigEndianBytesImpl( rgdSigElement,
...
ErrorExit:
    ...
    */
    OEM_SECURE_DIGITZEROMEMORY( rgdRandNum, DRM_NO_OF(rgdRandNum) );
    OEM_SECURE_DIGITZEROMEMORY( rgdRandNumInverse, DRM_NO_OF(rgdRandNumInverse) );
    OEM_SECURE_DIGITZEROMEMORY( rgdIntermediateVal1, DRM_NO_OF(rgdIntermediateVal1) );
    OEM_SECURE_DIGITZEROMEMORY( rgdIntermediateVal2, DRM_NO_OF(rgdIntermediateVal2) );
...
**
**  Synopsis: Creates an ECDSA P256 signature for a hash of a message, given a private keys.
**            The function uses HW accelerated crypto functions from Nvidia's Security Engine
**
**  Arguments:
**   IN   [f_rgbHash]:       Hash to create signature on
**   IN   [f_cbHash]:        Hash length in bytes
    ...
    __in                                 DRM_DWORD       f_cbBigCtx)
{
    ...
    */
    ...
    /*
    ** Convert msg hash to required form
    */
    ChkDR(Convert_P256_BigEndianBytesToDigitsModOrderImpl(f_rgbHash, &g_Ecurve, rgdMessageHash));

    /*
    ** Convert priv key to form needed for HW crypto functions
    */
    ChkDR(Convert_P256_BigEndianBytesToDigitsImpl(f_pPrivkey->m_rgbPrivkey, privKeyToDigits));
    ...
    ** We will use the pre-Montgomery conversion parameters directly, rather than declare a new curve.
    */
    if (seECDSASignHash((NvU32 *)g_Ecurve.fdesc->modulo->modulus, (NvU32 *)g_Ecurve.gorder,
                        (NvU32 *)&g_rgdConstA_P256_PreM[0],       (NvU32 *)&g_rgdConstB_P256_PreM[0],
                        (NvU32 *)&g_rgecConstG_P256_PreM[0],      (NvU32 *)&g_rgecConstG_P256_PreM[8],
                                  rgdMessageHash,                           privKeyToDigits,
                        (NvU32)   ECDSA_RETRY_LOOP,                         rgdSigElement0,
    ...
    */
    ChkDR(Convert_P256_DigitsToBigEndianBytesImpl(rgdSigElement0, f_pSignature->m_rgbSignature));
    ChkDR(Convert_P256_DigitsToBigEndianBytesImpl(rgdSigElement1, &f_pSignature->m_rgbSignature[ECC_P256_INTEGER_SIZE_IN_BYTES]));
    ...
    __in                                 DRM_DWORD       f_cbBigCtx )
{
    ...
    */
    DRM_RESULT  dr            = DRM_SUCCESS;
    OEM_SHA256_DIGEST  shaDigest;
    OEM_SHA256_CONTEXT shaData;
    ...
    ** Hash the message
    */
    ChkDR( OEM_SHA256_Init( &shaData ) );
    ChkDR( OEM_SHA256_Update( &shaData,
                               f_rgbMessage,
                               f_cbMessageLen ) );
    ChkDR( OEM_SHA256_Finalize( &shaData, &shaDigest ) );
