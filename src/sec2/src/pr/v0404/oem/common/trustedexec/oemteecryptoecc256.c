...
{
    DRM_BYTE rgbOpaque[BIGCTX_T_SIZE_FOR_ECC_ONLY];
    ...
    __inout                                               DRM_CRYPTO_CONTEXT_FOR_ECC_ONLY **f_ppCryptoCtx ) /*should not be that. */
{
    ...
    ChkDR( OEM_TEE_BASE_SelwreMemAlloc( f_pContextAllowNULL, sizeof( *pCryptoCtx ), (DRM_VOID**)&pCryptoCtx ) );

    DRMCASSERT( sizeof(DRM_CRYPTO_CONTEXT_FOR_ECC_ONLY) > sizeof(DRMBIGNUM_CONTEXT_STRUCT) );
    ( (DRMBIGNUM_CONTEXT_STRUCT *)pCryptoCtx->rgbOpaque )->oContext.pOEMTEEContext = f_pContextAllowNULL;
    ...
    __out_ecount( 1 )                                  OEM_TEE_KEY                  *f_pSigningKey )
{
    ...
    digit_t       rgdPrivKeyOriginal[ECC_P256_INTEGER_SIZE_IN_DIGITS];
    digit_t       rgdPrivKeyDoubled[ECC_P256_INTEGER_SIZE_IN_DIGITS];

    OEM_TEE_MEMCPY( f_pSigningKey->pKey, f_pEccKey->pKey, ECC_P256_INTEGER_SIZE_IN_BYTES );

    pPrivKey = OEM_TEE_INTERNAL_COLWERT_OEM_TEE_KEY_ECC256_TO_PRIVKEY_P256( f_pSigningKey );
    
    ChkDR( Colwert_BigEndianBytesToDigits( pPrivKey->m_rgbPrivkey, rgdPrivKeyOriginal, ECC_P256_INTEGER_SIZE_IN_BITS ) );
    ...
    ChkBOOL( add_mod(
        ...
        rgdPrivKeyDoubled, 
        g_Elwrve.gorder,
        ECC_P256_INTEGER_SIZE_IN_DIGITS ), DRM_E_P256_ECDSA_SIGNING_ERROR );

    ChkDR( Colwert_P256_DigitsToBigEndianBytes( rgdPrivKeyDoubled, pPrivKey->m_rgbPrivkey ) );

    f_pSigningKey->pKey = OEM_TEE_INTERNAL_COLWERT_ECC256_PRIVKEY_P256_TO_OEM_TEE_KEY( pPrivKey );
