...
**
** This function is used as an extension of OEM_TEE_BASE_GetVesion
**
** Synopsis:
**
** This function returns the version of your TEE firmware.
...
**  1. Return version information regarding the TEE.
**
** Arguments:
**
** f_pContext:           (in/out) The TEE context returned from
**                                OEM_TEE_BASE_AllocTEEContext.
** f_pcbVersion:         (out)    Number of bytes in the extended OEM Version information
** f_ppbVersion:         (out)    Values for the extended OEM Version information.
**                                This MUST be allocated using OEM_TEE_BASE_SelwreMemAlloc
**                                with a number of bytes equal to f_pcbVersion.
**                                Then, all bytes in this value MUST be set to yield any additional 
                                  version information you wish to provide to the REE.
...
**                                    This string MUST be allocated using OEM_TEE_BASE_SelwreMemAlloc
...
**                                    This MUST be set to DRM_TEE_PROPERTY_ARRAY_SIZE.
** f_ppbProperties:          (out)    Values from the DRM_TEE_PROPERTY enum.
**                                    This MUST be allocated using OEM_TEE_BASE_SelwreMemAlloc
**                                    with a number of bytes equal to DRM_TEE_PROPERTY_ARRAY_SIZE.
**                                    Then, this MUST then be zero-initialized.
**                                    Finally, any properties from the DRM_TEE_PROPERTY enum
**                                    should be set by calling DRM_TEE_PROPERTY_SET_VALUE.
**                                    This will be freed by the caller using OEM_TEE_BASE_SelwreMemFree.
**
** Returns:
**
    ...
    __deref_opt_out_bcount( *f_pcbProperties )            DRM_BYTE                **f_ppbProperties )
{
    ...
    */
    for( idx = 0; idx < DRM_METHOD_ID_DRM_TEE_Count - DRM_METHOD_ID_START_OF_OEM_FUNCTION_MAPPED_IDS; idx++ )
    {
        ...
        /* This if block would make DRM_TEE_AES128CTR_DecryptContentMultiple use structured serialization. */
        if (f_pdwFunctionMap[idx * 2] == DRM_METHOD_ID_DRM_TEE_AES128CTR_DecryptContentMultiple)
        {
            f_pdwFunctionMap[idx * 2 + 1] |= DRM_TEE_PROXY_BIT_USE_STRUCT_PARAMS;
        }

        /* This if block would make DRM_TEE_AES128CBC_DecryptContentMultiple use structured serialization. */
        if (f_pdwFunctionMap[idx * 2] == DRM_METHOD_ID_DRM_TEE_AES128CBC_DecryptContentMultiple)
        {
            f_pdwFunctionMap[idx * 2 + 1] |= DRM_TEE_PROXY_BIT_USE_STRUCT_PARAMS;
        }
    }
    ...
    *f_ppwszModelNumber = pwszModelNumber;
    ...
    if( f_pcbProperties != NULL && f_ppbProperties != NULL )
    {
