...
** supports Local Provisioning (DRM_SUPPORT_LOCAL_PROVISIONING=1) and you use
...
**
** If the device certificate is generated from the model certificate (default),
** then this function can just return DRM_E_NOTIMPL.
**
** This function is only used if the client supports Local Provisioning.
** Certain PlayReady clients may not need to include
** this function.
**
** Synopsis:
**
...
** f_pcbDeviceCertTemplate:            (in/out) On input, the size of the output device
**                                              certificate template buffer (f_pbDeviceCertTemplate).
**                                              On output, the size of the device certificate
**                                              chain template.
** f_pbDeviceCertTemplate:             (out)    Optional. If this value is NULL the size of the
**                                              device certificate chain template will be returned
**                                              via parameter f_pcbDeviceCertTemplate.  Otherwise,
**                                              the device certificate template will be copied into
**                                              this buffer.
