...
#ifndef SINGLE_METHOD_ID_REPLAY
    DRM_BOOL         bInLsMode;

    //
    // Make sure LWDEC is in LSMODE
    // This code is skipped for prMods profiles as the check will fail.
    //
...
**
** This function is only used if the client supports playback of encrypted
** content and you need to be given policy info from the license before
** DRM_TEE_DECRYPT_PrepareToDecrypt is called AND/OR content decryption is
...
** DRM_TEE_DECRYPT_PreparePolicyInfo, and the output blob should be
** the OEM-specific policy blob it requires before DRM_TEE_DECRYPT_PrepareToDecrypt.
...
** This means that the output blob will be returned from DRM_TEE_DECRYPT_PreparePolicyInfo.
...
**
** Otherwise, this function is being called by DRM_TEE_DECRYPT_PrepareToDecrypt
...
** This means that the output blob will be returned from DRM_TEE_DECRYPT_PrepareToDecrypt.
** In this case, protection and security of the key info lies entirely within your implementation.
** This scenario will mean that the DRM_TEE_*_Decrypt* functions should never be called
** because the SCP will be performing decryption.
** This scenario will only occur if DRM_TEE_DECRYPT_PrepareToDecrypt was passed an output
...
** requesting that a second copy of the CDKB be returned by DRM_TEE_DECRYPT_PrepareToDecrypt.
**
...
** OEM_TEE_DECRYPT_ParseAndVerifyExternalPolicyCryptoInfo will use this OEM-specific handle blob
...
**    If this option is set, OEM_TEE_DECRYPT_BuildExternalPolicyCryptoInfo will be initially
...
**    is validated by calling OEM_TEE_DECRYPT_EnforcePolicy and then the content decryption
...
**    information by calling OEM_TEE_DECRYPT_BuildExternalPolicyCryptoInfo with all the
...
**    will be accessed later via the function OEM_TEE_DECRYPT_ParseAndVerifyExternalPolicyCryptoInfo.
**
** 4. Process H.264 Slice Headers
**    During playback, it may be required to decrypt and process the H.264 slice headers. During
**    this process, a call will be made to OEM_TEE_DECRYPT_ParseAndVerifyExternalPolicyCryptoInfo
**    to retrieve  content decryption information required to decrypt the content. The parameters
**    returned from OEM_TEE_DECRYPT_ParseAndVerifyExternalPolicyCryptoInfo must match the input
**    parameters to OEM_TEE_DECRYPT_BuildExternalPolicyCryptoInfo exactly.  If your implementation
**    of OEM_TEE_DECRYPT_ParseAndVerifyExternalPolicyCryptoInfo returns DRM_E_NOTIMPL, then the
**    CDKB will be processed and its data will be used for decryption.
**
** 5. Content Decryption
**    During content decryption, a call will be made to OEM_TEE_DECRYPT_ParseAndVerifyExternalPolicyCryptoInfo
**    to retrieve content decryption information required to decrypt the content. The parameters
**    returned from OEM_TEE_DECRYPT_ParseAndVerifyExternalPolicyCryptoInfo must match the input
**    parameters to OEM_TEE_DECRYPT_BuildExternalPolicyCryptoInfo exactly.  If your implementation
**    of OEM_TEE_DECRYPT_ParseAndVerifyExternalPolicyCryptoInfo returns DRM_E_NOTIMPL, then the
...
** f_pidSession                 (in) The session ID.
** f_pCDKB                      (in) The CDKB from which the content key,
**                                   sample protection key, and policy was obtained.
**                                   Optional.  Only present when this function is
**                                   called from DRM_TEE_DECRYPT_CreateOEMBlobFromCDKB.
...
**                                   Optional.  Only present when this function is
**                                   called from DRM_TEE_DECRYPT_CreateOEMBlobFromCDKB.
** f_cbOEMData:                 (in) The size of the initialization data passed to
**                                   DRM_TEE_DECRYPT_CreateOEMBlobFromCDKB.
**                                   Optional.  Only present when this function is
**                                   called from DRM_TEE_DECRYPT_CreateOEMBlobFromCDKB
**                                   and when data was passed to that function.
** f_pbOEMData:                 (in) The initialization data passed to
**                                   DRM_TEE_DECRYPT_CreateOEMBlobFromCDKB.
**                                   Optional.  Only present when this function is
**                                   called from DRM_TEE_DECRYPT_CreateOEMBlobFromCDKB
**                                   and when data was passed to that function.
** f_pPolicy:                   (in) The policy structure obtained from either directly
**                                   from the license or indirectly via the CDKB.
...
**                                   This MUST be allocated using OEM_TEE_BASE_SelwreMemAlloc.
    ...
    __deref_out_bcount( *f_pcbInfo )                DRM_BYTE                  **f_ppbInfo )
{
    ...
    if (f_pPolicy->dwDecryptionMode == OEM_TEE_DECRYPTION_MODE_SAMPLE_PROTECTION)
    {
    ...
    pCDKBCryptoData->eSymmetricEncryptionType = f_pPolicy->eSymmetricEncryptionType;
...
** OEM_TEE_DECRYPT_BuildExternalPolicyCryptoInfo.  This function must return the
...
** the OEM_TEE_DECRYPT_BuildExternalPolicyCryptoInfo function. If you return
** DRM_E_NOTIMPL for this function, the CDKB will be used to decrypt content and
** provide policy information.
**
** This function is used by all decryption and H.264 slice header parser to
...
**    OEM_TEE_DECRYPT_BuildExternalPolicyCryptoInfo).
...
**                                   This MUST be allocated using OEM_TEE_BASE_SelwreMemAlloc.
**                                   This will be freed by the caller using OEM_TEE_BASE_SelwreMemFree.
**                                   This allocation MUST include any restrictions
**                                   and restriction configuration data as described in
**                                   comments surrounding the DRM_TEE_POLICY_INFO and
**                                   DRM_TEE_POLICY_RESTRICTION structures in drmteetypes.h.
...
**
** This function is only used if the client supports playback of encrypted content.
** Certain PlayReady clients may not need to implement this function.
**
** Synopsis:
**
** This function ensures that only supported decryption modes are used for content decryption.
**
** Operations Performed:
**
...
OEM_TEE_DECRYPT_RemapRequestedDecryptionModeToSupportedDecryptionMode(
    __inout                                         OEM_TEE_CONTEXT            *f_pContext,
    __inout                                         DRM_DWORD                  *f_pdwDecryptionMode )
{
    ...
    if( *f_pdwDecryptionMode != OEM_TEE_DECRYPTION_MODE_NOT_SELWRE
     && *f_pdwDecryptionMode != OEM_TEE_DECRYPTION_MODE_HANDLE
     && *f_pdwDecryptionMode != OEM_TEE_DECRYPTION_MODE_SAMPLE_PROTECTION )
    {
        *f_pdwDecryptionMode = OEM_TEE_DECRYPTION_MODE_HANDLE;
    }
}
    ...
    __in                                     const DRM_TEE_POLICY_INFO              *f_pPolicy )
{
    ...
    const DRM_BYTE                  *pbRestriction                = NULL;
    DRM_TEE_POLICY_RESTRICTION       oRestriction                 = { 0 };
    ...
    AssertChkArg(f_ePolicyCallingAPI == DRM_TEE_POLICY_INFO_CALLING_API_DECRYPT_PREPARETODECRYPT);
    ...
    pbRestriction = DRM_REINTERPRET_CONST_CAST(const DRM_BYTE, f_pPolicy) + sizeof(*f_pPolicy);
    for ( i = 0; i < f_pPolicy->cRestrictions; i++ )
    {
        // Find the next restriction.
        const DRM_BYTE *pbRestrictionConfiguration = NULL;
        DRM_DWORD       cbRestrictionConfiguration = 0;
        ...
        // Copy to a local variable because pbRestriction may be an unaligned pointer.
        ChkVOID(OEM_TEE_MEMCPY(&oRestriction, pbRestriction, sizeof(oRestriction)));
        cbRestrictionConfiguration = oRestriction.cbRestrictionConfiguration;
        pbRestrictionConfiguration = pbRestriction + sizeof(oRestriction);

        //
        // At this point, you should act on the restriction described by local variable oRestriction.
        // The configuration data is found in locals cbRestrictionConfiguration and pbRestrictionConfiguration.
        // Enforce the restriction as per PlayReady Compliance rules.
        //
        if ( OEM_SELWRE_ARE_EQUAL(&g_guidHDCPTypeRestriction, oRestriction.idRestrictionType.rgb, sizeof(DRM_GUID)) )
        {
            // HDCP type 1 restriction. Do some error checking first.

            // restriction category should be digital video.
            if ( DRM_XMRFORMAT_ID_EXPLICIT_DIGITAL_VIDEO_OUTPUT_PROTECTION_CONTAINER != oRestriction.wRestrictionCategory )
            {
                ChkDR( DRM_E_ILWALID_LICENSE );
            }
            ...
            if ( sizeof(DRM_DWORD) != cbRestrictionConfiguration )
            {
                ChkDR( DRM_E_ILWALID_LICENSE );
            }

            // Extract the BCD value and colwert to little endian
            DRM_DWORD bcdValue = *((DRM_DWORD*)(pbRestrictionConfiguration));
        ...
        else if ( OEM_SELWRE_ARE_EQUAL(&g_guidMaxResDecode, oRestriction.idRestrictionType.rgb, sizeof(DRM_GUID)) )
        {
            // restriction category should be digital video.
            if ( DRM_XMRFORMAT_ID_EXPLICIT_DIGITAL_VIDEO_OUTPUT_PROTECTION_CONTAINER != oRestriction.wRestrictionCategory )
            {
            ...
            // Size of the BCD has to be 64 bits i.e. 2 DWORDs
            if (2 * sizeof(DRM_DWORD) != cbRestrictionConfiguration)
            {
                ChkDR(DRM_E_ILWALID_LICENSE);
            }

            //
            // kwilson - MS has reconfirmed the PR3.0 stance that g_guidMaxResDecode is not a hardware backed
            //           policy and handling in UM (user mode) layers is fine. We will continue to return
            //           DRM_SUCCESS as we did for PR3.0
            //
        }

        else if ( OEM_SELWRE_ARE_EQUAL(&g_guidDigitalOnlyToken, oRestriction.idRestrictionType.rgb, sizeof(DRM_GUID)) )
        {
            // restriction category should be analog video.
            if ( DRM_XMRFORMAT_ID_EXPLICIT_ANALOG_VIDEO_OUTPUT_PROTECTION_CONTAINER != oRestriction.wRestrictionCategory )
            {
                ChkDR( DRM_E_ILWALID_LICENSE );
            }
        ...
        else if ( OEM_SELWRE_ARE_EQUAL(&g_guidVGA_MaxRes, oRestriction.idRestrictionType.rgb, sizeof(DRM_GUID))      ||
                  OEM_SELWRE_ARE_EQUAL(&g_guidBestEffortCGMSA, oRestriction.idRestrictionType.rgb, sizeof(DRM_GUID)) ||
                  OEM_SELWRE_ARE_EQUAL(&g_guidCGMSA, oRestriction.idRestrictionType.rgb, sizeof(DRM_GUID))           ||
                  OEM_SELWRE_ARE_EQUAL(&g_guidAGC, oRestriction.idRestrictionType.rgb, sizeof(DRM_GUID))             ||
                  OEM_SELWRE_ARE_EQUAL(&g_guidComponent_MaxRes, oRestriction.idRestrictionType.rgb, sizeof(DRM_GUID)) )
        {
            // restriction category should be analog video.
            if ( DRM_XMRFORMAT_ID_EXPLICIT_ANALOG_VIDEO_OUTPUT_PROTECTION_CONTAINER != oRestriction.wRestrictionCategory )
            {
            ...
            //
            // Error checks done. Lets take this as a valid license. We do not support analog outputs
            // so return DRM_E_TEE_OUTPUT_PROTECTION_REQUIREMENTS_NOT_MET.
            //
        ...
        {
            //
            // oRestriction.wRestrictionCategory is one of 3 values:
            ...
            //
            // LWPU TEE design cannot enforce restrictions/policies on audio output, hence we are
            // ignoring all the audio output requirements/restrictions as dislwssed with Microsoft.
            //
            // If this restriction/policy is not a digital audio policy/restriction, then it will
            // be either be a digital video or analog video policy/restriction that we are not aware
            // of and are not checking for. In this case, it is considered an unknown policy and we
            // will return an error.
            //
            // Check explicitly for the audio case and return success if this is an audio
            // restriction - otherwise this is an unknown digital or analog restriction.
            //
			if ( DRM_XMRFORMAT_ID_EXPLICIT_DIGITAL_AUDIO_OUTPUT_PROTECTION_CONTAINER == oRestriction.wRestrictionCategory )
			{
				dr = DRM_SUCCESS;
			}
			else
         ...
         // Move on to the next restriction.
         pbRestriction += oRestriction.cbThis;
    }
