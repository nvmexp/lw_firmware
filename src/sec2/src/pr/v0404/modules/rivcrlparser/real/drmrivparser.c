    ...
    __inout                          DRM_DWORD          *f_pidxRevInfo )
{
    ...
    *f_pidxRevInfo += sizeof( DRM_DWORD );      /* Can't overflow: *f_pidxRevInfo is iniitalized to zero and all adds are fixed size */
    ...
    *f_pidxRevInfo += 1;        /* Can't overflow: *f_pidxRevInfo is iniitalized to zero and all adds are fixed size */
    ...
    *f_pidxRevInfo += sizeof( DRM_DWORD );      /* Can't overflow: *f_pidxRevInfo is iniitalized to zero and all adds are fixed size */
    ...
    else if( RLVI_MAGIC_NUM_V2 == f_pRLVI->head.dwID )
    {
        ...
        *f_pidxRevInfo += sizeof( DRM_UINT64 );     /* Can't overflow: *f_pidxRevInfo is iniitalized to zero and all adds are fixed size */
    }
    else
    {
    ...
    __out_ecount_opt(1)              DRM_DWORD          *f_pcbSignedBytes )
{
    ...
    dwRevInfo += cbEntries;     /* Overflow cannot happen because "cbEntries + 1 + 4 + dwRevinfo" did not overflow. */
    ...
    dwRevInfo += sizeof( DRM_BYTE );  /* Overflow cannot happen because "cbEntries + 1 + 4 + dwRevinfo" did not overflow. */
    ...
    __out                               DRM_RLVI               *f_pRLVI )
{
    ...
    ChkDR( DRM_BCrl_VerifySignature( f_pvOemCtx,
                                     ...
                                     f_pbRevInfo,
                                     cbSignedBytes,
                                     f_pbRevInfo + f_pRLVI->signature.ibSignature,  /* Can't overflow: DRM_RVK_ParseRevocationInfo would have failed */
                                     f_pRLVI->signature.cbSignature,
                                     f_pbRevInfo + f_pRLVI->certchain.ibCertChain,  /* Can't overflow: DRM_RVK_ParseRevocationInfo would have failed */
