...
static DRM_GLOBAL_CONST DRM_ID g_idKeyDerivationCertificatePrivateKeysWrap PR_ATTR_DATA_OVLY(_g_idKeyDerivationCertificatePrivateKeysWrap) = {{ 0x9c, 0xe9, 0x34, 0x32, 0xc7, 0xd7, 0x40, 0x16, 0xba, 0x68, 0x47, 0x63, 0xf8, 0x01, 0xe1, 0x36 }};
static DRM_GLOBAL_CONST DRM_TEE_KEY_TYPE    g_rgeKeyTypes[] PR_ATTR_DATA_OVLY(_g_rgeKeyTypes) = { DRM_TEE_KEY_TYPE_PR_ECC_PRIVKEY_SIGN, DRM_TEE_KEY_TYPE_PR_ECC_PRIVKEY_ENCRYPT };
    ...
    __out_ecount( f_cECCKeys )      PUBKEY_P256                 *f_pPubKeys )
{
    ...
    DRMASSERT( f_cECCKeys <= DRM_NO_OF( g_rgeKeyTypes ) );
    ...
    {
        ChkDR( DRM_TEE_IMPL_BASE_AllocKeyECC256( pOemTeeCtx, g_rgeKeyTypes[ iKey ], &f_pECCKeys[ iKey ] ) );
...
static DRM_RESULT DRM_CALL _GenerateCertHash(
    ...
    __inout_bcount( sizeof(OEM_SHA256_DIGEST) ) DRM_BYTE          *f_pDigest )
{
    DRM_RESULT         dr           = DRM_SUCCESS;
    OEM_SHA256_CONTEXT SHAContext;  /* Initialized by OEM_TEE_BASE_SHA256_Init     */
    OEM_SHA256_DIGEST  oDigest;     /* Initialized by OEM_TEE_BASE_SHA256_Finalize */
    ...
    __inout_bcount( sizeof(SIGNATURE_P256) )    DRM_BYTE          *f_pSignature )
{
    ...
    ChkVOID( OEM_TEE_ZERO_MEMORY( &oSignature, sizeof( oSignature ) ) );
    ...
    /* We cannot use f_pSignature in OEM_TEE_BASE_ECDSA_P256_SignData because it may not be aligned */
...
**  4. If a previous PPKB was not given, generate a new Secure Store (SST) AES key.
**  5. Generate a new PlayReady Device Certificate based on data inside the
...
**  6. Append the new certificate to the given model certificate chain.
**  7. Via an OEM API, sign the new PlayReady Device Certificate using the
...
**  8. Build and sign a PPKB.  Include the SST key and all Previous TEE Keys
...
**  9. Return the new PPKB and (now complete) PlayReady Device Certificate Chain.
**
    ...
    __out                       DRM_TEE_BYTE_BLOB            *f_pNewCertificate )
{
   ...
   // DRM_DWORD                          cKeys               = 0;
    ...
    ChkVOID( OEM_TEE_ZERO_MEMORY( &oCertIndexes, sizeof( oCertIndexes ) ) );
    ...
    lwrtosENTER_CRITICAL();
    {
        ...
        ChkDR(DRM_TEE_IMPL_BASE_AllocBlob(f_pContext, DRM_TEE_BLOB_ALLOC_BEHAVIOR_WEAKREF, cbCertTemp, pbCertTemp, &oDevCertTemplate));
        ...
        DRMCASSERT( sizeof( SIGNATURE_P256 ) >= sizeof( PUBKEY_P256 ) );
        DRMCASSERT( sizeof( SIGNATURE_P256 ) >= sizeof( DRM_ID ) );
        DRMCASSERT( sizeof( SIGNATURE_P256 ) >= sizeof( OEM_SHA256_DIGEST ) );
        AssertChkBOOL( oDevCertTemplate.cb > sizeof( SIGNATURE_P256 ) );
        ChkDR( DRM_TEE_IMPL_BASE_AllocateKeyArray( f_pContext, DRM_NO_OF( g_rgeKeyTypes ), &pKeys ) );

        ChkDR( OEM_TEE_BASE_SelwreMemAlloc( pOemTeeCtx, DRM_NO_OF( g_rgeKeyTypes ) * sizeof(PUBKEY_P256), (DRM_VOID**)&pPubKeys ) );     
        ChkDR( _GenerateNewECCKeys( f_pContext, DRM_NO_OF( g_rgeKeyTypes ), pKeys, pPubKeys ) );

        /* Generate a unique Identifier for this certificate. */
        AssertChkBOOL( oCertIndexes.ibCertID <= ( oDevCertTemplate.cb - sizeof( DRM_ID ) ) );                                   /* Can't underflow: already guaranteed that cb >= sizeof( DRM_ID ) */
        ChkDR( OEM_TEE_BASE_GenerateRandomBytes( pOemTeeCtx, sizeof(DRM_ID), (DRM_BYTE *)&oDevCertTemplate.pb[oCertIndexes.ibCertID] ) );
        {
            ...
            ChkDR( DRM_TEE_IMPL_BASE_GetAppSpecificHWID( pOemTeeCtx, f_pidApplication, &oCertClientID ) );
            AssertChkBOOL( oCertIndexes.ibCertClientID <= ( oDevCertTemplate.cb - sizeof( oCertClientID ) ) );                  /* Can't underflow: already guaranteed that cb >= sizeof( DRM_ID ) */
            ChkVOID( OEM_TEE_MEMCPY_IDX( oDevCertTemplate.pb, oCertIndexes.ibCertClientID, &oCertClientID, 0, sizeof(oCertClientID) ) );
        }
        AssertChkBOOL(
            oCertIndexes.rgibPubKeys[ DRM_TEE_LPROV_PUBKEY_SIGN_IDX ] > 0
         && oCertIndexes.rgibPubKeys[ DRM_TEE_LPROV_PUBKEY_SIGN_IDX ] <= ( oDevCertTemplate.cb - sizeof( *pPubKeys ) ) );       /* Can't underflow: already guaranteed that cb >= sizeof( PUBKEY_P256 ) */
        ...
        AssertChkBOOL(
            oCertIndexes.rgibPubKeys[ DRM_TEE_LPROV_PUBKEY_ENCRYPT_IDX ] > 0
         && oCertIndexes.rgibPubKeys[ DRM_TEE_LPROV_PUBKEY_ENCRYPT_IDX ] <= ( oDevCertTemplate.cb - sizeof( *pPubKeys ) ) );    /* Can't underflow: already guaranteed that cb >= sizeof( PUBKEY_P256 ) */
        ...
        ** Update the certificate digest with a SHA256 hashing the public key.
        */
        AssertChkBOOL( oCertIndexes.ibCertDigest <= ( oDevCertTemplate.cb - sizeof( OEM_SHA256_DIGEST ) ) );    /* Can't underflow: already guaranteed that cb >= sizeof( OEM_SHA256_DIGEST ) */
        ...
        AssertChkBOOL( oCertIndexes.ibSignData <= oDevCertTemplate.cb );
        AssertChkBOOL( oCertIndexes.cbSignData <= ( oDevCertTemplate.cb - sizeof( SIGNATURE_P256 ) ) );     /* Can't underflow: already guaranteed that cb >= sizeof( SIGNATURE_P256 ) */
        AssertChkBOOL( oCertIndexes.ibSignature <= ( oDevCertTemplate.cb - sizeof( SIGNATURE_P256 ) ) );    /* Can't underflow: already guaranteed that cb >= sizeof( SIGNATURE_P256 )*/
...
/* #define _TEST_LPROV_CERT_GEN_ 1 */
    ...
    ChkDR( DRM_BCERTFORMAT_VerifyCertificateChain(
        ...
        oDevCertTemplate.pb ) );
#endif
        ChkDR( DRM_TEE_IMPL_KB_FinalizePPKB( f_pContext, DRM_NO_OF( g_rgeKeyTypes ), pKeys, f_pPrevPPKB, f_pPPKB ) );
        ChkVOID(DRM_TEE_IMPL_BASE_TransferBlobOwnership(f_pNewCertificate, &oDevCertTemplate));

   ErrorExit:
      
        ChkVOID( DRM_TEE_IMPL_BASE_FreeKeyArray( f_pContext, DRM_NO_OF( g_rgeKeyTypes ), &pKeys ) );
        ...
        //
        drTmp = DRM_TEE_IMPL_BASE_FreeBlob(f_pContext, &oDevCertTemplate);
        if (dr == DRM_SUCCESS)
        {
            dr = drTmp;
        }
