...
#include <drmteexbformat_generated.c>
...
static DRM_GLOBAL_CONST DRM_DWORD c_iSignatureFromObjectStart = XB_BASE_OBJECT_LENGTH + sizeof(DRM_WORD) + sizeof(DRM_WORD);
    ...
    __in_opt                        const DRM_BOOL                                      *f_pfPersist )
{
    ...
    ChkDR( DRM_DWordPtrAdd( (DRM_SIZE_T)f_pbKB, iSignature, (DRM_SIZE_T*)&pbSig ) );

    ChkDR( DRM_TEE_IMPL_BASE_DeriveTKDFromTK( f_pContext, NULL, f_eKBType, DRM_TEE_XB_KB_OPERATION_SIGN, f_pfPersist, &oTKD ) );
    ...
    __out_opt                                       DRM_DWORD                                    *f_pdwidTK )
{
    ...
    DRM_BYTE                        rgbUnused[ sizeof( DRM_SIZE_T ) ];
    ...
    */
    ...
    switch( f_eKBType )
    {
        case DRM_TEE_XB_KB_TYPE_LKB:
            ...
            {
            case sizeof( DRM_TEE_KB_LKBData ) :
            {
            ...
            case sizeof( DRM_TEE_KB_LKBData2 ) :
            {
                if( ( (const DRM_TEE_KB_LKBData2 *)pbCryptoDataWeakRef )->fPersist )
                {
                    *f_pdwidTK = ( (const DRM_TEE_KB_LKBData2 *)pbCryptoDataWeakRef )->dwidTK;
                    fPersistentSignature = TRUE;
                }
                break;
            }
        ...
        case DRM_TEE_XB_KB_TYPE_DKB:
            ...
            *f_pdwidTK = ( ( const DRM_TEE_KB_DKBData * )pbCryptoDataWeakRef )->dwidTK;
            break;
        case DRM_TEE_XB_KB_TYPE_SSKB:
            fPersistentSignature = TRUE;
            ChkArg( cbCryptoDataWeakRef == sizeof( DRM_TEE_KB_SSKBData ) );
            *f_pdwidTK = ( (const DRM_TEE_KB_SSKBData *)pbCryptoDataWeakRef )->dwidTK;
            break;
        ...
        case DRM_TEE_XB_KB_TYPE_NTKB : __fallthrough;
    ...
    ChkDR( DRM_TEE_IMPL_BASE_AllocKeyAES128( pOemTeeCtx, DRM_TEE_KEY_TYPE_TK, &oTKToFree ) );

    ChkDR( OEM_TEE_BASE_GetTKByID( pOemTeeCtx, *f_pdwidTK, &oTKToFree.oKey ) );

    ChkDR( DRM_TEE_IMPL_BASE_DeriveTKDFromTK( f_pContext, &oTKToFree, f_eKBType, DRM_TEE_XB_KB_OPERATION_SIGN, &fPersistentSignature, &oTKD ) );
    dr = OEM_TEE_BASE_VerifyOMAC1Signature(
        pOemTeeCtx,
        &oTKD.oKey,
        cbKBWithoutSignature,
        f_pbKB,
        XBBA_TO_CB( oxbKB.Signature.xbbaSignature ),
    ...
    __out                                                 DRM_TEE_BYTE_BLOB          *f_pKB )
{
    ...
    DRM_XB_BUILDER_CONTEXT       oXBBuilder;          /* OEM_TEE_ZERO_MEMORY */
    ...
    ChkVOID( OEM_TEE_ZERO_MEMORY( &oXBBuilder, sizeof( oXBBuilder ) ) );
    ...
    if( DRM_TEE_XB_KB_TYPE_LKB == f_eKBType )
    {
        ...
        switch( f_cbKBCryptoData )
        {
        case sizeof( DRM_TEE_KB_LKBData ) :
            fPersistentSignature = ( (DRM_TEE_KB_LKBData *)f_pbKBCryptoData )->fPersist;
            break;
        case sizeof( DRM_TEE_KB_LKBData2 ) :
            fPersistentSignature = ( (DRM_TEE_KB_LKBData2 *)f_pbKBCryptoData )->fPersist;
            break;
        default:
            ChkArgError();
        }
    ...
    {
        fPersistentSignature = DRM_TEE_IMPL_BASE_IsKBTypePersistedToDisk( f_eKBType );
    }
    ...
    ChkDR( DRM_TEE_IMPL_BASE_AllocBlobAndTakeOwnership( f_pContext, cbKB, &pbKB, f_pKB ) );
...
DRM_NO_INLINE DRM_API DRM_RESULT DRM_CALL DRM_TEE_IMPL_KB_ParseAndVerify(
    ...
    __deref_out_bcount( *f_pcbCryptoDataWeakRef )         DRM_BYTE                  **f_ppbCryptoDataWeakRef )
{
    return DRM_TEE_IMPL_KB_ParseAndVerifyWithReconstitution(
        f_pContext,
        f_pKB,
        f_eType,
        NULL,
