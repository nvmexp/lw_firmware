...
PREFAST_PUSH_DISABLE_EXPLAINED( __WARNING_WRITABLE_GLOBAL_FUNCTION_POINTER_25143, "Writable global function pointer is acceptable in DBG-only code." )
void( *g_pfDebugAnalyzeDROrig )( unsigned long, const char*, unsigned long, const char* ) = NULL;
PREFAST_POP /* __WARNING_WRITABLE_GLOBAL_FUNCTION_POINTER_25143 */
...
PREFAST_PUSH_DISABLE_EXPLAINED( __WARNING_POTENTIAL_RANGE_POSTCONDITION_VIOLATION_26071, "PREFAST thinks this function doesn't actually return the string length" )
PREFAST_PUSH_DISABLE_EXPLAINED( __WARNING_REQUIREMENT_NOT_SATISFIED_EXPR_NOT_TRUE_28196, "PREFAST thinks this function doesn't actually return the string length" )
static _Post_equal_to_( _String_length_( f_psz ) ) DRM_DWORD DRM_CALL _GetStrLen( __in_z const DRM_CHAR *f_psz )
{
    DRM_DWORD cch = 0;
    while( *f_psz != '\0' )
    {
...
PREFAST_POP  /* __WARNING_POTENTIAL_RANGE_POSTCONDITION_VIOLATION_26071 */

static DRM_RESULT DRM_CALL _BeginMessage( __in DRM_DWORD f_cch )
{
...
static DRM_VOID DRM_CALL _EndMessage( DRM_VOID )
{
    ...
    // is not being compiled into Lwpu TEE.
    //     
    (DRM_VOID)OEM_TEE_BASE_CRITSEC_Leave();
}

static DRM_VOID DRM_CALL _LogStrImpl( __in_ecount_z( f_cch ) const DRM_CHAR *f_pszToLog, __in DRM_DWORD f_cch )
{
    DRMASSERT( g_ichDebugCirlwlarLog < sizeof( g_rgbDebugCirlwlarLog ) );
    if( f_pszToLog != NULL )
    {
        ...
        */
        ...
        if( g_ichDebugCirlwlarLog < sizeof( g_rgbDebugCirlwlarLog ) )
        {
            ...
            while( ich < sizeof( g_rgbDebugCirlwlarLog ) && g_rgbDebugCirlwlarLog[ ich ] != '\0' )
            {
...
DRM_API_VOID DRM_VOID DRM_CALL DRM_TEE_IMPL_BASE_TRACE_LogStr( __in_z const DRM_CHAR *f_pszToLog )
{
    ...
    if( DRM_SUCCEEDED( _BeginMessage( cch ) ) )
    {
...
{
    if( DRM_SUCCEEDED( _BeginMessage( DRM_TEE_DEBUG_TRACE_CHARS_PER_NUMBER ) ) )
    {
...
{
    DRM_DWORD cch = 0;
    if( DRM_SUCCEEDED( _BeginMessage( _LogDecImpl( f_dwToLog, FALSE ) ) ) )
    {
...
static void _DebugAnalyzeDRFunc( unsigned long f_ulDR, const char *f_pszFile, unsigned long f_ulLine, const char *f_pszExpr )
{
    ...
    default:
        ...
        if( DRM_SUCCEEDED( ( _BeginMessage( 12 + cchFile + _LogDecImpl( (DRM_DWORD)f_ulLine, FALSE ) + cchExpr + DRM_TEE_DEBUG_TRACE_CHARS_PER_NUMBER ) ) ) )
        {
...
DRM_NO_INLINE DRM_API_VOID DRM_VOID DRM_CALL DRM_TEE_IMPL_BASE_DEBUG_DeInit( DRM_VOID )
{
    ...
    // is not being compiled into Lwpu TEE.
    //     
    (DRM_VOID)OEM_TEE_BASE_CRITSEC_Enter();
    ...
    __out                       DRM_TEE_BYTE_BLOB            *f_pLog )
{
...

    {
        ...
        if (DRM_SUCCEEDED(drRet = OEM_TEE_BASE_CRITSEC_Enter()))
        {
            drRet = DRM_TEE_IMPL_BASE_AllocBlob(
                f_pContext,
            ...
            if( DRM_SUCCEEDED( drRet ) )
            {
                ...
                ChkVOID( DRM_TEE_IMPL_BASE_TransferBlobOwnership( f_pLog, &oLog ) );
                *f_pdwLastHR = (DRM_DWORD)g_drLast;

                ChkVOID( DRM_TEE_IMPL_BASE_FreeBlob( f_pContext, &oLog ) );
            }
