...
typedef struct _tagXMBContext
{
    /*
    ** Usage of buffer:
    ** This context buffer is allocated by user at CreateDolwment(). Its size  can be reallocated
    ** via ReallocDolwment(). The XML string is built on the way when each of the API calls are
    ...
    */
    ...
    DRM_BOOL fInited;
    DRM_BOOL fIsSimMode;        /* TRUE to create a faked doc to callwlate the length needed */
...
/**********************************************************************
** Function:    DRM_XMB_CloseDolwment
** Synopsis:    Pop the opened nodes and close the document, if space allowed.
** Arguments:
** Returns:     DRM_SUCCESS on success
** Notes:
***********************************************************************/
DRM_API DRM_RESULT DRM_CALL DRM_XMB_CloseDolwment(
    __inout _XMBContext *pbXmlContext,
    __out   DRM_STRING  *pdstrXML   /* if opened with DRM_XMB_SimulateCreateDolwment, this contains the counts of WCHAR's needed for the real string */
    );
...
DRM_API DRM_RESULT DRM_CALL DRM_XMB_CloseLwrrNode(
    __inout   _XMBContext *pbXmlContext,
    __out_opt DRM_STRING  *pdstrXMLFragment );

DRM_API DRM_RESULT DRM_CALL DRM_XMB_GetLwrrNodeName(
...
**
** purpose: callwlates the number of XML characters required for
