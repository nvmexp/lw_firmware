...
enum XB_OBJECT_TYPE
{
    ...
    XB_OBJECT_TYPE_ILWALID                                          = DRM_MAX_UNSIGNED_TYPE(DRM_WORD)  , /* 0xFFFF = Invalid */
...
** a DWORD followed by cGUIDs conselwtive 16 byte GUID values.
*/
...
** a DWORD followed by cWORDs conselwtive WORD values.
*/
...
** a DWORD followed by cDWORDs conselwtive DWORD values.
*/
...
** a DWORD followed by cQWORDs conselwtive QWORD values.
*/
...
**  a DWORD followed by cbData conselwtive bytes or just cbData bytes if the
...
typedef struct __tagDRM_XB_ELEMENT_DESCRIPTION
{
    ...
    DRM_WORD             wOffsetInLwrrentStruct;    /* Where in the containing struct the element should be serialized to/from */
...
typedef struct __tagDRM_XB_ENTRY_DESCRIPTION
{
    ...
    DRM_WORD  wOffsetInLwrrentStruct;               /* Where in the containing structure should the data be serialized from/to */
...
*/
#define DRM_XB_EXTENDED_HEADER_SIZE_OFFSET_ILWALID ((DRM_WORD)0xFFFF)
...
DRM_NO_INLINE DRM_RESULT DRM_CALL _DRM_XB_AlignData(
    ...
    __out_opt _Post_satisfies_( f_pcbPadding == NULL || *f_pcbPadding == *f_piBuffer - _Old_( *f_piBuffer ) )   DRM_DWORD          *f_pcbPadding )
