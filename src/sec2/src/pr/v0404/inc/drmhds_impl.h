...
#define DRM_E_HDSILWALIDSTORE     DRM_E_ILWALID_LICENSE_STORE
...
typedef struct _tag_ChildBlock
{
    ...
    /* callwlated during init'd */
    DRM_DWORD nLwrrSlotPos;
...
typedef struct _tag_SlotContext
{
    ...
    _CommBlockHDR *pLwrrChildBlock; /* pointer of the _ChildBlockHDR containing the Slot, pointing somewhere in bBuff */
    ...
    DRM_DWORD dwSeekPointer;      /* the current position in Slot where next slot read/write oclwrs */
...
typedef struct _tag_EnumContext
{
    ...
    DRM_BOOL fLwrrBlockIsValid;
    _CommBlockHDR oLwrrBlock;  /* this field must be located at end of structure */
} _EnumContext;
...
typedef struct _tag_BlockScanContext
{
    ...
    DRM_DWORD                iKeyLwrrent; /* index of next unmarked KID in the array */
    DRM_BOOL                 fLwrrBlockIsValid;
    _ChildBlockHDR           blockheaderLwrrent;  /* this field must be located at end of structure */
} _BlockScanContext;
