...
DRM_NO_INLINE DRM_API DRM_RESULT DRM_CALL OEM_TEE_BASE_CRITSEC_Initialize( DRM_VOID )
{
    ...
    {
        Oem_CritSec_Initialize( &s_DrmTeeCritSec );
...
DRM_NO_INLINE DRM_API_VOID DRM_VOID DRM_CALL OEM_TEE_BASE_CRITSEC_Uninitialize( DRM_VOID )
{
    ...
    if( s_fDrmTeeCritSecInit )
    {
        Oem_CritSec_Delete( &s_DrmTeeCritSec );
...
**
** LWE (kwilson) Change the return type to DRM_RESULT because we can fail to release
** the mutex
*/
