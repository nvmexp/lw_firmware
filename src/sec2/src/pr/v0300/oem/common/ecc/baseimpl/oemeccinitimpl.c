...
DRM_NO_INLINE DRM_API_VOID DRM_VOID DRM_CALL bignum_free(DRM_VOID *pvMem, struct bigctx_t *f_pBigCtx)
{
    if ( pvMem != NULL &&  f_pBigCtx != NULL )
    {
        (DRM_VOID)DRM_STK_Free( (DRM_STACK_ALLOCATOR_CONTEXT*) f_pBigCtx, pvMem );
    }
}
    ...
    struct bigctx_t *f_pBigCtx )
{
    ...
    ** Colwert to proper structure and get pOEMContext.
    ...
    */
    ...
    if ( pBigNumCtx != NULL )
    {
        ...
        ** the normal world OEM or the TEE version of the underlying function will be ilwoked based on linkage.
        **
