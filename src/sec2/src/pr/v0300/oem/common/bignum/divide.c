...
*/
    ...
    const digit_t qestcomp
        = DRM_RADIXM1 - estimated_quotient_1(n1, n0, 0, recip);
                                             /* RADIX - 1 - qest */
         ...
         __in_ecount(lden)      const digit_t         *denom,
                                ...
                                const reciprocal_1_t  *supplied_reciprocal,
                                   /* reciprocal_t struct for this denominator,
                                      ...
                                      digit_t         *quot,     /* Quotient -- length DRM_MAX(lnum - lden + 1, 0) */
    ...
    if (OK) {
        ...
        if (dlead == 0) {
            ...
            if (used_reciprocal == NULL) {
                if( !divide_precondition_1(denom, lden,
                                      &computed_reciprocal) )
                {
                    return FALSE;
                }
...
*/
        ...
        OK = OK && div21(DRM_UI64HL(DRM_RADIXM1 - dshiftedhi,
                                     DRM_RADIXM1 - dshiftedlo),
...
*/
            for (iden = lden; (iden--) != 0 && rem < recip_multiplier;/*null*/){
                ...
                DRM_UINT64  test1 = DRM_UI64HL(rem,
                                      DRM_RADIXM1 - (iden > 0 ? denom[iden-1] : 0));
