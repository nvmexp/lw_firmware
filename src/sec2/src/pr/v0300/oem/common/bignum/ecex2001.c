...
#else
        //
        // LWE (nkuo) - As per Microsoft, we do not expect a function pointer here. Use falc_halt() to detect this.
        // We need to revisit this in production ucode and may want to return error to the caller to fail the methodID.
        //
        falc_halt(); 
#endif
    }
...
*/
        ...
        if (w == 2) {
            OEM_SELWRE_DIGITTCPY( Presult,P0,2*E->fdesc->elng );
            ...
            if (w == 3) {
                OEM_SELWRE_DIGITTCPY( Presult,p0location(3),2*E->fdesc->elng);
                                                           // P0 and 3*P0 saved
            } else {
...
// combining the ilwersions
    ...
    while (OK && delay) {
        ...
        const digit_t expon_partial3 = (const digit_t)( expon_partial - doublings3pwr );
                                         /* May be negative */
