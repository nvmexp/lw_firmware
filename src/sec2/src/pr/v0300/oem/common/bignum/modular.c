...
/*
        File modular.c.  Version 28 November 2002
                ...
                from_modular(a, b, modulo) --
                        The number a (mod modulo) is colwerted to
...
*/
{
    ...
    digit_t *dtemps_longterm
        = digit_allocate(3*lnga + lngred2, pbigctxGlobal);
    digit_t *dtemps_shortterm
        = digit_allocate(2*lnga + 2*lngred2 + 1, f_pBigCtx);
    ...
    if (dtemps_shortterm == NULL || dtemps_longterm == NULL) {
        ...
        pmodulo->right_reciprocal_1 = mod0ilw;
...
*/
                ...
                mptmp1[lnga] =
                    multiply_immediate(pmodulo->modulus, mod0ilw,
                                       mptmp1, lnga);
...
*/
{
    ...
    if (a != b) OEM_SELWRE_DIGITTCPY( b,a,lng);

    OK = OK && validate_modular_data(a, modulo->modulus, lng);

    if (OK && shift_remaining < 0 && DRM_IS_EVEN(modulo->modulus[0])) {
        OK = FALSE;
        SetMpErrno_clue(MP_ERRNO_ILWALID_DATA,
    ...
    while (OK && shift_remaining > 0) {          /* Do left shift, if any */
        const DRM_LONG shift_now = DRM_MIN(shift_remaining, DRM_RADIX_BITS);
                                /* 1 <= shift_now <= DRM_RADIX_BITS */
/*
...
*/
        ...
        if (OK) {
            ...
            carry_out -= delwmulate(modulo->modulus, qest, b,lng);
    ...
    while (OK && shift_remaining < 0) {
        ...
        const digit_t mul =   (0-modulo->right_reciprocal_1 * b[0])
                       & (DRM_RADIXM1 >> (DRM_RADIX_BITS - shift_now) );
