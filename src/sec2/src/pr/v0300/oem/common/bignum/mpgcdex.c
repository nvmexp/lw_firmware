...
{
    return 8*DRM_MAX(lnga, lngb) + 6;
} // mp_gcdex_ntemps
/****************************************************************************/
DRM_NO_INLINE DRM_DWORD DRM_CALL mp_ilwert_ntemps(const DRM_DWORD lng)
/* Return temporary count needed by mp_ilwert */
{
    return lng + mp_gcdex_ntemps(lng, lng);
} // mp_ilwert_ntemps
/****************************************************************************/
DRM_NO_INLINE DRM_API DRM_BOOL DRM_CALL mp_gcdex
        (const digit_t    *a,             // IN  (nonzero)
         ...
         const DRM_DWORD   lngb,          // IN
               digit_t    *ailwmodb,      // OUT
               digit_t    *bilwmoda,      // OUT (unless NULL)
               digit_t    *gcd,           // OUT
...
*/
    ...
    */
            (DRM_VOID)sub_same(temps_mul[1-igcd],
                     ailwmodb, temps_mul[1-igcd], lngb);
...
*/
{
    ...
    if (    lead0h == 0
         ...
         || ((lead0h | lead1h) & DRM_RADIX_HALF) == 0) {  // Neither normalized
             OK = FALSE;
             SetMpErrno_clue(MP_ERRNO_INTERNAL_ERROR,
...
DRM_NO_INLINE DRM_API DRM_BOOL DRM_CALL mp_ilwert
        (const digit_t    *denom,          // IN
         ...
         const DRM_CHAR   *caller,         // IN
               digit_t    *supplied_temps, // TEMPS -- length mp_ilwert_ntemps(lng)
         struct bigctx_t  *f_pBigCtx)
/*
...
*/
{
    ...
    if (!OK) {
        ...
        DRMASSERT(   gcdex_temps + mp_gcdex_ntemps(lng, lng)
        ...
        if (!OK) {
            ...
            TRACE( ( "Using data from %s, mp_ilwert got nontrivial GCD\n", caller ) );
        }
    }
