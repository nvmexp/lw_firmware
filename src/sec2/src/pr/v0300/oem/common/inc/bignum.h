...
typedef enum {
        ...
        MP_ERRNO_NOT_ILWERTIBLE,   /* Perhaps trying to ilwert modulo non-prime */
...
DRM_API digit_t DRM_CALL delwmulate( __in_ecount(lng) const digit_t *, __in const digit_t, __inout_ecount(lng) digit_t *, __in const DRM_DWORD lng);
...
DRM_API DRM_BOOL DRM_CALL mp_gcdex
       (const digit_t * a, const DRM_DWORD lnga,
        ...
        digit_t * ailwmodb, digit_t *bilwmoda,
...
DRM_EXTERN_INLINE DRM_DWORD DRM_CALL mp_ilwert_ntemps(const DRM_DWORD);
           /* Temporary count required by last argument to mp_ilwert */

DRM_API DRM_BOOL DRM_CALL mp_ilwert(const digit_t *, const digit_t *, const DRM_DWORD,
...
DRM_API DRM_BOOL DRM_CALL two_adic_ilwerse(const digit_t, digit_t*);
