...
typedef struct {
          //
          // LWE (nkuo) - Per MSFT's signoff, we are using exact functions instead of
          ...
          DRM_BOOL        (DRM_CALL *ilwerter1)   (const digit_t *, digit_t *,
                                             const struct field_desc_t*,
...
//
#define Kadd(f1, f2, f3, fdesc, ctx)   \
        Kprime_adder(      (f1), (f2),     (f3), 1, (fdesc), ctx)
#define Kequal(f1, f2,   fdesc, ctx)   \
        Kequaler_default(  (f1), (f2),           1, (fdesc), ctx)

#define Kiszero(f1,      fdesc, ctx)   \
        Kiszeroer_default( (f1),                 1, (fdesc), ctx)
#define Kmulpower2(f1, ishift, f3, fdesc, ctx)\
        Kprime_mulpower2er((f1), (ishift), (f3), 1, (fdesc), ctx)
#define Knegate(f1,  f3, fdesc, ctx)   \
        Kprime_negater(    (f1),           (f3), 1, (fdesc), ctx)
#define Ksqrt(f1,    f3, fdesc, is_square, ctx)  \
        Ksqrt_many(        (f1),           (f3), 1, (fdesc), (is_square), ctx)
#define Ksub(f1, f2, f3, fdesc, ctx)   \
        Kprime_subtracter( (f1), (f2),     (f3), 1, (fdesc), ctx)
#endif
...
/* Next items apply only for extension fields*/
          const struct field_desc_t *base_field;  /* Relwrsive data type*/
...
/* Next items apply only if ftype = FIELD_2_POLYNOMIAL or extension field.*/
          ...
          digit_t      *leading_ilwerse;
                     /* FLOOR(X^(2*degree-1) / minimal_polynomial)*/
                     /* N.B. Either leading_ilwerse = NULL or freduc = NULL.*/
...
/*     leading_ilwerse has signed bytes colwerting these low coefficients*/
...
DRM_API DRM_BOOL DRM_CALL Kinitialize_prime(const mp_modulus_t*, field_desc_t*,
                                     struct bigctx_t *,
                                     struct bigctx_t *f_pBigCtx);
DRM_API DRM_BOOL DRM_CALL Kilwert(const digit_t *, digit_t *,
                           const field_desc_t*, digit_t *, struct bigctx_t *f_pBigCtx);
DRM_API DRM_BOOL DRM_CALL Kilwert_many(const digit_t *, digit_t *,
