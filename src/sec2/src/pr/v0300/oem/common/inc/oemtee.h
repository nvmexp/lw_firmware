...
** PRND TX Operations
*/
DRM_NO_INLINE DRM_API DRM_RESULT DRM_CALL OEM_TEE_PRND_TX_CheckReceiverSelwrityLevel(
    __inout                                         OEM_TEE_CONTEXT            *f_pContext,
    __in                                            DRM_DWORD                   f_dwReceiverSelwrityLevel );
...
DRM_NO_INLINE DRM_API_VOID DRM_VOID DRM_CALL OEM_TEE_BASE_CRITSEC_Uninitialize( DRM_VOID );

//
// LWE (kwilson)  In Microsoft PK code, OEM_TEE_BASE_CRITSEC_Enter/Leave return DRM_VOID.
// Lwpu had to change the return values to DRM_RESULT in order to support a return
// code from acquiring/releasing the critical section, which may not always succeed.
//
