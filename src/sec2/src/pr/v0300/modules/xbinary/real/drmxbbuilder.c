    ...
    __inout_ecount( 1 )   DRM_DWORD                      *f_pcbLength )
{
    ...
    if( f_pElement->pElementDescription != NULL )
    {
        ...
        ChkDR( DRM_DWordPtrAdd( (DRM_DWORD_PTR)f_pvBaseObject, f_pElement->wOffsetInLwrrentStruct, ( DRM_DWORD_PTR * )&ppvStruct ) );
        pvStruct = *ppvStruct;
        ...
        for( iStruct = 0; iStruct < cStructs; iStruct++ )
        {
            ...
            ChkDR( DRM_DWordPtrAdd( (DRM_DWORD_PTR)pvStruct, f_pElement->xbSize.wMinSize, (DRM_DWORD_PTR*)&pvStruct) );
        }
...

{
    ...
    for( idx = 0; idx < f_pStructList->cElementDescription; idx++ )
    {
        ...
        if( f_pvObject != NULL )
        {
            ChkDR( DRM_DWordPtrAdd( (DRM_DWORD_PTR)f_pvObject, pElems[idx].wOffsetInLwrrentStruct, ( DRM_DWORD_PTR * )&pvElement ) );
        }
    ...
    __inout_ecount( 1 )          DRM_DWORD      *f_piBuffer )
{
    ...
    switch( f_pxbSize->eSizeType )
    {
        case XB_SIZE_IS_RELATIVE_WORD:
            ...
            ChkDR( DRM_DWordAddSame( &iBuffer, sizeof( DRM_WORD ) ) );
            break;
    ...
    __inout_ecount( 1 )          DRM_DWORD   *f_piBuffer )
{
    ...
    ChkDR( DRM_DWordPtrAdd( (DRM_DWORD_PTR)pqwordlist->pqwordBuffer, pqwordlist->iQwords, (DRM_DWORD_PTR*)&pbResult ) );
    
    for( iQWords = 0; iQWords < pqwordlist->cQWORDs; iQWords++ )
    {
        ChkDR( _DRM_XB_Serialize_QWORD( pbResult, f_pbBuffer, f_cbBuffer, &iBuffer ) );

        ChkDR( DRM_DWordPtrAddSame( ( DRM_DWORD_PTR * )&pbResult, DRM_SIZEOFQWORD ) );
    }
    ...
    __inout_ecount( 1 )          DRM_DWORD                          *f_piBuffer )
{
    ...
    ChkDR( DRM_DWordPtrAdd( (DRM_DWORD_PTR)f_pvBaseObject, f_pElement->wOffsetInLwrrentStruct, (DRM_DWORD_PTR*)&ppvObject ) );
    ...
    if( f_pElement->pElementDescription != NULL )
    {
        ...
        for( iStruct = 0; iStruct < cStructs; iStruct++ )
        {
            ...
            ChkDR( DRM_DWordPtrAdd( (DRM_DWORD_PTR)pvObject, f_pElement->xbSize.wMinSize, (DRM_DWORD_PTR*)&pvObject) );
        }        
    }
    else
    {
...
*/
static DRM_RESULT DRM_CALL _InsertNodeInChildListSorted(
    __in_ecount( 1 )    const DRM_XB_BUILDER_CONTEXT_INTERNAL    *f_pcontextBuilder,
...
******************************************************************************/
GCC_ATTRIB_NO_STACK_PROTECT()
static DRM_RESULT DRM_CALL _XB_AddHierarchy(
    ...
    __in_ecount( 1 ) const DRM_VOID                  *f_pvObject )
{
    ...
    for( iEntry = 0; iEntry < pcontextBuilder->pformat->cEntryDescriptions; iEntry++ )
    {
        ...
        if( pContainerEntry->wParent == f_wObjectType && pContainerEntry->wType != XB_OBJECT_TYPE_ROOT )
        {
            if( pContainerEntry->wType == XB_OBJECT_TYPE_UNKNOWN )
            {
                ...
                ChkDR( DRM_DWordPtrAdd( (DRM_DWORD_PTR)f_pvObject, pContainerEntry->wOffsetInLwrrentStruct, (DRM_DWORD_PTR*)&ppUnk ) );
            ...
            else if( pContainerEntry->wType == XB_OBJECT_TYPE_UNKNOWN_CONTAINER )
            {
                ...
                ChkDR( DRM_DWordPtrAdd( (DRM_DWORD_PTR)f_pvObject, pContainerEntry->wOffsetInLwrrentStruct, (DRM_DWORD_PTR*)&pUnkContainer ) );
            ...
            else if( pContainerEntry->wType   != XB_OBJECT_TYPE_ROOT )
            {
                ...
                */
                if( pContainerEntry->fDuplicateAllowed )
                {
                    ...
                    ChkDR( DRM_DWordPtrAdd( (DRM_DWORD_PTR)f_pvObject, pContainerEntry->wOffsetInLwrrentStruct, (DRM_DWORD_PTR*)&pvResult ) );
                ...
                else
                {
                    ...
                    ChkDR( DRM_DWordPtrAdd( (DRM_DWORD_PTR)f_pvObject, pContainerEntry->wOffsetInLwrrentStruct, (DRM_DWORD_PTR*)&pObject) );
    ...
    ** would be added with the _XB_AddHierarchy function.  However, the XMR builder allows empty containers to indicate behavior
...
/******************************************************************************
**
** Function :   _DRM_XB_AddUnknownHierarchy
**
** Synopsis :   Adds all the ancestors of a node, and builds the correct
**              hierarchy
**
...
**
******************************************************************************/
DRM_RESULT DRM_CALL  DRM_XB_AddUnknownHierarchy (
    __inout_ecount( 1 )  DRM_XB_BUILDER_CONTEXT_INTERNAL        *f_pcontextBuilder,
...

{
    ...
    {
        /*
        ** All known objects are deferred to _XB_AddHierarchy
        */
        ChkDR( _XB_AddHierarchy( f_pcontextBuilder, &( plistnodeChild->listNode ) ) );
    }
    else
    {
    ...
    __in_bcount_opt( f_cbObject ) const DRM_BYTE                *f_pbObject )
{
    ...
    ** Certain objects which are not marked external parse can also be so added.
    */
...
ErrorExit:
    ...
    {
        ChkVOID( DRM_STK_Free( pcontextBuilder->pcontextStack, ( DRM_VOID * )plistnode ) );
    }
    return dr;
}
