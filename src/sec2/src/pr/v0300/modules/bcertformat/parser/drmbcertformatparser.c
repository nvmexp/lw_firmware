...
static const PUBKEY_P256 g_ECC256MSPlayReadyRootIssuerPubKeyBCERT PR_ATTR_DATA_OVLY(_g_ECC256MSPlayReadyRootIssuerPubKeyBCERT) = {DRM_ECC256_MS_PLAYREADY_ROOT_ISSUER_PUBKEY};

#define DRM_BCERTFORMAT_VERIFY_SELWRITY_VERSION2( pParserCtx, pCert, pChailwer )                                            \
    do {                                                                                                                    \
        if( (pCert)->SelwrityVersion2.fValid )                                                                              \
        {                                                                                                                   \
            DRM_BCERTFORMAT_CHKVERIFICATIONERR(                                                                             \
                (pParserCtx),                                                                                               \
                (pCert)->SelwrityVersion2.dwPlatformID == (pChailwer)->dwPlatformID,                                        \
                DRM_E_BCERT_ILWALID_SELWRITY_VERSION );                                                                     \
                                                                                                                            \
            DRM_BCERTFORMAT_CHKVERIFICATIONERR(                                                                             \
                (pParserCtx),                                                                                               \
                (pCert)->SelwrityVersion2.dwSelwrityVersion == (pChailwer)->dwSelwrityVersion,                              \
                DRM_E_BCERT_ILWALID_SELWRITY_VERSION );                                                                     \
        }                                                                                                                   \
    } while(FALSE)


#define DRM_BCERTFORMAT_VERIFY_SELWRITY_VERSION( pParserCtx, pCert, pChailwer )                                             \
    do {                                                                                                                    \
        if( (pCert)->SelwrityVersion.fValid )                                                                               \
        {                                                                                                                   \
            DRM_BCERTFORMAT_CHKVERIFICATIONERR(                                                                             \
                (pParserCtx),                                                                                               \
                (pCert)->SelwrityVersion.dwPlatformID == (pChailwer)->dwPlatformID,                                         \
                DRM_E_BCERT_ILWALID_PLATFORM_IDENTIFIER );                                                                  \
                                                                                                                            \
            DRM_BCERTFORMAT_CHKVERIFICATIONERR(                                                                             \
                (pParserCtx),                                                                                               \
                (pCert)->SelwrityVersion.dwSelwrityVersion == (pChailwer)->dwSelwrityVersion,                               \
                DRM_E_BCERT_ILWALID_PLATFORM_IDENTIFIER );                                                                  \
        }                                                                                                                   \
    } while(FALSE)

#define DRM_BCERTFORMAT_INIT_CHAIN_SELWRITY_VERSION2( pChailwer, pCert )                                                    \
    do {                                                                                                                    \
        *(pChailwer)                = (pCert)->SelwrityVersion2;                                                            \
    } while( FALSE )

#define DRM_BCERTFORMAT_INIT_CHAIN_SELWRITY_VERSION( pChailwer, pCert )                                                     \
    do {                                                                                                                    \
    ...
    __inout                  DRM_BCERTFORMAT_SELWRITY_VERSION2          *f_poChainSelwrityVersion )
{
    ...
    */
    ...
    /*
    ** Only the child needs to be validated since chain security version is first set by the parent.
    ** Do not use "else if" here because the preceding block can modify f_poChainSelwrityVersion->fValid.
    */
    if( f_poChainSelwrityVersion->fValid )
    {
        DRM_BCERTFORMAT_VERIFY_SELWRITY_VERSION2( f_pParserCtx, f_poChildCert, f_poChainSelwrityVersion );
        DRM_BCERTFORMAT_VERIFY_SELWRITY_VERSION( f_pParserCtx, f_poChildCert, f_poChainSelwrityVersion );
    }
    ...
    */
    ...
    {
        AssertChkArg( XBBA_TO_CB( f_poChildCert->SignatureInformation.xbbaIssuerKey ) >= DRM_BCERT_MAX_PUBKEY_VALUE_LENGTH );
        AssertChkArg( XBBA_TO_CB( pKeyType[idx].xbbaKeyValue )                        >= DRM_BCERT_MAX_PUBKEY_VALUE_LENGTH );
    ...
    __in_ecount_opt( 1 )              const PUBKEY_P256                           *f_pRootPubKey )
{
    ...
    {
        AssertChkArg( DRM_BCERTFORMAT_PARSER_CTX_IS_FULL( &pCtx->oBaseCtx ) );
    }
    ...
    __out                                   DRM_BCERTFORMAT_CERT                    *f_pCertificate )
{
    ...
    */
    if( DoExpirationCheck( pCtx->ftLwrrentTime ) )
    {
        ...
        ChkBOOL( !DRM_UI64Les( uiCertTime, ui64LwrrentTime ), DRM_E_BCERT_BASICINFO_CERT_EXPIRED );
    }
    ...
    */
    ...
    {
        DRM_RESULT drTmp = DRM_STR_UTF8_IsUTF8NullTerminatedData( XBBA_TO_PB( f_pCertificate->DomainInformation.xbbaDomainUrl ),
    ...
    */
    ...
    {
        DRM_RESULT drTmp = DRM_STR_UTF8_IsUTF8NullTerminatedData( XBBA_TO_PB( f_pCertificate->MeteringInformation.xbbaMeteringUrl ),
    ...
    */
    if( f_pCertificate->ManufacturerInformation.fValid )
    {
        ...
        if( f_pCertificate->ManufacturerInformation.xbbaManufacturerName.fValid && f_pCertificate->ManufacturerInformation.xbbaManufacturerName.cbData > 0 )
        {
            drTmp = DRM_STR_UTF8_IsUTF8NullTerminatedData( XBBA_TO_PB( f_pCertificate->ManufacturerInformation.xbbaManufacturerName ),
                                                           XBBA_TO_CB( f_pCertificate->ManufacturerInformation.xbbaManufacturerName ) );
            DRM_BCERTFORMAT_CHKVERIFICATIONERR( pCtx, DRM_SUCCEEDED( drTmp ), drTmp );
        }
        if( f_pCertificate->ManufacturerInformation.xbbaModelName.fValid && f_pCertificate->ManufacturerInformation.xbbaModelName.cbData > 0 )
        {
            drTmp = DRM_STR_UTF8_IsUTF8NullTerminatedData( XBBA_TO_PB( f_pCertificate->ManufacturerInformation.xbbaModelName ),
                                                           XBBA_TO_CB( f_pCertificate->ManufacturerInformation.xbbaModelName ) );
            DRM_BCERTFORMAT_CHKVERIFICATIONERR( pCtx, DRM_SUCCEEDED( drTmp ), drTmp );
        }
        if( f_pCertificate->ManufacturerInformation.xbbaModelNumber.fValid && f_pCertificate->ManufacturerInformation.xbbaModelNumber.cbData > 0 )
        {
            drTmp = DRM_STR_UTF8_IsUTF8NullTerminatedData( XBBA_TO_PB( f_pCertificate->ManufacturerInformation.xbbaModelNumber ),
    ...
    if( ( f_pCertificate->BasicInformation.dwFlags & DRM_BCERT_FLAGS_EXTDATA_PRESENT ) == DRM_BCERT_FLAGS_EXTDATA_PRESENT )
    {
        if( f_pCertificate->ExDataContainer.fValid )
        {
            if( f_pCertificate->ExDataContainer.ExDataSignatureInformation.fValid )
            {
                ...
                ChkDR( DRM_DWordPtrAdd( (DRM_DWORD_PTR)pbSignData, DRM_BCERT_OBJECT_HEADER_LEN, (DRM_DWORD_PTR *)&pbSignData ) );
    ...
    __out_ecount_opt( 1 )                   DRM_BCERTFORMAT_CERT                    *f_pCert )
{
    ...
    AssertChkArg( f_pCertificateHeader->dwIndex < DRM_NO_OF(pCtx->rgoCertCache) );
    ...
    if( f_pCert != NULL )
    {
        *f_pCert = pCtx->rgoCertCache[ f_pCertificateHeader->dwIndex ];
    }
...
//
const DRM_DWORD rgdwKeyUsages[5] PR_ATTR_DATA_OVLY(_rgdwKeyUsages) = {
...
const DRM_DWORD rgdwChainDataKeyIndex[5] PR_ATTR_DATA_OVLY(_rgdwChainDataKeyIndex) = {
    DRM_BCERTFORMAT_CHAIN_DATA_KEY__SIGN,
    DRM_BCERTFORMAT_CHAIN_DATA_KEY__ENCRYPT,
    DRM_BCERTFORMAT_CHAIN_DATA_KEY__PRND_ENCRYPT,
    ...
    __inout       DRM_BCERTFORMAT_PARSER_CHAIN_DATA *f_pChainData )
{
    ...
    for( idxKey = 0; idxKey < f_pCert->KeyInformation.cEntries; idxKey++ )
    {
        ...
        DRMCASSERT( DRM_NO_OF(rgdwKeyUsages) == DRM_NO_OF(rgdwChainDataKeyIndex) );

        for( idxUsage = 0; idxUsage < DRM_NO_OF(rgdwKeyUsages); idxUsage++ )
        {
            if( DRM_BCERT_IS_KEYUSAGE_SUPPORTED( pKeyType[idxKey].dwUsageSet, rgdwKeyUsages[idxUsage] ) )
            {
                ChkBOOL( !DRM_BCERTFORMAT_PARSER_CHAIN_DATA_IS_KEY_VALID( f_pChainData->dwValidKeyMask, rgdwChainDataKeyIndex[idxUsage] ), DRM_E_BCERT_ILWALID_KEY_USAGE );
                ChkBOOL( sizeof(PUBKEY_P256) == XBBA_TO_CB( pKeyType[idxKey].xbbaKeyValue ), DRM_E_BCERT_ILWALID_KEY_LENGTH );
                ChkVOID( OEM_SELWRE_MEMCPY( &f_pChainData->rgoLeafPubKeys[ rgdwChainDataKeyIndex[idxUsage] ], XBBA_TO_PB( pKeyType[idxKey].xbbaKeyValue ), sizeof(PUBKEY_P256) ) );
                f_pChainData->dwValidKeyMask |= DRM_BCERTFORMAT_PARSER_CHAIN_DATA_KEY_BIT( rgdwChainDataKeyIndex[idxUsage] );
            }
        }
    }
...
#define DRM_BCERTFORMAT_ASSIGN_OR_ZERO(lhs,rhs)         \
do {                                                    \
    if( (lhs) != NULL )                                 \
    {                                                   \
    ...
    __in      const DRM_BCERTFORMAT_CERT                *f_pCertificate )
{
    ...
    if( pRootPubKey != NULL )
    {
        ...
        ChkBOOL( OEM_SELWRE_ARE_EQUAL( XBBA_TO_PB( f_pCertificate->SignatureInformation.xbbaIssuerKey ),
                                       pRootPubKey,
                                       sizeof( *pRootPubKey ) ),
    ...
    __out_opt                             DRM_BCERTFORMAT_PARSER_CHAIN_DATA       *f_pChainData )
{
    ...
    */
    ...
    for( iCert = 0; iCert < f_pChain->Header.cCerts; iCert++ )
    {
        ...
        ChkDR( _DRM_BCERTFORMAT_ParseCertificate(
            f_pParserCtx,
            f_cbCertData - dwOffset,
            f_pbCertData + dwOffset, /* Can't overflow because we check that dwOffset < f_cbCertData */
            &cbParsed,
            &dwCertKeyUsageSet,
            pParentCert ) );
        ...
        */
        ChkDR( Oem_Broker_SHA256_CreateDigest(
            ...
            &oDigest ) );
        ChkBOOL( XBBA_TO_CB( pParentCert->BasicInformation.xbbaDigestValue ) == sizeof( oDigest )
              && XBBA_TO_PB( pParentCert->BasicInformation.xbbaDigestValue ) != NULL
              && 0 == MEMCMP( &oDigest, XBBA_TO_PB( pParentCert->BasicInformation.xbbaDigestValue ), sizeof(oDigest) ), DRM_E_BCERT_ILWALID_DIGEST );
        ...
        */
        if( iCert == (f_pChain->Header.cCerts - 1) )
        {
    ...
    */
    ...
    ChkBOOL( !DRM_BCERTFORMAT_PARSER_CTX_IS_FULL( pCtx ) || DRM_BCERTFORMAT_PARSER_CTX_TO_FULL( pCtx )->iResult == 0, DRM_E_BCERT_VERIFICATION_ERRORS );
...
{
    DRM_RESULT                     dr         = DRM_SUCCESS;
    DRM_BCERTFORMAT_PARSER_CONTEXT oParserCtx = {0};
    ...
    __out_ecount_opt( 1 )               DRM_DWORD                       *f_pdwCertKeyIndex )
{
    ...
    const DRM_BCERTFORMAT_PARSER_CONTEXT_FULL *pCtx                 = DRM_BCERTFORMAT_PARSER_CTX_TO_FULL( f_pParserCtx );
    ...
    */
    ...
    OEM_SELWRE_MEMCPY( f_pPubkey->m_rgbPubkey,
                       XBBA_TO_PB( pKeyInfo->pHead[ iFoundKey ].xbbaKeyValue ),
                       sizeof( PUBKEY_P256 ) );
    ...
    __inout_ecount( 1 )       PUBKEY_P256                       *f_pPubkey )
{
    ...
    OEM_SELWRE_MEMCPY( f_pPubkey->m_rgbPubkey,
                       XBBA_TO_PB( f_pCert->KeyInformation.pHead[ 0 ].xbbaKeyValue ),
                       sizeof( PUBKEY_P256 ) );
...
**              specified cert in a certificate chain.
**
** Arguments:   [f_pParserCtx]      : Optional. The BCERT format parser context.
**              [f_pOemTeeContext]  : The OEM TEE context (if available).
**              [f_cbCertData]      : The size in bytes of the certificate chain.
...
#ifdef NONE
/*****************************************************************************
** Function:    DRM_BCERTFORMAT_GetDigestValuesOutOfCertChain
**
** Synopsis:    Get digest values of all certs in a cert chain
**
** Arguments:   [f_pCertParserCtx]     : The BCERT format parser context.
**              [f_pCertChain]         : The certificate chain object created in DRM_BCERTFORMAT_ParseCertificateChain.
**              [f_pcDigestValues]     : The size of buffer
**              [f_pDigestValues]      : The buffer that will contain digest entries
**
** Returns:     DRM_SUCCESS          - on success
**              DRM_E_ILWALIDARG     - one of the arguments was NULL
**              DRM_E_BUFFERTOOSMALL - the digest buffer was not large enough
**
** Note: the values of f_pDigestValues are undefined if function fails.
**
******************************************************************************/
DRM_NO_INLINE DRM_API DRM_RESULT DRM_CALL DRM_BCERTFORMAT_GetDigestValuesOutOfCertChain(
    ...
    __out_ecount_opt( *f_pcDigestValues )           DRM_RevocationEntry                       *f_pDigestValues )
{
    DRM_RESULT           dr             = DRM_SUCCESS;
    DRM_DWORD            iCert          = 0;
    DRM_DWORD            cDigestValues  = 0;

    ChkArg( f_pCertParserCtx  != NULL );
    ChkArg( f_pCertChain      != NULL );
    ChkArg( f_pcDigestValues  != NULL );

    cDigestValues = f_pCertChain->Certificates.cEntries;

    if (*f_pcDigestValues < cDigestValues)
    {
        *f_pcDigestValues = cDigestValues;
        ChkDR( DRM_E_BUFFERTOOSMALL );
    }

    ChkArg( f_pDigestValues  != NULL );
    /*
    ** Navigate from the leaf most cert to the root to find all digest value
    */
    for( iCert = 0; iCert < cDigestValues; iCert++ )
    {
        DRM_BCERTFORMAT_CERT oCert = {0};
        ChkDR( DRM_BCERTFORMAT_GetCertificate(
            f_pCertParserCtx,
            &f_pCertChain->Certificates.pCertHeaders[iCert],
            NULL,
            &oCert ) );

        ChkBOOL( XBBA_TO_PB( oCert.BasicInformation.xbbaDigestValue ) != NULL
              && XBBA_TO_CB( oCert.BasicInformation.xbbaDigestValue ) >= sizeof(DRM_RevocationEntry), DRM_E_BCERT_ILWALID_DIGEST );

        ChkVOID( OEM_SELWRE_MEMCPY( &f_pDigestValues[iCert], XBBA_TO_PB( oCert.BasicInformation.xbbaDigestValue ), sizeof(DRM_RevocationEntry) ) );
    }
