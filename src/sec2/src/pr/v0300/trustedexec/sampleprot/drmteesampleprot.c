...
DRM_NO_INLINE DRM_API DRM_BOOL DRM_CALL DRM_TEE_SAMPLEPROT_IMPL_IsSAMPLEPROTSupported( DRM_VOID )
{
    return TRUE;
}
    ...
    __out                       DRM_TEE_BYTE_BLOB            *f_pEncryptedKey )
{
    ...
    CIPHERTEXT_P256                    oSPKeyEncrypted = {{0}};  // LWE (nkuo) - changed due to compile error "missing braces around initializer"
...

    {
        ...
        AssertChkArg( pChainData->dwLeafSelwrityLevel <= DRM_BCERT_SELWRITYLEVEL_2000 );
    ...
    ChkDR( DRM_TEE_KB_ParseAndVerifyRKB(
        f_pContext,
        f_pRKB,
        &oRKBCtx,
        pChainData->cDigests,
    ...
    ChkDR( DRM_TEE_BASE_IMPL_AllocKeyAES128( pOemTeeCtx, DRM_TEE_KEY_TYPE_SAMPLEPROT, &oSPKey ) );
    ...
    ChkDR( DRM_TEE_BASE_AllocBlob( f_pContext, DRM_TEE_BLOB_ALLOC_BEHAVIOR_COPY, ECC_P256_CIPHERTEXT_SIZE_IN_BYTES, &oSPKeyEncrypted.m_rgbCiphertext[0], &oSPKBEncrypted ) );

    ChkDR( DRM_TEE_KB_BuildSPKB(
        f_pContext,
        &oSPKey,
    ...
    ChkVOID( DRM_TEE_BASE_TransferBlobOwnership( f_pEncryptedKey, &oSPKBEncrypted ) );
    ChkVOID( DRM_TEE_BASE_TransferBlobOwnership( f_pSPKB, &oSPKB ) );
...
ErrorExit:
    ...
    //
    drTmp = DRM_TEE_BASE_FreeBlob( f_pContext, &oSPKBEncrypted );
    if (dr == DRM_SUCCESS)
    {
        dr = drTmp;
    }

    drTmp = DRM_TEE_BASE_FreeBlob( f_pContext, &oSPKB );
    if (dr == DRM_SUCCESS)
    {
        dr = drTmp;
    }
...
{
    if( f_dwMode == OEM_TEE_AES128CTR_DECRYPTION_MODE_HANDLE
     || f_dwMode == OEM_TEE_AES128CTR_DECRYPTION_MODE_SAMPLE_PROTECTION )
    {
        return TRUE;
    }
    else
