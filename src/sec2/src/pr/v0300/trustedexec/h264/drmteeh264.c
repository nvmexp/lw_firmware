...
DRM_NO_INLINE DRM_API DRM_BOOL DRM_CALL DRM_TEE_H264_IMPL_IsH264Supported( DRM_VOID )
{
    return TRUE;
}
#endif
    ...
    __out                                           DRMFILETIME                *f_pftSystemTime )
{
    ...
    return OEM_TEE_CLOCK_GetSystemTime( (OEM_TEE_CONTEXT*)f_pCallbackContextAllowNULL, f_pftSystemTime );
}
...
**                                               from DRM_TEE_AES128CTR_PrepareToDecrypt.
...
**
**                                               f_pEncryptedRegionMapping->cb is set to the number of
**                                               of DRM_DWORDs times sizeof(DRM_DWORD) [i.e.
**                                               f_pEncryptedRegionMapping->cb is a multiple of 8].
**
    ...
    __inout_tee_opt             DRM_TEE_BYTE_BLOB            *f_pEncryptedTranscryptedFullFrame )
{
    ...
    AssertChkArg( cKeys == 2 || cKeys == 3 );
    ChkDRAllowENOTIMPL( OEM_TEE_AES128CTR_EnforcePolicy( &f_pContext->oContext, DRM_TEE_POLICY_INFO_CALLING_API_H264_PREPROCESSENCRYPTEDDATA, pPolicyRef ) );
...

    {
        ...
        */
        for( iCare = 0; iCare < cCares; iCare += 2 )
        {
        ...
        */
        ChkDR( DRM_TEE_BASE_AllocBlob( f_pContext, DRM_TEE_BLOB_ALLOC_BEHAVIOR_COPY, f_pEncryptedPartialFrame->cb, f_pEncryptedPartialFrame->pb, &oCCD ) );
    ...
    {
        ChkDR( DRM_TEE_BASE_AllocBlob( f_pContext, DRM_TEE_BLOB_ALLOC_BEHAVIOR_COPY, cbOpaqueFrameData, pbOpaqueFrameData, &oOpaqueFrameData ) );
    }
    if( pbOpaqueStateUpdated != NULL )
    {
        ChkDR( DRM_TEE_BASE_IMPL_AllocBlobAndTakeOwnership( f_pContext, cbOpaqueStateUpdated, &pbOpaqueStateUpdated, &oOpaqueStateUpdated ) );
    }
    ChkDR( DRM_TEE_BASE_IMPL_AllocBlobAndTakeOwnership( f_pContext, cbSliceHeaders, &pbSliceHeaders, &oSliceHeaders ) );

    ChkVOID( DRM_TEE_BASE_TransferBlobOwnership( f_pOpaqueSliceHeaderStateUpdated, &oOpaqueStateUpdated ) );
    ChkVOID( DRM_TEE_BASE_TransferBlobOwnership( f_pSliceHeaders, &oSliceHeaders ) );
    ChkVOID( DRM_TEE_BASE_TransferBlobOwnership( f_pOpaqueFrameData, &oOpaqueFrameData ) );
...
ErrorExit:
    ...
    ChkVOID( DRM_TEE_BASE_FreeBlob( f_pContext, &oCCD ) );
    ...
    ChkVOID( DRM_TEE_BASE_FreeBlob( f_pContext, &oOpaqueStateUpdated ) );
    ChkVOID( DRM_TEE_BASE_FreeBlob( f_pContext, &oSliceHeaders ) );
    ChkVOID( DRM_TEE_BASE_FreeBlob( f_pContext, &oOpaqueFrameData ) );
    return dr;
}
#endif
