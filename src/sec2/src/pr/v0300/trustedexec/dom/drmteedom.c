...
DRM_NO_INLINE DRM_API DRM_BOOL DRM_CALL DRM_TEE_DOM_IMPL_IsDOMSupported( DRM_VOID )
{
    return TRUE;
}

static DRM_FRE_INLINE DRM_RESULT DRM_CALL _TEE_DOM_ParsePrivateKey(
   ...
   __deref_out_bcount( ECC_P256_PRIVKEY_SIZE_IN_BYTES ) const DRM_BYTE   **f_ppbEncryptedDomainKey )
{
    ...
    *f_ppbEncryptedDomainKey = f_pbData + ibData;
    *f_pibData               = ibData + oPrivKey.dwKeyLength;
    ...
    __out                       DRM_TEE_BYTE_BLOB            *f_pDKBs )
{
    ...
    ChkDR( DRM_TEE_BASE_IMPL_AllocKeyAES128( pOemTeeCtx, DRM_TEE_KEY_TYPE_DOMAIN_SESSION, &rgoSessionKeys[ 0 ] ) );
    ChkDR( DRM_TEE_BASE_IMPL_AllocKeyAES128( pOemTeeCtx, DRM_TEE_KEY_TYPE_DOMAIN_SESSION, &rgoSessionKeys[ 1 ] ) );
    ...
    ChkDR( DRM_TEE_BASE_MemAlloc( f_pContext, cDomains * sizeof(DRM_DWORD), ( DRM_VOID ** )&oRevisions.pdwData ) );

    while( ibDomainKeysData + DRM_DOMKEYXMR_PRIVKEYOBJ_MIN_LENGTH < cbDomainKeysData )
    {
        DRM_DWORD       dwRevision           = 0;
        const DRM_BYTE *pbEncryptedDomainKey = NULL;
        DRM_DWORD      *pdwRevision          = ( DRM_DWORD * )&oRevisions.pdwData[iDomain];

        ChkDR( _TEE_DOM_ParsePrivateKey( cbDomainKeysData, pbDomainKeysData, &ibDomainKeysData, &dwRevision, &pbEncryptedDomainKey ) );
        DRMASSERT( ibDomainKeysData <= cbDomainKeysData );

        *pdwRevision = dwRevision;

        ChkDR( DRM_TEE_BASE_IMPL_AllocKeyECC256( pOemTeeCtx, DRM_TEE_KEY_TYPE_PR_ECC_PRIVKEY_ENCRYPT, &oDomainPrivKey ) );
        ...
        ChkDR( DRM_TEE_BASE_FreeBlob( f_pContext, &oDKB ) );

        ChkDR( DRM_TEE_KB_BuildDKB(
            f_pContext,
            &oDomainPrivKey,
            dwRevision,
            &oDKB ) );
        ...
        if( oDKBs.pb == NULL )
        {
            ...
            */
            ...
            ChkDR( DRM_TEE_BASE_AllocBlob( f_pContext, DRM_TEE_BLOB_ALLOC_BEHAVIOR_NEW, cbDKBs, NULL, &oDKBs ) );
        }

        DRMASSERT( IsBlobAssigned( &oDKBs ) );
        __analysis_assume( IsBlobAssigned( &oDKBs ) );
        ChkVOID( OEM_TEE_MEMCPY( (DRM_VOID*)( oDKBs.pb + ( iDomain * oDKB.cb ) ), oDKB.pb, oDKB.cb ) );
    ...
    ChkVOID( DRM_TEE_BASE_TransferBlobOwnership( f_pDKBs, &oDKBs ) );
...
ErrorExit:
    ...
    //
    drTmp = DRM_TEE_BASE_FreeBlob( f_pContext, &oDKBs );
    if (dr == DRM_SUCCESS)
    {
        dr = drTmp;
    }

    drTmp = DRM_TEE_BASE_FreeBlob( f_pContext, &oDKB  );
    if (dr == DRM_SUCCESS)
    {
        dr = drTmp;
    }

    ChkVOID( DRM_TEE_BASE_MemFree( f_pContext, ( DRM_VOID ** )&oRevisions.pdwData ) );
    ChkVOID( OEM_TEE_BASE_FreeKeyECC256( pOemTeeCtx, &oDomainPrivKey.oKey ) );
