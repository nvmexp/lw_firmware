    ...
    __inout_bcount_opt( *f_pcbResponseMessage )         DRM_BYTE           *f_pbResponseMessage )
{
    ...
    */
    ...
    do
    {
        ChkVOID( DRM_TEE_BASE_MemFree( NULL, ( DRM_VOID ** )&pbStack ) );
        ChkDR( DRM_TEE_BASE_MemAlloc( NULL, cbStack, ( DRM_VOID ** )&pbStack ) );
        ...
        if( DRM_FAILED( dr ) )
        {
            ...
            /* needs to zero out as oReq is inout */
            ChkVOID( OEM_TEE_ZERO_MEMORY( &oReq, sizeof(oReq) ) );
        }
    ...
    ChkBOOL( oReq.fValid && oReq.MethodInfo.fValid, DRM_E_XB_ILWALID_OBJECT );

#if DRM_PC_TEST_CAPTURE_TEE_HEAP_USAGE
    g_dwLwrrentTeeFunctionID = oReq.MethodInfo.dwMethodID;
#endif /* DRM_PC_TEST_CAPTURE_TEE_HEAP_USAGE */
    ...
    if( oReq.MethodInfo.dwMethodID != DRM_TEE_PROXY_METHOD_ID( DRM_TEE_BASE_AllocTEEContext ) )
    {
#ifdef SINGLE_METHOD_ID_REPLAY
        /*
        ...
        *(((DRM_BYTE*)&g_rgoCache[0].idSession)+15) = *(((DRM_BYTE*)f_pbRequestMessage)+0x3f);
        ...
        _InitializeTKeys();
#endif
        ChkDR( DRM_TEE_BASE_MemAlloc( NULL, sizeof(*pTeeCtx), ( DRM_VOID ** )&pTeeCtx ) );
        ChkVOID( OEM_TEE_ZERO_MEMORY( pTeeCtx, sizeof(*pTeeCtx) ) );

        ChkDR( DRM_TEE_STUB_XB_TEECtxFromXBTEECtx( &oReq.MethodContext, pTeeCtx ) );

        ChkDR( DRM_TEE_CACHE_ReferenceContext( pTeeCtx ) );
    }

    ChkDR( DRM_TEE_STUB_ReadParameters( pTeeCtx, &oReq, rgParameters ) );

#ifndef BUILD_NO_METHODS
    switch( oReq.MethodInfo.dwMethodID )
    {
#ifdef NONE
        case DRM_TEE_PROXY_METHOD_ID( DRM_TEE_AES128CTR_PreparePolicyInfo ):
            ...
            drTEEResult = DRM_TEE_AES128CTR_PreparePolicyInfo(
                pTeeCtx,
        ...
        case DRM_TEE_PROXY_METHOD_ID( DRM_TEE_AES128CTR_PrepareToDecrypt ):
            ChkArg( rgParameters[1].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobAssigned( &rgParameters[1].uParameterValue.blobValue ) );
            ...
            drTEEResult = DRM_TEE_AES128CTR_PrepareToDecrypt(
                pTeeCtx,
               (const DRM_TEE_BYTE_BLOB  *)&rgParameters[1].uParameterValue.blobValue,
...
#endif
#if defined (SEC_COMPILE)
        case DRM_TEE_PROXY_METHOD_ID( DRM_TEE_AES128CTR_CreateOEMBlobFromCDKB ):
            ChkArg( rgParameters[1].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobAssigned( &rgParameters[1].uParameterValue.blobValue ) );
            ChkArg( rgParameters[2].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobConsistent( &rgParameters[2].uParameterValue.blobValue ) );
            ChkArg( rgParameters[3].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__ILWALID && IsBlobEmpty( &rgParameters[3].uParameterValue.blobValue ) );

            drTEEResult = DRM_TEE_AES128CTR_CreateOEMBlobFromCDKB(
                pTeeCtx,
...
#endif
#ifdef NONE
        case DRM_TEE_PROXY_METHOD_ID( DRM_TEE_PRNDRX_ProcessRegistrationResponseMessage ):
            ChkArg( rgParameters[1].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobAssigned( &rgParameters[1].uParameterValue.blobValue ) );
            ChkArg( rgParameters[2].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobConsistent( &rgParameters[2].uParameterValue.blobValue ) );
            ChkArg( rgParameters[3].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobAssigned( &rgParameters[3].uParameterValue.blobValue ) );

            drTEEResult = DRM_TEE_PRNDRX_ProcessRegistrationResponseMessage(
                pTeeCtx,
               (const DRM_TEE_BYTE_BLOB  *)&rgParameters[1].uParameterValue.blobValue,
               &rgParameters[2].uParameterValue.blobValue,
               (const DRM_TEE_BYTE_BLOB  *)&rgParameters[3].uParameterValue.blobValue );
            ...
            rgParameters[2].fIsOutputParameter = TRUE;
            break;
        case DRM_TEE_PROXY_METHOD_ID( DRM_TEE_PRNDRX_GenerateProximityResponseNonce ):
            ChkArg( rgParameters[1].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobAssigned( &rgParameters[1].uParameterValue.blobValue ) );
            ChkArg( rgParameters[2].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_ID );

            drTEEResult = DRM_TEE_PRNDRX_GenerateProximityResponseNonce(
                pTeeCtx,
               (const DRM_TEE_BYTE_BLOB  *)&rgParameters[1].uParameterValue.blobValue,
               &rgParameters[2].uParameterValue.idValue );
            ...
            rgParameters[2].fIsOutputParameter = TRUE;
            break;
        case DRM_TEE_PROXY_METHOD_ID( DRM_TEE_PRNDRX_CompleteLicenseRequestMessage ):
            ChkArg( rgParameters[1].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobAssigned( &rgParameters[1].uParameterValue.blobValue ) );
            ChkArg( rgParameters[2].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobConsistent( &rgParameters[2].uParameterValue.blobValue ) );

            drTEEResult = DRM_TEE_PRNDRX_CompleteLicenseRequestMessage(
                pTeeCtx,
               (const DRM_TEE_BYTE_BLOB  *)&rgParameters[1].uParameterValue.blobValue,
               &rgParameters[2].uParameterValue.blobValue );
            ...
            rgParameters[2].fIsOutputParameter = TRUE;
            break;
        case DRM_TEE_PROXY_METHOD_ID( DRM_TEE_PRNDRX_ProcessLicenseTransmitMessage ):
            ChkArg( rgParameters[1].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobAssigned( &rgParameters[1].uParameterValue.blobValue ) );
            ChkArg( rgParameters[2].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobAssigned( &rgParameters[2].uParameterValue.blobValue ) );

            drTEEResult = DRM_TEE_PRNDRX_ProcessLicenseTransmitMessage(
                pTeeCtx,
               (const DRM_TEE_BYTE_BLOB  *)&rgParameters[1].uParameterValue.blobValue,
               (const DRM_TEE_BYTE_BLOB  *)&rgParameters[2].uParameterValue.blobValue );

            break;
        case DRM_TEE_PROXY_METHOD_ID( DRM_TEE_PRNDTX_ProcessRegistrationRequestMessage ):
            ChkArg( rgParameters[1].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobConsistent( &rgParameters[1].uParameterValue.blobValue ) );
            ChkArg( rgParameters[2].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobAssigned( &rgParameters[2].uParameterValue.blobValue ) );
            ChkArg( rgParameters[3].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobAssigned( &rgParameters[3].uParameterValue.blobValue ) );
            ChkArg( rgParameters[4].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__ILWALID );

            drTEEResult = DRM_TEE_PRNDTX_ProcessRegistrationRequestMessage(
                pTeeCtx,
               &rgParameters[1].uParameterValue.blobValue,
               (const DRM_TEE_BYTE_BLOB  *)&rgParameters[2].uParameterValue.blobValue,
               (const DRM_TEE_BYTE_BLOB  *)&rgParameters[3].uParameterValue.blobValue,
               &rgParameters[4].uParameterValue.idValue );

            rgParameters[1].eParameterType     = DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB;
            rgParameters[1].fIsOutputParameter = TRUE;
            rgParameters[4].eParameterType     = DRM_TEE_PROXY_PARAMETER_TYPE__DRM_ID;
            rgParameters[4].fIsOutputParameter = TRUE;
            break;
        case DRM_TEE_PROXY_METHOD_ID( DRM_TEE_PRNDTX_CompleteRegistrationResponseMessage ):
            ChkArg( rgParameters[1].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobConsistent( &rgParameters[1].uParameterValue.blobValue ) );
            ChkArg( rgParameters[2].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobConsistent( &rgParameters[2].uParameterValue.blobValue ) );
            ChkArg( rgParameters[3].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobConsistent( &rgParameters[3].uParameterValue.blobValue ) );

            drTEEResult = DRM_TEE_PRNDTX_CompleteRegistrationResponseMessage(
                pTeeCtx,
               (const DRM_TEE_BYTE_BLOB  *)&rgParameters[1].uParameterValue.blobValue,
               &rgParameters[2].uParameterValue.blobValue,
               &rgParameters[3].uParameterValue.blobValue );
            ...
            rgParameters[3].fIsOutputParameter = TRUE;
            break;
        case DRM_TEE_PROXY_METHOD_ID( DRM_TEE_PRNDTX_GenerateProximityDetectionChallengeNonce ):
            ChkArg( rgParameters[1].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobConsistent( &rgParameters[1].uParameterValue.blobValue ) );
            ChkArg( rgParameters[2].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__ILWALID );

            drTEEResult = DRM_TEE_PRNDTX_GenerateProximityDetectionChallengeNonce(
                pTeeCtx,
               &rgParameters[1].uParameterValue.blobValue,
               &rgParameters[2].uParameterValue.idValue );
            ...
            rgParameters[2].fIsOutputParameter = TRUE;
            break;
        case DRM_TEE_PROXY_METHOD_ID( DRM_TEE_PRNDTX_VerifyProximityDetectionResponseNonce ):
            ChkArg( rgParameters[1].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobConsistent( &rgParameters[1].uParameterValue.blobValue ) );
            ChkArg( rgParameters[2].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_ID );

            drTEEResult = DRM_TEE_PRNDTX_VerifyProximityDetectionResponseNonce(
                pTeeCtx,
               &rgParameters[1].uParameterValue.blobValue,
            ...
            rgParameters[1].fIsOutputParameter = TRUE;
            break;
        case DRM_TEE_PROXY_METHOD_ID( DRM_TEE_PRNDTX_ProcessLicenseRequestMessage ):
            ChkArg( rgParameters[1].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobConsistent( &rgParameters[1].uParameterValue.blobValue ) );
            ChkArg( rgParameters[2].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobAssigned( &rgParameters[2].uParameterValue.blobValue ) );

            drTEEResult = DRM_TEE_PRNDTX_ProcessLicenseRequestMessage(
                pTeeCtx,
               &rgParameters[1].uParameterValue.blobValue,
               (const DRM_TEE_BYTE_BLOB  *)&rgParameters[2].uParameterValue.blobValue );
            ...
            rgParameters[1].fIsOutputParameter = TRUE;
            break;
        case DRM_TEE_PROXY_METHOD_ID( DRM_TEE_PRNDTX_CompleteLicenseTransmitMessage ):
            ChkArg( rgParameters[1].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobConsistent( &rgParameters[1].uParameterValue.blobValue ) );
            ChkArg( rgParameters[2].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobAssigned( &rgParameters[2].uParameterValue.blobValue ) );
            ChkArg( rgParameters[3].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobConsistent( &rgParameters[3].uParameterValue.blobValue ) );

            drTEEResult = DRM_TEE_PRNDTX_CompleteLicenseTransmitMessage(
                pTeeCtx,
               &rgParameters[1].uParameterValue.blobValue,
               (const DRM_TEE_BYTE_BLOB  *)&rgParameters[2].uParameterValue.blobValue,
               &rgParameters[3].uParameterValue.blobValue );
            ...
            rgParameters[3].fIsOutputParameter = TRUE;
            break;
        case DRM_TEE_PROXY_METHOD_ID( DRM_TEE_PRNDTX_RebindLicenseToReceiver ):
            ChkArg( rgParameters[1].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobConsistent( &rgParameters[1].uParameterValue.blobValue ) );
            ChkArg( rgParameters[2].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobAssigned( &rgParameters[2].uParameterValue.blobValue ) );
            ChkArg( rgParameters[3].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobAssigned( &rgParameters[3].uParameterValue.blobValue ) );
            ChkArg( rgParameters[4].eParameterType == DRM_TEE_PROXY_PARAMETER_TYPE__DRM_TEE_BYTE_BLOB && IsBlobAssigned( &rgParameters[4].uParameterValue.blobValue ) );
            ...
            drTEEResult = DRM_TEE_PRNDTX_RebindLicenseToReceiver(
                pTeeCtx,
               &rgParameters[1].uParameterValue.blobValue,
               (const DRM_TEE_BYTE_BLOB  *)&rgParameters[2].uParameterValue.blobValue,
               (const DRM_TEE_BYTE_BLOB  *)&rgParameters[3].uParameterValue.blobValue,
...
#endif //BUILD_NO_METHODS

    if( oReq.MethodInfo.dwMethodID != DRM_TEE_PROXY_METHOD_ID( DRM_TEE_BASE_FreeTEEContext ) )
    {
        ...
        do
        {
            if( dr2 == DRM_E_OUTOFMEMORY || cbStack < DRM_TEE_PROXY_MESSAGE_RESPONSE_OVERHEAD )
            {
                ...
                if( DRM_FAILED( dr2 ) )
                {
                    ChkVOID( OEM_SELWRE_ZERO_MEMORY( &oResp, sizeof( oResp ) ) );
...
ErrorExit:
    ...
    if( oResp.MethodContext.xbbaContext.pbDataBuffer != NULL )
    {
        ChkVOID( DRM_TEE_BASE_MemFree( pTeeCtx, ( DRM_VOID ** )&oResp.MethodContext.xbbaContext.pbDataBuffer ) );
    }

    ChkVOID( DRM_TEE_BASE_MemFree( pTeeCtx, ( DRM_VOID ** )&pbStack ) );
