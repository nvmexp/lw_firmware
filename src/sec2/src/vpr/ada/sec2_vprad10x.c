...
/*!
 * @file   sec2_vprad10x.c
 * @brief  VPR HAL Functions for AD10X
 *
 ...
 */
FLCN_STATUS
vprCheckIfBuildIsSupported_AD10X(void)
{
    LwU32       chip;
 ...
 */
FLCN_STATUS
vprIsDisplayIpVersionSupported_AD10X(void)
{
    LwU32       ipVersion;
 ...
 */
FLCN_STATUS
vprSetupClientTrustLevel_AD10X(void)
{
    ...
    // TODO: Drop debug board check for making LWJPG VPR_AWARE
    if (!FLD_TEST_DRF(_CSEC, _SCP_CTL_STAT, _DEBUG_MODE, _DISABLED, REG_RD32(CSB, LW_CSEC_SCP_CTL_STAT)))
    {
        cyaHi  = cyaHi | DRF_NUM(_PFB, _PRI_MMU, _VPR_CYA_HI_TRUST_LWJPG,  LW_PFB_PRI_MMU_VPR_CYA_VPR_AWARE);
    }
