 ...
 *
 * When a descriptor returned by this macro is passed to scpEncrypt() or
 * scpDecrypt(), it instructs the callee to use the first SCP_KEY_SIZE bytes
 * from keyPa as the encryption key for the requested operation.
 *
 ...
 *
 * When a descriptor returned by this macro is passed to scpEncrypt() or
 * scpDecrypt(), it instructs the callee to use the data located in the
 * register specified by regIndex as the encryption key for the requested
 * operation. This mode is intended to support keys pulled in from KMEM.
 *
 * Note that the contents of the register will be cleared following use of the
 * returned key-descriptor in an encryption or decryption operation.
 ...
 *
 * When a descriptor returned by this macro is passed to scpEncrypt() or
 * scpDecrypt(), it instructs the callee to perform AES-ECB encryption on the
 * first SCP_BLOCK_SIZE bytes from saltPa, using the hardware-secret indicated
 * by secretIndex as the key. The result of this procedure is then used as the
 * encryption key for the requested encryption or decryption operation.
 *
 ...
 *
 * When a descriptor returned by this macro is passed to scpEncrypt() or
 * scpDecrypt(), it instructs the callee to perform AES-CBC encryption on the
 * first SCP_BLOCK_SIZE bytes from saltPa, using the hardware-secret indicated
 * by secretIndex as the key and the first SCP_BLOCK_SIZE bytes from ivPa as
 * the initialization vector. The result of this procedure is then used as the
 * encryption key for the requested encryption or decryption operation.
 *
 ...
 *                          less than SCP_SECRET_COUNT.
 *
 * @param[in] bForget       A boolean value indicating whether the engine's
 *                          signature/hash should be cleared from the signature
 *                          register ("forgotten") at the conclusion of the
 ...
 * When a descriptor returned by this macro is passed to scpEncrypt() or
 * scpDecrypt(), it instructs the callee to perform AES-ECB encryption on the
 * engine's signature/hash, using the hardware-secret indicated by secretIndex
 * as the key. The result of this procedure is then used as the encryption key
 * for the requested encryption or decryption operation.
 *
 * scpEncrypt() and scpDecrypt() will also issue forget_sig to clear the
 * engine's signature/hash from the signature register before returning, unless
 * overridden by setting bForget to false. Applications should only override
 * this behaviour if they require the engine's signature/hash for further use.
 *
 * Note that key-descriptors returned by this macro cannot be used when in an
 * inselwre context, or when the engine's signature/hash is already invalid.
 */
#define SCP_KEY_DERIVED_SIG(secretIndex, bForget) ( \
    (const SCP_KEY_DESC *)&(SCP_KEY_DESC)           \
    {                                               \
        .keyType = SCP_KEY_TYPE_DERIVED_SIG,        \
        .sigSecretIndex = (secretIndex),            \
        .bSigForget = (bForget),                    \
    }                                               \
)
...
typedef struct SCP_KEY_DESC
{
    ...
    union
    {
        ...
        // Information for DERIVED_SIG-type keys.
        struct
        {
            // The hardware secret to be used in the key-derivation process.
            uint8_t sigSecretIndex;

            // Whether to forget the engine's signature/hash after deriving.
            bool bSigForget;
        };
    };
}SCP_KEY_DESC;
 ...
 * memory locations (i.e. to facilitate encrypting data in-place). However, the
