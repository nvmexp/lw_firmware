 ...
 * are preserved through the virtual-to-physical colwersion process, if needed.
 *
...
typedef enum SCP_INIT_FLAG
{
    ...
    //
    // Do not issue forget_sig to clear the engine's signature/hash from the
    // signature register. Should only be used if the application intends to
    // derive a key from the engine signature/hash later on. This flag is
    // invalid when scpInit() is called from an inselwre context.
    //
    SCP_INIT_FLAG_KEEP_SIG = 1,
 ...
 *                                   SCP unit.
 *
 *    ERR_INSUFFICIENT_PERMISSIONS   if the SCP unit is blocked in the host
 *                                   engine's device map.
 *
 ...
 * again, unless otherwise noted. Keep in mind that the engine signature/hash
 * cannot be recovered after shutdown.
 */
