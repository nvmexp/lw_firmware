 ...
 * lwctassert.h
 *
 * LWpu ct_assert (compile time assertion) function.
 *
 * Copyright (c) 1997-2019 LWPU Corporation.  All rights reserved.
 ...
 * alternate method which ilwolves array declarations is employed in that case,
 * described below.
 *
...
#define LW_CTASSERT_STRINGIFY_MACRO(b) #b

#if !defined(LWOC) && ((defined(__STDC_VERSION__) && __STDC_VERSION__ >= 201112L) || CLANG_C_STATIC_ASSERT)
 // ISO C11 defines the _Static_assert keyword
# define ct_assert(b)  _Static_assert((b), "Compile time assertion failed: " LW_CTASSERT_STRINGIFY_MACRO(b))
# define ct_assert_i(b,line)  _Static_assert((b), "Compile time assertion failed: " LW_CTASSERT_STRINGIFY_MACRO(b)LW_CTASSERT_STRINGIFY_MACRO(line))
#elif (defined(__cplusplus) && __cplusplus >= 201103L) || (defined(_MSVC_LANG) && _MSVC_LANG >= 201103L)
 // ISO C++11 defines the static_assert keyword
# define ct_assert(b)  static_assert((b), "Compile time assertion failed: " LW_CTASSERT_STRINGIFY_MACRO(b))
# define ct_assert_i(b,line)  static_assert((b), "Compile time assertion failed: " LW_CTASSERT_STRINGIFY_MACRO(b)LW_CTASSERT_STRINGIFY_MACRO(line))
#else
