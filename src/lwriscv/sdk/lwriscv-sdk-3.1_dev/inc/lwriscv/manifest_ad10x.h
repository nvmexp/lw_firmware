 ...
 */
#ifndef LWRISCV_MANIFEST_AD10X_H
#define LWRISCV_MANIFEST_AD10X_H

#include "lwriscv/manifest.h"

//
// Synced on 2021-10-27 to
// https://confluence.lwpu.com/display/FalconSelwrityIPUserSpace/AD10x+LWRISCV+BootROM+Design+Spec
//

typedef struct
{
    // 384 bytes of RSA signature is prepended to this structure by siggen
    uint8_t version;                    // [MISC] version of the manifest; it needs to be 0x1 in AD10x
    ...
    uint8_t bAttester;                  // [Attestation flag] if true, BR DECFUSEKEY and AK subroutines are exelwted (false is defined as 0x55; true is defined as 0xAA; other values will be reported as an error) [AD10x Update]
    uint8_t reservedBytes0[10];         // [reserved space] all 0s; 1 byte for bHashSaving, 4 bytes for MAX_DELAY_USEC in the furture [AD10x Update]
...
#endif // LWRISCV_MANIFEST_AD10X_H
