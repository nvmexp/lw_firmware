...
/* Crypto system config file for target: Trusty TOS */
...
/* The SE HW mutex locks TZ only access to the engines, but since
 * this runs in multiple CPUs we need to block access from other Trusty
 * instances in other CPUs when we use the SE HW
 *
 * The locking order is:
 *
 * 1) take the hwse_driver_lock
 * 2) take HW mutex
 * 3) do what ever you want to do
 * 4) release HW mutex
 * 5) release hwse_driver_lock
 */
extern mutex_t hwse_driver_lock;
...
#define HAVE_DEPRECATED 0	  // If non-zero => compiles in deprecated functions for
				  ...
				  // Lwrrently: AES && RSA keyslot clear functions
...
#define HAVE_NC_REGIONS	1	// Handle non-contiguous phys memory regions for SE engine DMA
				// (XXX Not complete: AES cipher data is COPIED around =>FIXME)

#define HAVE_USER_MODE	1	// Support secure AARCH-32 user mode (Trusty TA's)
...
#ifndef HAVE_P521
#define HAVE_P521 		      0 // XXX NIST-P521 lwrve not fully implemented yet...
#endif

#define HAVE_NIST_LWRVES	      1 // Support NIST EC lwrves
#define HAVE_NIST_LWRVE_ALL	      1 // Support all NIST EC lwrves (or select individual lwrves)

#define HAVE_BRAINPOOL_LWRVES	      1	// Support brainpool EC lwrves
#define HAVE_BRAINPOOL_LWRVES_ALL     1	// Support brainpool EC lwrves (or select individual lwrves)

#if HAVE_BRAINPOOL_LWRVES
#define HAVE_BRAINPOOL_TWISTED_LWRVES 1		// Support brainpool EC twisted lwrves
#define HAVE_BRAINPOOL_TWISTED_LWRVES_ALL 1	// Support brainpool EC twisted lwrves (or select individual lwrves)
#endif

#ifndef HAVE_ELLIPTIC_20
#define HAVE_ELLIPTIC_20 0	// Code running in T194 with Elliptic-2.0 FW (with new lwrves and algos)
#endif
...
#define HAVE_LWRVE_ED25519 1
#else
#define HAVE_LWRVE_ED25519 0
#endif

#if HAVE_PKA1_X25519
#define HAVE_LWRVE_C25519 1
#else
...
#define TRUSTY_LOG(level, msg,...) misra_dprintf(level, msg, ## __VA_ARGS__)

/* SE driver log macros for Trusty */
// #define LOG_NOISE(msg,...)  TRUSTY_LOG(SPEW,    msg, ## __VA_ARGS__)
#define LOG_INFO(msg,...)   TRUSTY_LOG(INFO,    msg, ## __VA_ARGS__)
#define LOG_ERROR(msg,...)  TRUSTY_LOG(CRITICAL,msg, ## __VA_ARGS__)
#define LOG_ALWAYS(msg,...) TRUSTY_LOG(CRITICAL,msg, ## __VA_ARGS__)
...
#define GET_ADDR32(addr32) (addr32)
#define GET_DOMAIN_ADDR(domain, addr) \
  ((TE_CRYPTO_DOMAIN_TA == (domain)) ? GET_ADDR32(addr) : (addr))

/* System specific virtual to physical address mapping function,
 * defined in the config file. Prototype declared in tegra_se.h.
 */
#define SYSTEM_DOMAIN_V2P trusty_domain_v2p

/* System dependent virtual address to phys address mapping.
 *
 * In case of a TA address (TE_CRYPTO_DOMAIN_TA) the addr may be
 * either a 64 bit Trusty address (with upper 32 bits nonzero) or a TA
 * address (upper 32 bits zero).
 *
 * With TE_CRYPTO_DOMAIN_KERNEL the address must be a 64 bit Trusty kernel address.
 */
#define DOMAIN_VADDR_TO_PADDR(domain, addr) SYSTEM_DOMAIN_V2P(domain, addr)

/* XX Make more generic, move somewhere else and use this more!!! FIXME
 *
 * Do not assert this anymore; just return an error if this fails.
 *
 * If memory is contiguous => then all secure carveout addresses are valid
 * for SE DMA access. This *COULD* also verify DMA target address is
 * in writable memory regions and reject the others => TODO
 */
#define DEBUG_ASSERT_PHYS_DMA_OK(addr,size)				\
  do {									\
    ret = se_is_contiguous_memory_region(addr, size, NULL);		\
    if (NO_ERROR != ret) {						\
      ERROR_MESSAGE("Buffer %p (%u bytes) is not contiguous!\n", addr, size); \
      goto fail;							\
    }									\
  } while(false)
 ...
 */
#define DOMAIN_VADDR_TO_PADDR(domain, addr) vaddr_to_paddr((void *)addr)
...
#define SE_CACHE_FLUSH_ILWALIDATE(vaddr, size) do { CF; arch_clean_ilwalidate_cache_range(vaddr,size); } while(false)
#define SE_CACHE_ILWALIDATE(vaddr, size) do { CF; arch_ilwalidate_cache_range(vaddr,size); } while(false)
 ...
 */
#define SW_MUTEX_TAKE    (void)mutex_acquire(&hwse_driver_lock)
#define SW_MUTEX_RELEASE (void)mutex_release(&hwse_driver_lock)

struct engine_s;
void trusty_engine_reserve(const struct engine_s *engine);
void trusty_engine_release(const struct engine_s *engine);

#define SYSTEM_ENGINE_RESERVE(engine) trusty_engine_reserve(engine)
#define SYSTEM_ENGINE_RELEASE(engine) trusty_engine_release(engine)
...
#define SE_PHYS_RANGE_CHECK(paddr, size)				\
  do {									\
    if (!VALID_PADDR_SE_RANGE((paddr),(size))) {			\
      ...
      LTRACEF("Phys addr 0x%lx..0x%lx (size %u) not SE HW accessible (%d)\n", \
 ...
 * TODO => FIXME!!!!
 */
void *trusty_get_contiguous_zeroed_memory(uint32_t align, size_t size, const char *fun, uint32_t line);
void trusty_release_contiguous_memory(void *addr, const char *fun, uint32_t line);

#define GET_CONTIGUOUS_ZEROED_MEMORY(align, size) \
  trusty_get_contiguous_zeroed_memory(align, size, __func__, __LINE__)

#define RELEASE_CONTIGUOUS_MEMORY(addr) \
  trusty_release_contiguous_memory(addr, __func__, __LINE__)
 ...
 */
void *trusty_get_zeroed_memory(size_t size, const char *fun, uint32_t line);
void trusty_release_memory(void *addr, const char *fun, uint32_t line);

#define GET_ZEROED_MEMORY(size)			\
  trusty_get_zeroed_memory(size, __func__, __LINE__)

#define RELEASE_MEMORY(addr) \
  trusty_release_memory(addr, __func__, __LINE__)
...
/* Otherwise the cirlwlar log buffer will overflow */
 ...
 */
#define SYSTEM_MAP_DEVICE trusty_map_crypto_device
