...
/* Crypto system config file for SE-lite LWPKA */
...
static inline void writeWrapper(const volatile void* addr, uint32_t val)
{
    localWrite((uint32_t)(uintptr_t)addr, val);
}

static inline uint32_t readWrapper(const volatile void* addr)
{
...
#define HAVE_SE_ENGINE_AES0 0    /* SE HW does not have AES0 engine */
#define HAVE_AES_XTS      0    /* Support XTS-AES mode */
#define HAVE_AES_GCM      1    /* Support AES-GCM mode */
...
#define HAVE_SE_GENRND    1    /* Support SE DRNG random numbers with AES0 engine seeded from LWRNG */
#define HAVE_ALLOW_CACHED_RANDOM 0 /* Use cache of fresh random numbers */
#define HAVE_LWRNG_DRNG   0    /* Support DRNG random numbers with LWRNG engine */
...
#define HAVE_LWPKA_RSA     0    /* Add LWPKA RSA (LW unit with up to 4096 bit RSA keys) */
#define HAVE_LWPKA_ECC     0    /* Add LWPKA ECC (LW unit with for ECDH, ECDSA, EC point operations) */

#define HAVE_LWPKA_ECDH    0    /* Add LWPKA ECDH operation (requires HAVE_LWPKA_ECC) */
#define HAVE_LWPKA_ECDSA   0    /* Add LWPKA ECDSA (requires HAVE_LWPKA_ECC, HAVE_LWPKA_MODULAR) */
#define HAVE_SHAMIR_TRICK 0     /* Enable Shamir's trick for PKA1/LWPKA */
#define HAVE_LWPKA_MODULAR 0    /* Add LWPKA modular library (e.g. mandatory for ECDSA, requires HAVE_LWPKA_ECC) */
...
#define HAVE_ECDSA_SIGNING   0 /* ECDSA signature generation */
...
#define HAVE_NIST_LWRVES     0 /* Support NIST EC lwrves */
...
#else
#define TEGRA_READ32(a)     ({ readWrapper(a); })
#define TEGRA_WRITE32(a, v) ({ writeWrapper(a, v); })
#endif
 ...
 */
#define LW_ADDRESS_MAP_SE_AES_BASE      0x01500000
#define LW_ADDRESS_MAP_SE_LWPKA_BASE    0x0
#define LW_ADDRESS_MAP_SE_LWRNG_BASE    0x0
/**
...
static inline status_t map_crypto_device(uint32_t device_id, void **base)
{
    ...
    switch (device_id) {
        case SE_CDEV_SE0:
            *base = (void *)LW_ADDRESS_MAP_SE_AES_BASE;
            ret = NO_ERROR;
            break;
...
/**
 *  @brief return kac user for host engine.
 *
 *  @param[in] N/A
 *  @param[out] kac_user        engine-specific kac user
 */
#include "arse0.h"
#include "ccc_lwrm_drf.h"
static inline uint32_t se_kac_get_keyslot_user(void)
{
    uint32_t regval = localRead(LW_PRGNLCL_SE0_OWNERSHIP);
    uint32_t kac_user = LW_DRF_VAL(SE0, OWNERSHIP, UID, regval);
    return kac_user;
}
#define SE_KAC_KEYSLOT_USER se_kac_get_keyslot_user()
