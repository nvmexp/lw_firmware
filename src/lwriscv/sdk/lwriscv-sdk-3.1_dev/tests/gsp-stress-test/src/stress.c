...
__attribute__((noinline)) int lz4CompressionTest(void *param)
{
	...
	LwU32 crc, crc2;
	char *buf, *out;
	unsigned int size = 0, o_size = 0;
	buf = (char *)cfg->src;
	crc = crc32(buf, cfg->src_size, 0);
...
__attribute__((noinline)) static LwU64 bp_loop2(LwU64 loopSz, LwU64 mask) // aka Branch Predictor Torture
{
	LwU64 mraow = 0;
	__asm__ (
		"mv a4, %[loopSz]\n"
...
__attribute__((noinline)) static LwU64 bp_mul_torture(LwU64 loopSz, LwU64 mask) // aka Branch Predictor Torture
{
	LwU64 mraow = 0;
	LwU64 checkval;
	__asm__ (
...
__attribute__((noinline)) int branchPredictorTortureTest(LwU64 iterations) // expected failure mode is "hang"
{
...
__attribute__((noinline)) int branchPredictorMultiplyTortureTest(LwU64 iterations)
{
	LwU64 a, b, c, d;
	a = bp_mul_torture(iterations, 0x1); //50%
	b = bp_mul_torture(iterations, 0x3); //75%
	c = bp_mul_torture(iterations, 0x7); //87.5%
	d = bp_mul_torture(iterations, 0xf); //93.75%
	...
	//printf("bpmul torture FAILURE DETECTED\n");
	return -1;
...
__attribute__((noinline)) void lfsrTest(unsigned size, void *dest, LwBool print)
{
	const char meow[] = "mraaaow!";
	int match = -1;
	for (int x = 0; x < (int)(sizeof(lfsr_data)/sizeof(LFSR_DATA)); x++)
	{
...
extern LwU64 g_sysmem_block_addr;

__attribute__((noinline)) void init_crc_table()
{
	// CRC table in Sysmem memory allocated by Lwgpu.
	crc32_slice4_table = (LwU32 *)(void *) g_sysmem_block_addr;
...
__attribute__((noinline)) LwU32 crc32(void *data, LwU64 length, LwU32 crc_in)
{
...
__attribute__((noinline)) LwU32 slow_crc32(void *data, LwU64 length, LwU32 crc_in)
{
	LwU8 *data_asU8 = (LwU8 *)data;
...
__attribute__((noinline)) void verify(void *data, LwU64 length, LwU32 expectedCRC)
{
	LwU32 crc[2] = {0,};
	crc[0] = crc32(data, length, 0);
	crc[1] = slow_crc32(data, length, 0);
	for (LwU64 i=0; i<2; i++)
	{
...
void* LZ4_calloc(size_t n, size_t s)
{
	...
	fastAlignedMemSet(retVal, 0, n * s);
	return retVal;
}
