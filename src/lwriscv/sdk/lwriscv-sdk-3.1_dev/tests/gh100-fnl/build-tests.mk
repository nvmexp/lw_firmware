...
# MK TODO: this should not be needed but CCC "needs help" working properly for now
ifeq ($(SAMPLE_NEEDS_LIBCCC),true)
    INCLUDES += $(LIBLWRISCV_DIR)/inc/common_crypto
    INCLUDES += $(LIBLWRISCV_DIR)/inc/common_crypto/include
    INCLUDES += $(LIBLWRISCV_DIR)/inc/common_crypto_private
    INCLUDES += $(LIBLWRISCV_DIR)/inc/common_crypto_private/lwpka
    DEFINES  += -DUPROC_ARCH_RISCV -DCCC_USE_LIBLWRISCV -DCCC_ENGINE_${LIBCCC_ENGINE_UPPERCASE}
    LIBS+= -L$(LIBLWRISCV_DIR)/lib/$(LWRISCV_IS_ENGINE) -lCCC
endif
...
CFLAGS += $(INCLUDES)
CFLAGS += $(DEFINES)
CFLAGS += -Wno-error=unused-function -Wno-error=sign-colwersion -Wno-error
CFLAGS += $(EXTRA_CFLAGS)
...
TARGET:=$(BUILD_DIR)/$(FNL_TEST_NAME)-$(SAMPLE_PROFILE_FILE)
OBJS += main.o
...
BUILD_FILES := $(FINAL_FMC_TEXT) $(FINAL_FMC_DATA) $(FINAL_MANIFEST) $(FMC_TEXT) $(FMC_DATA) $(ELF) $(OBJDUMP_SOURCE) $(MANIFEST_HEX)
