...
is
    ...
    type External_Policy_Array is array (Partition_ID) of External_Policy with
            Dynamic_Predicate => (
                                  -- Predicate ensures the following:
                                  -- 1  - There can be only one partition with Hash_LWMPU_Enable
                                  -- 1' - Plcy_Mpu_Control predicate ensures Start_Index and Entry_Count specifying all MPU entries if Hash_LWMPU is enabled
                                  -- 2  - There can not be MPU entry overlaps
                                  (for all I in Partition_ID =>
                                    (for all J in Partition_ID =>
                                        (if I = J then True
                                            elsif ((External_Policy_Array(I).Mpu_Control.Hash_LWMPU_Enable = 0 and then External_Policy_Array(J).Mpu_Control.Hash_LWMPU_Enable = 0) or else
                                                    (External_Policy_Array(I).Mpu_Control.Hash_LWMPU_Enable = 1 and then External_Policy_Array(J).Mpu_Control.Hash_LWMPU_Enable = 0) or else
                                                    (External_Policy_Array(I).Mpu_Control.Hash_LWMPU_Enable = 0 and then External_Policy_Array(J).Mpu_Control.Hash_LWMPU_Enable = 1))
                                                and then External_Policy_Array(I).Mpu_Control.Entry_Count > 0 and then External_Policy_Array(J).Mpu_Control.Entry_Count > 0 then
                                                    ((LwU8(External_Policy_Array(I).Mpu_Control.Start_Index) + External_Policy_Array(I).Mpu_Control.Entry_Count - 1) < LwU8(External_Policy_Array(J).Mpu_Control.Start_Index)
                                                or else
                                                    (LwU8(External_Policy_Array(J).Mpu_Control.Start_Index) + External_Policy_Array(J).Mpu_Control.Entry_Count - 1) < LwU8(External_Policy_Array(I).Mpu_Control.Start_Index)))))
                                  and then
                                 ...
                                 (for all I in Partition_ID => (for all index in Io_Pmp_Array_Index => (External_Policy_Array(I).Io_Pmp(index).Cfg.Lock = LOCK_UNLOCKED) or else
                                                                 (for all J in Partition_ID => (External_Policy_Array(J).Io_Pmp(index).Mode = External_Policy_Array(I).Io_Pmp(index).Mode) and then
                                                                                               ...
                                                                                               (External_Policy_Array(J).Io_Pmp(index).Cfg.Fbdma_Imem = External_Policy_Array(I).Io_Pmp(index).Cfg.Fbdma_Imem) and then
                                                                                               (External_Policy_Array(J).Io_Pmp(index).Cfg.Fbdma_Dmem = External_Policy_Array(I).Io_Pmp(index).Cfg.Fbdma_Dmem) and then
                                                                                               ...
                                                                                               (External_Policy_Array(J).Io_Pmp(index).Cfg.Gdma_Chan0_0 = External_Policy_Array(I).Io_Pmp(index).Cfg.Gdma_Chan0_0) and then
                                                                                               (External_Policy_Array(J).Io_Pmp(index).Cfg.Gdma_Chan0_1 = External_Policy_Array(I).Io_Pmp(index).Cfg.Gdma_Chan0_1) and then
                                                                                               (External_Policy_Array(J).Io_Pmp(index).Cfg.Gdma_Chan1_0 = External_Policy_Array(I).Io_Pmp(index).Cfg.Gdma_Chan1_0) and then
                                                                                               (External_Policy_Array(J).Io_Pmp(index).Cfg.Gdma_Chan1_1 = External_Policy_Array(I).Io_Pmp(index).Cfg.Gdma_Chan1_1) and then
                                                                                               (External_Policy_Array(J).Io_Pmp(index).Cfg.Gdma_Chan2_0 = External_Policy_Array(I).Io_Pmp(index).Cfg.Gdma_Chan2_0) and then
                                                                                               (External_Policy_Array(J).Io_Pmp(index).Cfg.Gdma_Chan2_1 = External_Policy_Array(I).Io_Pmp(index).Cfg.Gdma_Chan2_1) and then
                                                                                               (External_Policy_Array(J).Io_Pmp(index).Cfg.Gdma_Chan3_0 = External_Policy_Array(I).Io_Pmp(index).Cfg.Gdma_Chan3_0) and then
                                                                                               (External_Policy_Array(J).Io_Pmp(index).Cfg.Gdma_Chan3_1 = External_Policy_Array(I).Io_Pmp(index).Cfg.Gdma_Chan3_1) and then
                                                                                               (External_Policy_Array(J).Io_Pmp(index).Cfg.SE_AES_0 = External_Policy_Array(I).Io_Pmp(index).Cfg.SE_AES_0) and then
                                                                                               (External_Policy_Array(J).Io_Pmp(index).Cfg.SE_AES_1 = External_Policy_Array(I).Io_Pmp(index).Cfg.SE_AES_1) and then
                                                                                               (External_Policy_Array(J).Io_Pmp(index).Cfg.SE_HASH = External_Policy_Array(I).Io_Pmp(index).Cfg.SE_HASH) and then
