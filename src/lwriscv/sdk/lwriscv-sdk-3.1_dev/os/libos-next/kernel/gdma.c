...
LIBOS_SECTION_TEXT_COLD LibosStatus gdmaCfgChannel(LwU8 chanIdx)
{
    LwU32 reg;
...
LIBOS_SECTION_TEXT_COLD LibosStatus gdmaFlush(LwU8 chanIdx)
{
    LwU32 reg;
...
)
{
    ...
    gdmaXferReg(srcVa, dstVa, size, lwrChanIdx, subChanIdx);
...
LIBOS_NORETURN LIBOS_SECTION_TEXT_COLD void KernelSyscallGdmaFlush(void)
{
    for (LwU8 chanIdx = 0; chanIdx < gdmaNumChannels; chanIdx++)
    {
        LibosStatus status = gdmaFlush(chanIdx);
        if (status != LibosOk)
        {
