...
void LibosTreeInsertFixup(LibosTreeHeader *x, void (*mutator)(LibosTreeHeader * node))
{
    ...
    // Update parents state to root
    // @todo: Could stop early if mutator returned bool
    if (mutator)
    {
        j = x->parent;
        while (!j->isNil) {
    ...
    // @todo: This file is compiled into both user-space and kernel
    //        without different macrospace, we need to call KernelAssert here
...
{
    // Rightmost child exists? Find the minimum value in that subtree
    if (!i->right->isNil)
        return LibosTreeLeftmost(i->right);
 
    // Walk up the tree until we're the leftmost child
    LibosTreeHeader * p = i->parent;
    while (i == p->right && !p->isNil) {
        i = p;
        p = p->parent;
    }
