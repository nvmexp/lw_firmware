...
void gsp::csr_write(LwU32 addr, LwU64 value)
{
    // Flush the tlb
    tlbVersion++;
...
void gsp::priv_write(LwU64 addr, LwU32 value)
{
    switch (addr)
    {
    ...
    case LW_PRGNLCL_GDMA_CHAN_TRANS_CONFIG0(1):
        ...
        gdmaTransfer((addr - LW_PRGNLCL_GDMA_CHAN_TRANS_CONFIG0(0)) / 4096);
        break;
...
LwBool gsp::probe_address(LwU64 i, LwU64 va, LwU64 &pa, TranslationKind & mask)
{
    mask = (TranslationKind)0;
    if (!(mmpuVa[i] & 1))
        return LW_FALSE;
    ...
    {
        if (DRF_VAL64(_RISCV, _CSR_SMPUATTR, _SX, mmpuAttr[i]))
            mask = (TranslationKind)(mask | translateSupervisorExelwte);

        if (DRF_VAL64(_RISCV, _CSR_SMPUATTR, _UX, mmpuAttr[i]))
            mask = (TranslationKind)(mask | translateUserExelwte);

        if (DRF_VAL64(_RISCV, _CSR_SMPUATTR, _SR, mmpuAttr[i]))
            mask = (TranslationKind)(mask | translateSupervisorLoad);

        if (DRF_VAL64(_RISCV, _CSR_SMPUATTR, _UR, mmpuAttr[i]))
            mask = (TranslationKind)(mask | translateUserLoad);

        if (DRF_VAL64(_RISCV, _CSR_SMPUATTR, _SW, mmpuAttr[i]))
            mask = (TranslationKind)(mask | translateSupervisorStore);

        if (DRF_VAL64(_RISCV, _CSR_SMPUATTR, _UW, mmpuAttr[i]))
            mask = (TranslationKind)(mask | translateUserStore);
...
LwBool
gsp::translate_address(LwU64 va, LwU64 &pa, TranslationKind & mask)
{
#if LIBOS_CONFIG_MPU_HASHED
    if (DRF_VAL64(_RISCV_CSR, _SATP, _MODE, satp) == LW_RISCV_CSR_SATP_MODE_HASH_LWMPU)
    {
        LwU64 params[3] = { smpuhp & 0xFFFF, (smpuhp>>16) & 0xFFFF, (smpuhp>>32) & 0xFFFF};

        for (int order = 0; order < 3; order ++)
        {
            uint16_t s0 = params[order] & 0x3f;
            uint16_t s1 = (params[order]>>8) & 0x3f;
            uint8_t group = ((va >> s0) ^ (va >> s1)) & (LIBOS_CONFIG_MPU_INDEX_COUNT/4 - 1);
            for (int i = group * 4; i != (group+1)*4; i++)            
                if (probe_address(i, va, pa, mask))
                    return LW_TRUE;
        }
        return LW_FALSE;
    }
    ...
    else
    {
        // MPU is disabled
        pa = va;
        mask = (TranslationKind)-1ULL;
        return LW_TRUE;
    }
}

void gsp::dmaTransfer(void)
{
    LwU64 src, dest;
    LwU64 tcmAperture;
    LwU32 size;
    ...
    pmp_assert_partition_has_access(dest, size, translateSupervisorStore);
    pmp_assert_partition_has_access(src, size, translateSupervisorLoad);
...
#if LIBOS_CONFIG_GDMA_SUPPORT
void gsp::gdmaTransfer(LwU8 chanIdx)
{
    ...
    pmp_assert_partition_has_access(dstPa, size, translateSupervisorStore);
    pmp_assert_partition_has_access(srcPa, size, translateSupervisorLoad);
...
LwBool gsp::csr_validate_access(LwU32 addr)
{
