...
struct Section
{
   T * begin_, * end_;
   LwU64  va_;
   Section(LibosElfImage * image, const char * name)
   {
    LibosElf64SectionHeader * debugLine = LibosElfFindSectionByName(image, name);
    if (debugLine && LibosOk == LibosElfMapSection(image, debugLine, (LwU8**)&begin_, (LwU8**)&end_))
       va_ = debugLine->addr;
    else
       begin_ = end_ = 0;
   }
...
int compile(const char * in, const char * out)
{
    ...
    void * elfBytes = (LibosElf64Header *)malloc(fsize);
    if (fread(elfBytes, 1, fsize, f) != fsize) {
        fprintf(stderr, "Error reading ELF\n");
        exit(1);
    }
    fclose(f);
    LibosElfImage elf;
    if (LibosOk != LibosElfImageConstruct(&elf, elfBytes, fsize))
      return -1;
    ...
    LwU64 shuttleSyncSendId, shuttleSyncRecvId, shuttleTrapId, shuttleReplayId;
    ...
    for (struct LibosManifestTaskInstance * i = manifestTasks.begin(); i!=manifestTasks.end(); ++i) {
        LibosTaskHandle id = (LibosTaskHandle)(LwU64)(i->name);
    ...
    for (int tries = 0; tries < 1000000; tries++)
    {
      ...
      for (auto i = manifestShuttlesEx.begin(); i != manifestShuttlesEx.end();i++) {
          ...
          hash ^= compiledTaskArray[(LibosTaskHandle)(LwU64)i->task].tableKey;
    ...
    for (int i = 0; i < object_table_plan.table.size(); i++) {
      ...
      objectTable[i].owner   = (LibosTaskHandle)(LwU64)entry->task;
      objectTable[i].id      = (LibosPortHandle)(LwU64)entry->name;

      // Default to not grant
      objectTable[i].grant = 0;
      ...
      {
        uint32_t timerIndex = ManifestTimers.index_of(entry->instance);
        objectTable[i].offset = compiledTimerArray.va_index(timerIndex) - offsetReference;
        objectTable[i].typeMask = TinyTypeTimer | TinyTypePort;

        if (entry->access == LibosManifestObjectGrant::timerGrantWait)
          objectTable[i].grant = LibosGrantWait;        
        if (entry->access == LibosManifestObjectGrant::timerGrantSet)
          objectTable[i].grant = LibosGrantTimerSet;
        else if (entry->access == LibosManifestObjectGrant::timerGrantAll)
          objectTable[i].grant = LibosGrantTimerAll;
      }
      else if (ManifestLocks.contains(entry->instance))
      {
        ...
        objectTable[i].typeMask = TinyTypeLock | TinyTypePort;
        objectTable[i].grant = LibosGrantWait | LibosGrantLockAll;
      }
    }
    ...
    for (auto i = manifestTasks.begin(); i != manifestTasks.end(); i++)
    {
      ...
      if (i->waitingShuttle) {
        ...
        shuttle->waitingOnPort = (Port *)(compiledPortArray.va_ptr(port));
