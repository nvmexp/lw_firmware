...
void IntegerMapInsert(IntegerMap * integerMap, IntegerMapNode * node)
{
    ...
    while (!(*slot)->isNil)
    {
        ...
        if (CONTAINER_OF(node, IntegerMapNode, header)->key < CONTAINER_OF(*slot, IntegerMapNode, header)->key)
            slot = &(*slot)->left;
        else
            slot = &(*slot)->right;
    }
