 ...
 * @param void
 */
LIBOS_SECTION_TEXT_COLD void KernelTimerInit(void)
{
...
{
    KernelTrace("Wakeup: Task=%d time=%llx\n", target->id, target->timestamp);
...
{
    KernelTrace("Wakeup-Cancel: Task=%d time=%llx\n", target->id, target->timestamp);
...
LIBOS_SECTION_IMEM_PINNED void KernelTimerArmInterrupt(LwU64 nextPreempt)
{
    ...
    KernelSbiTimerSet(LIBOS_CONFIG_TIME_NS(nextPreempt));
#else
...
LIBOS_SECTION_IMEM_PINNED LwU64 KernelTimerProcessElapsed(LwU64 mtime)
{
        ...
        header != &timeoutsPending;)
    {
        ...
        KernelTrace("Wakeup-Elapsed: Task=%d time=%llx\n", task->id, task->timestamp);

        // Port timeout will unlink the node
        ...
        header != &timersPending;)
    {
        ...
        while (!ListEmpty(&timer->port.waitingReceivers))
        {
            Shuttle *waiter = CONTAINER_OF(timer->port.waitingReceivers.next, Shuttle, portLink);
