-- Copyright (c) 2020 LWPU Corporation - All Rights Reserved
--
-- This source module contains confidential and proprietary information
-- of LWPU Corporation. It is not to be disclosed or used except
-- in accordance with applicable agreements. This copyright notice does
-- not evidence any actual or intended publication of such source code.

-- @summary
-- Defining intrinsic of LwUx types.

package Lw_Types_Intrinsic
with SPARK_Mode => On
is
   pragma Pure;

   type  LwU1_Wrap is mod 2 ** 1  with Size => 1;
   type  LwU2_Wrap is mod 2 ** 2  with Size => 2;
   type  LwU3_Wrap is mod 2 ** 3  with Size => 3;
   type  LwU4_Wrap is mod 2 ** 4  with Size => 4;
   type  LwU5_Wrap is mod 2 ** 5  with Size => 5;
   type  LwU6_Wrap is mod 2 ** 6  with Size => 6;
   type  LwU7_Wrap is mod 2 ** 7  with Size => 7;
   type  LwU8_Wrap is mod 2 ** 8  with Size => 8;
   type  LwU9_Wrap is mod 2 ** 9  with Size => 9;
   type LwU10_Wrap is mod 2 ** 10 with Size => 10;
   type LwU11_Wrap is mod 2 ** 11 with Size => 11;
   type LwU12_Wrap is mod 2 ** 12 with Size => 12;
   type LwU13_Wrap is mod 2 ** 13 with Size => 13;
   type LwU14_Wrap is mod 2 ** 14 with Size => 14;
   type LwU15_Wrap is mod 2 ** 15 with Size => 15;
   type LwU16_Wrap is mod 2 ** 16 with Size => 16;
   type LwU17_Wrap is mod 2 ** 17 with Size => 17;
   type LwU18_Wrap is mod 2 ** 18 with Size => 18;
   type LwU19_Wrap is mod 2 ** 19 with Size => 19;
   type LwU20_Wrap is mod 2 ** 20 with Size => 20;
   type LwU21_Wrap is mod 2 ** 21 with Size => 21;
   type LwU22_Wrap is mod 2 ** 22 with Size => 22;
   type LwU23_Wrap is mod 2 ** 23 with Size => 23;
   type LwU24_Wrap is mod 2 ** 24 with Size => 24;
   type LwU25_Wrap is mod 2 ** 25 with Size => 25;
   type LwU26_Wrap is mod 2 ** 26 with Size => 26;
   type LwU27_Wrap is mod 2 ** 27 with Size => 27;
   type LwU28_Wrap is mod 2 ** 28 with Size => 28;
   type LwU29_Wrap is mod 2 ** 29 with Size => 29;
   type LwU30_Wrap is mod 2 ** 30 with Size => 30;
   type LwU31_Wrap is mod 2 ** 31 with Size => 31;
   type LwU32_Wrap is mod 2 ** 32 with Size => 32;
   type LwU33_Wrap is mod 2 ** 33 with Size => 33;
   type LwU34_Wrap is mod 2 ** 34 with Size => 34;
   type LwU35_Wrap is mod 2 ** 35 with Size => 35;
   type LwU36_Wrap is mod 2 ** 36 with Size => 36;
   type LwU37_Wrap is mod 2 ** 37 with Size => 37;
   type LwU38_Wrap is mod 2 ** 38 with Size => 38;
   type LwU39_Wrap is mod 2 ** 39 with Size => 39;
   type LwU40_Wrap is mod 2 ** 40 with Size => 40;
   type LwU41_Wrap is mod 2 ** 41 with Size => 41;
   type LwU42_Wrap is mod 2 ** 42 with Size => 42;
   type LwU43_Wrap is mod 2 ** 43 with Size => 43;
   type LwU44_Wrap is mod 2 ** 44 with Size => 44;
   type LwU45_Wrap is mod 2 ** 45 with Size => 45;
   type LwU46_Wrap is mod 2 ** 46 with Size => 46;
   type LwU47_Wrap is mod 2 ** 47 with Size => 47;
   type LwU48_Wrap is mod 2 ** 48 with Size => 48;
   type LwU49_Wrap is mod 2 ** 49 with Size => 49;
   type LwU50_Wrap is mod 2 ** 50 with Size => 50;
   type LwU51_Wrap is mod 2 ** 51 with Size => 51;
   type LwU52_Wrap is mod 2 ** 52 with Size => 52;
   type LwU53_Wrap is mod 2 ** 53 with Size => 53;
   type LwU54_Wrap is mod 2 ** 54 with Size => 54;
   type LwU55_Wrap is mod 2 ** 55 with Size => 55;
   type LwU56_Wrap is mod 2 ** 56 with Size => 56;
   type LwU57_Wrap is mod 2 ** 57 with Size => 57;
   type LwU58_Wrap is mod 2 ** 58 with Size => 58;
   type LwU59_Wrap is mod 2 ** 59 with Size => 59;
   type LwU60_Wrap is mod 2 ** 60 with Size => 60;
   type LwU61_Wrap is mod 2 ** 61 with Size => 61;
   type LwU62_Wrap is mod 2 ** 62 with Size => 62;
   type LwU63_Wrap is mod 2 ** 63 with Size => 63;
   type LwU64_Wrap is mod 2 ** 64 with Size => 64;


end Lw_Types_Intrinsic;
