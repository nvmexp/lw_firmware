...
{
    init();
    char* hello = "Hellow rts world. You are running on ";
    char* buildTime = ", this binary was built on: ";
#if LWRISCV_HAS_PRI
#if !LWRISCV_PLATFORM_IS_CMOD
    uint32_t platform = priRead(LW_PTOP_PLATFORM);

    printf("%sLW_PTOP_PLATFORM=%x%s%s.\n", hello, platform, buildTime, __DATE__ " " __TIME__);
#else // LWRISCV_PLATFORM_IS_CMOD
    printf("%sCMOD%s%s.\n", hello, buildTime, __DATE__ " " __TIME__);
...
#endif
#else // LWRISCV_HAS_PRI
    printf("%sengine without PRI%s%s.\n", hello, buildTime, __DATE__ " " __TIME__);
#endif // LWRISCV_HAS_PRI


    // Execute unit tests for libRTS
   libRTS_RunUnitTests();
