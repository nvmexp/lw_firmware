...
int main(void)
{
    ...
    // Setting Priv Level 3
    csr_set(LW_RISCV_CSR_MRSP, DRF_NUM64(_RISCV, _CSR_MRSP, _MRPL, LW_RISCV_CSR_MRSP_MRPL_LEVEL3));

#if defined LWRISCV_IS_ENGINE_sec || defined LWRISCV_IS_ENGINE_gsp || defined LWRISCV_IS_ENGINE_fsp
    printf("\n\n***** ATOMIC ECDSA PRIMITIVE SIGNATURE GENERATION *****\n");
#if defined LWRISCV_IS_ENGINE_sec || defined LWRISCV_IS_ENGINE_gsp
    uint32_t keystoreOwner = priRead(LW_FUSE_OPT_SECHUB_LWPKA_KEYSTORE_OWNER);
    keystoreOwner = DRF_VAL(_FUSE, _OPT_SECHUB_LWPKA_KEYSTORE_OWNER, _DATA, keystoreOwner);
#if defined LWRISCV_IS_ENGINE_sec
    if(keystoreOwner == 0) {
#else
    if(keystoreOwner == 1) {
#endif
        verifyEcdsaSigningAtomicPrimitive();
    } else {
        printf("Engine is not the keystore owner for LWPKA ECC keystore. Skipping...\n");
    }
#else
    verifyEcdsaSigningAtomicPrimitive();
#endif // LWRISCV_IS_ENGINE_sec || LWRISCV_IS_ENGINE_gsp
#endif // LWRISCV_IS_ENGINE_sec || LWRISCV_IS_ENGINE_gsp || LWRISCV_IS_ENGINE_fsp

    // Clearing Priv Level 3 i.e. indirectly setting it back to Priv Level 0
    csr_clear(LW_RISCV_CSR_MRSP, DRF_NUM64(_RISCV, _CSR_MRSP, _MRPL, LW_RISCV_CSR_MRSP_MRPL_LEVEL3));
