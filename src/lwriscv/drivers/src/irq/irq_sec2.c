...
/*!
 * @file    irq_ext.c
 * @brief   Interrupt driver - SEC2 specific part
 *
 *  Most of this driver is actually platform independent, it's SEC2 only because
 *  we assume interrupt tree belongs to SEC2 (and parse it as such).
 */
...
irqInit(void)
{
    lwrtosTaskCriticalEnter();
    {
        ...
        irqMode = FLD_SET_DRF(_PSEC, _FALCON_IRQMODE, _LVL_MTHD,        _FALSE, irqMode); // MTHD
        irqMode = FLD_SET_DRF(_PSEC, _FALCON_IRQMODE, _LVL_CTXSW,       _FALSE, irqMode); // CTXSW
        irqMode = FLD_SET_DRF(_PSEC, _FALCON_IRQMODE, _LVL_SWGEN0,      _FALSE, irqMode);
        irqMode = FLD_SET_DRF(_PSEC, _FALCON_IRQMODE, _LVL_SWGEN1,      _FALSE, irqMode);
        irqMode = FLD_SET_DRF(_PSEC, _FALCON_IRQMODE, _LVL_EXT_EXTIRQ5, _FALSE, irqMode);
        irqMode = FLD_SET_DRF(_PSEC, _FALCON_IRQMODE, _LVL_EXT_EXTIRQ8, _FALSE, irqMode);
        irqMode = FLD_SET_DRF(_PSEC, _FALCON_IRQMODE, _LVL_MEMERR,      _TRUE,  irqMode);
        irqMode = FLD_SET_DRF(_PSEC, _FALCON_IRQMODE, _LVL_CTXSW_ERROR, _TRUE,  irqMode);
