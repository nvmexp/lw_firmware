...
_Static_assert(sizeof(__stack_chk_guard) <= STACK_CANARY_SIZE,
    "Stack canary exceeds DMEM carveout!");

__attribute__ ((noreturn))
void __stack_chk_fail(void)
{
    dbgExit(BOOTSTUB_ERROR_STACK_CORRUPTION);
}
...
static LwUPtr getTimeBasedRandomCanary(void)
{
    ...
    //
    // Mix with SSP_MAGIC_NUMBER because get64BitRandomNumber()
    // is not truly random.
    //
