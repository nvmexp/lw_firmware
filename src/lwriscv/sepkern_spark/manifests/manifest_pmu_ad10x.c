...
const PKC_VERIFICATION_MANIFEST manifest = {
    ...
    .mspm         = {
        ...
        .msecm = 0x1,                               // This allows secure SCPDMATRANSFER etc. writes. Needed due to SCP lockdown!
    },
    ...
    .corePmp = {
        .pmpcfg = {
            ...
            PMP_ENTRY(_DENIED, _DENIED, _PERMITTED, _NAPOT, _LOCK), // IMEM FMC, read/write blocked *MUST BE LOCKED*
    ...
    .numberOfValidPairs      = 32,  // 32 registers
    .registerPair            = {
        .addr    = {
            // SCP little-hammer lockdown
            LW_PRGNLCL_SCP_CTL_CFG,
            // ICD config
            ...
            //
            // PLMs that are accessed by RISC-V but can b fully disabled, because
            ...
            LW_PRGNLCL_FALCON_IRQTMR_PRIV_LEVEL_MASK,
        ...
        .andMask = {
            // SCP little-hammer lockdown
            0x0, // LW_PRGNLCL_SCP_CTL_CFG - see orMask
            // ICD setup
            ...
            // Non PLMs
            0x0,  // LW_PRGNLCL_FBIF_CTL2, has no other field
            0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,      // All the rest are PLMs and we program all fields
            0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, // 13-20
            0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, // 21-28
            0x0, 0x0, 0x0, // 29-31
        },
        .orMask  = {
            // SCP little-hammer lockdown
            DRF_DEF(_PRGNLCL, _SCP_CTL_CFG, _LOCKDOWN_SCP, _ENABLE),
            // ICD setup
            ...
            // PLMs that are programmed to fully disabled
            ...
            //
            // PLMs that are accessed by RISC-V but can be fully disabled
            // (RISC-V bypassess PLM checks for Peregrine)
            //
            PLM_ALL_DISABLE_WITH_REPORT_ERROR(FALCON_DIODT  ),
            PLM_ALL_DISABLE_WITH_REPORT_ERROR(FALCON_SHA_RAL),
            PLM_ALL_DISABLE_WITH_REPORT_ERROR(FALCON_TMR    ),
            PLM_ALL_DISABLE_WITH_REPORT_ERROR(FALCON_WDTMR  ),
            PLM_ALL_DISABLE_WITH_REPORT_ERROR(RISCV_MSIP    ),
            PLM_ALL_DISABLE_WITH_REPORT_ERROR(RISCV_LWCONFIG),

            //
            // For IMEM and DMEM we need to permit L0 access because DMA does PLM check
            // And fails if it has no access.
            //
            PLM_SET_SOURCE_WITH_REPORT_ERROR(FALCON_IMEM,       PLM_ALL_LEVELS, PLM_ALL_LEVELS, PLM_SOURCE_ID(PMU)),
            PLM_SET_SOURCE_WITH_REPORT_ERROR(FALCON_DMEM,       PLM_ALL_LEVELS, PLM_ALL_LEVELS, PLM_SOURCE_ID(PMU)),

            // PLMs with values
            PLM_SET_ALL_SOURCES_WITH_REPORT_ERROR(FBIF_REGIONCFG,   PLM_LEVEL3,     PLM_LEVEL3          ), // ACR needs it
            ...
            PLM_SET_ALL_SOURCES_WITH_REPORT_ERROR(FALCON_IRQSCMASK, PLM_ALL_LEVELS, PLM_LEVEL2_AND_ABOVE),
            PLM_SET_ALL_SOURCES_WITH_REPORT_ERROR(FALCON_IRQTMR,    PLM_ALL_LEVELS, PLM_LEVEL2_AND_ABOVE),
            ...
            PLM_SET_ALL_SOURCES_WITH_REPORT_ERROR(RISCV_IRQ,        PLM_ALL_LEVELS, PLM_LEVEL3          ),
        },
    },
};
