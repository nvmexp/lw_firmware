...
#
# Syslib-config.pl and other LWRISCV Libraries share one chip-config client.
...
if (!grep {$_ =~ m/--output-dir/} @ARGV) {
    ...
    my $outDir = '_out';
    if (not -e $outDir) {
        print "[syslib-config] creating output directory '$outDir'\n";
...
unshift @ARGV, "--config", "syslib-config.cfg";
