...
#
# sepkern-config.pl and other LWRISCV Libraries share one chip-config client.
...
BEGIN {
    ...
    my $cfgFilePath = (dirname __FILE__).'/../../config';
...
if (!grep {$_ =~ m/--output-dir/} @ARGV) {
    ...
    my $outDir = '_out';
    if (not -e $outDir) {
        print "[sepkern-config] creating output directory '$outDir'\n";
...
# "--mode riscvlib-config" must be the first argument
unshift @ARGV, "--config", "sepkern-config.cfg";
unshift @ARGV, "--mode", "riscvlib-config";
