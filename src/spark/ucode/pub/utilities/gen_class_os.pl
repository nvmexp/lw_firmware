#!perl -w
use Time::Local;
use constant OS_CODE => 1;
use constant SEC_CODE => 6;

#
# On Turing, around 30-35K is allocated for ACR phases in BSIRAM, check Bug 1809325 for latest
# Note that BSI_LOCK which runs on SEC2 takes twice the size to be stored in BSIRAM
# And after ACR_LOAD split, we run ASB on GSP which will take twice the space of binary size
# The below values are defined to limit the actual size of the binaries
#
use constant BSI_BOOT_ADDR_MARKER_GM20X_to_PASCAL => 0x1100;
use constant BSI_BOOT_ADDR_MARKER_TURING_and_later => 0x2000;
use constant BSI_LOCKDOWN_ADDR_MARKER_GM20X_to_PASCAL => 0xB00;
use constant BSI_LOCKDOWN_ADDR_MARKER_TURING_and_later => 0x1F00;

my $bin_file = "";
my $verbose = 0;
my $cheetah = 0;
my $hdr_file = "", my $dmp_file = "", my $path_vpm = "$ELW{'LW_TOOLS'}/falcon-gcc/falcon6/6.5.4/MinGW/falcon-elf-gcc/bin";
...
sub create_offsets_array
{
  ...
  for($cnt = 1; $cnt < 2; $cnt++)
  {
    my $app_size       = dec2hex(getsize_roundup("imem_pub"));
    if ($app_size > 0)
    {
  ...
  for($cnt = 1; $cnt < 2; $cnt++)
  {
    my $app_size       = dec2hex(getsize_roundup("data_pub"));
    if($app_size > 0)
    {
      @offsets_array = (@offsets_array, $app_offset, $app_size);
      $num_elems+=2;
...
sub getsize
{
  ...
  $cmd = "$path_vpm\\falcon-elf-readelf -S $bin_file |";
...
###############################################################################
# @brief Verifies the size of binary
#
# @param[in] Binary Name
# @param[in] Binary size
#
# @return Error if Binary size exceeds marker
###############################################################################
sub verify_size
{
    my ($bin_name, $bin_size) = @_;
    $bootSize = 0;
    $lockdownSize = 0;

    if ($bin_name =~ "gm20" || $bin_name =~ "gp10")
    {
        $bootSize = BSI_BOOT_ADDR_MARKER_GM20X_to_PASCAL;
        $lockdownSize = BSI_LOCKDOWN_ADDR_MARKER_GM20X_to_PASCAL;
    }
    else
    {
        $bootSize = BSI_BOOT_ADDR_MARKER_TURING_and_later;
        $lockdownSize = BSI_LOCKDOWN_ADDR_MARKER_TURING_and_later;
    }
    if ($bin_name =~ "bsi_boot")
    {
        print "Size of BSI_BOOT Binary is : \n";
        if($bin_size > $bootSize)
        {
            exit_error("size exceeded");
        }        
    }
    if ($bin_name =~ "bsi_lockdown")
    {
        print "Size of BSI_LOCKDOWN Binary is :  \n";
        if($bin_size > $lockdownSize)
        {
            exit_error("size exceeded");
        }        
    }
...
sub write_hdr
{
  ...
  print SRC_FILE " *"."\n";
  print SRC_FILE " * Copyright 2013-2018 by LWPU Corporation.  All rights reserved.  All"."\n";
  ...
  print SRC_FILE ""."\n";
  print SRC_FILE ""."\n";
  print SRC_FILE "LwU32 pub_ucode_data_".$chip."[] = {\n";
  ...
  print SRC_FILE "// \n";
  print SRC_FILE "// \n";
  print SRC_FILE " LwU32 pub_ucode_header_".$chip."[] = {\n";
  my $count;
  my $arrsize = @offsets_array;
# print("array size: $arrsize\n");
  verify_size($chip,  ($offsets_array[OS_CODE]+$offsets_array[SEC_CODE]));
  ...
  print SRC_FILE "\nLwU32 pub_ucode_data"."_size_".$chip." = $ucode_size_dwords;\n"; #ucode_size in DWORDS
