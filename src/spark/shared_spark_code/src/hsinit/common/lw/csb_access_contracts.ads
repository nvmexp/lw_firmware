...
package Csb_Access_Contracts
with SPARK_Mode => On,
  Ghost
is

   function Csb_Access_Post_Contract(Status : LW_UCODE_UNIFIED_ERROR_TYPE) return Boolean
   is
     (if Ghost_Csb_Pri_Error_State = CSB_PRI_ERROR_TRUE then
        ...
        with
          Global => (Input => Ghost_Csb_Pri_Error_State),
     Depends => ( Csb_Access_Post_Contract'Result => (Status,
                                                      Ghost_Csb_Pri_Error_State));          

   function Csb_Access_Pre_Contract(Status : LW_UCODE_UNIFIED_ERROR_TYPE) return Boolean
   is
     ...
     Depends => ( Csb_Access_Pre_Contract'Result => (Status,
                                                     Ghost_Csb_Pri_Error_State,
                                                     Ghost_Csb_Err_Reporting_State));

end Csb_Access_Contracts;
