...
// Tabs have been replaced with spaces. All functions have had their sections specified
// via GCC_ATTRIB_SECTION.
//
...
typedef struct {
    ...
    spdm_device_version_t selwred_message_version;
    //
    // My Certificate
    //
...
typedef struct {
    ...
    // Peer CertificateChain
    //
// LWE (tandrewprinz) LW_REFACTOR_CONTEXT: Remove unused buffers to save size.
...
//
// LWE (tandrewprinz) LW_REFACTOR_TRANSCRIPT: Create structs for each transcript
// message, so we can limit their size by exactly what is needed. In below
// spdm_transcript_t and spdm_session_transcript_t, use custom structs.
//
typedef struct {
    uintn max_buffer_size;
    uintn buffer_size;
    uint8 buffer[MAX_SPDM_MESSAGE_A_BUFFER_SIZE];
} managed_buffer_message_a_t;

typedef struct {
    uintn max_buffer_size;
    uintn buffer_size;
    uint8 buffer[MAX_SPDM_MESSAGE_B_BUFFER_SIZE];
} managed_buffer_message_b_t;

typedef struct {
    uintn max_buffer_size;
    uintn buffer_size;
    uint8 buffer[MAX_SPDM_MESSAGE_C_BUFFER_SIZE];
} managed_buffer_message_c_t;

typedef struct {
    uintn max_buffer_size;
    uintn buffer_size;
    uint8 buffer[MAX_SPDM_MESSAGE_M_BUFFER_SIZE];
} managed_buffer_message_m_t;

typedef struct {
    uintn max_buffer_size;
    uintn buffer_size;
    uint8 buffer[MAX_SPDM_MESSAGE_K_BUFFER_SIZE];
} managed_buffer_message_k_t;

typedef struct {
    uintn max_buffer_size;
    uintn buffer_size;
    uint8 buffer[MAX_SPDM_MESSAGE_F_BUFFER_SIZE];
} managed_buffer_message_f_t;
...
typedef struct {
    ...
    //
    managed_buffer_message_a_t message_a;
    managed_buffer_message_b_t message_b;
    managed_buffer_message_c_t message_c;
...
//  small_managed_buffer_t message_mut_c;
    ...
    //
    managed_buffer_message_m_t message_m;
} spdm_transcript_t;
...
typedef struct {
    ...
    //
    managed_buffer_message_k_t message_k;
    managed_buffer_message_f_t message_f;
    //
...
//
// LWE (tandrewprinz) LW_REFACTOR_CONTEXT LW_REMOVE_FP:
// Remove unused FP to save size and prevent accidental use.
//
//  //
//  // IO information
//  //
...
//  spdm_transport_decode_message_func transport_decode_message;
    ...
    //
    // Cache session_id in this spdm_message, only valid for selwred message.
    //
...
//
// LWE (tandrewprinz) LW_REFACTOR_CONTEXT LW_REMOVE_FP:
// Remove unused FP structs to prevent accidental use.
//
//  //
...
//
// LWE (tandrewprinz) LW_REFACTOR_CONTEXT: Remove unused structs to save size.
// We will not be using the message caches below, as we will not be supporting
// RESPOND_IF_READY message, given blocking properties of transport layer.
//
//  //
...
void internal_dump_hex_str(IN uint8 *data, IN uintn size)
    GCC_ATTRIB_SECTION("imem_libspdm", "internal_dump_hex_str");
...
void internal_dump_data(IN uint8 *data, IN uintn size)
    GCC_ATTRIB_SECTION("imem_libspdm", "internal_dump_data");
...
void internal_dump_hex(IN uint8 *data, IN uintn size)
    GCC_ATTRIB_SECTION("imem_libspdm", "internal_dump_hex");
...
return_status append_managed_buffer(IN OUT void *managed_buffer_t,
    ...
    GCC_ATTRIB_SECTION("imem_libspdm", "append_managed_buffer");
...
return_status shrink_managed_buffer(IN OUT void *managed_buffer_t,
    ...
    GCC_ATTRIB_SECTION("imem_libspdm", "shrink_managed_buffer");
...
void reset_managed_buffer(IN OUT void *managed_buffer_t)
    GCC_ATTRIB_SECTION("imem_libspdm", "reset_managed_buffer");
...
uintn get_managed_buffer_size(IN void *managed_buffer_t)
    GCC_ATTRIB_SECTION("imem_libspdm", "get_managed_buffer_size");
...
void *get_managed_buffer(IN void *managed_buffer_t)
    GCC_ATTRIB_SECTION("imem_libspdm", "get_managed_buffer");
...
void init_managed_buffer(IN OUT void *managed_buffer_t,
    ...
    GCC_ATTRIB_SECTION("imem_libspdm", "init_managed_buffer");
...
void spdm_session_info_init(IN spdm_context_t *spdm_context,
    ...
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_session_info_init");
...
uint16 spdm_allocate_req_session_id(IN spdm_context_t *spdm_context)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_allocate_req_session_id");
...
uint16 spdm_allocate_rsp_session_id(IN spdm_context_t *spdm_context)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_allocate_rsp_session_id");
...
boolean spdm_is_version_supported(IN spdm_context_t *spdm_context,
                  IN uint8 version)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_is_version_supported");
...
spdm_is_capabilities_flag_supported(IN spdm_context_t *spdm_context,
    ...
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_is_capabilities_flag_supported");

/*
  This function callwlates m1m2.
  ...
  @retval RETURN_SUCCESS  m1m2 is callwlated.
*/
boolean spdm_callwlate_m1m2(IN void *context, IN boolean is_mut,
                IN OUT uintn *m1m2_buffer_size,
                OUT void *m1m2_buffer)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_callwlate_m1m2");

/*
  This function callwlates l1l2.

  @param  spdm_context                  A pointer to the SPDM context.
  @param  l1l2_buffer_size               size in bytes of the l1l2
  @param  l1l2_buffer                   The buffer to store the l1l2

  @retval RETURN_SUCCESS  l1l2 is callwlated.
*/
boolean spdm_callwlate_l1l2(IN void *context, IN OUT uintn *l1l2_buffer_size,
                OUT void *l1l2_buffer)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_callwlate_l1l2");
...
boolean spdm_generate_cert_chain_hash(IN spdm_context_t *spdm_context,
    ...
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_generate_cert_chain_hash");
...
boolean spdm_verify_peer_digests(IN spdm_context_t *spdm_context,
    ...
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_verify_peer_digests");
...
boolean spdm_verify_peer_cert_chain_buffer(IN spdm_context_t *spdm_context,
    ...
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_verify_peer_cert_chain_buffer");
...
boolean spdm_generate_challenge_auth_signature(IN spdm_context_t *spdm_context,
                           IN boolean is_requester,
                           OUT uint8 *signature)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_generate_challenge_auth_signature");
...
spdm_verify_certificate_chain_hash(IN spdm_context_t *spdm_context,
    ...
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_verify_certificate_chain_hash");
...
boolean spdm_verify_challenge_auth_signature(IN spdm_context_t *spdm_context,
    ...
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_verify_challenge_auth_signature");

/**
  This function callwlate the measurement summary hash size.
...
spdm_get_measurement_summary_hash_size(IN spdm_context_t *spdm_context,
    ...
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_get_measurement_summary_hash_size");

/**
  This function callwlate the measurement summary hash.
...
spdm_generate_measurement_summary_hash(IN spdm_context_t *spdm_context,
    ...
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_generate_measurement_summary_hash");
...
boolean spdm_generate_measurement_signature(IN spdm_context_t *spdm_context,
                        OUT uint8 *signature)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_generate_measurement_signature");
...
boolean spdm_verify_measurement_signature(IN spdm_context_t *spdm_context,
                      IN void *sign_data,
                      IN uintn sign_data_size)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_verify_measurement_signature");
...
spdm_generate_key_exchange_rsp_signature(IN spdm_context_t *spdm_context,
                     IN spdm_session_info_t *session_info,
                     OUT uint8 *signature)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_generate_key_exchange_rsp_signature");
...
spdm_generate_key_exchange_rsp_hmac(IN spdm_context_t *spdm_context,
                    IN spdm_session_info_t *session_info,
                    OUT uint8 *hmac)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_generate_key_exchange_rsp_hmac");
...
boolean spdm_verify_key_exchange_rsp_signature(
    ...
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_verify_key_exchange_rsp_signature");
...
boolean spdm_verify_key_exchange_rsp_hmac(IN spdm_context_t *spdm_context,
    ...
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_verify_key_exchange_rsp_hmac");
...
boolean spdm_generate_finish_req_signature(IN spdm_context_t *spdm_context,
                       IN spdm_session_info_t *session_info,
                       OUT uint8 *signature)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_generate_finish_req_signature");
...
boolean spdm_generate_finish_req_hmac(IN spdm_context_t *spdm_context,
                      IN spdm_session_info_t *session_info,
                      OUT void *hmac)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_generate_finish_req_hmac");
...
boolean spdm_verify_finish_req_signature(IN spdm_context_t *spdm_context,
    ...
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_verify_finish_req_signature");
...
boolean spdm_verify_finish_req_hmac(IN spdm_context_t *spdm_context,
    ...
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_verify_finish_req_hmac");
...
boolean spdm_generate_finish_rsp_hmac(IN spdm_context_t *spdm_context,
                      IN spdm_session_info_t *session_info,
                      OUT uint8 *hmac)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_generate_finish_rsp_hmac");
...
boolean spdm_verify_finish_rsp_hmac(IN spdm_context_t *spdm_context,
                    ...
                    IN void *hmac_data,
                    IN uintn hmac_data_size)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_verify_finish_rsp_hmac");
...
spdm_generate_psk_exchange_rsp_hmac(IN spdm_context_t *spdm_context,
                    IN spdm_session_info_t *session_info,
                    OUT uint8 *hmac)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_generate_psk_exchange_rsp_hmac");
...
boolean spdm_verify_psk_exchange_rsp_hmac(IN spdm_context_t *spdm_context,
                      ...
                      IN void *hmac_data,
                      IN uintn hmac_data_size)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_verify_psk_exchange_rsp_hmac");
...
spdm_generate_psk_exchange_req_hmac(IN spdm_context_t *spdm_context,
                    IN spdm_session_info_t *session_info,
                    OUT void *hmac)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_generate_psk_exchange_req_hmac");
...
boolean spdm_verify_psk_finish_req_hmac(IN spdm_context_t *spdm_context,
                    IN spdm_session_info_t *session_info,
                    IN uint8 *hmac, IN uintn hmac_size)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_verify_psk_finish_req_hmac");
...
uintn spdm_get_opaque_data_supported_version_data_size(
    ...
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_get_opaque_data_supported_version_data_size");
...
spdm_build_opaque_data_supported_version_data(IN spdm_context_t *spdm_context,
                          ...
                          OUT void *data_out)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_build_opaque_data_supported_version_data");
...
spdm_process_opaque_data_version_selection_data(IN spdm_context_t *spdm_context,
                        ...
                        IN void *data_in)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_process_opaque_data_version_selection_data");
...
uintn spdm_get_opaque_data_version_selection_data_size(
    IN spdm_context_t *spdm_context)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_get_opaque_data_version_selection_data_size");
...
spdm_build_opaque_data_version_selection_data(IN spdm_context_t *spdm_context,
                          IN OUT uintn *data_out_size,
                          OUT void *data_out)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_build_opaque_data_version_selection_data");
...
spdm_process_opaque_data_supported_version_data(IN spdm_context_t *spdm_context,
                        IN uintn data_in_size,
                        IN void *data_in)
    GCC_ATTRIB_SECTION("imem_libspdm", "spdm_process_opaque_data_supported_version_data");
