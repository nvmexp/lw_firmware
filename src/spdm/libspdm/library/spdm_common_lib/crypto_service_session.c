...
//
// LWE (utesic): Including LWPU copyright, as file contains LWPU modifications.
//
	...
	OPTIONAL IN OUT uintn *th_data_buffer_size, OUT void *th_data_buffer)
{
	...
	large_managed_buffer_t th_lwrr;
	...
	init_managed_buffer(&th_lwrr, MAX_SPDM_MESSAGE_BUFFER_SIZE);
	...
	if (cert_chain_data != NULL) {
		...
		status = append_managed_buffer(&th_lwrr, cert_chain_data_hash,
					       hash_size);
		if (RETURN_ERROR(status)) {
			return FALSE;
		}
	...
	*th_data_buffer_size = get_managed_buffer_size(&th_lwrr);
	copy_mem(th_data_buffer, get_managed_buffer(&th_lwrr),
		 *th_data_buffer_size);
...
/*
  ...
  @retval RETURN_SUCCESS  current TH data is callwlated.
*/
boolean spdm_callwlate_th_for_finish(IN void *context,
				     ...
				     OUT void *th_data_buffer)
{
	...
	if (mut_cert_chain_data != NULL) {
		...
		status = append_managed_buffer(&th_lwrr, MutCertChainDataHash,
					       hash_size);
		if (RETURN_ERROR(status)) {
			return FALSE;
		}
					 ...
					 OUT uint8 *signature)
{
	...
	uint32 hash_size;
	uint8 th_lwrr_data[MAX_SPDM_MESSAGE_BUFFER_SIZE];
	uintn th_lwrr_data_size;
	...
	th_lwrr_data_size = sizeof(th_lwrr_data);
	result = spdm_callwlate_th_for_exchange(
		spdm_context, session_info, cert_chain_data,
		cert_chain_data_size, &th_lwrr_data_size, th_lwrr_data);
	if (!result) {
		return FALSE;
	}
	...
	spdm_hash_all(spdm_context->connection_info.algorithm.bash_hash_algo,
		      th_lwrr_data, th_lwrr_data_size, hash_data);
	DEBUG((DEBUG_INFO, "th_lwrr hash - "));
...
// LWE (utesic) LW_MULTIPLE_CERTS: Add support for multiple signing keys
		...
		th_lwrr_data, th_lwrr_data_size, signature, &signature_size);
	if (result) {
				    ...
				    OUT uint8 *hmac)
{
	...
	spdm_hmac_all_with_response_finished_key(
		session_info->selwred_message_context, th_lwrr_data,
		th_lwrr_data_size, hmac_data);
	DEBUG((DEBUG_INFO, "th_lwrr hmac - "));
	...
	IN void *sign_data, IN intn sign_data_size)
{
	...
	result = spdm_asym_verify(
		...
		th_lwrr_data, th_lwrr_data_size, sign_data, sign_data_size);
					  ...
					  IN uintn hmac_data_size)
{
	...
	spdm_hmac_all_with_response_finished_key(
		session_info->selwred_message_context, th_lwrr_data,
		th_lwrr_data_size, calc_hmac_data);
	DEBUG((DEBUG_INFO, "th_lwrr hmac - "));
					   ...
					   OUT uint8 *signature)
{
	...
	th_lwrr_data_size = sizeof(th_lwrr_data);
	result = spdm_callwlate_th_for_finish(
		spdm_context, session_info, cert_chain_data,
		cert_chain_data_size, mut_cert_chain_data,
		mut_cert_chain_data_size, &th_lwrr_data_size, th_lwrr_data);
	if (!result) {
		return FALSE;
	}
				    ...
				    OUT uint8 *hmac)
{
	...
	th_lwrr_data_size = sizeof(th_lwrr_data);
	result = spdm_callwlate_th_for_exchange(spdm_context, session_info,
						NULL, 0, &th_lwrr_data_size,
						th_lwrr_data);
	if (!result) {
		return FALSE;
	}
				    ...
				    OUT void *hmac)
{
	...
	th_lwrr_data_size = sizeof(th_lwrr_data);
	result = spdm_callwlate_th_for_finish(spdm_context, session_info, NULL,
					      0, NULL, 0, &th_lwrr_data_size,
					      th_lwrr_data);
	if (!result) {
		return FALSE;
	}
					...
					IN uint8 *hmac, IN uintn hmac_size)
{
	...
	spdm_hmac_all_with_request_finished_key(
		session_info->selwred_message_context, th_lwrr_data,
		th_lwrr_data_size, hmac_data);
	DEBUG((DEBUG_INFO, "Calc th_lwrr hmac - "));
	internal_dump_data(hmac_data, hash_size);
	DEBUG((DEBUG_INFO, "\n"));
...
*/
return_status spdm_callwlate_th1_hash(IN void *context,
				      IN void *spdm_session_info,
				      ...
				      OUT uint8 *th1_hash_data)
{
	...
	spdm_hash_all(spdm_context->connection_info.algorithm.bash_hash_algo,
		      th_lwrr_data, th_lwrr_data_size, th1_hash_data);
...
*/
return_status spdm_callwlate_th2_hash(IN void *context,
				      IN void *spdm_session_info,
				      IN boolean is_requester,
				      OUT uint8 *th2_hash_data)
{
	...
	spdm_hash_all(spdm_context->connection_info.algorithm.bash_hash_algo,
		      th_lwrr_data, th_lwrr_data_size, th2_hash_data);
