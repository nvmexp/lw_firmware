...
{
//
// LWE (tandrewprinz) LW_REFACTOR_CONTEXT: Remove functionality, as we have
// removed all usage of the peer cert chain buffer to save size.
//
    return FALSE;
//
                ...
                OUT void *m1m2_buffer)
{
    ...
    if (is_mut) {
        return FALSE;
//
// LWE (tandrewprinz) LW_REFACTOR_CONTEXT: Remove functionality, we do not
// expect to support mutual authentication, and have therefore removed the
// corresponding message transcripts to save space.
//
                           ...
                           OUT uint8 *signature)
{
    ...
    result = spdm_callwlate_m1m2(spdm_context, is_requester,
                        ...
                        OUT uint8 *signature)
{
    ...
    result = spdm_callwlate_l1l2(spdm_context, &l1l2_buffer_size,
                     l1l2_buffer);
    if (!result) {
        return FALSE;
    }
