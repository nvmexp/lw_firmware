...
boolean need_session_info_for_data(IN spdm_data_type_t data_type)
{
    ...
    case SPDM_DATA_SESSION_END_SESSION_ATTRIBUTES:
        return TRUE;
    // LWE (tandrewprinz): Compiler requires default case for enum.
    default:
        break;
    }
    return FALSE;
                ...
                IN uintn data_size)
{
    ...
    // LWE (tandrewprinz): Compiler requests default value for session_info.
    session_info = NULL;
    ...
    case SPDM_DATA_SPDM_VERSION:
        ...
        break;
    case SPDM_DATA_SELWRED_MESSAGE_VERSION:
        if (data_size >
            sizeof(spdm_version_number_t) * MAX_SPDM_VERSION_COUNT) {
            return RETURN_ILWALID_PARAMETER;
        }
        if (parameter->location == SPDM_DATA_LOCATION_CONNECTION) {
            spdm_context->connection_info.selwred_message_version
                .spdm_version_count = (uint8)(
                data_size / sizeof(spdm_version_number_t));
            copy_mem(spdm_context->connection_info
                     .selwred_message_version.spdm_version,
                 data,
                 spdm_context->connection_info
                         .selwred_message_version
                         .spdm_version_count *
                     sizeof(spdm_version_number_t));
        } else {
            spdm_context->local_context.selwred_message_version
                .spdm_version_count = (uint8)(
                data_size / sizeof(spdm_version_number_t));
...
// LWE (tandrewprinz) LW_COVERITY: Adding enum cast for type correctness.
        spdm_context->connection_info.connection_state =
            (spdm_connection_state_t)(*(uint32 *)data);
        break;
...
// LWE (tandrewprinz) LW_COVERITY: Adding enum cast for type correctness.
        spdm_context->response_state =
            (spdm_response_state_t)(*(uint32 *)data);
        break;
    ...
    case SPDM_DATA_LOCAL_USED_CERT_CHAIN_BUFFER:
        ...
        spdm_context->connection_info.local_used_cert_chain_buffer =
            data;
        break;
// LWE (tandrewprinz) LW_REFACTOR_CONTEXT: Remove usage of unneeded structs.
...
//
// LWE (tandrewprinz) LW_REFACTOR_CONTEXT: Remove below functions, as in order
// to save space, the associated transcript buffers should not be used.
//
//  /**
...
// Remove unused functions to register FP to prevent accidental use.
//
//  /**
...
void spdm_init_context(IN void *context)
{
    ...
    uintn SelwredMessageContextSize;
    uintn index;
...
//
// LWE (tandrewprinz) LW_REFACTOR_TRANSCRIPT:
// Initialize transcript buffers to exact size, rather than compiler constant.
//
...
//
// LWE (tandrewprinz) LW_REFACTOR_CONTEXT:
// Remove mut_b and mut_c buffers, as they are unused.
//
...
// LWE (tandrewprinz) LW_REFACTOR_CONTEXT: Remove unused RESPOND_IF_READY structs.
//  spdm_context->retry_times = MAX_SPDM_REQUEST_RETRY_TIMES;
//  spdm_context->lwrrent_token = 0;
    ...
    spdm_context->local_context.version.spdm_version[1]
        .update_version_number = 0;
    spdm_context->local_context.selwred_message_version.spdm_version_count =
        1;
    spdm_context->local_context.selwred_message_version.spdm_version[0]
        .major_version = 1;
    spdm_context->local_context.selwred_message_version.spdm_version[0]
        .minor_version = 1;
    spdm_context->local_context.selwred_message_version.spdm_version[0]
    ...
    spdm_context->local_context.selwred_message_version.spdm_version[0]
        .update_version_number = 0;
// LWE (tandrewprinz) LW_REFACTOR_CONTEXT: Remove unused buffer.
//  spdm_context->encap_context.certificate_chain_buffer.max_buffer_size =
//      MAX_SPDM_MESSAGE_BUFFER_SIZE;

    selwred_message_context = (void *)((uintn)(spdm_context + 1));
    SelwredMessageContextSize = spdm_selwred_message_get_context_size();
    for (index = 0; index < MAX_SPDM_SESSION_COUNT; index++) {
        spdm_context->session_info[index].selwred_message_context =
            (void *)((uintn)selwred_message_context +
                 SelwredMessageContextSize * index);
        spdm_selwred_message_init_context(
            spdm_context->session_info[index]
                .selwred_message_context);
    }
...
uintn spdm_get_context_size(void)
{
    return sizeof(spdm_context_t) +
           spdm_selwred_message_get_context_size() * MAX_SPDM_SESSION_COUNT;
}
