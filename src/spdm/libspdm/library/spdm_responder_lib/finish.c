...
// LWE (eddichang): Including LWPU copyright, as file contains LWPU modifications.
				       ...
				       OUT void *response)
{
	...
	if (!spdm_is_capabilities_flag_supported(
		...
		if (!spdm_context->last_spdm_request_session_id_valid) {
			spdm_generate_error_response(
				context, SPDM_ERROR_CODE_ILWALID_REQUEST, 0,
				response_size, response);
			return RETURN_SUCCESS;
		}
	} else {
...
//
// LWE (eddichang): Need to implement HMAC functions. Lwrrently, just make FINISH command pass.
// GSP-CC CONFCOMP-575 to track
//
#ifndef GSP_CC
	...
	*response_size = sizeof(spdm_finish_response_t) + hmac_size;
	...
	status = spdm_callwlate_th2_hash(spdm_context, session_info, FALSE,
					 th2_hash_data);
	if (RETURN_ERROR(status)) {
		spdm_generate_error_response(spdm_context,
					     SPDM_ERROR_CODE_ILWALID_REQUEST, 0,
	...
	status = spdm_generate_session_data_key(
		session_info->selwred_message_context, th2_hash_data);
	if (RETURN_ERROR(status)) {
		spdm_generate_error_response(spdm_context,
					     SPDM_ERROR_CODE_ILWALID_REQUEST, 0,
					     response_size, response);
