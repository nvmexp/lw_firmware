...
//
// LWE (tandrewprinz) LW_COVERITY: This check will never be true.
// Replace with local check for specific slot id.
//
...
// LWE (tandrewprinz) LW_COVERITY: Replacement check for specific slot id.
	if (spdm_context->local_context.local_cert_chain_provision[slot_id]
		== NULL) {
		spdm_generate_error_response(
			spdm_context, SPDM_ERROR_CODE_UNSUPPORTED_REQUEST,
			SPDM_GET_CERTIFICATE, response_size, response);
