...
void spdm_encap_move_to_next_op_code(IN spdm_context_t *spdm_context)
{
	...
	if (spdm_context->encap_context.lwrrent_request_op_code == 0) {
		spdm_context->encap_context.lwrrent_request_op_code =
	...
	for (index = 0;
	     ...
	     index++) {
		if (spdm_context->encap_context.lwrrent_request_op_code ==
	...
	OUT void *encap_request)
{
	...
	if (spdm_context->encap_context.lwrrent_request_op_code == 0) {
		...
		*encap_request_size = 0;
		spdm_context->encap_context.lwrrent_request_op_code = 0;
				    ...
				    IN uint8 mut_auth_requested)
{
	...
	spdm_context->encap_context.lwrrent_request_op_code = 0x00;
	...
	IN OUT uintn *response_size, OUT void *response)
{
	...
	if (RETURN_ERROR(status)) {
		spdm_generate_error_response(
			spdm_context, SPDM_ERROR_CODE_ILWALID_RESPONSE_CODE, 0,
			response_size, response);
