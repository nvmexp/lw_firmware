...
// LWE (tandrewprinz): Specify DMEM section of buffer.
GLOBAL_REMOVE_IF_UNREFERENCED uint8 m_zero_filled_buffer[64]
	GCC_ATTRIB_SECTION("dmem_spdm", "m_zero_filled_buffer");
	...
	IN uint8 *major_secret, OUT uint8 *key, OUT uint8 *iv)
{
	...
	hash_size = selwred_message_context->hash_size;
	key_length = selwred_message_context->aead_key_size;
	iv_length = selwred_message_context->aead_iv_size;
	...
	ret_val = spdm_hkdf_expand(selwred_message_context->bash_hash_algo,
				    ...
				    IN uint8 *th1_hash_data)
{
	...
	if (selwred_message_context->use_psk) {
		// No handshake_secret generation for PSK.
	} else {
		DEBUG((DEBUG_INFO, "[DHE Secret]: "));
		internal_dump_hex_str(
			selwred_message_context->master_secret.dhe_secret,
			selwred_message_context->dhe_key_size);
		DEBUG((DEBUG_INFO, "\n"));
		ret_val = spdm_hmac_all(
			selwred_message_context->bash_hash_algo,
			m_zero_filled_buffer, hash_size,
			selwred_message_context->master_secret.dhe_secret,
			selwred_message_context->dhe_key_size,
			selwred_message_context->master_secret.handshake_secret);
		ASSERT(ret_val);
	...
	if (selwred_message_context->use_psk) {
		ret_val = spdm_psk_handshake_secret_hkdf_expand(
			selwred_message_context->bash_hash_algo,
			selwred_message_context->psk_hint,
			selwred_message_context->psk_hint_size, bin_str1,
			bin_str1_size,
	...
	internal_dump_data(selwred_message_context->handshake_secret
				   .request_handshake_secret,
			   hash_size);
	DEBUG((DEBUG_INFO, "\n"));
	...
	if (selwred_message_context->use_psk) {
		ret_val = spdm_psk_handshake_secret_hkdf_expand(
			selwred_message_context->bash_hash_algo,
			selwred_message_context->psk_hint,
			selwred_message_context->psk_hint_size, bin_str2,
			bin_str2_size,
			selwred_message_context->handshake_secret
	...
	spdm_generate_finished_key(
		...
		selwred_message_context->handshake_secret
			.request_handshake_secret,
		selwred_message_context->handshake_secret.request_finished_key);

	spdm_generate_finished_key(
		selwred_message_context,
		selwred_message_context->handshake_secret
			.response_handshake_secret,
		selwred_message_context->handshake_secret.response_finished_key);

	spdm_generate_aead_key_and_iv(selwred_message_context,
				      selwred_message_context->handshake_secret
					      .request_handshake_secret,
				      selwred_message_context->handshake_secret
			       ...
			       IN uint8 *th2_hash_data)
{
	...
	if (selwred_message_context->use_psk) {
		...
		ret_val = spdm_hmac_all(
			...
			selwred_message_context->master_secret.master_secret);
		ASSERT(ret_val);
	...
	if (selwred_message_context->use_psk) {
		ret_val = spdm_psk_master_secret_hkdf_expand(
			selwred_message_context->bash_hash_algo,
			selwred_message_context->psk_hint,
			selwred_message_context->psk_hint_size, bin_str3,
			bin_str3_size,
	...
	internal_dump_data(
		selwred_message_context->application_secret.request_data_secret,
		hash_size);
	DEBUG((DEBUG_INFO, "\n"));
	...
	if (selwred_message_context->use_psk) {
		ret_val = spdm_psk_master_secret_hkdf_expand(
			selwred_message_context->bash_hash_algo,
			selwred_message_context->psk_hint,
			selwred_message_context->psk_hint_size, bin_str4,
			bin_str4_size,
			selwred_message_context->application_secret
	...
	internal_dump_data(
		selwred_message_context->application_secret.response_data_secret,
		hash_size);
	DEBUG((DEBUG_INFO, "\n"));
	...
	if (selwred_message_context->use_psk) {
		ret_val = spdm_psk_master_secret_hkdf_expand(
			selwred_message_context->bash_hash_algo,
			selwred_message_context->psk_hint,
			selwred_message_context->psk_hint_size, bin_str8,
			bin_str8_size,
			selwred_message_context->handshake_secret
				    ...
				    IN spdm_key_update_action_t action)
{
	...
	if ((action & SPDM_KEY_UPDATE_ACTION_REQUESTER) != 0) {
		copy_mem(&selwred_message_context->application_secret_backup
				  .request_data_secret,
			 &selwred_message_context->application_secret
				  .request_data_secret,
			 MAX_HASH_SIZE);
		...
		copy_mem(&selwred_message_context->application_secret_backup
			 ...
			 &selwred_message_context->application_secret
				  .request_data_salt,
			 MAX_AEAD_IV_SIZE);
		selwred_message_context->application_secret_backup
		...
		DEBUG((DEBUG_INFO, "RequestDataSecretUpdate (0x%x) - ",
		       hash_size));
		internal_dump_data(selwred_message_context->application_secret
					   .request_data_secret,
				   hash_size);
		DEBUG((DEBUG_INFO, "\n"));
				      ...
				      IN boolean use_new_key)
{
	...
	if ((action & SPDM_KEY_UPDATE_ACTION_REQUESTER) != 0) {
		zero_mem(&selwred_message_context->application_secret_backup
				  .request_data_secret,
			 MAX_HASH_SIZE);
		zero_mem(&selwred_message_context->application_secret_backup
				  .request_data_encryption_key,
					...
					OUT uint8 *hmac_value)
{
	...
	return spdm_hmac_all(
		selwred_message_context->bash_hash_algo, data, data_size,
		selwred_message_context->handshake_secret.request_finished_key,
		selwred_message_context->hash_size, hmac_value);
}
