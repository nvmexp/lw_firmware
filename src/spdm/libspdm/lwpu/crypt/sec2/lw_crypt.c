...
// libspdm (library/spdm_crypt_lib/crypt.c), but heavily modified by LWPU.
//

/*!
 * @file   lw_crypt.c
 * @brief  Fork of crypt.c from libspdm. Provides hooks to Falcon crypto
 *         implementations and primitives. Only included those we expect
 *         to support, other crypto functions have been removed or are unwired.
 */
...
)
{
    ...
    {
        case SPDM_ALGORITHMS_BASE_HASH_ALGO_TPM_ALG_SHA_256:
            return hkdf_sha256_expand(prk, prk_size, info, info_size, out,
                                      out_size);
        default:
            break;
    }
...
{
    // We don't expect responder to ever have to verify anything
    return FALSE;
}
...
{
    LwU8  messageHash[MAX_HASH_SIZE];
    LwU32 hashSize;
    LwU32 hashNid;

    hashNid = get_spdm_hash_nid(bash_hash_algo);

    switch (base_asym_algo)
    {
        case SPDM_ALGORITHMS_BASE_ASYM_ALGO_TPM_ALG_ECDSA_ECC_NIST_P256:
        {
            hashSize = spdm_get_hash_size(bash_hash_algo);
            if (!spdm_hash_all(bash_hash_algo, message, message_size,
                               messageHash))
            {
                return FALSE;
            }
            return ecdsa_sign(context, hashNid, messageHash, hashSize,
                              signature, sig_size);
        }
        default:
            break;
...
)
{
    ...
    {
        case SPDM_ALGORITHMS_DHE_NAMED_GROUP_SECP_256_R1:
            return ec_new_by_nid(CRYPTO_NID_SECP256R1);
        default:
            break;
    }
...
)
{
    ...
    {
        case SPDM_ALGORITHMS_AEAD_CIPHER_SUITE_AES_128_CTR_HMAC_SHA_256:
            return 16;
...
)
{
    ...
    {
        case SPDM_ALGORITHMS_AEAD_CIPHER_SUITE_AES_128_CTR_HMAC_SHA_256:
            return aead_aes_ctr_hmac_sha_encrypt(key, key_size, iv, iv_size,
...
)
{
    ...
    {
        case SPDM_ALGORITHMS_AEAD_CIPHER_SUITE_AES_128_CTR_HMAC_SHA_256:
            return aead_aes_ctr_hmac_sha_decrypt(key, key_size, iv, iv_size,
                                                 a_data, a_data_size, data_in,
                                                 data_in_size, tag, tag_size,
                                                 data_out, data_out_size);
        default:
            break;
    }
