...
sub setup_client_info
{
    ...
    my %client_info = (
        # General client Info
        NAME            => 'ACR',
        CONFIG_FILE     => $self->{LW_ROOT}.'/uproc/acr_riscv/build/mcheck/mcheck.config',
        ...
        RMCFG_SRC_PATH  => $self->{LW_ROOT}.'/uproc_riscv/acr/src',
        RMCFG_ARGS      => '--mode acr-config '.
                           '--config '.$self->{LW_ROOT}.'/uproc_riscv/acr/config/acr-config.cfg ',
