 ...
 */
static LwU8 pmuMutexPhysicalToLogical_GP100[] =
{
 ...
 */
static LwU8 pmgrMutexPhysicalToLogical_GP100[] =
{
 ...
 */
void
mutexEstablishMapping_GP100(void)
{
    LwU8 i;
    LwU8 j;
    LwBool bFind;
    ...
    {
        bFind = LW_FALSE;
        for (j = 0; ((j < sizeof(pmuMutexPhysicalToLogical_GP100)) && !bFind); j++)
        {
            if (pmuMutexPhysicalToLogical_GP100[j] == i)
            {
        ...
        for (j = 0; ((j < sizeof(pmgrMutexPhysicalToLogical_GP100)) && !bFind); j++)
        {
            if (pmgrMutexPhysicalToLogical_GP100[j] == i)
            {
