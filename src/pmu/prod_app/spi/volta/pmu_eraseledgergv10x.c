...
/*!
 * @file   pmu_eraseledger.c
 * @brief  Container-object for PMU SPI ROM Erase Ledger routines.
 *
 * This file contains Erase ledger routines.
 * @ref https://confluence.lwpu.com/display/CSSRM/Erase+Ledger+Design
 */
...
/* ------------------------- Static Variables ------------------------------- */
/* ------------------------- Global Variables ------------------------------- */
ERASE_LEDGER eraseLedger
    GCC_ATTRIB_SECTION("dmem_spi", "eraseLedger");

/* ------------------------- Macros and Defines ----------------------------- */
/* ------------------------- Prototypes ------------------------------------- */
static FLCN_STATUS s_spiRomInitLedgerSectors(SPI_DEVICE_ROM *pSpiRom);
static FLCN_STATUS s_spiRomInitLedgerDataSectors(SPI_DEVICE_ROM *pSpiRom,
    ERASE_LEDGER_SECTOR *pLedgerSector);
static FLCN_STATUS s_spiRomInitLedgerSession(SPI_DEVICE_ROM *pSpiRom,
    ERASE_LEDGER_SECTOR *pLedgerSector, ERASE_LEDGER_DATA_SECTOR *pSector,
    LwU32 dataSectorSize);
static FLCN_STATUS s_spiRomFindTallyBitOffset(SPI_DEVICE_ROM *pSpiRom,
    LwU32 sessionStartOffset, LwU32 sessionEndOffset, LwU32 *pTallyBitOffset);
static FLCN_STATUS s_spiRomGetLedgerSector(SPI_DEVICE_ROM *pSpiRom,
    LwU32 eraseAddress, ERASE_LEDGER_SECTOR **ppLedgerSector);
static FLCN_STATUS s_spiRomCopyLedgerSector(SPI_DEVICE_ROM *pSpiRom,
    ERASE_LEDGER_SECTOR *pSrc, ERASE_LEDGER_SECTOR *pDest, LwU32 eraseAddress);

/* ------------------------- Public Functions ------------------------------- */
/*!
 * Initializes all of Erase Ledger state from EEPROM.
 *
 * This function reads Erase Ledger Image header from EEPROM, validates the
 * ledger image sign and initializes all erase ledger structures.
 *
...
)
{
    ...
    ERASE_LEDGER_REGION    *pLedgerRegion = NULL;
        ...
        (!LW_ROM_REGION_INITIALIZED(pSpiRom->pLedgerRegion)))
    {
        ...
        goto spiRomInitEraseLedger_GV10X_exit;
    }
    ...
    status = spiRomReadDirect_HAL(&Spi, pSpiRom, headerOffset,
...
spiRomInitEraseLedger_GV10X_exit:
    if (status != FLCN_OK)
    {
 ...
 *
 * @return LW_TRUE   If data sector erases has reached the limit.
 * @return LW_FALSE  If data sector erases has not reached the limit.
 */
LwBool
spiRomIsEraseLimitReached_GV10X
(
    SPI_DEVICE_ROM    *pSpiRom,
    LwU32              eraseAddress 
)
{ 
    ERASE_LEDGER_DATA_SECTOR   *pDataSector;
    ERASE_LEDGER_SECTOR        *pLedgerSector;
    LwU32                       dataSectorAddr;
    LwU32                       lifetimeEraseCount;

    if (FLCN_OK != s_spiRomGetLedgerSector(pSpiRom, eraseAddress, &pLedgerSector))
    {
        return LW_FALSE;
    }
    ...
    SPI_FOR_EACH_SECTOR(pLedgerSector, pDataSector)
    {
 ...
 * @param[in]   eraseAddress            Starting address of tracked data Sector.
 *
 * @return FLCN_ERR_ILWALID_STATE       If accessed an invalid pointer.
 * @return FLCN_OK                      If tacked data sector erases were tallied
                                        successfully.
 * @return other                        Propagates return values from other calls.
 */
FLCN_STATUS
spiRomEraseTally_GV10X
(
    SPI_DEVICE_ROM    *pSpiRom,
    LwU32              eraseAddress 
)
{
    ERASE_LEDGER_DATA_SECTOR   *pDataSector;
    ERASE_LEDGER_SECTOR        *pLedgerSector;
    ERASE_LEDGER_SECTOR        *pSrcSector;
    ERASE_LEDGER_SECTOR        *pDestSector;
    ERASE_LEDGER               *pEL = &eraseLedger;
    ...
    status = s_spiRomGetLedgerSector(pSpiRom, eraseAddress, &pLedgerSector);
    if (status != FLCN_OK)
    {
        return status;
    }
    ...
    SPI_FOR_EACH_SECTOR(pEL, pLedgerSector)
    {
...
{
    ERASE_LEDGER           *pEraseLedger = &eraseLedger;
    ERASE_LEDGER_SECTOR    *pSector = NULL;
    ERASE_LEDGER_SECTOR    *pPrev   = NULL;
    ...
    for (i = 0; i < (pEraseLedger->header.ledgerImageSize); i++)
    {
        ...
        pSector = lwosCallocType(OVL_INDEX_DMEM(spi), 1, ERASE_LEDGER_SECTOR);
        if (pSector == NULL)
        {
            PMU_BREAKPOINT();
        ...
        status = spiRomReadDirect_HAL(&Spi, pSpiRom, pSector->ledgerSectorOffset,
        ...
        if (pEraseLedger->pSectorHead == NULL)
        {
...
{
    ERASE_LEDGER                *pEraseLedger = &eraseLedger;
    ERASE_LEDGER_DATA_SECTOR    *pSector = NULL;
    ERASE_LEDGER_DATA_SECTOR    *pPrev   = NULL;
    ...
    for (i = 0; i < (pEraseLedger->header.ledgersPerSector); i++)
    {
        ...
        pSector = lwosCallocType(OVL_INDEX_DMEM(spi), 1,
                      ERASE_LEDGER_DATA_SECTOR);
        if (pSector == NULL)
        {
            PMU_BREAKPOINT();
            status = FLCN_ERR_NO_FREE_MEM;
        ...
        status = spiRomReadDirect_HAL(&Spi, pSpiRom, pSector->dataSectorOffset,
        ...
        // Insert node into the linked list.
        if (pLedgerSector->pSectorHead == NULL)
        {
 ...
 *
 * @return FLCN_OK                     If active ledger sessions were initialized
 *                                     successfully.
 * @return FLCN_ERR_ILWALID_ARGUMENT   If argument is an invalid pointer
 * @return FLCN_ERR_NO_FREE_MEM        If there is no free memory.
 * @return other                       Propagates return values from various calls
...
(
    SPI_DEVICE_ROM              *pSpiRom,
    ERASE_LEDGER_SECTOR         *pLedgerSector,
    ERASE_LEDGER_DATA_SECTOR    *pSector,
    LwU32                        dataSectorSize
)
{
    LwU32          dword;
    ...
    {
        status = spiRomReadDirect_HAL(&Spi, pSpiRom, startOffset,
...
)
{
    ...
    {
        status = spiRomReadDirect_HAL(&Spi, pSpiRom, sessionStartOffset,
                     sizeof(LwU32), (LwU8 *)&dword);
        if (status != FLCN_OK)
        {
 ...
 * @param[out]  ppLedgerSector         ERASE_LEDGER_SECTOR *pointer.
 *
...
(
    SPI_DEVICE_ROM         *pSpiRom,
    LwU32                   eraseAddress,
    ERASE_LEDGER_SECTOR   **ppLedgerSector
)
{
    ERASE_LEDGER_SECTOR    *pLedgerSector;
    ERASE_LEDGER           *pEL = &eraseLedger;
...
(
    SPI_DEVICE_ROM         *pSpiRom,
    ERASE_LEDGER_SECTOR    *pSrc,
    ERASE_LEDGER_SECTOR    *pDest,
    LwU32                   eraseAddress
)
{
    LwU32                     sectorHeader;
    ...
    LwU32                     lifetimeEraseCount;
    LwU32                     dataSectorAddr;
    ERASE_LEDGER_DATA_SECTOR *pDestDataSector;
    ERASE_LEDGER_DATA_SECTOR *pDataSector;
    ...
    SPI_FOR_EACH_SECTOR(pSrc, pDataSector)
    {
        if (dataSectorAddr == eraseAddress)
        {
