...
/* ------------------------- Types and Enums -------------------------------- */
/* ------------------------- Macros and Defines ----------------------------- */
#define SMBPBI_DEM_FIFO_PTR(x)      ((x) % INFOROM_DEM_OBJECT_V1_00_FIFO_SIZE)
#define SMBPBI_DEM_RECORD_SIZE_MIN  sizeof(LW_MSGBOX_DEM_RECORD)
#define SMBPBI_DEM_RECORD_SIZE_MAX  (sizeof(LW_MSGBOX_DEM_RECORD)       \
                                    + LW_MSGBOX_MAX_DRIVER_EVENT_MSG_TXT_SIZE)
#define SMBPBI_DEM_READ_VAR(memb, var)                                  \
({                                                                      \
    ct_assert(sizeof(var) <=                                            \
              sizeof(((PINFOROMHALINFO_GK104)0)->DEM.memb));            \
    smbpbiHostMemRead_INFOROM_GK104(LW_OFFSETOF(INFOROMHALINFO_GK104,   \
                                    DEM.memb),                          \
                                    sizeof(var),                        \
                                    (LwU8 *)&(var));                    \
})
...
)
{
    pSmbpbi->config.cap[LW_MSGBOX_DATA_CAP_4] =
        FLD_SET_DRF(_MSGBOX, _DATA_CAP_4, _GET_DRIVER_EVENT_MSG, _AVAILABLE,
                    pSmbpbi->config.cap[LW_MSGBOX_DATA_CAP_4]);
    SMBPBI_DEM_READ_VAR(bValid, pSmbpbi->bDemActive);
}
...
smbpbiDemEventSource(void)
{
    ...
    if (Smbpbi.bDemActive)
    {
        ...
        status = SMBPBI_DEM_READ_VAR(object.v1.readOffset, readOffset);
        if (status != LW_MSGBOX_CMD_STATUS_SUCCESS)
        {
            goto smbpbiEventSource_exit;
        }

        status = SMBPBI_DEM_READ_VAR(object.v1.writeOffset, writeOffset);
        if (status != LW_MSGBOX_CMD_STATUS_SUCCESS)
        {
            goto smbpbiEventSource_exit;
        }
...
)
{
    ...
    LwU16           rem;
    LwU8            recSize;
    LwU8            buffer[SMBPBI_DEM_RECORD_SIZE_MAX];
    LwU8            *bufPtr;
    ...
    status = smbpbiScratchPtr2Offset(arg2, SMBPBI_SCRATCH_BANK_READ, &dstOffset);
    if (status != LW_MSGBOX_CMD_STATUS_SUCCESS)
    {
        goto smbpbiGetDem_exit;
    }
    ...
    status = SMBPBI_DEM_READ_VAR(object.v1.fifoBuffer[readOffset +
    ...
    if ((rem < SMBPBI_DEM_RECORD_SIZE_MIN) || (rem > SMBPBI_DEM_RECORD_SIZE_MAX))
    {
        // Record size is invalid, hence the FIFO is corrupted
        status = LW_MSGBOX_CMD_STATUS_ERR_MISC;
        PMU_TRUE_BP();

        // Empty the FIFO
        readOffset = SMBPBI_DEM_FIFO_PTR(writeOffset);
        goto smbpbiGetDem_writeReadOffset;
    }
    ...
    while (rem > 0)
    {
        ...
        status = smbpbiHostMemRead_INFOROM_GK104(
                    LW_OFFSETOF(INFOROMHALINFO_GK104,
                                DEM.object.v1.fifoBuffer[readOffset]),
                    copySz,
                    bufPtr);
        if (status != LW_MSGBOX_CMD_STATUS_SUCCESS)
        {
        ...
        rem -= copySz;
        bufPtr += copySz;
        readOffset = SMBPBI_DEM_FIFO_PTR(readOffset + copySz);
    }
...
smbpbiGetDem_writeReadOffset:

    // Write back the read pointer
    flcnStatus = dmaWrite(&readOffset, &Smbpbi.config.memdescInforom,
                      PMU_SMBPBI_HOST_MEM_OFFSET(PMU_SMBPBI_HOST_MEM_TYPE_INFOROM_GK104) +
                            LW_OFFSETOF(INFOROMHALINFO_GK104, DEM.object.v1.readOffset),
                      sizeof(readOffset));
