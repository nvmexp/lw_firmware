...
/*!
 * @file  pwrdev_gpuadc11.c
 * @brief GPUADC11 Driver
 *
...
/* ------------------------- Macros and Defines ----------------------------- */
//
// The value that the LW_CPWR_THERM_IPC_CTRL_INDEX_INDEX field should have so
// that the LW_CPWR_THERM_IPC_CTRL_DATA_VALUE_BY_DS field may me modified. This
// is because the index value for our concerned field should be ((i)*17+5).
// Here, i = 0 as we are patching IPC 0.
//
#define BUG_3282024_IPC_PARAMS_INDEX            0x5

// The new value that the LW_CPWR_THERM_IPC_CTRL_DATA_VALUE_BY_DS field should have.
#define BUG_3282024_IPC_PARAMS_BY_DS_NEW_VALUE  0x06

/* ------------------------- Static Function Prototypes --------------------- */
static FLCN_STATUS s_pwrDevGpuAdc11ProvChannelMapConstruct(PWR_DEVICE_GPUADC11 *pGpuAdc11)
    GCC_ATTRIB_SECTION("imem_pmgrLibConstruct", "s_pwrDevGpuAdc11ProvChannelMapConstruct");
static void s_pwrDevGpuAdc11PatchIPCBug3282024(void)
    GCC_ATTRIB_SECTION("imem_pmgrLibLoad", "s_pwrDevGpuAdc11PatchIPCBug3282024");
...
static FLCN_STATUS s_pwrDevGpuAdc11UpdateFuseParams(PWR_DEVICE_GPUADC11 *pGpuAdc11)
    GCC_ATTRIB_SECTION("imem_pmgrLibLoad", "s_pwrDevGpuAdc11UpdateFuseParams");
static FLCN_STATUS s_pwrDevGpuAdc11PreLoad(PWR_DEVICE_GPUADC11 *pGpuAdc11)
    GCC_ATTRIB_SECTION("imem_pmgrLibLoad", "s_pwrDevGpuAdc11PreLoad");
 ...
 *
 * @param[in]   pGpuAdc11
 *     Pointer to @ref PWR_DEVICE_GPUADC11.
 * @param[in]   provIdx
...
pwrDevGpuAdc11ProvGet
(
    PWR_DEVICE_GPUADC11 *pGpuAdc11,
    LwU8                 provIdx
)
{
    if (provIdx < LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PHYSICAL_PROV_NUM)
    {
       return &pGpuAdc11->prov[provIdx];
    }
...
/*!
 * Construct a GPUADC11 PWR_DEVICE object.
 *
 * @copydoc BoardObjGrpIfaceModel10ObjSet
 */
FLCN_STATUS
pwrDevGrpIfaceModel10ObjSetImpl_GPUADC11
(
...
{
    PWR_DEVICE_GPUADC11                               *pGpuAdc11;
    RM_PMU_PMGR_PWR_DEVICE_DESC_GPUADC11_BOARDOBJ_SET *pGpuAdc11Set =
        (RM_PMU_PMGR_PWR_DEVICE_DESC_GPUADC11_BOARDOBJ_SET *)pBoardObjDesc;
    ...
    OSTASK_ATTACH_OR_DETACH_OVERLAYS_ENTER(ovlDescList);
    {
        ...
        PMU_ASSERT_OK_OR_GOTO(status,
            ...
            pwrDevGrpIfaceModel10ObjSetImpl_GPUADC11_done);

        // Sanity check the VFE VAR index
        if (!VFE_VAR_IS_VALID(pGpuAdc11Set->vcmCoarseOffsetVfeVarIdx))
        {
            status = FLCN_ERR_ILWALID_STATE;
            PMU_BREAKPOINT();
            goto pwrDevGrpIfaceModel10ObjSetImpl_GPUADC11_done;
        }

        pGpuAdc11 = (PWR_DEVICE_GPUADC11 *)*ppBoardObj;

        pGpuAdc11->bPatchIpcBug3282024 = pGpuAdc11Set->bPatchIpcBug3282024;
        pGpuAdc11->operatingMode       = pGpuAdc11Set->operatingMode;
        pGpuAdc11->numSamples          = pGpuAdc11Set->numSamples;
        pGpuAdc11->ovrm                = pGpuAdc11Set->ovrm;
        pGpuAdc11->ipc                 = pGpuAdc11Set->ipc;
        pGpuAdc11->beacon              = pGpuAdc11Set->beacon;
        pGpuAdc11->offset              = pGpuAdc11Set->offset;
        pGpuAdc11->sum                 = pGpuAdc11Set->sum;
        pGpuAdc11->vcmCoarseOffsetVfeVarIdx =
            pGpuAdc11Set->vcmCoarseOffsetVfeVarIdx;

        pGpuAdc11->tupleParams.numSamples = pGpuAdc11Set->numSamples;
        ...
        //
        pGpuAdc11->tupleParams.lwrrShiftBits = 25;
        ...
        // 25 fractional bits will be discarded by multUFXP32
        //
        pGpuAdc11->tupleParams.voltShiftBits = 25;
        ...
        //
        pGpuAdc11->tupleParams.pwrShiftBits = 14;

        // Callwlate lwrrColwFactor and pwrColwFactor
        for (i = 0; i < pGpuAdc11->super.activeProvCount; i++)
        {
            LwU32 temp;
            PWR_DEVICE_GPUADC1X_PROV *pProv = pwrDevGpuAdc11ProvGet(pGpuAdc11, i);
            PMU_ASSERT_TRUE_OR_GOTO(status,
                (pProv != NULL),
                FLCN_ERR_ILWALID_STATE,
                pwrDevGrpIfaceModel10ObjSetImpl_GPUADC11_done);

            pProv->fullRangeLwrrent = pGpuAdc11Set->fullRangeLwrrent[i];

            // Colwert fullRangeLwrrent to mA
            fullRangeLwrrentmA = pwrDevGpuAdc11FullRangeLwrrmAGet(pGpuAdc11, pProv);

            //
            // Callwlate lwrrColwFactor (mA/code)
            // lwrrColwFactor = fullRangeLwrrentmA (mA) / adcMaxValue (code)
            // Consider fullRangeLwrrentmA of 0x1FFFFF00 (2097151 mA) in FXP21.8
            // adcMaxValue of 0x7F is FXP7.0
            //   21.8 mA      /  7.0 code => 14.8 mA/code
            //   14.8 mA/code >> 4        => 14.4 mA/code
            //
            pProv->lwrrColwFactor = LW_TYPES_U32_TO_UFXP_X_Y_SCALED(
                28, 4, fullRangeLwrrentmA, pGpuAdc11->super.adcMaxValue);
            ...
            //
            temp = multUFXP32(2, pProv->lwrrColwFactor,
                pGpuAdc11->super.accColwusPerCycle);
            pProv->lwrr.scaleFactor = LW_UNSIGNED_ROUNDED_DIV(temp,
                pGpuAdc11->numSamples);
            pProv->lwrr.shiftBits   = 35;
            ...
            //
            temp = pGpuAdc11->super.voltColwFactor *
                pGpuAdc11->super.accColwusPerCycle;
            pProv->volt.scaleFactor = LW_UNSIGNED_ROUNDED_DIV(temp,
                pGpuAdc11->numSamples);
            pProv->volt.shiftBits   = 37;
            ...
            pProv->pwrColwFactor = LW_TYPES_UFXP_X_Y_TO_U32_ROUNDED(
                28, 4, ((pProv->lwrrColwFactor *
                         pGpuAdc11->super.voltColwFactor) / (LwU32)1000));
            ...
            //
            temp = multUFXP32(2, pProv->pwrColwFactor,
                pGpuAdc11->super.accColwusPerCycle);
            pProv->pwr.scaleFactor = LW_UNSIGNED_ROUNDED_DIV(temp,
                (pGpuAdc11->numSamples * pGpuAdc11->numSamples));
            pProv->pwr.shiftBits   = 24;
        ...
        // channels that it constitutes. Also, this function verifies the beacon
        // provider is specified correctly and updates the offset channel
        // provider index for Gen2
        //
        PMU_ASSERT_OK_OR_GOTO(status,
            s_pwrDevGpuAdc11ProvChannelMapConstruct(pGpuAdc11),
            pwrDevGrpIfaceModel10ObjSetImpl_GPUADC11_done);

        pGpuAdc11->bAdcRawCorrection = LW_FALSE;

        // Set boolean for ADC_RAW_CORRECTION based on Offset configuration
        if ((pGpuAdc11->offset.instance[0].provIdx ==
                LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_IDX_NONE) &&
            (pGpuAdc11->offset.instance[1].provIdx ==
                LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_IDX_NONE))
        {
            pGpuAdc11->bAdcRawCorrection = LW_TRUE;
        }
        ...
        // device
        //
        if ((pGpuAdc11->beacon.instance[0].provIdx !=
                LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_IDX_NONE) ||
            (pGpuAdc11->beacon.instance[1].provIdx !=
                LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_IDX_NONE))
        {
...
/*!
 * GPUADC11 power device implementation.
 *
 * @copydoc PwrDevLoad
 */
FLCN_STATUS
...
{
    PWR_DEVICE_GPUADC11      *pGpuAdc11 = (PWR_DEVICE_GPUADC11 *)pDev;
    FLCN_STATUS               status    = FLCN_OK;
    ...
    //
    // Patch IPC based on Bug 3282024 is required,
    // based on information provided by RM.
    //
    if (pGpuAdc11->bPatchIpcBug3282024)
    {
        s_pwrDevGpuAdc11PatchIPCBug3282024();
    }

    // Registers which use fuse values must be programmed first
    OSTASK_ATTACH_OR_DETACH_OVERLAYS_ENTER(ovlDescListFuse);
    {
        status = s_pwrDevGpuAdc11UpdateFuseParams(pGpuAdc11);
    }
    OSTASK_ATTACH_OR_DETACH_OVERLAYS_EXIT(ovlDescListFuse);

    PMU_ASSERT_OK_OR_GOTO(status, status, pwrDevLoad_GPUADC11_done);

    // Settings to be done before enabling ADC sensing
    PMU_ASSERT_OK_OR_GOTO(status,
        s_pwrDevGpuAdc11PreLoad(pGpuAdc11),
        pwrDevLoad_GPUADC11_done);
    ...
    reg32 = FLD_SET_DRF_NUM(_PTRIM_SYS_NAFLL, _SYS_PWR_SENSE_ADC_MULTISAMPLER,
        _NUM_SAMPLES, pGpuAdc11->numSamples - 1, reg32);
    ...
    for (i = 0; i < LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_NUM; i++)
    {
        reg32 = 0;
        switch (pGpuAdc11->ipc.instance[i].sourceType)
        {
            case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_SRC_TYPE_ADC:
            {
                pProv = pwrDevGpuAdc11ProvGet(pGpuAdc11,
                            pGpuAdc11->ipc.instance[i].index.provider);
                PMU_ASSERT_TRUE_OR_GOTO(status,
                    (pProv != NULL),
                    FLCN_ERR_ILWALID_STATE,
                    pwrDevLoad_GPUADC11_done);
                ...
                switch (pGpuAdc11->ipc.instance[i].providerUnit)
                {
                    case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_PROV_UNIT_MV:
                    {
                        PMU_ASSERT_TRUE_OR_GOTO(status,
                    ...
                    case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_PROV_UNIT_MA:
                    {
                        PMU_ASSERT_TRUE_OR_GOTO(status,
                    ...
                    case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_PROV_UNIT_MW:
                    {
                        PMU_ASSERT_TRUE_OR_GOTO(status,
            ...
            case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_SRC_TYPE_BA:
            {
                reg32 = FLD_SET_DRF(_CPWR_THERM, _IPC_CTRL, _EN, _ON, reg32);
                reg32 = FLD_SET_DRF_NUM(_CPWR_THERM, _IPC_CTRL, _OP,
                    LW_CPWR_THERM_IPC_CTRL_OP_BA_W(
                        pGpuAdc11->ipc.instance[i].index.window), reg32);
                break;
            }
            case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_SRC_TYPE_ADC_SUM:
            {
                reg32 = FLD_SET_DRF(_CPWR_THERM, _IPC_CTRL, _EN, _ON, reg32);
                reg32 = FLD_SET_DRF_NUM(_CPWR_THERM, _IPC_CTRL, _OP,
                    LW_CPWR_THERM_IPC_CTRL_OP_SUM(
                        pGpuAdc11->ipc.instance[i].index.sum), reg32);
                break;
            }
            case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_SRC_TYPE_NONE:
            {
    ...
    if (pGpuAdc11->super.bFirstLoad)
    {
        pGpuAdc11->super.bFirstLoad = LW_FALSE;
    }
...
)
{
    ...
    if ((provIdx >= pGpuAdc11->super.activeProvCount) &&
        (provIdx < LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_NUM))
    {
        //
        ...
        //
        status = FLCN_OK;
        goto pwrDevTupleGet_GPUADC11_exit;
    }
    else
    {
        PMU_ASSERT_TRUE_OR_GOTO(status,
            ...
            pwrDevTupleGet_GPUADC11_exit);
    }

    pProv = pwrDevGpuAdc11ProvGet(pGpuAdc11, provIdx);
    PMU_ASSERT_TRUE_OR_GOTO(status,
        (pProv != NULL),
        FLCN_ERR_ILWALID_ARGUMENT,
        pwrDevTupleGet_GPUADC11_exit);
    ...
    PMU_ASSERT_OK_OR_GOTO(status,
        pwrDevTupleGet_GPUADC1X(pDev, pProv, pTuple, &(pGpuAdc11->tupleParams)),
        pwrDevTupleGet_GPUADC11_exit);
...
)
{
        ...
        (provIdx < LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_NUM))
    {
        ...
        //
        status = FLCN_OK;
        goto pwrDevTupleAccGet_GPUADC11_exit;
    }
    else
    {
        PMU_ASSERT_TRUE_OR_GOTO(status,
            (provIdx < LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_NUM),
            FLCN_ERR_ILWALID_ARGUMENT,
            pwrDevTupleAccGet_GPUADC11_exit);
    }
    ...
    // Call super-class implementation
    status = pwrDevTupleAccGet_GPUADC1X(pDev, pProv, pTupleAcc);
    if (status == FLCN_ERR_ACC_SEQ_MISMATCH)
    {
        goto pwrDevTupleAccGet_GPUADC11_exit;
    }

    PMU_ASSERT_OK_OR_GOTO(status, status, pwrDevTupleAccGet_GPUADC11_exit);
...
/*!
 * GPUADC11 power device implementation to return full range tuple.
 *
 * @copydoc PwrDevTupleGet
 */
FLCN_STATUS
pwrDevTupleFullRangeGet_GPUADC11
(
    PWR_DEVICE                 *pDev,
    LwU8                        provIdx,
    LW2080_CTRL_PMGR_PWR_TUPLE *pTuple
)
{
        ...
        (provIdx < LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_NUM))
    {
        ...
        //
        status = FLCN_OK;
        goto pwrDevTupleFullRangeGet_GPUADC11_exit;
    }
    else
    {
        PMU_ASSERT_TRUE_OR_GOTO(status,
            (provIdx < LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_NUM),
            FLCN_ERR_ILWALID_ARGUMENT,
            pwrDevTupleFullRangeGet_GPUADC11_exit);
    }
...
)
{
    ...
    PMU_ASSERT_TRUE_OR_GOTO(status,
        (limitIdx < pwrDevThresholdNumGet_GPUADC11(pDev)),
        FLCN_ERR_ILWALID_ARGUMENT,
        pwrDevSetLimit_GPUADC11_exit);

    // Sanity check limitUnit matches IPC(limitIdx) unit
    devLimitUnit = s_pwrDevColwertPolicyLimitUnit(limitUnit);
    PMU_ASSERT_TRUE_OR_GOTO(status,
        (devLimitUnit == pGpuAdc11->ipc.instance[limitIdx].providerUnit),
        FLCN_ERR_ILWALID_ARGUMENT,
        pwrDevSetLimit_GPUADC11_exit);

    // Sanity check if provIdx corresponds to the IPC(limitIdx) instance
    switch (pGpuAdc11->ipc.instance[limitIdx].sourceType)
    {
        case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_SRC_TYPE_ADC:
        {
            // Additionally, sanity check that the provider is active
            PMU_ASSERT_TRUE_OR_GOTO(status,
                ((pGpuAdc11->ipc.instance[limitIdx].index.provider == provIdx) &&
                 (provIdx < pGpuAdc11->super.activeProvCount)),
                FLCN_ERR_ILWALID_ARGUMENT,
                pwrDevSetLimit_GPUADC11_exit);
            break;
        }
        ...
        {
            PMU_ASSERT_TRUE_OR_GOTO(status,
                (pGpuAdc11->ipc.instance[limitIdx].index.window == (provIdx -
                    LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_BA_IPC_PROV_IDX_START)),
                FLCN_ERR_ILWALID_ARGUMENT,
                pwrDevSetLimit_GPUADC11_exit);
            break;
        }
        ...
        {
            PMU_ASSERT_TRUE_OR_GOTO(status,
                (pGpuAdc11->ipc.instance[limitIdx].index.sum == (provIdx -
                    LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PHYSICAL_PROV_NUM)),
                FLCN_ERR_ILWALID_ARGUMENT,
                pwrDevSetLimit_GPUADC11_exit);
            break;
        }
    ...
    switch (pGpuAdc11->ipc.instance[limitIdx].sourceType)
    {
        case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_SRC_TYPE_ADC:
        {
            ...
            if (pGpuAdc11->ipc.instance[limitIdx].providerUnit ==
                    LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_PROV_UNIT_MA)
            {
                //
                ...
                // max possible value.
                //
                if (limitValue > pwrDevGpuAdc11FullRangeLwrrmAGet(pGpuAdc11, pProv))
                {
                    reg32 = LW_U32_MAX;
                }
                else
                {
                    reg32 = LW_TYPES_U32_TO_UFXP_X_Y(7, 25, pGpuAdc11->numSamples);
                    reg32 = LW_UNSIGNED_ROUNDED_DIV(reg32, pProv->lwrrColwFactor);
                    reg32 = reg32 * limitValue;
                }
            }
            else if (pGpuAdc11->ipc.instance[limitIdx].providerUnit ==
                         LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_PROV_UNIT_MW)
            {
                //
                ...
                //
                if (limitValue >
                        pwrDevGpuAdc11FullRangePwrmWGet(pGpuAdc11, pProv))
                {
                    reg32 = LW_U32_MAX;
                }
                else
        ...
        case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_SRC_TYPE_ADC_SUM:
        {
            LwU8  sumIdx = pGpuAdc11->ipc.instance[limitIdx].index.sum;
            LwU8  i;
                    ...
                    LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_PROV_UNIT_MA)
            {
                ...
                for (i = 0;
                     i < LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_SUM_INPUT_NUM;
                     i++)
                {
                    if (pGpuAdc11->sum.instance[sumIdx].input[i].provIdx !=
                            LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_IDX_NONE)
                    {
                        pProv = pwrDevGpuAdc11ProvGet(pGpuAdc11,
                            pGpuAdc11->sum.instance[sumIdx].input[i].provIdx);
                        PMU_ASSERT_TRUE_OR_GOTO(status,
                            (pProv != NULL),
                            FLCN_ERR_ILWALID_ARGUMENT,
                            pwrDevSetLimit_GPUADC11_exit);
                        ...
                        fullRangeLwrrentmASum +=
                            pwrDevGpuAdc11FullRangeLwrrmAGet(pGpuAdc11, pProv);
                    }
                }
                ...
                else
                {
                    ...
                    reg32 = LW_UNSIGNED_ROUNDED_DIV(reg32,
                        pGpuAdc11->sum.instance[sumIdx].colwFactor.lwrr);
                    reg32 = reg32 * limitValue;
                }
            }
            else if (pGpuAdc11->ipc.instance[limitIdx].providerUnit ==
                         LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_PROV_UNIT_MW)
            {
                ...
                else
                {
                    reg32 = LW_TYPES_U32_TO_UFXP_X_Y(18, 14,
                        (pGpuAdc11->numSamples * pGpuAdc11->numSamples));
                    reg32 = LW_UNSIGNED_ROUNDED_DIV(reg32,
                        pGpuAdc11->sum.instance[sumIdx].colwFactor.pwr);
                    reg32 = reg32 * limitValue;
                }
            }
            REG_WR32(CSB, LW_CPWR_THERM_IPC_REF(limitIdx), reg32);
...
)
{
    ...
    switch (pGpuAdc11->ipc.instance[limitIdx].sourceType)
    {
        ...
        {
            // Additionally, sanity check that the provider is active
            if ((pGpuAdc11->ipc.instance[limitIdx].index.provider != provIdx) ||
                (provIdx >= pGpuAdc11->super.activeProvCount))
            {
                pStatus->providers[provIdx].thresholds[limitIdx] = 0;
                goto pwrDevGetLimit_GPUADC11_exit;
            }
            break;
        }
        case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_SRC_TYPE_BA:
        {
            if (pGpuAdc11->ipc.instance[limitIdx].index.window != (provIdx -
                    LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_BA_IPC_PROV_IDX_START))
            {
                pStatus->providers[provIdx].thresholds[limitIdx] = 0;
                goto pwrDevGetLimit_GPUADC11_exit;
        ...
        case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_SRC_TYPE_ADC_SUM:
        {
            if (pGpuAdc11->ipc.instance[limitIdx].index.sum != (provIdx -
                    LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PHYSICAL_PROV_NUM))
            {
                pStatus->providers[provIdx].thresholds[limitIdx] = 0;
                goto pwrDevGetLimit_GPUADC11_exit;
    ...
    switch (pGpuAdc11->ipc.instance[limitIdx].sourceType)
    {
        case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_SRC_TYPE_ADC:
        {
            PWR_DEVICE_GPUADC1X_PROV *pProv = pwrDevGpuAdc11ProvGet(pGpuAdc11,
                pGpuAdc11->ipc.instance[limitIdx].index.provider);
            PMU_ASSERT_TRUE_OR_GOTO(status,
                (pProv != NULL),
                FLCN_ERR_ILWALID_STATE,
                pwrDevGetLimit_GPUADC11_exit);
                     ...
                     LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_PROV_UNIT_MA))
            {
                ...
                // 25 fractional bits will be discarded by multUFXP32
                //
                reg32 = LW_UNSIGNED_ROUNDED_DIV(reg32, pGpuAdc11->numSamples);
                reg32 = multUFXP32(25, reg32, pProv->lwrrColwFactor);
            }
            else if ((pGpuAdc11->ipc.instance[limitIdx].providerUnit ==
                          LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_PROV_UNIT_MW))
            {
                ...
                reg32 = LW_UNSIGNED_ROUNDED_DIV(reg32,
                        (pGpuAdc11->numSamples * pGpuAdc11->numSamples));
                reg32 = multUFXP32(14, reg32, pProv->pwrColwFactor);
            }
            break;
        }
        case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_SRC_TYPE_BA:
        ...
        case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_SRC_TYPE_ADC_SUM:
        {
                     ...
                     LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_PROV_UNIT_MA))
            {
                ...
                reg32 = multUFXP32(25, reg32,
                    pGpuAdc11->sum.instance[pGpuAdc11->ipc.instance[limitIdx].index.sum].colwFactor.lwrr);
            }
            else if ((pGpuAdc11->ipc.instance[limitIdx].providerUnit ==
                          LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_PROV_UNIT_MW))
            {
                ...
                else
                {
                    ...
                    reg32 = multUFXP32(14, reg32,
                        pGpuAdc11->sum.instance[pGpuAdc11->ipc.instance[limitIdx].index.sum].colwFactor.pwr);
                }
            }
            break;
        }
 ...
 */
FLCN_STATUS
pwrDevGpuAdc1xBaSetLimit_GPUADC11
(
...
)
{
    ...
    // https://confluence.lwpu.com/display/RMPWR/GPUADC+1.1+-+Software+Architecture+and+Design+Document#GPUADC1.1-SoftwareArchitectureandDesignDolwment-LogicalProviders
    //
    provIdx = baWindowIdx +
        LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_BA_IPC_PROV_IDX_START;

    return pwrDevSetLimit_GPUADC11(&pGpuAdc1x->super, provIdx, limitIdx,
 ...
 */
LwU32
pwrDevGpuAdc1xFullRangeVoltmVGet_GPUADC11
(
    PWR_DEVICE_GPUADC1X      *pGpuAdc1x,
    PWR_DEVICE_GPUADC1X_PROV *pProv
)
{
    PWR_DEVICE_GPUADC11 *pGpuAdc11 = (PWR_DEVICE_GPUADC11 *)pGpuAdc1x;

    return pwrDevGpuAdc11FullRangeVoltmVGet(pGpuAdc11, pProv);
}
 ...
 */
LwU32
pwrDevGpuAdc1xFullRangeLwrrmAGet_GPUADC11
(
    PWR_DEVICE_GPUADC1X      *pGpuAdc1x,
    PWR_DEVICE_GPUADC1X_PROV *pProv
)
{
    ...
    return pwrDevGpuAdc11FullRangeLwrrmAGet(pGpuAdc11, pProv);
}
 ...
 */
LwU32
pwrDevGpuAdc1xFullRangePwrmWGet_GPUADC11
(
    PWR_DEVICE_GPUADC1X      *pGpuAdc1x,
    PWR_DEVICE_GPUADC1X_PROV *pProv
)
{
    ...
    return pwrDevGpuAdc11FullRangePwrmWGet(pGpuAdc11, pProv);
}
 ...
 */
LwUFXP28_4
pwrDevGpuAdc1xVoltColwFactorGet_GPUADC11
(
    PWR_DEVICE_GPUADC1X      *pGpuAdc1x,
    PWR_DEVICE_GPUADC1X_PROV *pProv
)
 ...
 */
void
pwrDevGpuAdc1xSwResetLengthSet_GPUADC11
(
    PWR_DEVICE_GPUADC1X *pGpuAdc1x
)
{
 ...
 */
void
pwrDevGpuAdc1xSnapTrigger_GPUADC11
(
    PWR_DEVICE_GPUADC1X *pGpuAdc1x
)
{
 ...
 */
void
pwrDevGpuAdc1xSwResetTrigger_GPUADC11
(
    PWR_DEVICE_GPUADC1X *pGpuAdc1x
)
{
...
/* ------------------------- Static Functions ------------------------------- */
/*!
 * Helper function to patch IPC based on Bug 3282024
 */
static void
s_pwrDevGpuAdc11PatchIPCBug3282024(void)
{
    LwU32 regIpcCtrlIdx;
    LwU32 ipcParamsOldIdx;
    LwU32 regIpcCtrlData;

    regIpcCtrlIdx = REG_RD32(CSB, LW_CPWR_THERM_IPC_CTRL_INDEX);

    // Save the old value for restoration.
    ipcParamsOldIdx = regIpcCtrlIdx;

    // Set the index value.
    regIpcCtrlIdx = FLD_SET_DRF_NUM(_CPWR_THERM, _IPC_CTRL_INDEX, _INDEX,
        BUG_3282024_IPC_PARAMS_INDEX, regIpcCtrlIdx);
    REG_WR32(CSB, LW_CPWR_THERM_IPC_CTRL_INDEX, regIpcCtrlIdx);

    regIpcCtrlData = REG_RD32(CSB, LW_CPWR_THERM_IPC_CTRL_DATA);

    // Set the IPC parameter to the new value.
    regIpcCtrlData = FLD_SET_DRF_NUM(_CPWR_THERM, _IPC_CTRL_DATA, _VALUE_BY_DS,
        BUG_3282024_IPC_PARAMS_BY_DS_NEW_VALUE, regIpcCtrlData);
    REG_WR32(CSB, LW_CPWR_THERM_IPC_CTRL_DATA, regIpcCtrlData);

    // Restore the old index value.
    REG_WR32(CSB, LW_CPWR_THERM_IPC_CTRL_INDEX, ipcParamsOldIdx);
 ...
 * https://confluence.lwpu.com/download/attachments/335317040/GPUADC1.1%20Channel%20Programming%20Guide.xlsx?version=1&modificationDate=1576042654267&api=v2
 *
 * @param[in/out]  pGpuAdc11 Pointer to PWR_DEVICE_GPUADC11
 * @return
 ...
 */
static FLCN_STATUS
s_pwrDevGpuAdc11ProvChannelMapConstruct
(
    PWR_DEVICE_GPUADC11 *pGpuAdc11
)
{
    ...
    for (devIdx = 0;
         devIdx < LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_OVRM_NUM;
         devIdx++)
    {
        if (pGpuAdc11->ovrm.device[devIdx].gen ==
            LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_OVRM_GEN_2)
        {
            LwU8 i;
            LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_OVRM_DATA_GEN2 *pGen2 =
                &(pGpuAdc11->ovrm.device[devIdx].data.gen2);

            for (i = 0; i < pGen2->tupleProvNum; i++)
            {
                pProv = pwrDevGpuAdc11ProvGet(pGpuAdc11, provIdx);

                // Sanity check the pointer and fullRangeLwrrent
                PMU_ASSERT_TRUE_OR_GOTO(status,
                    ((pProv != NULL) &&
                     (pProv->fullRangeLwrrent != 0)),
                    FLCN_ERR_ILWALID_STATE,
                    s_pwrDevGpuAdc11ProvChannelMapConstruct_exit);

                //
                // The ordering of the bus and shunt channels is important here.
                // As per the GPUADC1X spec, bus channel always comes first.
                //
                pProv->channelMap.bus   = chIdx++;
                pProv->channelMap.shunt = chIdx++;
                pProv->channelMap.pair  = chPairIdx++;
                provIdx++;
            }

            for (i = 0; i < pGen2->imonNum; i++)
            {
                pProv = pwrDevGpuAdc11ProvGet(pGpuAdc11, provIdx);
                ...
                // IMON channel is always the shunt channel in a provider
                pProv->channelMap.bus   =
                    PWR_DEVICE_GPUADC1X_PROV_CH_MAP_IDX_NONE;
                pProv->channelMap.shunt = chIdx++;
                pProv->channelMap.pair  =
                    PWR_DEVICE_GPUADC1X_PROV_CH_MAP_IDX_NONE;
                provIdx++;
            }
            ...
            {
                //
                // If IMON is present, a provider is already allotted and the
                // bus channel of that provider can be re-used. Else, use new
                // provider and mark the bus and pair indexes to be invalid.
                //
                if (pGen2->imonNum != 0)
                {
                    provIdx--;
                ...
                if (pGpuAdc11->beacon.instance[devIdx].provIdx !=
                    provIdx)
                {
                    status = FLCN_ERR_ILWALID_STATE;
                    PMU_BREAKPOINT();
                    goto s_pwrDevGpuAdc11ProvChannelMapConstruct_exit;
                }

                // Set the offset provider index to ground
                pGpuAdc11->offset.instance[devIdx].provIdx = provIdx;
                provIdx++;
            }
            else
            {
                // No offset channel
                pGpuAdc11->offset.instance[devIdx].provIdx =
                    LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_IDX_NONE;
            }
        }
        else if (pGpuAdc11->ovrm.device[devIdx].gen ==
            LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_OVRM_GEN_1)
        {
            ...
            for (provIdx = pGpuAdc11->super.activeProvCount;
                 provIdx < LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PHYSICAL_PROV_NUM;
                 provIdx++)
            {
                pProv = pwrDevGpuAdc11ProvGet(pGpuAdc11, provIdx);
                PMU_ASSERT_TRUE_OR_GOTO(status,
            ...
            LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_OVRM_GEN_ILWALID)
        {
            //
            ...
            pGpuAdc11->offset.instance[devIdx].provIdx =
                LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_IDX_NONE;
            PMU_ASSERT_TRUE_OR_GOTO(status,
                (pGpuAdc11->beacon.instance[devIdx].provIdx ==
                     LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_IDX_NONE),
                FLCN_ERR_ILWALID_STATE,
                s_pwrDevGpuAdc11ProvChannelMapConstruct_exit);
        }
 ...
 *
 * @param[in] pGpuAdc11    Pointer to PWR_DEVICE_GPUADC11
 *
 * @return    FLCN_OK
 ...
 */
static FLCN_STATUS
s_pwrDevGpuAdc11UpdateFuseParams
(
    PWR_DEVICE_GPUADC11 *pGpuAdc11
)
{
    ...
    // Read the VCM_OFFSET.
    if (VFE_VAR_IS_VALID(pGpuAdc11->super.vcmOffsetVfeVarIdx))
    {
        PMU_ASSERT_OK_OR_GOTO(status,
            vfeVarEvaluate(pGpuAdc11->super.vcmOffsetVfeVarIdx, &vcmOffsetFloat),
            s_pwrDevGpuAdc11UpdateFuseParams_done);
    }

    // Read the DIFF_OFFSET.
    if (VFE_VAR_IS_VALID(pGpuAdc11->super.diffOffsetVfeVarIdx))
    {
        PMU_ASSERT_OK_OR_GOTO(status,
            vfeVarEvaluate(pGpuAdc11->super.diffOffsetVfeVarIdx, &diffOffsetFloat),
            s_pwrDevGpuAdc11UpdateFuseParams_done);
    }

    // Read the DIFF_GAIN.
    if (VFE_VAR_IS_VALID(pGpuAdc11->super.diffGailwfeVarIdx))
    {
        PMU_ASSERT_OK_OR_GOTO(status,
            vfeVarEvaluate(pGpuAdc11->super.diffGailwfeVarIdx, &diffGainFloat),
            s_pwrDevGpuAdc11UpdateFuseParams_done);
    }

    // Read the VCM_COARSE_OFFSET.
    if (VFE_VAR_IS_VALID(pGpuAdc11->vcmCoarseOffsetVfeVarIdx))
    {
        PMU_ASSERT_OK_OR_GOTO(status,
            vfeVarEvaluate(pGpuAdc11->vcmCoarseOffsetVfeVarIdx, &vcmCoarseOffsetFloat),
            s_pwrDevGpuAdc11UpdateFuseParams_done);
    }
    ...
    if (pGpuAdc11->bAdcRawCorrection)
    {
        // Colwert floating point values to unsigned 32bit integer.
...
)
{
    ...
    if (pGpuAdc11->operatingMode ==
            LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_OPERATING_MODE_ALT)
    {
        reg32 = REG_RD32(CSB, LW_CPWR_THERM_ADC_CTRL);
        ...
        reg32 = FLD_SET_DRF_NUM(_CPWR_THERM, _ADC_CTRL4, _NEG2POS_BIAS,
            pGpuAdc11->numSamples * pGpuAdc11->super.adcMaxValue, reg32);
    ...
    // BEACON channel
    for (idx = 0;
         idx < LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_BEACON_NUM;
         idx++)
    {
        if (pGpuAdc11->beacon.instance[idx].provIdx !=
                LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_IDX_NONE)
        {
            pProv = pwrDevGpuAdc11ProvGet(pGpuAdc11,
                pGpuAdc11->beacon.instance[idx].provIdx);
            PMU_ASSERT_TRUE_OR_GOTO(status,
                ((pProv != NULL) &&
                 (pwrDevGpuAdc1xProvChBusGet(pProv) !=
                      PWR_DEVICE_GPUADC1X_PROV_CH_MAP_IDX_NONE)),
                FLCN_ERR_ILWALID_STATE,
                s_pwrDevGpuAdc11PreLoad_exit);
    ...
    // OFFSET channel
    for (idx = 0;
         idx < LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_OFFSET_NUM;
         idx++)
    {
        if (pGpuAdc11->offset.instance[idx].provIdx !=
                LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_IDX_NONE)
        {
            pProv = pwrDevGpuAdc11ProvGet(pGpuAdc11,
                pGpuAdc11->offset.instance[idx].provIdx);
            PMU_ASSERT_TRUE_OR_GOTO(status,
                (pProv != NULL),
                FLCN_ERR_ILWALID_STATE,
                s_pwrDevGpuAdc11PreLoad_exit);

            if (pGpuAdc11->ovrm.device[idx].gen ==
                    LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_OVRM_GEN_2)
            {
         ...
         idx++)
    {
                ...
                LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_IDX_NONE)
        {
            ...
            reg32 = FLD_IDX_SET_DRF_NUM(_CPWR_THERM, _ADC_CTRL3, _BEACON_THRESH,
                idx, pGpuAdc11->beacon.instance[idx].threshold * pGpuAdc11->numSamples,
                reg32);
            switch (pGpuAdc11->beacon.instance[idx].compFunc)
            {
                case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_BEACON_COMP_GT:
                {
                ...
                case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_BEACON_COMP_LT:
                {
                    reg32 = FLD_IDX_SET_DRF_NUM(_CPWR_THERM, _ADC_CTRL3,
    ...
    //
    PMU_ASSERT_TRUE_OR_GOTO(status,
        ((pGpuAdc11->offset.instance[0].provMask &
          pGpuAdc11->offset.instance[1].provMask) == 0),
        FLCN_ERR_ILWALID_STATE,
        s_pwrDevGpuAdc11PreLoad_exit);
    ...
    for (idx = 0;
         ...
         idx++)
    {
        FOR_EACH_INDEX_IN_MASK(16, i, pGpuAdc11->offset.instance[idx].provMask)
        {
            pProv = pwrDevGpuAdc11ProvGet(pGpuAdc11, i);
            PMU_ASSERT_TRUE_OR_GOTO(status,
                (pProv != NULL),
                FLCN_ERR_ILWALID_STATE,
                s_pwrDevGpuAdc11PreLoad_exit);
    ...
    if (pGpuAdc11->sum.instance[0].provUnit !=
            LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_SUM_PROV_UNIT_DISABLED)
    {
        // SUM1_INPUT
        reg32 = REG_RD32(CSB, LW_CPWR_THERM_IPC_CTRL_SUM1_INPUT);

        for (i = 0; i < LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_SUM_INPUT_NUM; i++)
        {
            if (pGpuAdc11->sum.instance[0].input[i].provIdx ==
                    LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_IDX_NONE)
            {
            ...
            {
                pProv = pwrDevGpuAdc11ProvGet(pGpuAdc11,
                    pGpuAdc11->sum.instance[0].input[i].provIdx);
                PMU_ASSERT_TRUE_OR_GOTO(status,
                    (pProv != NULL),
                    FLCN_ERR_ILWALID_STATE,
                    s_pwrDevGpuAdc11PreLoad_exit);

                switch (pGpuAdc11->sum.instance[0].provUnit)
                {
                    case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_SUM_PROV_UNIT_MV:
                    {
                        PMU_ASSERT_TRUE_OR_GOTO(status,
                            (pwrDevGpuAdc1xProvChBusGet(pProv) !=
                                 PWR_DEVICE_GPUADC1X_PROV_CH_MAP_IDX_NONE),
                    ...
                    case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_SUM_PROV_UNIT_MA:
                    {
                        PMU_ASSERT_TRUE_OR_GOTO(status,
                            (pwrDevGpuAdc1xProvChShuntGet(pProv) !=
                                 PWR_DEVICE_GPUADC1X_PROV_CH_MAP_IDX_NONE),
                    ...
                    case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_SUM_PROV_UNIT_MW:
                    {
                        PMU_ASSERT_TRUE_OR_GOTO(status,
                            (pwrDevGpuAdc1xProvChPairGet(pProv) !=
                                 PWR_DEVICE_GPUADC1X_PROV_CH_MAP_IDX_NONE),
        ...
        for (i = 0; i < LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_SUM_INPUT_NUM; i++)
        {
            reg32 = FLD_IDX_SET_DRF_NUM(_CPWR_THERM, _IPC_CTRL_SUM1_SF,
                _SF, i, pGpuAdc11->sum.instance[0].input[i].scaleFactor, reg32);
        }
        REG_WR32(CSB, LW_CPWR_THERM_IPC_CTRL_SUM1_SF, reg32);
    }

    if (pGpuAdc11->sum.instance[1].provUnit !=
            LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_SUM_PROV_UNIT_DISABLED)
    {
        // SUM2_INPUT
        ...
        for (i = 0; i < LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_SUM_INPUT_NUM; i++)
        {
            if (pGpuAdc11->sum.instance[1].input[i].provIdx ==
                    LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_PROV_IDX_NONE)
            {
                idx = LW_CPWR_THERM_IPC_CTRL_SUM_INPUT_NO_CH;
            }
            ...
            {
                pProv = pwrDevGpuAdc11ProvGet(pGpuAdc11,
                    pGpuAdc11->sum.instance[1].input[i].provIdx);
                PMU_ASSERT_TRUE_OR_GOTO(status,
                    (pProv != NULL),
                    FLCN_ERR_ILWALID_STATE,
                    s_pwrDevGpuAdc11PreLoad_exit);

                switch (pGpuAdc11->sum.instance[1].provUnit)
                {
                    case LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_SUM_PROV_UNIT_MV:
                    {
                        PMU_ASSERT_TRUE_OR_GOTO(status,
        ...
        for (i = 0; i < LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_SUM_INPUT_NUM; i++)
        {
            reg32 = FLD_IDX_SET_DRF_NUM(_CPWR_THERM, _IPC_CTRL_SUM2_SF,
                _SF, i, pGpuAdc11->sum.instance[1].input[i].scaleFactor, reg32);
        }
...
/*!
 * Helper function to colwert from policy limit unit to GPUADC11 provider limit
 * unit
 *
 ...
 *            @ref LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_PROV_UNIT_<xyz>
 */
static LwU8
...
)
{
    switch (limitUnit)
    {
        case LW2080_CTRL_PMGR_PWR_POLICY_LIMIT_UNIT_LWRRENT_MA:
            return LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_PROV_UNIT_MA;
        case LW2080_CTRL_PMGR_PWR_POLICY_LIMIT_UNIT_POWER_MW:
            return LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_PROV_UNIT_MW;
    }

    return LW2080_CTRL_PMGR_PWR_DEVICE_GPUADC11_IPC_PROV_UNIT_ILWALID;
}
