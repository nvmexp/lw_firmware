...
/*!
 * @file   pmu_clkadc_v30_extended.c
 * @brief  File for ADC device version 3.0 routines applicable only to DGPUs and
...
)
{
    ...
    OSTASK_ATTACH_OR_DETACH_OVERLAYS_ENTER(ovlDescList);
    {
        ...
        tempCoefficientDen = pAdcsV20->data.highTemp - pAdcsV20->data.lowTemp;
        ...
        // instead of uV.
        //
        voltTempCoefficientDen = ((pAdcsV20->data.highVoltuV - pAdcsV20->data.lowVoltuV) *
                                  (pAdcsV20->data.highTemp - pAdcsV20->data.lowTemp) / 1000);
...
)
{
    ...
    OSTASK_ATTACH_OR_DETACH_OVERLAYS_ENTER(ovlDescList);
    {
        ...
        if (!bTempChange && (pTargetVoltList == NULL))
        {
            ...
            PMU_BREAKPOINT();
            goto _clkAdcComputeCodeOffset_V30_exit;
        }
        ...
        else
        {
            ...
            tempBasedOffsetCorrection = lwF32Mul(pAdcDevV30->statusData.tempCoefficient,
                                                (LwF32)(lwrrTemp - pAdcsV20->data.refTemp));
            ...
            voltTempBasedGainCorrection = lwF32Mul(pAdcDevV30->statusData.voltTempCoefficient,
                (LwF32)(lwrrTemp - pAdcsV20->data.refTemp) * (lwrrVoltuV - pAdcsV20->data.refVoltuV));
        ...
        if (adcCodeCorrectionOffset <
                lwF32ColwertFromS32(pAdcsV20->data.adcCodeCorrectionOffsetMin))
        {
            pAdcDevV30->targetAdcCodeCorrectionOffset =
                    pAdcsV20->data.adcCodeCorrectionOffsetMin;
        }
        else if (adcCodeCorrectionOffset >
                    lwF32ColwertFromS32(pAdcsV20->data.adcCodeCorrectionOffsetMax))
        {
            pAdcDevV30->targetAdcCodeCorrectionOffset =
                    pAdcsV20->data.adcCodeCorrectionOffsetMax;
        }
        else
        {
            pAdcDevV30->targetAdcCodeCorrectionOffset =
...
_clkAdcComputeCodeOffset_V30_exit:
        lwosNOP();
    }
    OSTASK_ATTACH_OR_DETACH_OVERLAYS_EXIT(ovlDescList);
...
/*!
 * @brief Program the ADC code offset to inlwlcate voltage and temperature based
 *        evaluations for calibration
 *
 * @param[in]   pAdcDev             Pointer to the ADC device
 * @param[in]   bPreVoltOffsetProg  Boolean to indicate if we're programming the
 *      offset prior to an anticipated voltage change or post a voltage/temperature
 *          change.
 *
 * @return FLCN_OK Successful programming of ADC offset
 * @return other   Descriptive error code from sub-routines on failure
 */
FLCN_STATUS
clkAdcProgramCodeOffset_V30
(
    CLK_ADC_DEVICE *pAdcDev,
...
)
{
    ...
    status = clkAdcCalOffsetProgram_HAL(&Clk, pAdcDev);
    if (status != FLCN_OK)
    {
        PMU_BREAKPOINT();
        goto _clkAdcProgramCodeOffset_V30_exit;
    }

    // Update the cached value with the latest
    pAdcDevV30->statusData.adcCodeCorrectionOffset = pAdcDevV30->targetAdcCodeCorrectionOffset;

    //
    // Update the concerned mask to indicate that the concerned ADC is programmed
    // with the offset computed in @ref clkAdcComputeCodeOffset_V30
    //
    if (bPreVoltOffsetProg)
    {
        boardObjGrpMaskBitClr(&pAdcsV20->preVoltAdcMask, BOARDOBJ_GET_GRP_IDX(&pAdcDev->super));
    }
    else
    {
        boardObjGrpMaskBitClr(&pAdcsV20->postVoltOrTempAdcMask, BOARDOBJ_GET_GRP_IDX(&pAdcDev->super));
    }

_clkAdcProgramCodeOffset_V30_exit:
    return status;
}
