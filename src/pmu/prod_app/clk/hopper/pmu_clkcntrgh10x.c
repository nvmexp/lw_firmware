 ...
 * {SYS, LWD, XBAR, PWR, HUB, UTILS, DISP, 4 VCLKs, MCLK,
 *  Bcast GPC, 7 unicast GPCs}
 */
#define CLK_CNTR_ARRAY_SIZE_MAX_GH10X              21
 ...
 */
CLK_CNTR *
clkCntrArrayGet_GH10X()
{
    static CLK_CNTR ClkCntrArray_GH10X[CLK_CNTR_ARRAY_SIZE_MAX_GH10X] = {{ 0 }};
    return ClkCntrArray_GH10X;
}
 ...
 */
LwU32
clkCntrDomMaskGet_GH10X()
{
    LwU32  clkDomMask;

    //
    // Make sure to update CLK_CNTR_ARRAY_SIZE_MAX_GH10X if
    // number of clock counters are changing.
    //
    clkDomMask =
#if (LW_PTRIM_SYS_PHYSICAL_CLKS_0_SYSCLK_CAPABILITY_INIT != 0)
                    CLK_DOMAIN_MASK_SYS     |
#endif
#if (LW_PTRIM_SYS_PHYSICAL_CLKS_0_LWDCLK_CAPABILITY_INIT != 0)
                    CLK_DOMAIN_MASK_LWD     |
#endif
#if (LW_PTRIM_SYS_PHYSICAL_CLKS_0_XBARCLK_CAPABILITY_INIT != 0)
