 ...
 * 7) Lower voltage in LW_PFB_FBPA_FBIO_VTT_CTRL0
 * 8) Lower voltage in LW_PFB_FBPA_FBIO_VTT_CTRL1
 */
void
msFbPadPwrDown_TU10X(void)
{
    LwU32     val;
    ...
    //
    // VAUXC controls moved to VTT_CTRL0/1 for DQ / CMD / CDB
    // VTT_CTRL0/1 registers are in serial priv range
    //
    val = pSwAsr->regs.fbpaFbioVttCntl0 = REG_RD32(FECS, LW_PFB_FBPA_FBIO_VTT_CTRL0);
    val = FLD_SET_DRF(_PFB, _FBPA_FBIO_VTT_CTRL0, _BYTE_LV_VAUXC, _850mV, val);
    val = FLD_SET_DRF(_PFB, _FBPA_FBIO_VTT_CTRL0, _CMD_LV_VAUXC, _850mV, val);
    REG_WR32(FECS, LW_PFB_FBPA_FBIO_VTT_CTRL0, val);
    ...
    val = pSwAsr->regs.fbpaFbioVttCntl1 = REG_RD32(FECS, LW_PFB_FBPA_FBIO_VTT_CTRL1);

    val = FLD_SET_DRF(_PFB, _FBPA_FBIO_VTT_CTRL1, _CDB_LV_VAUXC, _850mV, val);
    REG_WR32(FECS, LW_PFB_FBPA_FBIO_VTT_CTRL1, val);
}
...
msFbPadPwrUp_TU10X(void)
{
    ...
    // These below steps are for Turing and later GPUs
    REG_WR32(FECS, LW_PFB_FBPA_FBIO_VTT_CTRL0,
                      pSwAsr->regs.fbpaFbioVttCntl0);
    REG_WR32(FECS, LW_PFB_FBPA_FBIO_VTT_CTRL1,
                      pSwAsr->regs.fbpaFbioVttCntl1);
