...
/*!
 * @file   pmu_msghxxx.c
 * @brief  HAL-interface for the GH100 Memory System Engine.
 */
 ...
 */
LwBool
msIsHostMemOpPending_GHXXX(void)
{
    LwU32 val;

    val = REG_RD32(BAR0, LW_XAL_EP_POWER_STATUS);
    if (FLD_TEST_DRF(_XAL_EP, _POWER_STATUS, _BAR_BIND_STATUS,
                     _IN_PROGRESS, val)                                  ||
        FLD_TEST_DRF(_XAL_EP, _POWER_STATUS, _FLUSH_STATUS,
                     _IN_PROGRESS, val)                                  ||
        FLD_TEST_DRF(_XAL_EP, _POWER_STATUS, _L2_SYSMEM_ILWALIDATE_STATUS,
                     _IN_PROGRESS, val)                                  ||
        FLD_TEST_DRF(_XAL_EP, _POWER_STATUS, _L2_PEERMEM_ILWALIDATE_STATUS,
                     _IN_PROGRESS, val)                                  ||
        FLD_TEST_DRF(_XAL_EP, _POWER_STATUS, _L2_CLEAN_COMPTAGS_STATUS,
                     _IN_PROGRESS, val)                                  ||
        FLD_TEST_DRF(_XAL_EP, _POWER_STATUS, _L2_FLUSH_DIRTY_STATUS,
                     _IN_PROGRESS, val))
    {
        return LW_TRUE;
    }
