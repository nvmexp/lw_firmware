/* _LWRM_COPYRIGHT_BEGIN_
 *
 * Copyright 2014-2021 by LWPU Corporation.  All rights reserved.  All
 * information contained herein is proprietary and confidential to LWPU
 * Corporation.  Any use, reproduction, or disclosure without the written
 * permission of LWPU Corporation is prohibited.
 *
 * _LWRM_COPYRIGHT_END_
 */

/*!
 * @file   lpwr_cg_elcggmxxx_only.c
 * @brief  ELCG related functions.
 */

/* ------------------------ System includes -------------------------------- */
#include "pmusw.h"

#include "dev_pwr_csb.h"
#include "dev_therm.h"

/* ------------------------ Application includes --------------------------- */
#include "main.h"
#include "pmu_objlpwr.h"
#include "pmu_objms.h"
#include "pmu_objfifo.h"
#include "pmu_objlpwr.h"
#include "pmu_cg.h"
#include "config/g_lpwr_private.h"

//
// Compile time check to ensure LW_CPWR_THERM_GATE_CTRL__SIZE_1 is
// supported by SW framework
//
ct_assert(LW_CPWR_THERM_GATE_CTRL__SIZE_1 <= RM_PMU_LPWR_CG_ELCG_CTRL__COUNT);

/* ------------------------- Type Definitions ------------------------------- */
/* ------------------------- External Definitions --------------------------- */
/* ------------------------- Macros and Defines ----------------------------- */
/* ------------------------- Prototypes ------------------------------------- */
static FLCN_STATUS s_lpwrCgElcgStateChange_GM20X(LwU32 engHwId, LwBool bEnable);
static void        s_lpwrCgGrElcgWar_GM20X(void);

/* ------------------------- Public Functions ------------------------------- */
/*!
 * @brief ELCG Pre-initialization
 */
void
lpwrCgElcgPreInit_GM20X(void)
{
    LwU32 regVal;
    LwU32 engHwId;

    // Initialize all ELCG domains
    for (engHwId = 0; engHwId < LW_CPWR_THERM_GATE_CTRL__SIZE_1; engHwId++)
    {
        regVal = THERM_GATE_CTRL_REG_RD32(engHwId);

        // Initialize Idle threshold
        regVal = FLD_SET_DRF(_CPWR_THERM, _GATE_CTRL, _ENG_IDLE_FILT_EXP,
                               __PROD, regVal);
        regVal = FLD_SET_DRF(_CPWR_THERM, _GATE_CTRL, _ENG_IDLE_FILT_MANT,
                               __PROD, regVal);

        // Initialize delays for CG
        regVal = FLD_SET_DRF(_CPWR_THERM, _GATE_CTRL, _ENG_DELAY_BEFORE,
                               __PROD, regVal);
        regVal = FLD_SET_DRF(_CPWR_THERM, _GATE_CTRL, _ENG_DELAY_AFTER,
                               __PROD, regVal);

        // Initialize AUTO holdoff engagement
        regVal = FLD_SET_DRF(_CPWR_THERM, _GATE_CTRL, _ENG_PWR,
                             _AUTO, regVal);

        // Ensure that ELCG is by default disabled
        regVal = FLD_SET_DRF(_CPWR_THERM, _GATE_CTRL, _ENG_CLK,
                             _RUN, regVal);

        THERM_GATE_CTRL_REG_WR32(engHwId, regVal);
    }

    // Initialize FECS idle filter
    regVal = REG_RD32(CSB, LW_CPWR_THERM_FECS_IDLE_FILTER);
    regVal = FLD_SET_DRF(_CPWR_THERM, _FECS_IDLE_FILTER, _VALUE,
                         __PROD, regVal);
    REG_WR32(CSB, LW_CPWR_THERM_FECS_IDLE_FILTER, regVal);

    // Initialize HUBMMU idle filter
    regVal = REG_RD32(CSB, LW_CPWR_THERM_HUBMMU_IDLE_FILTER);
    regVal = FLD_SET_DRF(_CPWR_THERM, _HUBMMU_IDLE_FILTER, _VALUE,
                         __PROD, regVal);
    REG_WR32(CSB, LW_CPWR_THERM_HUBMMU_IDLE_FILTER, regVal);
}

/*!
 * @brief ELCG post initialization
 */
FLCN_STATUS
lpwrCgElcgPostInit_GM20X(void)
{
    Lpwr.grElcgDisableReasonMask = BIT(LPWR_CG_ELCG_CTRL_REASON_RM);

    return FLCN_OK;
}

/*!
 * @brief API to enable ELCG Ctrl using HW engine ID
 *
 * @param[in] engHwId   Engine ID
 * @param[in] reasonId  Enable reason ID
 *
 * @return  FLCN_OK on success
 */
FLCN_STATUS
lpwrCgElcgCtrlEnable_GM20X
(
    LwU32 engHwId,
    LwU32 reasonId
)
{
    LwU32       engHwIdGr = GET_FIFO_ENG(PMU_ENGINE_GR);
    FLCN_STATUS status    = FLCN_OK;

    if (engHwId == engHwIdGr)
    {
        Lpwr.grElcgDisableReasonMask &= ~BIT(reasonId);

        if (Lpwr.grElcgDisableReasonMask == 0)
        {
            status = s_lpwrCgElcgStateChange_GM20X(engHwId, LW_TRUE);
        }
    }
    else
    {
        status = s_lpwrCgElcgStateChange_GM20X(engHwId, LW_TRUE);
    }

    return status;
}

/*!
 * @brief API to Disable ELCG Ctrl using HW engine ID
 *
 * This API disables ELCG based on request from client.
 *
 * API does special handling for MS client to implement GR-ELCG <-> OSM WAR.
 * MS client gates the GPC and LTC clocks from OSM. As part of that it needs
 * to disable GR-ELCG SM. SM can be disablement only when -
 * - ENG_CLK == _RUN    AND
 * - ENG_PWR == _ON
 * Thus API sets ENG_PWR to _ON when request came from MS otherwise it set
 * ENG_PWR to _AUTO.
 *
 * @param[in] engHwId   Engine ID
 * @param[in] reasonId  Disable reason ID
 *
 * @return  FLCN_OK on success
 */
FLCN_STATUS
lpwrCgElcgCtrlDisable_GM20X
(
    LwU32 engHwId,
    LwU32 reasonId
)
{
    LwU32       engHwIdGr = GET_FIFO_ENG(PMU_ENGINE_GR);
    FLCN_STATUS status    = FLCN_OK;

    if (engHwId == engHwIdGr)
    {
        //
        // GR-ELCG <-> OSM CG WAR
        // Disable GR-ELCG state machine if disable request is issued by MS.
        // Issues SM disablement independent of current state of ELCG.
        //
        if ((PMUCFG_FEATURE_ENABLED(PMU_CG_GR_ELCG_OSM_WAR)) &&
            (reasonId == LPWR_CG_ELCG_CTRL_REASON_MS))
        {
            s_lpwrCgGrElcgWar_GM20X();
        }
        //
        // Request came from non-MS client. Disable GR-ELCG if its not
        // already disabled.
        //
        else if (Lpwr.grElcgDisableReasonMask == 0)
        {
            status = s_lpwrCgElcgStateChange_GM20X(engHwId, LW_FALSE);
        }

        Lpwr.grElcgDisableReasonMask |= LWBIT(reasonId);
    }
    else
    {
        status = s_lpwrCgElcgStateChange_GM20X(engHwId, LW_FALSE);
    }

    return status;
}

/* ------------------------- Private Functions ------------------------------ */
/*!
 * @brief To enable/disable the ELCG.
 *
 * @param[in] engHwId     - ELCG Eng HW ID
 * @param[in] bEnable     - LW_TRUE  -> Enable ELCG
 *                          LW_FALSE -> Disable ELCG
 */
static FLCN_STATUS
s_lpwrCgElcgStateChange_GM20X
(
    LwU32  engHwId,
    LwBool bEnable
)
{
    FLCN_STATUS status = FLCN_OK;
    LwU32       gateCtrl;

    //
    // Ensure that HW ID requested by RM does not exceed
    // LW_CPWR_THERM_GATE_CTRL__SIZE_1
    //
    if (engHwId >= LW_CPWR_THERM_GATE_CTRL__SIZE_1)
    {
        PMU_BREAKPOINT();
        status = FLCN_ERR_RPC_ILWALID_INPUT;

        goto lpwrCgElcgStateChange_GM20X_exit;
    }

    gateCtrl = THERM_GATE_CTRL_REG_RD32(engHwId);

    if (bEnable)
    {
        gateCtrl = FLD_SET_DRF(_CPWR_THERM, _GATE_CTRL, _ENG_CLK,
                                _AUTO, gateCtrl);
        gateCtrl = FLD_SET_DRF(_CPWR_THERM, _GATE_CTRL, _ENG_PWR,
                                _ON, gateCtrl);
    }
    else
    {
        gateCtrl = FLD_SET_DRF(_CPWR_THERM, _GATE_CTRL, _ENG_CLK,
                                _RUN, gateCtrl);
        gateCtrl = FLD_SET_DRF(_CPWR_THERM, _GATE_CTRL, _ENG_PWR,
                                _AUTO, gateCtrl);
    }

    // Program the clock gating.
    THERM_GATE_CTRL_REG_WR32(engHwId, gateCtrl);

lpwrCgElcgStateChange_GM20X_exit:

    return status;
}

/*!
 * @brief Disable GR-ELCG State machine
 *
 * This function disables GR-ELCG SM as part of ELCG<->OSM CG WAR. This
 * WAR disables GR-ELCG SM before gating GPC and LTC clocks from OSM. It
 * restores state of GR-ELCG SM after ungating clocks from OSM.
 *
 * Refer @lpwr_cg.c for details.
 */
static void
s_lpwrCgGrElcgWar_GM20X(void)
{
    LwU32 engHwIdGr = GET_FIFO_ENG(PMU_ENGINE_GR);
    LwU32 gateCtrl;

    gateCtrl = THERM_GATE_CTRL_REG_RD32(engHwIdGr);

    // Disable GR ELCG SM
    gateCtrl = FLD_SET_DRF(_CPWR_THERM, _GATE_CTRL, _ENG_CLK, _RUN, gateCtrl);
    gateCtrl = FLD_SET_DRF(_CPWR_THERM, _GATE_CTRL, _ENG_PWR, _ON, gateCtrl);

    THERM_GATE_CTRL_REG_WR32(engHwIdGr, gateCtrl);
}
