...
/*!
 * @file   fb_stop_war_gv10x_only.c
 * @brief  Functions for FB stop WAR to get MCLK switch up on FB falcon for
 *         Volta only.
 *         https://lwbugswb.lwpu.com/LwBugs5/SWBug.aspx?bugid=200167665&cmtNo=34
 */
...
//
// The following two functions (fbStopPmuFbflcnWar_GV10X &
// fbStartPmuFbflcnWar_GV10X) disable/enable all traffic on the host to FB
// interfaces so that nothing can access the FB when disabled. For example,
// MCLK switching requires FB not be accessed by anything else but the PMU.
//
// NOTE: These functions have been tweaked to be used for the FB stop WAR
// implemented to get MCLK switch up on FB falcon and for anything that doesn't
// need these tweaks should use fbStop_HAL().
// For more details regarding the WAR: Bug 1898603.
//
...
fbStopPmuFbflcnWar_GV10X(void)
{
    ...
    //
    // Note: We don't have the code to poll for FB_STOP_ACK here as FB falcon
    // will poll on the FB_STOP_ACK instead of PMU.
    //
}
 ...
 */
void
fbStartPmuFbflcnWar_GV10X(void)
{
    LwU32 reg32;
    ...
    // To do (akshatam): Since this is a tight loop in an interrupt service
    // routine check how long are we usually stuck in this loop.
    //
    do
    {
    ...
    //
    // Set FB falcon's mailbox register to magic value as an ack to convey
    // that the FB has been restarted.
    //
    REG_WR32(BAR0, LW_PFBFALCON_FIRMWARE_MAILBOX(1),
                LW_PFBFALCON_FIRMWARE_MAILBOX_VALUE_MAGIC);
}
