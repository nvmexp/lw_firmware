 ...
 */
static inline FLCN_STATUS _soeCsbErrorChkRegRd32_LR10(LwU32  addr, LwU32  *pData)
    GCC_ATTRIB_ALWAYSINLINE();
static inline FLCN_STATUS _soeCsbErrorChkRegWr32NonPosted_LR10(LwU32  addr, LwU32 data)
    GCC_ATTRIB_ALWAYSINLINE();
static inline FLCN_STATUS _soeBar0ErrorChkRegRd32_LR10(LwU32  addr, LwU32  *pData)
    GCC_ATTRIB_ALWAYSINLINE();
static inline FLCN_STATUS _soeBar0ErrorChkRegWr32NonPosted_LR10(LwU32  addr, LwU32  data)
    GCC_ATTRIB_ALWAYSINLINE();
 ...
 * calls.
 */
static inline void _soeBar0ClearCsrErrors_LR10(void)
    GCC_ATTRIB_ALWAYSINLINE();
...
)
{
    ...
    csbErrStatVal = falc_ldxb_i ((LwU32*)(LW_CSOE_FALCON_CSBERRSTAT), 0);

    // If FALCON_CSBERRSTAT_VALID == _FALSE, CSB read succeeded
    if (FLD_TEST_DRF(_CSOE, _FALCON_CSBERRSTAT, _VALID, _FALSE, csbErrStatVal))
    {
        //
        ...
        {
            if (addr == LW_CSOE_FALCON_PTIMER1)
            {
                //
            ...
            else if (addr == LW_CSOE_FALCON_PTIMER0)
            {
                //
...
_soeCsbErrorChkRegWr32NonPosted_LR10
(
    LwU32  addr,
    LwU32  data
)
{
    ...
    if (FLD_TEST_DRF(_CSOE, _FALCON_CSBERRSTAT, _VALID, _TRUE, csbErrStatVal))
    {
...
)
{
    ...
    // call inline the inline function, _soeBar0RegRd32_LR10, to execute
    ...
    flcnStatus = _soeBar0WaitIdle_LR10(LW_TRUE);
    ...
    // Set up the read
    CHECK_FLCN_STATUS(_soeCsbErrorChkRegWr32NonPosted_LR10(LW_CSOE_BAR0_ADDR, addr));
    CHECK_FLCN_STATUS(_soeCsbErrorChkRegWr32NonPosted_LR10(LW_CSOE_BAR0_CSR,
            DRF_DEF(_CSOE, _BAR0_CSR, _CMD ,        _READ) |
            DRF_NUM(_CSOE, _BAR0_CSR, _BYTE_ENABLE, 0xF  ) |
            DRF_DEF(_CSOE, _BAR0_CSR, _TRIG,        _TRUE)));
    CHECK_FLCN_STATUS(_soeCsbErrorChkRegRd32_LR10(LW_CSOE_BAR0_CSR, NULL));
    ...
    //
    // Bad reads should be caught by checking BAR0_CSR_STATUS in _soeBar0WaitIdle_LR10,
    // but check the read return value in case the failure is missed by HW.
    // Exceptions are added for timer registers.
    //
    CHECK_FLCN_STATUS(_soeCsbErrorChkRegRd32_LR10(LW_CSOE_BAR0_DATA, &val));
    ...
    if ((val & FLCN_BAR0_PRIV_PRI_ERROR_MASK) == FLCN_BAR0_PRIV_PRI_ERROR_CODE)
    {
        ...
        // TODO_Apoorvg: Removed PGC6_SCI_PTIMER_TIME_0 and LW_PTIMER_TIME_0. Confirm if it is required for SOE
        else if (addr == LW_CSOE_FALCON_PTIMER0)
        {
            LwU32   val1 = 0;
            LwU32   i;
            ...
            for (i = 0; i < 3; i++)
            {
                val1 = _soeBar0RegRd32_LR10(addr);
                if (val1 != val)
                {
                    val = val1;
                    flcnStatus = FLCN_OK;
 ...
 */
static inline FLCN_STATUS
_soeBar0ErrorChkRegWr32NonPosted_LR10
(
    LwU32  addr,
    LwU32  data
)
{
    ...
    // 
    // For PEH, we need to check the return value of WFI, so rather than
    // call the inline function, _soeBar0RegWr32NonPosted_LR10, to execute
    ...
    // Do the write
    CHECK_FLCN_STATUS(_soeCsbErrorChkRegWr32NonPosted_LR10(LW_CSOE_BAR0_ADDR, addr));
    CHECK_FLCN_STATUS(_soeCsbErrorChkRegWr32NonPosted_LR10(LW_CSOE_BAR0_DATA, data));
    CHECK_FLCN_STATUS(_soeCsbErrorChkRegWr32NonPosted_LR10(LW_CSOE_BAR0_CSR,
 ...
 *
 * Once we have an error of LW_CSOE_BAR0_CSR_STATUS_TMOUT or
 * case LW_CSOE_BAR0_CSR_STATUS_DIS (or others),  the status will
 * remain until we have a successful CSR read.  Unless the error
 * status is cleared,  all subsequent calls to  soeBar0WaitIdle
 * will fail.
 *
 * See https://sc-refmanuals.lwpu.com/~gpu_refmanuals/gp104/LW_CSOE.ref#125
 * STATUS :3 (R)
 * Determines the Status of the previous Command. When status is ERR or TOUT, 
 * the register is reset to IDLE on read access.
 */
static inline void
_soeBar0ClearCsrErrors_LR10(void)
{
    // Initiate a BAR0_CSR CSB read access (to any) to clear errors
    REG_WR32(CSB, LW_CSOE_BAR0_ADDR, LW_PSMC_BOOT_0);
    ...
    (void)REG_RD32(CSB, LW_CSOE_BAR0_DATA);
}
