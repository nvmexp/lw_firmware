...
/* ------------------------- Private Functions ------------------------------ */
static LwU8 _smbpbiDmemScratchRd(LwU16 offset, LwU32 *pData);
static LwU8 _smbpbiDmemScratchWr(LwU16 offset, LwU32 data);
static LwU8 _smbpbiDmemScratchCp(LwU32 dst, LwU32 src, LwU32 szBytes);
static LwU32 _smbpbisrvDmemScratchPageOffset(void);
static LwU8 _smbpbiAsyncRequestPoll(LwU8 arg2, LwU32 *pData);
static LwU8 _smbpbiHandleAsyncRequest(LwU8 arg1, LwU8 arg2, LwU32 *pData);
static LwU8 _smbpbiHandleDeviceDisableRequest(LwU16 flags);
static LwU8 _smbpbiAsyncReqHandleDeviceModeControlSet(LW_MSGBOX_DEVICE_MODE_CONTROL_PARAMS *pParams);

/* ------------------------- Static Variables ------------------------------- */
/* ------------------------- Global Variables ------------------------------- */
LwU32 scratchMemory[LW_MSGBOX_SCRATCH_BUFFER_SIZE_D];
LwU32 registerFile[LW_MSGBOX_REGISTER_FILE_SIZE];
