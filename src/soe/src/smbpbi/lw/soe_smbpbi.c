...
/* ------------------------- Type Definitions ------------------------------- */
/* ------------------------- Macros and Defines ----------------------------- */
#ifndef DBG_PRINTF_SMBPBI   //TODOmy
#define DBG_PRINTF_SMBPBI(x)
#endif
/* ------------------------- External Definitions --------------------------- */
/* ------------------------- Private Functions ------------------------------ */
static void _smbpbiSetStatus(OBJSMBPBI *pSmbpbi, LwU32 cmd, LwU32 data, LwU32 extData);
static LwU8 _smbpbiGetTemp(LwU8 sensor, LwU32 *pTemp, LwBool  bExtended);
static LwU8 _smbpbiGetThermParam(LwU8 paramId, LwU32 *pData);
static void _smbpbiPrepareDataCopy(LwU32 *pCmd, LwU32 data, LwU32 extData);
static LwU8 _smbpbiGetLwlinkInfoHandler(LwU8 arg1, LwU8 arg2, LwU32 *pCmd,
                                        LwU32 *pData, LwU32 *pExtData);
static LwU8 _smbpbiGetPcieLinkInfoHandler(LwU8 arg1, LwU8 arg2, LwU32 *pData, LwU32 *pExtData);
static LwU8 _smbpbiGetMiscDeviceFlagsHandler(LwU8 arg1, LwU8 arg2, LwU32 *pData, LwU32 *pExtData);
static LwU8 _smbpbiGetDeviceDisableState(LwU32 *pData);
static void _smbpbiInforomInitCapDword(void);
static LwU8 _smbpbiGetSysId(LwU32 attr, LwU16 offset, LwU32 *pData);
static FLCN_STATUS _smbpbiSetupSharedSurface(RM_FLCN_U64 dmaHandle);
static OsTimerCallback (_smbpbiMonitorDriverState)
     GCC_ATTRIB_USED();
static void _smbpbiSetupDriverStateCallback(LwU32 driverPollingPeriodUs);
static FLCN_STATUS _smbpbiProcessInitCmd(RM_SOE_SMBPBI_CMD_INIT *pCmd);
static FLCN_STATUS _smbpbiProcessUnloadCmd(RM_SOE_SMBPBI_CMD_UNLOAD *pCmd);
static FLCN_STATUS _smbpbiProcessSetLinkErrorInfoCmd(RM_SOE_SMBPBI_CMD_SET_LINK_ERROR_INFO *pCmd);
static LwU8 s_smbpbiRegWrClient(LwU8 reg, LwU32 data);
static LwU8 s_smbpbiEventSet(LwMsgboxEventType event);
static LwU8 _smbpbiGetWriteProtectionMode(LwU8 arg1, LwU32 *pData);

/* ------------------------- Static Variables ------------------------------- */
/* ------------------------- Global Variables ------------------------------- */
RM_SOE_SMBPBI_INFOROM_DATA *pInforomData = NULL;
...
GCC_ATTRIB_SECTION("imem_resident", "smbpbiDispatch");

/* ------------------------- Constants -------------------------------------- */
static const LwU32 smbpbiEventsEdgeTriggered =
...
)
{
    ...
    switch (pDispSmbpbi->eventType)
    {
        case DISP2UNIT_EVT_COMMAND:
        {
            ...
            if (status == FLCN_OK)
            {
                osCmdmgmtCmdQSweep(&pCmd->hdr, pDispSmbpbi->cmdQueueId);
...
)
{
    ...
    osTimerScheduleCallback(
        &SmbpbiOsTimer,                                           // pOsTimer
        SMBPBI_OS_TIMER_ENTRY_DRIVER_STATE,                       // index
        lwrtosTaskGetTickCount32(),                               // ticksPrev
        driverPollingPeriodUs,                                    // usDelay
        DRF_DEF(_OS_TIMER, _FLAGS, _RELWRRING, _YES_MISSED_EXEC), // flags
        _smbpbiMonitorDriverState,                                // pCallback
        NULL,                                                     // pParam
...
)
{
    ...
    switch (sensor)
    {
        ...
        {
            //
            // The attribute is necessary to pacify the compiler,
            // because in reality thermGetTsenseMax_HAL() is a macro
            // that is not using its argument, as of now (11/2019).
            //
            OBJTHERM    *pTherm __attribute__((unused)) = &Therm;

            temp = thermGetTsenseMax_HAL(pTherm);
            break;
        }
