...
/* ------------------------ Static Variables ------------------------------- */
static RM_FLCN_CMD_SOE  smbpbiCmd;
 ...
 */
void
smbpbiService_LR10(void)
{
    DISP2UNIT_CMD   disp2Smbpbi = { 0 };
    LwU32           regVal;

    // Verify _MSGBOX_INTR bit and clear it.
    regVal = ISR_REG_RD32(LW_THERM_MSGBOX_COMMAND);
    if (FLD_TEST_DRF(_THERM, _MSGBOX_COMMAND, _INTR, _PENDING, regVal))
    {
        ...
        ISR_REG_WR32(LW_THERM_MSGBOX_COMMAND, regVal);
        (void) ISR_REG_RD32(LW_THERM_MSGBOX_COMMAND);
 ...
 */
void
smbpbiSetDeviceDisableMode_LR10
(
    LwU16 flags
)
{
 ...
 */
void
smbpbiGetDeviceDisableModeStatus_LR10
(
    LwU16 *pDriverState,
    LwU16 *pDeviceState,
