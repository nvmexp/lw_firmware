...
/*!
 * @file  soe_os.c
 * @brief Top-Level interfaces between the SOE application layer and the RTOS
 *
...
{
    return soeDmaNackCheckAndClear_HAL();
}
...
{
    if (SOECFG_FEATURE_ENABLED(DYNAMIC_FLCN_PRIV_LEVEL))
    {
        soeFlcnPrivLevelSet_HAL(&Soe, privLevelExt, privLevelCsb);
    }
}
...
vApplicationFlcnPrivLevelReset(void)
{
    if (SOECFG_FEATURE_ENABLED(DYNAMIC_FLCN_PRIV_LEVEL))
    {
        soeFlcnPrivLevelSet_HAL(&Soe,
                                 Soe.flcnPrivLevelExtDefault,
                                 Soe.flcnPrivLevelCsbDefault);
    }
}
