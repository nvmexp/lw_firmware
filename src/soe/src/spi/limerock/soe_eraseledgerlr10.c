...
)
{
        ...
        (!LW_ROM_REGION_INITIALIZED(pSpiRom->pLedgerRegion)))
    {
        SOE_HALT();
        status = FLCN_ERR_ILWALID_ARGUMENT;
        goto spiRomInitEraseLedger_LR10_exit;
    }
...
spiRomInitEraseLedger_LR10_exit:
    if (status != FLCN_OK)
    {
 ...
 */
FLCN_STATUS
spiRomInitLedgerRegionFromScratch_LR10
(
    PSPI_DEVICE_ROM pSpiRom
)
{
 ...
 */
LwBool
spiRomIsEraseLimitReached_LR10
(
    PSPI_DEVICE_ROM    pSpiRom,
    LwU32              eraseAddress
)
