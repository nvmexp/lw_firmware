...
/* ------------------------- Prototypes ------------------------------------- */
static FLCN_STATUS s_spiRomInitAccessRange(PSPI_DEVICE_ROM pSpiRom);
static FLCN_STATUS s_spiRomReadId(PSPI_DEVICE_ROM pSpiRom, LwU32 *pJedecId);
static FLCN_STATUS s_spiRomWriteProtect(PSPI_DEVICE_ROM pSpiRom, LwBool bEnable);
static FLCN_STATUS s_spiRomReadStatusReg(PSPI_DEVICE_ROM pSpiRom, LwU32 *pStatus);
static FLCN_STATUS s_spiRomWriteStatusReg(PSPI_DEVICE_ROM pSpiRom, LwU8 wrMask);
static FLCN_STATUS s_spiRomWriteEnable(PSPI_DEVICE_ROM pSpiRom);
static FLCN_STATUS s_spiRomSendPPCmd(PSPI_DEVICE_ROM pSpiRom, LwU32 address);
static FLCN_STATUS s_spiRomPollForReady(PSPI_DEVICE_ROM pSpiRom, LwU32 waitTimeUs);

/* ------------------------- Macros and Defines ----------------------------- */
#define LW_GET_BYTE(v, n)            (((v) >> (8 * (n))) & 0xff)

/* ------------------------- Global Variables ------------------------------- */
LwU8 dmaBuffer[SOE_DMA_MAX_SIZE] __attribute__ ((aligned (SOE_DMA_MAX_SIZE)));
static LwBool _spiRomInitialized = LW_FALSE;
