...
sub setup_client_info
{
    ...
    my %client_info = (
        # General client Info
        NAME            => 'SOE',
        CONFIG_FILE     => $self->{LW_ROOT}.'/uproc/soe/build/mcheck/mcheck.config',
        ...
        RMCFG_SRC_PATH  => $self->{LW_ROOT}.'/uproc/soe/src',
        RMCFG_ARGS      => '--mode soe-config '.
                           '--config '.$self->{LW_ROOT}.'/uproc/soe/config/soe-config.cfg ',
