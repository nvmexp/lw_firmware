...
_icServiceMthd_LS10(void)
{
    ...
    lwrtosQueueSendFromISR(SoeChnMgmtCmdDispQueue, &dispatch, sizeof(dispatch));
...
icService_LS10(void)
{
    ...
    for (irqLine=0; irqLine <= DRF_BASE(LW_SOE_FALCON_IRQSTAT_MEMERR); ++irqLine)
    {
         if (irqstat & BIT(irqLine))
         {
             ...
             switch(irqLine)
             {
             case DRF_BASE(LW_SOE_FALCON_IRQSTAT_MTHD):
                 _icServiceMthd_LS10();
                 dbgPuts(LEVEL_INFO, "MTHD Interrupt\n");
                 break;
             case DRF_BASE(LW_SOE_FALCON_IRQSTAT_CTXSW):
                 icServiceCtxSw_LS10();
                 dbgPuts(LEVEL_INFO, "CTXSW Interrupt\n");
                 break;
             case DRF_BASE(LW_SOE_FALCON_IRQSTAT_EXT_EXTIRQ5):
                 dbgPuts(LEVEL_INFO, "EXT Interrupt\n");
                 cmdqIrqHandler();
                 break;
             case DRF_BASE(LW_SOE_FALCON_IRQSTAT_MEMERR):
 ...
 */
sysSYSLIB_CODE void
icHostIfIntrUnmask_LS10(void)
{
    csbWrite(LW_CSOE_RISCV_IRQMSET,
            (DRF_DEF(_CSOE, _RISCV_IRQMSET, _MTHD,  _SET) |
             DRF_DEF(_CSOE, _RISCV_IRQMSET, _CTXSW, _SET)));
}
 ...
 */
sysSYSLIB_CODE void
icSemaphoreDTrapIntrTrigger_LS10(void)
{
    csbWrite(LW_CSOE_FALCON_IRQSSET, DRF_SHIFTMASK(LW_CSOE_FALCON_IRQSSET_SWGEN1));
}
 ...
 *   */
sysSYSLIB_CODE void
icSetNonstallIntrCtrlGfid_LS10
(
    LwU32 gfid
)
{
    // The notification (nonstall) tree is at index 1
    LwU32 intrCtrl = csbRead(LW_CSOE_FALCON_INTR_CTRL(1));

    //
    // HW unit registers, including SOE's, don't have field defines since the units
    ...
    csbWrite(LW_CSOE_FALCON_INTR_CTRL(1), intrCtrl);
}
 ...
 */
sysSYSLIB_CODE void
icCmdQueueIntrUnmask_LS10(LwU8 queueId)
{
    LwU32 regVal = 0;

    // For now, we only support RM queue
    if (queueId != SOE_CMDMGMT_CMD_QUEUE_RM)
    {
        SOE_BREAKPOINT();
    }
    ...
    regVal = FLD_IDX_SET_DRF(_CSOE, _CMD_INTREN, _HEAD_UPDATE,
 ...
 */
sysSYSLIB_CODE void
icCmdQueueIntrClear_LS10(LwU8 queueId)
{
    LwU32 regVal = 0;
    ...
    regVal = FLD_IDX_SET_DRF(_CSOE, _CMD_HEAD_INTRSTAT, _FIELD,
 ...
 */
sysSYSLIB_CODE void
icHaltIntrMask_LS10(void)
{
    LwU32 regVal = 0;

    regVal = FLD_SET_DRF(_CSOE, _RISCV_IRQMCLR, _HALT, _SET, regVal);
    csbWrite(LW_CSOE_RISCV_IRQMCLR, regVal);
}
...
icIsCtxSwIntrPending_LS10(void)
{
    return FLD_TEST_DRF(_CSOE, _FALCON_IRQSTAT, _CTXSW, _TRUE,
                        csbRead(LW_CSOE_FALCON_IRQSTAT));
}
...
*/
sysSYSLIB_CODE void
icHostIfIntrCtxswUnmask_LS10(void)
{
    csbWrite(LW_CSOE_FALCON_IRQMSET,
             DRF_DEF(_CSOE, _FALCON_IRQMSET, _CTXSW,  _SET));
}
