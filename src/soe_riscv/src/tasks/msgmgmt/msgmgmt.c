...
//
// TODO sahilc : This is a placeholder value required for CMD QUEUE entry in RM_SOE_INIT_MSG_SOE_INIT (always index 0)
//               since MSQ QUEUE entry is always index 1.It should be replaced with correct entry once
//               command queue mgmt task is setup.
//
extern LwU32 cmdqOffsetShared;
...
GCC_ATTRIB_SECTION("globalCode", "mapRmOffsetToPtr")
...
GCC_ATTRIB_SECTION("globalCode", "mapPtrToRmOffset")
...
{
    LwU32 head = *pHead;
    LwU32 tail = csbRead(ENGINE_REG(_MSGQ_TAIL(SOE_MSG_QUEUE_RM)));
...
msgQueuePostBlocking(PRM_FLCN_QUEUE_HDR pHdr, const void *pBody)
{
    ...
    head = csbRead(ENGINE_REG(_MSGQ_HEAD(SOE_MSG_QUEUE_RM)));
    ...
    csbWrite(ENGINE_REG(_MSGQ_HEAD(SOE_MSG_QUEUE_RM)), head);
...
initMessageQueue(void)
{
    ...
    csbWrite(ENGINE_REG(_MSGQ_TAIL(SOE_MSG_QUEUE_RM)), msgqOffset);
    csbWrite(ENGINE_REG(_MSGQ_HEAD(SOE_MSG_QUEUE_RM)), msgqOffset);
}
...
sendRmInitMessage(FLCN_STATUS status)
{
    ...
    csbWrite(SOE_STATUS_SCRATCH_REGISTER, SOE_BOOTSTRAP_SUCCESS);
}
